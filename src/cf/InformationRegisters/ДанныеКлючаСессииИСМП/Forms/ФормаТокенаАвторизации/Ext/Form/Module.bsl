...
Процедура ПолучитьТокен()
	...
	ДанныеКлючаСессии = ИнтерфейсАвторизацииИСМПСлужебный.ПолучитьСохраненныеДанныеКлючаСессии(
		ИнтерфейсИСМПКлиентСервер.ИмяДанныхКлючаСессии(ТипТокенаАвторизации));
...
Процедура ЗаписатьТокен()
	...
	Иначе
		...
		Если РезультатРабора <> Неопределено Тогда
			ТокенДействуетДо = ИнтеграцияИС.ДатаИзСтрокиUNIX(РезультатРабора.exp, 1);
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыЗапроса.Вставить("ПроизводственныйОбъект", ПроизводственныйОбъект);
