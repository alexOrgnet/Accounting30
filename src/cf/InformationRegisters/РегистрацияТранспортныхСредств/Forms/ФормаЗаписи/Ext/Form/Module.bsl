...
Процедура ЗаполнитьКодыВидовТС()
	...
	Если СтарыйГод = 0 ИЛИ СтарыйГод <> ТекущийПоказательГод Тогда 
		...
		ТаблицаКодовВидовТС = РегистрыСведений.РегистрацияТранспортныхСредств.КодыВидовТС(ТекущийПоказательГод);
...
Процедура УстановитьДоступностьНалоговыхЛьгот(Форма)
	...
	Запись = Форма.Запись;
	НалоговаяЛьгота = Запись.НалоговаяЛьгота;
	...
	РегиональнаяЛьгота = СтрНачинаетсяС(Запись.КодНалоговойЛьготы, "2");
	...
	Элементы.ДекорацияКодЛьготы.Видимость = 
		ОсвобождениеОтНалогообложения И РегиональнаяЛьгота ИЛИ СнижениеНалоговойСтавки ИЛИ УменьшениеСуммыНалогаВПроцентах;
