...
Процедура СкорректироватьПредставлениеСертификатаАбонентаПослеПоискаСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Выполнено И Результат.СертификатНайден Тогда
		...
		ЭтоФизическоеЛицо = (КриптографияЭДКОКлиентСервер.ТипСубъектаСертификата(СвойстваСертификата.Владелец) = "ФЛ");
		Если ЭтоФизическоеЛицо Тогда
			ИННСертификата = КриптографияЭДКОКлиентСервер.ИННСертификата(СвойстваСертификата.Владелец);
			Если ЗначениеЗаполнено(ИННСертификата) И ИННСертификата =
				ИННОрганизации(Запись.Организация) Тогда
				
				// Портал ФСРАР принимает сертификаты физических лиц без ОГРНИП в качестве сертификатов ИП по ИНН
...
Функция ИННОрганизации(Организация)
	...
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	Возврат КонтекстЭДОСервер.ИННОрганизации(Организация, "");
