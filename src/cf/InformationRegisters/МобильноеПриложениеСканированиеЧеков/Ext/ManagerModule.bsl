...
Функция ДоступноПодключениеПоЛогинуИПаролю() Экспорт
	...
	// Ограничение будет введено позже - при реализации для коробочных версий.
...
Функция КодДоступа(Пользователь = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Константы.АдресШиныПрикладныхПриложений.Получить()) Тогда
	...
	Иначе
		...
		// В процессе подключения мог быть получен адрес Шины прикладных приложений
		Если КодДоступа = Неопределено И ЗначениеЗаполнено(Константы.АдресШиныПрикладныхПриложений.Получить()) Тогда
			КодДоступа = ШинаПрикладныхПриложений.КодДоступа(ИмяМобильногоПриложения(), НачальныеДанные);
		КонецЕсли;
	КонецЕсли;
...
Функция УдалитьКодДоступа(Пользователь) Экспорт
	...
	Для Каждого Идентификатор Из ИдентификаторыПользователя Цикл
		...
		Иначе 
			КодУдален = ШинаМобильныхПриложений.УдалитьКодДоступа(ИмяМобильногоПриложения(), Запись.КодДоступа);
		КонецЕсли;
...
// Если не выполнен переход на Шину прикладных приложений, то получение выполняется с Шины мобильных приложений.
// Для работы с Шиной мобильных приложений это - запасной вариант доставки сообщений.
...
Функция НачатьПроверкуСообщений() Экспорт
	...
	Если ЗначениеЗаполнено(Константы.АдресШиныПрикладныхПриложений.Получить()) Тогда
		ШинаПрикладныхПриложений.ПолучитьСообщенияШиныОтложенно(ИмяМобильногоПриложения());
	Иначе
