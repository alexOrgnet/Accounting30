...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ИнтеграцияЕГАИСКлиент.ПодключитьОбработчикВыполненияОбменаНаКлиентеПоРасписанию(Ложь);
...
Функция КодФСРАРИзУТМНаКлиенте(НастройкаОбмена)
	...
	#Иначе
		...
		Результат = ИнтеграцияЕГАИСКлиентСервер.КодФСРАРv3(НастройкаОбмена);
		Если Результат.ПроверитьУТМv4 Тогда
			Результат = ИнтеграцияЕГАИСКлиентСервер.КодФСРАРv4(НастройкаОбмена);
		КонецЕсли;
