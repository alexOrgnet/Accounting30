...
Процедура ПодготовитьДанные(Параметры, АдресХранилища) Экспорт
	...
	|	Правила.Ссылка,
	|	Правила.Владелец,
	|	ВключенныеПравила.ВключеноПользователем,
	|	ИСТИНА
	|ИЗ
	|	Справочник.ПравилаПредставленияОтчетовУплатыНалогов КАК Правила
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ВключенныеПравила КАК ВключенныеПравила
...
Процедура ЗаписатьДанные(Параметры, АдресХранилища) Экспорт
	...
	Для Каждого Строка Из Дерево.Строки.НайтиСтроки(Отбор, Истина) Цикл
		...
		ИначеЕсли Справочники.ПравилаПредставленияОтчетовУплатыНалогов.ОписаниеТипаУсловияПримененияТребованияЗаконодательства().СодержитТип(ТипЗнч(Строка.Ссылка)) Тогда
...
Процедура ВключитьПравилоСтатистикаП2ГодоваяПоОрганизации(Организация, НастройкаНапоминания)
	...
	ВключенныеПравила = Справочники.ПравилаПредставленияОтчетовУплатыНалогов.ВключенныеПравилаОрганизации(
