...
Функция ДопустимыеЗначения(Отбор = Неопределено, Знач СтрокаПоиска = Неопределено) Экспорт
	...
	ИначеЕсли ТипОтбора = Тип("ФиксированныйМассив") Тогда
		...
		Запрос.УстановитьПараметр("Отбор", Отбор);
	ИначеЕсли ТипОтбора = Тип("Структура") И Отбор.Свойство("Исключение") Тогда
		ТекстыОтбора.Добавить("ВидыОперацийОплатаПлатежнойКартой.Ссылка <> &Исключение");
		Запрос.УстановитьПараметр("Исключение", Отбор.Исключение);
	КонецЕсли;
