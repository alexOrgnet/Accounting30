...
Процедура ОбработкаВыбораПодборВставкаИзБуфераНаСервере(ВыбранноеЗначение, ИмяТаблицы)
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		// Пропускаем услуги
		Если ЭтоВставкаИзБуфера И СведенияОНоменклатуре <> Неопределено И СведенияОНоменклатуре.Услуга = Истина Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеШапкаИВидимость()
	...
	// В таблице билетов для расходов без НДС поля НДС не доступны
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Билеты.СтавкаНДС", ВидСравненияКомпоновкиДанных.Равно, ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС"));
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Доступность", Ложь);	
	
	// Если в таблице билетов отсуствует "входящий" НДС, то скрываем настройку учета НДС
