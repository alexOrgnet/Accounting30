...
Процедура ЗаполнитьПрисоединенныеФайлы(ЗаполнятьУдаленные = Ложь) 
	...
	Для каждого ЭлементКоллекции Из ДанныеФайлов Цикл   
		...
		НовСтрока.Наименование = ОбменСБанкамиСлужебныйКлиентСервер.ПредставлениеПрисоединенногоФайла(
			ДанныеФайла.Наименование, ДанныеФайла.Расширение, ДанныеФайла.Размер);
...
Процедура ДобавитьПрисоединенныйФайлВТаблицу(АдресФайла, ИмяФайла)   
	...
	НовСтрока.Наименование = ОбменСБанкамиСлужебныйКлиентСервер.ПредставлениеПрисоединенногоФайла(
		СтруктураФайла.ИмяБезРасширения, СтруктураФайла.Расширение, Размер);
...
Процедура ОформитьПрисоединенныеФайлы() 
	...
	ОбщийРазмер = ПрисоединенныеФайлыТаблица.Итог("Размер"); 
	Доступно = (МаксимальныйРазмерВложений - Мин(МаксимальныйРазмерВложений,ОбщийРазмер))/1024;
	Всего = МаксимальныйРазмерВложений/1024;

	ЗаголовокПрисоединенныеФайлы = НСтр("ru = 'Файлы (доступно %1 из %2 Кб.)'");
	Элементы.ПрисоединенныеФайлыНаименование.Заголовок = 
		СтрШаблон(ЗаголовокПрисоединенныеФайлы,Формат(Доступно,"ЧДЦ=2"),Формат(Всего,"ЧДЦ=2"));
