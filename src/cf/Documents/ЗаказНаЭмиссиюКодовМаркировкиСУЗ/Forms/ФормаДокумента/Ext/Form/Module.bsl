...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы(Форма)
	...
	Форма.ИспользуютсяНаборы            = ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетНаборы(Объект.ВидПродукции);
	Форма.ИспользуютсяГрупповыеУпаковки = ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(Объект.ВидПродукции);
...
// 
// Параметры:
//  РезультатАвторизации - Неопределено, (см. ИнтерфейсИСМПКлиент.ПараметрыЗавершенияОбработки) - Результат авторизации.
...
Процедура ЗаполнитьДоступныеGTINНаКлиенте(РезультатАвторизации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ИнтерфейсИСМПКлиент.АвторизацияГИСМТВыполненаСОшибкой(РезультатАвторизации) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТребуетсяОбновитьКлючСессии
		...
		ПараметрыАвторизации = ИнтерфейсИСМПКлиент.ПараметрыЗапросаДанных();
		...
		ИнтерфейсИСМПКлиент.ВыполнитьАвторизациюГИСМТ(ПараметрыАвторизации);
...
Функция ПараметрыЗавершенияВводаШтрихкода(ИсходныеДанные, РезультатОбработки, ПараметрыСканирования)
	...
	ПараметрыЗавершенияВводаШтрихкода = ШтрихкодированиеИСКлиент.ПараметрыЗавершенияОбработкиШтрихкода(,,"ОбработатьОчереднойШтрихкодОбработчикОповещения");
...
Процедура ЗаполнитьПустойGTINВСтроках(МассивСтрок)
	...
	ШтрихкодированиеИС.ЗаполнитьПроверяемыеGTIN(ТаблицаНоменклатуры, МассивGTIN, СоответствиеGTIN, ЭтоМаркировкаОстатков);
