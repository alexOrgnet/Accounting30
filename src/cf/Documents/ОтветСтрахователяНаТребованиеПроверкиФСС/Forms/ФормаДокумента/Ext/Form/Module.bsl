...
Процедура СохранитьРезультатОтправки(РезультатОтправки)
	...
	Если НЕ ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.СвойстваОбменаПоСЭДОЧерезОператора(
		...
		ПредыдущийИдентификаторСообщенияОтвета = Неопределено;
		Для каждого Строка из Объект.ПредоставленныеДокументы Цикл
			Если ПредыдущийИдентификаторСообщенияОтвета = Неопределено
				ИЛИ ПредыдущийИдентификаторСообщенияОтвета <> ИдентификаторыСообщенияОтвета[Строка.НомерСтроки] Тогда
				
				ЭлектронныйДокументооборотСФСС.ДобавитьОтправкуФССПоСЭДО(
					Объект.Ссылка,
					ИдентификаторыСообщенияОтвета[Строка.НомерСтроки],
					Объект.Организация,
					Объект.ДатаОтправкиВФСС);
				ПредыдущийИдентификаторСообщенияОтвета = ИдентификаторыСообщенияОтвета[Строка.НомерСтроки];
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
