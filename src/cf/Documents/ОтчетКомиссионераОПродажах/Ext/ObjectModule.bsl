...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЕстьПрослеживаемыйВозвращенныйТовар Тогда
		...
		КонецЕсли;
		Для Каждого СтрокаСТоваром ИЗ ПрослеживаемыйВозвращенныйТовар Цикл
			СтрокаСТоваром.НомерГТД = Неопределено;
		КонецЦикла;
	КонецЕсли; 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураТаблицУСН = Новый Структура("ТаблицаТМЦ, ТаблицаРасчетовПоПриобретению, ТаблицаРасчетовПоРеализации, ТаблицаВознаграждение, ДолгПокупателя",
		ТаблицаСписанныеТовары, ТаблицаВзаиморасчетовВознаграждение, ТаблицаВзаиморасчетовСВозвратом, ПараметрыПроведения.ВознаграждениеТаблица, ТаблицаВзаиморасчетыКомиссияПоПродаже);
	...
	Если Не ПроведениеСервер.ГрупповоеПерепроведение(ЭтотОбъект)
	 Или ПараметрыДействия.ВалютаДоговора <> ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета() Тогда
