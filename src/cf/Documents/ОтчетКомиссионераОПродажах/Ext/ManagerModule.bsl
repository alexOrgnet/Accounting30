...
Функция СоздатьДокументИзСтруктуры(ДанныеСтруктуры, ТекстПрогресса = "") Экспорт
	...
	ИсключатьВыкупы = (НовыйДокумент.Дата <= '20230901') ИЛИ (УчетнаяПолитика.ПлательщикНДС(НовыйДокумент.Организация, НовыйДокумент.Дата));
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("ТаблицаТоваров") Тогда
		...
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			...
			Если ВидДокумента = "Отчет_WB_детальный" Тогда
				Если ИсключатьВыкупы И ОбменСМаркетплейс.ПродажаЧерезВыкупТовара(СтрокаТовара.КодСтраны, СтрокаТовара.НазваниеСтраны) Тогда
				...
				ИначеЕсли Нрег(СтрокаТовара.ВидДвижения) = "возврат"
					...
					Если ЗначениеЗаполнено(СтрокаТовара.ДатаПродажи)
						И НачалоНедели(НовыйДокумент.Дата) <> НачалоНедели(СтрокаТовара.ДатаПродажи) Тогда
		...
		ДокументыРеализацииВозвратов  = НайтиДокументыРеализацииВозвратов(ДатыРеализацииВозвратов, НовыйДокумент.Организация, НовыйДокумент.Контрагент);
...
Функция НайтиДокументыРеализацииВозвратов(ДатыРеализацииВозвратов, Организация, Контрагент)
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("ТаблицаТоваров") Тогда
		...
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			...
			Если ВидДокумента = "Отчет_Ozon_списание"
				Или ОбменСМаркетплейс.СписаниеТовараМаркетплейсом(СтрокаТовара.ВидДвижения, СтрокаТовара.ТипДокумента) Тогда
...
Функция ТекстЗапросаЗачетАвансовНаВозврат(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|			ТОГДА &Ссылка
	|		ИНАЧЕ ТаблицаВозвраты.Сделка
	|	КОНЕЦ" + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|						ИЛИ ОтчетКомиссионераОПродажахПокупатели.Покупатель.Самозанятый
	|					ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.РеализацияРозничная)
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|			ТОГДА ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ТаблицаДокумента.СуммаВключаетНДС
...
Функция ОписаниеСчетаФактуры(СчетФактура) Экспорт
