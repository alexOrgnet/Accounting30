...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	СчетаУчетаВДокументах.ДобавитьПравилоЗаполнения(Правила, "", "СчетУчетаРасчетовСКонтрагентом", "ВыплатыСамозанятым");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ВыплатаСамозанятому");
...
Функция ВидыОперацийСчетаУчета()
	...
	ВидыОпераций.Добавить("ВыплатаСамозанятому");
...
Функция ПолучитьНазначениеПлатежа(Объект, ТолькоСумму = Ложь, ПолучитьСгенерированноеНазначение = Ложь, ФормаДокумента = Неопределено) Экспорт
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатаСамозанятому Тогда
		...
		ТекстНазначение = НСтр("ru = 'Перевод средств самозанятому согласно реестру'");
		ЭтоВыплатыСамозанятым = Истина;
...
Функция ПредставлениеПлатежногоПоручения(ПлатежноеПоручение, ВидОперации, Проведен = Ложь, ЦифровыеДеньги = Ложь) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.СнятиеНаличных
		...
		ИЛИ ЦифровыеДеньги Тогда
		
		НаименованиеДокумента = НСтр("ru = 'Платежное поручение не требуется'");
		
	ИначеЕсли ЗначениеЗаполнено(ПлатежноеПоручение) И ТипЗнч(ПлатежноеПоручение) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
...
Процедура ПодготовитьПараметрыРеквизитыДокумента(Запрос, ПараметрыПроведения, Отказ)
	...
	|	Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникуПоДоговоруПодряда) КАК ЭтоВыплатаПоДоговоруПодряда,
...
Функция ПолучитьТаблицыОплатыДоговоровПодряда(ПараметрыПроведения, ДокументСсылка)
	...
	Если ТаблицаВедомостей.Количество() > 0 Тогда
		...
		ТаблицаВедомостей.ЗаполнитьЗначения(ДанныеДокумента.СчетУчета,                     "СчетУчета");
		ТаблицаВедомостей.ЗаполнитьЗначения(ДанныеДокумента.БанковскийСчет,                "БанковскийСчет");
		ТаблицаВедомостей.ЗаполнитьЗначения(Ложь,                                          "ИспользуетсяЭлектронныйОбмен");
	КонецЕсли;
...
Функция ТекстЗапросаВыплатыСамозанятым(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Не (Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатаСамозанятому
		Или Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым) Тогда
	...
	Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатаСамозанятому Тогда
