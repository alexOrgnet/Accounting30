...
Процедура ОбработкаПроведения(Отказ)
	...
	ПараметрыПроведения = Документы.ОбесценениеОС.ПодготовитьПараметрыПроведения(Ссылка, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	...
	УчетОС.СформироватьДвиженияОбесценениеОС(
		ПараметрыПроведения.ОбесценениеОСТаблица,
		ПараметрыПроведения.ОбесценениеОС,
		Движения, Отказ);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ УчетнаяПолитика.ПрименяетсяФСБУ6(Организация, Дата) И НЕ МежотчетныйПериод Тогда
