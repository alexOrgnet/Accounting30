...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|			ТОГДА ПоступлениеТоваровУслугСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ТаблицаДокумента.СуммаВключаетНДС
...
Функция ТекстЗапросаРасчетыВВалютеТовары()
	...
	|	СуммыТаблицыТовары.СуммаПодтвержденнаяРуб,
	|	СуммыТаблицыТовары.СуммаНДСПодтвержденнаяРуб
...
Функция ТекстЗапросаПоступлениеМПЗИП(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЕстьТопливо Тогда
		...
		НомераТаблиц.Вставить("ИПТаблицаПрочее", НомераТаблиц.Количество());
		ТекстЗапроса = ТекстЗапроса + 
		"ВЫБРАТЬ
		...
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК НоменклатурнаяГруппа,
		|	&СтатьяЗатратРасходыГСМ КАК СтатьяЗатрат,
		|	ВЫРАЗИТЬ(&СтатьяЗатратРасходыГСМ КАК Справочник.СтатьиЗатрат).ВидРасходовНУ КАК ВидРасходовНУ,
		|	ИСТИНА КАК ПринятиеКНалоговомуУчету,
		|	ВЫРАЗИТЬ(&СтатьяЗатратРасходыГСМ КАК Справочник.СтатьиЗатрат).ВидДеятельностиДляНалоговогоУчетаЗатрат КАК ВидДеятельностиДляНалоговогоУчетаЗатрат,
		|	НЕОПРЕДЕЛЕНО КАК Партия
		|ИЗ
		|	ТаблицаТопливо КАК ТаблицаТопливо
		|
...
Функция ТекстЗапросаРасчетыВВалютеОсновныеСредства()
	...
	|	ВременнаяТаблицаОсновныеСредства.Количество,
	|	ВременнаяТаблицаОсновныеСредства.ИнвентарныйНомер,
	|	ВременнаяТаблицаОсновныеСредства.ЕдиницаУчета,
	|	СуммыТаблицыОсновныеСредства.СуммаВзаиморасчетов,
	|	СуммыТаблицыОсновныеСредства.СуммаНДСВзаиморасчетов,
	|	СуммыТаблицыОсновныеСредства.СуммаРуб,
	|	СуммыТаблицыОсновныеСредства.СуммаНДСРуб,
	...
	|	ВременнаяТаблицаОсновныеСредства.ИдентификаторСтроки
	|ПОМЕСТИТЬ ТаблицаОсновныеСредства
	|ИЗ
	|	ВременнаяТаблицаОсновныеСредства КАК ВременнаяТаблицаОсновныеСредства
...
Функция ТекстЗапросаПоступлениеРасходовУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если НЕ УчестьРасходыУСНТовары
		...
		И Не Реквизиты.ЕстьТопливо Тогда
		ПараметрыПроведения.Вставить("УСНТаблицаРасходов", Неопределено);
		Возврат ТекстЗапроса;
	Иначе
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.Контрагенты) Тогда
		...
		КомандаПечати.СписокФорм    = "ФормаСписка,ФормаВыбора,ФормаДокументаОбщая,ФормаДокументаТовары,ФормаДокументаУслуги,ФормаДокументаАренда";
