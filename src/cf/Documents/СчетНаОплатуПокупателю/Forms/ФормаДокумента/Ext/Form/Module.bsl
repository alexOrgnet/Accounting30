...
Процедура ПриОткрытии(Отказ)
	...
	Если ПоказатьЗаполнениеПоДаннымИнтернетМагазина Тогда
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменСИнтернетМагазином")
...
Процедура ПодготовитьКЗаполнениюПоДаннымИнтернетМагазина(ТекущийОбъект)
	...
	Результат = ОбменСИнтернетМагазином.ДозаполнитьПоДаннымИнтернетМагазина(Объект.Ссылка, ЭтотОбъект.УникальныйИдентификатор);
	ПоказатьЗаполнениеПоДаннымИнтернетМагазина = Результат.ТребуетсяДозаполнение;
	ДанныеИнтернетМагазинаДляЗаполнения = Результат;
...
Процедура ДекорацияРеквизитыОрганизацииКонтрагентаНажатие(Элемент)
	...
	ОткрытьФорму("Документ.СчетНаОплатуПокупателю.Форма.ФормаРеквизитыОрганизацииКонтрагента", ПараметрыФормы, ЭтотОбъект);
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Не Копирование Тогда
			...
			ТекущиеДанныеСтроки.СтавкаНДС              = УчетНДСКлиентСервер.СтавкаНДСПользователя(Объект.Дата, ПлательщикНДС);
...
Процедура ТоварыНоменклатураПриИзмененииНаКлиенте(СтрокаТаблицы)
	...
	ДанныеСтрокаТаблицы = Новый Структура("Номенклатура, Количество, Цена, Сумма, СтавкаНДС, СуммаНДС, ЭтоУслуга, СуммаСкидки, ПроцентСкидки, ЕдиницаИзмерения, Артикул, Код");
...
Процедура ЗаполнитьСписокАдресовКонтрагента()
	...
	МассивАдресовДоставкиКонтрагента = УправлениеКонтактнойИнформациейБП.СписокАдресовОбъекта(Объект.Контрагент, Объект.Дата);
	Если МассивАдресовДоставкиКонтрагента.Количество() > 0 Тогда
		...
		Для каждого АдресДоставки Из МассивАдресовДоставкиКонтрагента Цикл
			Элементы.АдресДоставки.СписокВыбора.Добавить(АдресДоставки.Значение, АдресДоставки.Представление);
		КонецЦикла; 
	КонецЕсли;
...
Процедура КонтрагентОбработатьИзменение()
	...
	Если Элементы.АдресДоставки.СписокВыбора.Количество() > 0 Тогда
		Объект.АдресДоставки = Элементы.АдресДоставки.СписокВыбора.Получить(0).Значение;
	Иначе
		Объект.АдресДоставки = "";
	КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ПараметрыОбъекта)
	...
	Иначе
		...
		Иначе
			ИменаРеквизитов = "Наименование, НаименованиеПолное, ПериодичностьУслуги, Услуга, ЕдиницаИзмерения, Артикул, Код";
		КонецЕсли;
		...
		Иначе
			...
			Если ИмяТаблицы = "Товары" Тогда
				...
				Иначе
					...
					НоваяСтрока.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПоУмолчанию(Объект.Дата, ПлательщикНДС);
				КонецЕсли;
...
Процедура ОбработкаВыбораПодборОСНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТаблицыОС Из ТаблицаОС Цикл
		...
		Иначе
			...
			ЭлементКоллекции.СтавкаНДС        = УчетНДСКлиентСервер.СтавкаНДСПоУмолчанию(Объект.Дата, ПлательщикНДС);
...
Процедура ПоказатьНесопоставленныеОбъектыНаКлиенте()
	...
	Если ДанныеИнтернетМагазинаДляЗаполнения.ВсеДанныеСопоставленыАвтоматически Тогда
		ОбработатьОповещениеОЗаполненииДанныхИнтернетМагазина(ДанныеИнтернетМагазинаДляЗаполнения.ДанныеСопоставленныеАвтоматически);
	Иначе
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ДокументСсылка", Объект.Ссылка);
		ПараметрыФормы.Вставить("АдресХранилища", ДанныеИнтернетМагазинаДляЗаполнения.ХранилищеДанныхДляСопоставления);
...
Процедура ОбработатьОповещениеОЗаполненииДанныхИнтернетМагазина(Параметры)
	...
	Если ЗначениеЗаполнено(Параметры.Номенклатура) Тогда
		...
		Для Каждого СоответствиеНоменклатуры из Параметры.Номенклатура Цикл
			СтрокаТаблицыТовары = Объект.Товары.НайтиПоИдентификатору(СоответствиеНоменклатуры.Ключ);
	...
	Если Параметры.Свойство("ВсеДанныеСопоставленыАвтоматически")
		И Параметры.ВсеДанныеСопоставленыАвтоматически Тогда
		Записать();
...
Процедура СнятьВыделениеЭлементаПодсказкиНаФорме() Экспорт
	...
	Если ВыделенныйЭлементПодсказки = "ЗагрузитьИзФайла"
		Или ВыделенныйЭлементПодсказки = "ИзменитьТовары" Тогда
		Элемент = Элементы[ВыделенныйЭлементПодсказки];
