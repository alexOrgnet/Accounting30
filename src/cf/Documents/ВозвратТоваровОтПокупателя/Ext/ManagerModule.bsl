...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ДанныеКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Реквизиты.Контрагент, "ЮридическоеФизическоеЛицо, ИНН, Самозанятый");
...
Функция СоздатьДокументПоОтчетуЯндекса(АдресХранилища, ПараметрыДокумента, Период) Экспорт
	...
	Для Каждого СтраницаОтчета Из ДанныеФайла Цикл
		Если СтраницаОтчета.ВидДокумента = ВидДокумента Тогда
	...
	Иначе
		...
		НовыйДокумент.ВидОперации = ?(ВидДокумента = "Яндекс_Невыкуплено",
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|	СведенияПрослеживаемостиДоПосле.СуммаВозвращено КАК СуммаВозвращено,
	...
	|	СведенияПрослеживаемостиДоПосле.СуммаВозвращено,
	...
	|	ВЫБОР
	|		КОГДА СведенияОПрослеживаемостиПредварительная.СуммаВозвращено = 0
	|			ТОГДА СведенияОПрослеживаемостиПредварительная.КоличествоТовараПослеИзменения * ТаблицаДокумента.ЦенаПослеИзменения
	|		ИНАЧЕ СведенияОПрослеживаемостиПредварительная.СуммаВозвращено
