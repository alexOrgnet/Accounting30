...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если ИспользоватьРасчетПервойПоловиныМесяца Тогда
			...
			Иначе
				ДатыВыплаты = РасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатыВыплатыОрганизации(Объект.Организация, Объект.Дата);
				Если Объект.Дата > ДобавитьМесяц(ДатыВыплаты.Зарплата, -1) Тогда
					Объект.МесяцНачисления = НачалоМесяца(Объект.Дата);
					Объект.ПерваяПоловинаМесяца = КонецДня(Объект.Дата) <> КонецМесяца(Объект.Дата);
				КонеЦесли;
			КонеЦесли;
		КонеЦесли;
...
Процедура ДобавитьНачисление(Команда)
	...
	ДатаВыплатыДохода = КонецМесяца(Объект.МесяцНачисления);
	Если Объект.ПерваяПоловинаМесяца Тогда
		ДатаВыплатыДохода = ДатаВыплаты;
	КонецЕсли;
	...
	ПараметрыВвода.Вставить("ДатаВыплатыДохода", ДатаВыплатыДохода);
...
Процедура ОбработатьИзменениеМесяцНачисленияНаСервере()
	...
	ПрочитатьДатуВыплаты();
	Если Объект.ПерваяПоловинаМесяца Тогда
		Объект.Дата = НачалоДня(ДатаВыплаты);
	Иначе
...
Процедура ПоказатьВсеНачисления(ПараметрыВвода)
	...
	ПараметрыОтбора.Вставить("КатегорияНачисленияИлиНеоплаченногоВремени", Категории);
	ПараметрыОтбора.Вставить("КатегорияДохода", КатегорииДохода);
...
Процедура ПереместитьСтроку(Направление)
	...
	Если НЕ ПоддержкаСовместительства Тогда
...
Процедура ЗаполнитьКнопкиПодменю()
	...
	КатегорииДоходов = РасчетЗарплатыДляНебольшихОрганизаций.КатегорииДоходовНДФЛПриАУСН(Объект.Организация, Объект.МесяцНачисления);
...
Функция АдресПараметровВХранилище(Сотрудник, ФизическоеЛицо, ИменаТаблиц)
	...
	ПараметрыРасчета.Вставить("ДатаВыплатыДохода",                      ДатаВыплатыДохода);
	...
	ПараметрыРасчета.Вставить("Договоры",                               Объект.НачисленияПоДоговорам.Выгрузить(Объект.Начисления.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник))));
