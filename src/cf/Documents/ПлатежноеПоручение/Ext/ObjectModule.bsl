...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым Тогда
		...
		Если СуммаРеестра <> СуммаДокумента Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "СуммаДокумента", "Объект", Отказ);
		КонецЕсли;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатаСамозанятому Тогда
...
Процедура ПроверитьОбновитьРеквизитыПлатежаВБюджет(ОбъектКопирования)
	...
	НалогиУплачиваемыеОтдельно = ЕдиныйНалоговыйСчетПовтИсп.МассивДоступныхВидовНалогов();
...
Процедура ЗаполнитьДокументПоВедомости(Основание)
	...
	Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникуПоДоговоруПодряда Тогда
		ОчередностьПлатежа = УчетДенежныхСредствКлиентСервер.ОчередностьПлатежаПоУмолчанию();
		РеквизитыПолучателяПлатежа = РегистрыСведений.ОсновныеВариантыПеречисленияСотруднику.ПолучитьОсновнойВариантПеречисления(
			Новый Структура("Организация, ФизическоеЛицо, ВидОперации, БанковскийСчет",
				Организация, Контрагент, ВидОперации, СчетКонтрагента));
		СчетКонтрагента = РеквизитыПолучателяПлатежа.БанковскийСчет;
	Иначе
