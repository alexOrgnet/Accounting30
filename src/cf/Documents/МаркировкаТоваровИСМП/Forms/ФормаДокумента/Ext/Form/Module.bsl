...
Процедура ОперацияПриИзменении(Элемент)
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Объект.ВидПродукции) Тогда
	...
	Действия.Вставить("СнятьФлагОтчетПроизводственнойЛинии",
		...
		И Не ИнтеграцияИСКлиентСервер.ЭтоПродукцияМОТП(Объект.ВидПродукции));
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПроверки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.НормализованноеЗначениеШтрихкодаУпаковки)
			...
			Если КэшДанныхЗапроса[СтрокаТаблицы.НормализованноеЗначениеШтрихкодаУпаковки] = Неопределено Тогда
				...
				СтруктураЗначения = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(СтрокаТаблицы.НормализованноеЗначениеШтрихкодаУпаковки, ВидМаркируемойПродукции);
		...
		Если (ПолучитьСтатусыВсехКодов
			...
			СтруктураЗначения = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(СтрокаТаблицы.НормализованноеЗначениеШтрихкода, ВидМаркируемойПродукции);
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	ВидПродукцииИспользуетНаборы = ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетНаборы(ВидПродукции);
...
Процедура ОбработатьКодМаркировки(ИсходныеДанные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ШтрихкодированиеИСКлиентСервер.ДекодироватьШтрихкодДанныхBase64(ИсходныеДанные);
...
Процедура ЗаполнитьДоступныеОперации()
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Объект.ВидПродукции, Ложь, Истина, Ложь) Тогда
...
Функция ЗаполнитьКоличествоПоДаннымОтчетаНаСервере(КОпределениюВидаУпаковки = Неопределено)
	...
	ОписанияНоменклатуры = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(Номенклатура);
...
Процедура ОбработатьПолученныеШтрихкодыНаСервере(ШтрихКоды, ПараметрыСканирования)
	...
	Для Каждого ДанныеШтрихкода Из ШтрихКоды Цикл
		...
		ВозможнаГрупповаяУпаковка = ШтрихкодированиеИСКлиентСервер.ВозможнаГрупповаяУпаковкаИлиНабор(
...
Функция КлючСвязиСтатусаСтрок(Форма, Номенклатура = Неопределено)
	...
	Если ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(Форма.Объект.Операция) <> Неопределено
		...
		Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Форма.Объект.ВидПродукции) Тогда
...
Процедура ЗаполнитьДанныеВЕТИСПоСтроке(Форма, ДобавленныеСтроки)
	...
	Если Не ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Форма.Объект.ВидПродукции)
...
Процедура ЗаполнитьСлужебныеРеквизитыВЕТИСВКоллекции()
	...
	Если Не ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Объект.ВидПродукции)
...
Функция ИспользуетсяУказаниеИдентификатораВЕТИС(Форма)
	...
	Если Форма.ИспользуетсяПодсистемаВЕТИС
		И ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Форма.Объект.ВидПродукции)
