...
Функция ПоследовательностьОпераций(ДокументСсылка, ЛинейныйСписок = Ложь) Экспорт
	...
	Если ЛинейныйСписок
		...
		Иначе
			...
			ИнтеграцияИС.ДобавитьОперациюВПоследовательность(Таблица, 106,
				Исходящий,
...
Функция ВводВОборотJSON(ДокументСсылка, ДополнительныеПараметры, Повторно = Ложь)
	...
	Если Не (ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции, Ложь, Истина)
...
Функция ОтчетОНанесенииШтрихкодыУпаковокJSON(РезультатЗапроса, ДополнительныеПараметры, Повторно)
	...
	ЭтоПродукцияПодконтрольнаяВЕТИС = ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Шапка.ВидПродукции);
...
#КонецОбласти
	...
	Если ЭтоПиво Или ЭтоБАПиво Тогда
		...
		Если ЗначениеЗаполнено(Шапка.Номенклатура) Тогда
			...
			Если Объем <> 0 Тогда
				ОписанияНоменклатуры = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(Шапка.Номенклатура);
...
Процедура ЗаполнитьДанныеОбъектаФормыПоДеревуУпаковок(Форма, ДеревоУпаковок, ПараметрыЗаполнения,
	...
	ЭтоПродукцияПодконтрольнаяВЕТИС = ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Форма.Объект.ВидПродукции);
...
Процедура ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыЗаполнения)
	...
	КонецЕсли;
	Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(ВидПродукции)
		...
		Если СтрокаИсточникаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			...
			Если Не ЭтоМаркировкаОстатков И Настройки.УказыватьСертификацию
				...
				Если Настройки.УказыватьСуммы И ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво Тогда
					...
					КонецЦикла;
					ХэшДанныхСертификации = ИнтеграцияИС.ХэшСуммаСтроки(СтрСоединить(МассивСтрок));
...
Функция ЗаполнитьДанныеСтрокиВводВОборотПоДаннымОтчета(СтрокаИсточникаДанных, СведенияОТоварах, Настройки)
	...
	Если СтрокаИсточникаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
		...
		Если Настройки.УказыватьИдентификаторПроисхожденияВЕТИС
			И ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(СведенияОТоварах.ВидПродукции) Тогда
...
Функция ПроверитьИРазбитьНаГруппыПолныеКодыМаркировки(СообщенияJSON, СообщениеJSON, ГруппыКодов,
	...
	Для Каждого СтрокаКодМаркировки Из ПолныеКодыМаркировки Цикл
		...
		Иначе
			...
			ПолныйКодМаркировки = ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(СтрокаКодМаркировки.ПолныйКодМаркировки);
...
#КонецОбласти
	...
	Для Каждого ДанныеСообщения Из ГруппыКодов Цикл
		...
		ТекстСообщенияJSON = ИнтерфейсМОТПСлужебный.ОбъектВТекстJSON(ТелоЗапросаТекущейГруппы, Истина);
...
Функция СообщенияJSONОтчетОНанесении(Шапка, ДополнительныеПараметры)
	...
	ЭтоПродукцияМОТП     = ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(Шапка.ВидПродукции);
...
Функция ЗаводскойСерийныйНомерЗаполнен(ПолныеКодыМаркировки)
	...
	Для Каждого СтрокаТЧ Из ПолныеКодыМаркировки Цикл
		Штрихкод = ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(СтрокаТЧ.ПолныйКодМаркировки);
