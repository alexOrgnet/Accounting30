...
Функция ПоследовательностьОпераций(ДокументСсылка) Экспорт
	...
	ИнтеграцияЕГАИС.ДобавитьОперациюВПоследовательность(Таблица, 0, Исходящий, Операция,                 ДокументСсылка);
	ИнтеграцияЕГАИС.ДобавитьОперациюВПоследовательность(Таблица, 0, Входящий,  Перечисления.ВидыДокументовЕГАИС.УведомлениеОРегистрацииДвиженияОтчетаОбИмпортеПроизводстве);
	ИнтеграцияЕГАИС.ДобавитьОперациюВПоследовательность(Таблица, 0, Исходящий, Перечисления.ВидыДокументовЕГАИС.ЗапросНаОтменуПроведенияОтчетаОПроизводстве, ДокументСсылка);
...
#Область РазборКодовМаркировки
		...
		ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(Шапка.Ссылка, Неопределено, Перечисления.ВидыПродукцииИС.Пиво);
		...
		Если Не КодыМаркировкиНормализованы Тогда
			...
			Если РезультатСохраненияДанныхОтчета.ЕстьОшибки Тогда
				...
				СообщениеXML.Описание = ИнтеграцияЕГАИС.ОписаниеОперацииПередачиДанных(
					Шапка.Операция, ДокументСсылка);
				ИнтеграцияЕГАИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, РезультатСохраненияДанныхОтчета.ТекстОшибки);
				СообщенияXML.Добавить(СообщениеXML);
				Возврат СообщенияXML;
			КонецЕсли;
		КонецЕсли;
...
#КонецОбласти
	...
	Если Не Шапка.ОтчетПроизводственнойЛинии Тогда
		...
		Для Каждого СтрокаТЧ Из ВложенныеШтрихкодыУпаковок.МаркированныеТовары Цикл
			...
			Если НайденнаяСтрока.Количество() = 0 Тогда
				ИнтеграцияЕГАИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, НСтр("ru = 'Ошибка при связывании кодов маркировки и номенклатуры'"));
			КонецЕсли;
	...
	ОтчетXDTO = ИнтеграцияЕГАИС.ОбъектXDTO(ПространствоИмен, "RepProducedType_v5");
	
	ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ОтчетXDTO, "Identity", Шапка.Идентификатор, СообщениеXML, 3);
	
	Header = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(ОтчетXDTO, "Header");
	ОтчетXDTO.Header = Header;
	
	ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Header, "Number",       СокрЛП(Шапка.Номер),      СообщениеXML);
	ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Header, "Date",         Шапка.Дата,               СообщениеXML);
	ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Header, "ProducedDate", Шапка.ДатаПроизводства,   СообщениеXML);
	ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Header, "Type",         Операция(Шапка.Операция), СообщениеXML);
	ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Header, "Note",         Шапка.Комментарий ,       СообщениеXML);
	
	ИнтеграцияЕГАИС.ЗаполнитьВXDTOОрганизацию_v2(Header, "Producer", Шапка, "Производитель", СообщениеXML);
	
	ОтчетXDTO.Content = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(ОтчетXDTO, "Content");
	НомерСтрокиСырье = 0;
	...
	Для Каждого СтрокаТЧ Из ТоварыИтоги Цикл
		...
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Identity",    НомерСтроки,                   СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Quantity",    СтрокаТЧ.Количество,           СообщениеXML);
		
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "ProductCode",   Детали.АлкогольнаяПродукция, СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Comment1",      Детали.Комментарий1,         СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Comment2",      Детали.Комментарий2,         СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Comment3",      Детали.Комментарий3,         СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Party",         Детали.НомерПартии,          СообщениеXML);
		Если ЗначениеЗаполнено(Детали.Крепость) Тогда
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "alcPercent",    Детали.Крепость,    СообщениеXML);
		Иначе
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "alcPercentMin", Детали.КрепостьОт,  СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "alcPercentMax", Детали.КрепостьДо,  СообщениеXML);
		КонецЕсли;
		...
		Если СтрокаТЧ.КодыМаркировки.Количество() Тогда
			...
			Для Каждого КодАкцизнойМарки Из СтрокаТЧ.КодыМаркировки Цикл
				ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position.MarkInfo, "ms", КодАкцизнойМарки, СообщениеXML);
			КонецЦикла;
		...
		Если СырьеСтроки.Количество() Тогда
			Position.ContentResource = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(Position, "ContentResource");
			Для Каждого СтрокаТЧСырье Из СырьеСтроки Цикл
				НомерСтрокиСырье = НомерСтрокиСырье + 1;
				Resource = ИнтеграцияЕГАИС.ОбъектXDTO(ПространствоИмен, "UsedResourceType");
				ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Resource, "IdentityRes", НомерСтрокиСырье,            СообщениеXML);
				ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Resource, "Quantity",    СтрокаТЧСырье.Количество,    СообщениеXML);
				ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Resource, "RegForm2",    СтрокаТЧСырье.НомерСправки2, СообщениеXML);
				
				Resource.Product = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(Resource, "Product");
	...
	КонецЦикла;
	ТекстСообщенияXML = ИнтеграцияЕГАИС.ОбъектXDTOВXML(ОтчетXDTO, Шапка.ИдентификаторФСРАР, ПространствоИмен, ИмяТипа);
