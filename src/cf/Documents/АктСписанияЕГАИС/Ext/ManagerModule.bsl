...
Функция АктСписанияЕГАИСXML(ДокументСсылка)
	...
	Если Шапка.Операция = Перечисления.ВидыДокументовЕГАИС.АктСписанияИзРегистра1 Тогда
		...
		Иначе
			...
			АктXDTO = ИнтеграцияЕГАИС.ОбъектXDTO(ПространствоИмен, "ActWriteOffType_v4");
			...
			Для Каждого СтрокаТЧ Из ТоварыИтоги Цикл
				...
				ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Identity", Формат(НомерСтроки, "ЧГ=0"), СообщениеXML, 5);
				ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position, "SumSale", Сумма, СообщениеXML);
				
				Position.writeoffvolume = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(Position, "writeoffvolume");
				ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position.writeoffvolume, ИмяПоляКоличество, Количество, СообщениеXML);
				...
				Если СтрокаТЧ.АкцизныеМарки.Количество() > 0 Тогда
					Position.MarkCodeInfo = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(Position, "MarkCodeInfo");
					Для Каждого АкцизнаяМарка Из СтрокаТЧ.АкцизныеМарки Цикл
						ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Position.MarkCodeInfo, "amc", АкцизнаяМарка, СообщениеXML);
					КонецЦикла;
				КонецЕсли;
