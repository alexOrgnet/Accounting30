...
Процедура РазбитьСтрокуЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт
	...
	Если НоваяСтрока <> Неопределено Тогда
		...
		ОбновитьДанные(ТекущиеДанные.Количество, Новый Структура(
			"ИмяПоля, ИдентификаторСтроки",
			"ТоварыКоличество", ИдентификаторИсходнойСтроки));
...
Функция ЭлеваторПоГрузополучателю(Грузополучатель)
	...
	ИННКПП = ИнтеграцияЗЕРНОВызовСервера.ИННКПППоОрганизацииКонтрагенту(Грузополучатель);
