...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "СчетРасчетов",         "Контрагент");
...
Функция ТаблицаСделокДляВозврата(ДокументСсылка, Дата)
	...
	|ИЗ
	|	ВТ_ДокументыПродажи КАК ВТ_ДокументыПродажи
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВозвратыТоваров.Обороты(, &конДата, Регистратор, ) КАК ВозвратыТоваровОбороты
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ПрослеживаемаяРеализацияОтображаетсяВДекларацииНДС",
		Реквизиты.ЕстьНДСТовары И Не ПрименяетсяОсвобождениеОтУплатыНДС И Не Реквизиты.ПрименяетсяУСН);
	...
	Если Реквизиты.ЕстьВозвраты Тогда
		Запрос.УстановитьПараметр("ОснованияВозвратов", ТаблицаСделокДляВозврата(ДокументСсылка, Реквизиты.Период));
	КонецЕсли; 
...
Функция ТекстЗапросаВозвратыОплаты(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Иначе
		...
		ТекстЗапроса = ТекстЗапроса + СтрСоединить(МассивЗапросов, Символы.ПС + "Объединить все"+Символы.ПС) + 
	...
	// Если возвраты только наличными, то 62.Р не используется и проводки делаются в корреспонденции со счетом выручки
	Если НЕ Реквизиты.ЕстьВозвратыПредоплаты И Не Реквизиты.ЕстьБезналичныеВозвратыОплаты Тогда 
	...
	Иначе
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(ВложенныйЗапрос.СуммаРуб) > 0" + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ВложенныйЗапрос", СтрСоединить(МассивТекстЗапроса, Символы.ПС + " ОБЪЕДИНИТЬ ВСЕ " + Символы.ПС));
	КонецЕсли;
...
Функция ТекстЗапросаБезналичнаяОплата(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Иначе
		...
		+ "ВЫБРАТЬ
		  ...
		  |	ЛОЖЬ КАК УчетАгентскогоНДС,
		  |	ВЫБОР
		  |		КОГДА ТаблицаОплата.ТипОплаты В (ЗНАЧЕНИЕ(Перечисление.ТипыОплат.ПодарочныйСертификатСобственный), ЗНАЧЕНИЕ(Перечисление.ТипыОплат.ПодарочныйСертификатСторонний), ЗНАЧЕНИЕ(Перечисление.ТипыОплат.РасчетыСРозничнымиПокупателями))
