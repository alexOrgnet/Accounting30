...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|			ТОГДА ВозвратТоваровПоставщикуСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ТаблицаДокумента.СуммаВключаетНДС
...
Функция ПодготовитьТаблицуНДСВключенВСтоимость(ТаблицаТоварыНДС, ТаблицаСписанныеТовары, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если РаздельныйУчетНДСНаСчете19 Тогда
		Возврат ТаблицаСписанныеПартииНДС.Скопировать(Новый Структура("НовыйСпособУчетаНДС", Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости));
	Иначе
