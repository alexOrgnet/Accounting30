...
Функция СоздатьДокументИзСтруктуры(ДанныеСтруктуры, ТекстПрогресса = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.ДоговорКонтрагента)
		...
		РаботаСДоговорамиКонтрагентовБП.УстановитьДоговорКонтрагента(ДанныеЗаполнения.ДоговорКонтрагента,
			...
			МассивВидовДоговоров, ДополнительныеПараметры);
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ДанныеЗаполнения.ДоговорКонтрагента)
			...
			НовыйДоговор.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПоУмолчанию(
...
Функция ПодготовитьСтруктуруТаблицИП(ТаблицаРеквизиты, ТаблицаВзаиморасчеты) Экспорт
	...
	Если Не ПлательщикНДФЛ Или Не ЕстьАвансыОплаченныеПлатежнойКартой(ТаблицаВзаиморасчеты) Тогда
		СтруктураВозврата.Вставить("ТаблицаВзаиморасчетыИП",
			ПодготовитьПараметрыСтруктурыТаблицыВзаиморасчетыИП(ТаблицаВзаиморасчеты));
		Возврат СтруктураВозврата;
	КонецЕсли;
	...
	ДокументыАвансовПоПлатежнымКартам = УчетВзаиморасчетов.ДокументыОплатыЧерезПосредника(Взаиморасчеты);
	...
	ЭлементБлокировки.ИсточникДанных = ДокументыАвансовПоПлатежнымКартам;
	...
	Если ТаблицаОстатковПрочихРасчетов.Количество() > 0 Тогда
		...
		СуммаАвансаПлатежнойКартой = 0;
		Для Каждого СтрокаТаблицы Из Взаиморасчеты Цикл
			Если ТипЗнч(СтрокаТаблицы.ДокументРасчетов) = Тип("ДокументСсылка.ОплатаПлатежнойКартой") Тогда
				СуммаАвансаПлатежнойКартой = СуммаАвансаПлатежнойКартой + СтрокаТаблицы.СуммаРуб;
			КонецЕсли
		КонецЦикла;
		СуммаАвансаПлатежнойКартой = Мин(СуммаАвансаПлатежнойКартой, СуммаРаспределения);
		
		Если СуммаАвансаПлатежнойКартой > 0 Тогда 
			...
			ТаблицаТолькоНеоплаченныхОстатков.Сортировать("ДатаДокументаРасчетов, РасчетныйДокумент", Новый СравнениеЗначений);
			
			Для Каждого СтрокаТаблицы Из ТаблицаТолькоНеоплаченныхОстатков Цикл
				
				Если ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) <> Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет") Тогда
					Продолжить;
				КонецЕсли;
				...
				СуммаДвижения = Мин(СуммаАвансаПлатежнойКартой, СтрокаТаблицы.СуммаОстаток);
				...
				СуммаАвансаПлатежнойКартой = СуммаАвансаПлатежнойКартой - СуммаДвижения;
				СуммаРаспределения = СуммаРаспределения - СуммаДвижения;
		...
		Если СуммаРаспределения > 0 Тогда
			...
			Для Каждого СтрокаТаблицы Из ТаблицаВзаиморасчеты Цикл
				...
				Для Каждого ЭлементМассива Из НайденныеСтроки Цикл
					...
					СуммаРаспределения = СуммаРаспределения - СуммаДвижения;
					СуммаДляЗакрытия = СуммаДляЗакрытия - СуммаДвижения;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС",
		Реквизиты.ЕстьНДС И Не ПрименяетсяОсвобождениеОтУплатыНДС И Не Реквизиты.ПрименяетсяУСН);
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц, ЭтоОтложенноеПроведение)
	...
	|	Реквизиты.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	|	Реквизиты.Перевозчик КАК Перевозчик,
	|	ЕСТЬNULL(СпособыДоставки.ДоговорКонтрагента, НЕОПРЕДЕЛЕНО) КАК ПеревозчикДоговор,
	|	ЕСТЬNULL(СпособыДоставки.ОплатаПриПолучении, ЛОЖЬ) КАК ОплатаПриПолучении,
	|	ЕСТЬNULL(СпособыДоставки.СчетУчетаРасчетов, НЕОПРЕДЕЛЕНО) КАК СчетРасчетовСПеревозчиком,
...
Функция ДанныеРазделаРабот(ТаблицаРаздела, ТаблицаСметы, СсылкаНаДокумент, СтрокаДанныхПечати)
	...
	Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
		...
		СтрокаДанныхРаздела.РаботыНаименование = ?(ЗначениеЗаполнено(СтрокаТаблицы.НоменклатураНаименованиеПолное), 
...
Функция ПодготовитьТаблицуПереносаЗадолженности(ТаблицаВзаиморасчетов, ТаблицаРеквизитыПереносЗадолженности, Отказ) Экспорт
	...
	Для каждого СтрокаВзаиморасчетов Из ТаблицаВзаиморасчетовПереносЗадолженности Цикл
		...
		СтрокаТаблицы.Содержание           = НСтр("ru = 'Оплата принята курьером транспортной организации'");
...
Функция ТекстЗапросаРеализованнаяНоменклатураПоСчету() Экспорт
	...
	|			ИЛИ ВТ_РеализацииПоСчету.СчетНаОплату = &СчетНаОплату)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
