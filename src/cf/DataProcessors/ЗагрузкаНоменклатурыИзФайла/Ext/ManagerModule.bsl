...
Процедура ПроизвестиПоискНоменклатуры(Таблица, ЕстьКолонкаЕдиницаИзмерения)
	...
	Запрос = ЗапросПоискаНоменклатуры(Таблица);
...
Функция ЗапросПоискаНоменклатуры(Таблица) Экспорт
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Таблица.НомерСтроки КАК НомерСтроки,
	|	ЕСТЬNULL(НоменклатураШтрихкод.Ссылка, ЕСТЬNULL(НоменклатураПолноеНаименование.Ссылка, НоменклатураКраткоеНаименование.Ссылка)) КАК Номенклатура,
	|	ЕСТЬNULL(НоменклатураШтрихкод.ЕдиницаИзмерения, ЕСТЬNULL(НоменклатураПолноеНаименование.ЕдиницаИзмерения, НоменклатураКраткоеНаименование.ЕдиницаИзмерения)) КАК ЕдиницаИзмерения,
	|	ЕСТЬNULL(НоменклатураШтрихкод.ВидСтавкиНДС, ЕСТЬNULL(НоменклатураПолноеНаименование.ВидСтавкиНДС, НоменклатураКраткоеНаименование.ВидСтавкиНДС)) КАК ВидСтавкиНДС,
	|	ВЫБОР
	|		КОГДА НоменклатураШтрихкод.Ссылка ЕСТЬ NULL
	|				И ЕСТЬNULL(НоменклатураКраткоеНаименование.НаименованиеПолное, """") = ЕСТЬNULL(НоменклатураПолноеНаименование.НаименованиеПолное, """")
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ + ВЫБОР
	|		КОГДА Таблица.Артикул = ЕСТЬNULL(НоменклатураШтрихкод.Артикул, ЕСТЬNULL(НоменклатураПолноеНаименование.Артикул, НоменклатураКраткоеНаименование.Артикул))
	|			ТОГДА 2
	|		ИНАЧЕ 0
	|	КОНЕЦ + ВЫБОР
	|		КОГДА Таблица.ЕдиницаИзмерения = ЕСТЬNULL(НоменклатураШтрихкод.ЕдиницаИзмерения, ЕСТЬNULL(НоменклатураПолноеНаименование.ЕдиницаИзмерения, НоменклатураКраткоеНаименование.ЕдиницаИзмерения))
	|			ТОГДА 2
	|		ИНАЧЕ 0
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК РегистрШтрихкодыНоменклатуры
	|		ПО Таблица.Штрихкод = РегистрШтрихкодыНоменклатуры.Штрихкод
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК НоменклатураШтрихкод
	|		ПО (РегистрШтрихкодыНоменклатуры.Номенклатура = НоменклатураШтрихкод.Ссылка)
	|			И (НЕ НоменклатураШтрихкод.ЭтоГруппа)
	|			И (НЕ НоменклатураШтрихкод.ПометкаУдаления)
	|ГДЕ
	...
	|			ИЛИ НЕ НоменклатураШтрихкод.Ссылка ЕСТЬ NULL)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Таблица", ТаблицаДляПроверки);
