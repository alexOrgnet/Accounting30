...
Процедура ПолучитьПоказателиДляРасчетаНалога()
	...
	СоставРазделов = Новый Структура("Операции, Расчет, Уведомление, Уплата",
		НЕ АвтоматическийПомощник, АвтоматическийПомощник, ПлательщикЕНП, Истина);
...
Процедура ОпределитьСвязанныеПравила()
	...
	Иначе
		ПравилоУплатыВыполняетсяЕдинымПомощником = УчетНДСПереопределяемый.ВедетсяПростойУчетЕНП(Срок);
	КонецЕсли;
