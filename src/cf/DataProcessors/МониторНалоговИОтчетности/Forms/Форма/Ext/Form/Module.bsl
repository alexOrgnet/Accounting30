...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОгобработка)
	...
	Элементы.НужноОплатить.Гиперссылка = ПомощникиПоУплатеНалоговИВзносов.ПоказыватьКомандыОплаты();
	
	Элементы.ОстатокЕНС.Гиперссылка = Не ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком();
...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПополнитьЕНС" Тогда
...
&НаКлиенте
Процедура НужноОплатитьНажатие(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура Подключаемый_ПлатежЕНСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыПлатежныхДокументов = ПомощникиПоУплатеНалоговИВзносовКлиент.ПараметрыОбработкиПлатежныхДокументов(
		ПлатежиЕНС, "ПлатежЕНС", ОповещениеУдаленияПлатежногоДокумента());
...
Процедура РазместитьВИнтерфейсе()
	...
	Если УпрощеннаяНулеваяОтчетность Тогда
		Элементы.Шапка.Ширина = 82;
	Иначе
		Элементы.Шапка.Ширина = 104;
	КонецЕсли;
	...
	ЗаполнитьБлокЕНС(ДанныеЕНС);
	
	// Формирование.
	РядыБлоков = Монитор.СформироватьРядыБлоков(ВключенныеЗадачи, НалоговыеИндикаторы, ОтчетныеИндикаторы);
	
	Элементы.Ряд0.Видимость = НЕ УпрощеннаяНулеваяОтчетность;
	Элементы.РядЕНС.Видимость = НЕ УпрощеннаяНулеваяОтчетность;
...
&НаСервере
Функция ЗаполнитьБлокЕНС(ДанныеЕНС)
	...
	ПлатежиЕНС.Загрузить(ДанныеЕНС.ПлатежиЕНС);
	ВывестиПлатежиНаФорму();

	Если ДанныеЕНС.НужноОплатить = 0 Тогда
		Элементы.РядЕНС.ЦветФона = ЦветЗеленыйБлок;
		Элементы.СтрокаОплатаНеТребуется.Видимость = Истина;
		Элементы.НужноОплатить.Видимость = Ложь;
	Иначе
		Элементы.РядЕНС.ЦветФона = ЦветЖелтыйБлок;
		Элементы.СтрокаОплатаНеТребуется.Видимость = Ложь;
		Элементы.НужноОплатить.Видимость = Истина;
	КонецЕсли;
...
Процедура Подключаемый_ПроверитьВыполнениеАктуализации()
	...
	Если СтатусЗадания = "Выполнено" Тогда
		...
		Иначе
			...
			ИдентификаторЗаданияАктуализации = Неопределено;
		КонецЕсли;
	ИначеЕсли СтатусЗадания = "Выполняется" Тогда
	...
	ИначеЕсли СтатусЗадания = "Отменено" Тогда
		ОтключитьОбработчикОжидания("Подключаемый_ПроверитьВыполнениеАктуализации");
		ЗаданиеЗапущеноВДругойФорме = Ложь;
		ИдентификаторЗаданияАктуализации = Неопределено;
		АктуализироватьДанные();
...
Процедура ОбработатьСсылкуНаНалогОтчет(Элемент, АдресХранилища)
	...
	Если ТипЗнч(ОписаниеДействия) = Тип("Структура") Тогда
		Если ОписаниеДействия.ИмяФормы = "Обработка.МониторНалоговИОтчетности.Форма.ФормаРасшифровкаСписанияЕНС" Тогда
			ОткрытьФорму("Обработка.МониторНалоговИОтчетности.Форма.ФормаРасшифровкаСписанияЕНС",
				ОписаниеДействия.ПараметрыФормы,,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
...
Процедура СоздатьДокументУплатыНалога()
	...
	Если НужноОплатить = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеДействияПлатеж = ОписаниеДействияДляНовогоДокументаУплатыНалога(Организация,
		НужноОплатить, СпособУплатыНалога, ТекущийДень);
...
Функция ОписаниеДействияДляНовогоДокументаУплатыНалога(Организация, Сумма, СпособУплатыНалога, ПериодСобытия)
	...
	СтруктураРеквизитовУплатыНалога = ВыполнениеЗадачБухгалтера.НовыйСтруктураРеквизитовУплатыНалогаЕНС();
	...
	Возврат ВыполнениеЗадачБухгалтера.ОписаниеДействияПлатежЕНС(СтруктураРеквизитовУплатыНалога);
	
КонецФункции

&НаСервере
Процедура ВывестиПлатежиНаФорму()
	
	ПомощникиПоУплатеНалоговИВзносов.ОтобразитьПлатежи(ЭтотОбъект, ЭтотОбъект.ПлатежиЕНС, "ПлатежЕНС");
