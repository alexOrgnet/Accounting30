...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("НачалоРаботы") Тогда
		ЗаполнятьРеквизитыОрганизации = Параметры.НачалоРаботы;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзмененоСостояниеИнтеграцииСПлатформойСамозанятые"
		...
		ОпросПлатформы = Ложь;
		Если ЗаполнятьРеквизитыОрганизации И СостояниеПодключения = ПредопределенноеЗначение(
			...
			ПодключитьОбработчикОжидания("Подключаемый_ОбновитьРеквизитыНалогоплательщика", 10, Истина);
		ИначеЕсли ЗаполнятьРеквизитыОрганизации И СостояниеПодключения = ПредопределенноеЗначение(
			"Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.ЗапросОтправлен") Тогда
			ПодключитьОбработчикОжидания("Подключаемый_ОбновитьСостояниеИнтеграцииСПлатформойСамозанятые", 10, Истина);
		Иначе
	...
	Если ОбновитьСтатусСообщенийИзСервиса Тогда
		...
		Если ОбсужденияПодключены() Тогда
			ОповещенияПлатформыСамозанятыеКлиент.Подключить();
...
Процедура ПериодСправкиОПостановкеНаУчетПриИзменении(Элемент)
	...
	Если НачалоГода(ПериодСправкиОПостановкеНаУчет) < ДатаНачалаЭксперимента Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ПериодСправкиОПостановкеНаУчет");
	КонецЕсли;
...
Процедура ПериодСправкиОДоходахПриИзменении(Элемент)
	...
	Если НачалоГода(ПериодСправкиОДоходах) < ДатаНачалаЭксперимента Тогда
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Справку нельзя сформировать за период раньше %1 года'"),
			Формат(ДатаНачалаЭксперимента, "ДФ=yyyy"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ПериодСправкиОДоходах");
	КонецЕсли;
...
Процедура ПолучитьДанныеНаСервере()
	...
	Если Не ЗначениеЗаполнено(Объект.НомерТелефонаПредставление) И ЗаполнятьРеквизитыОрганизации Тогда
...
Процедура УправлениеФормой(Форма)
	...
	ИначеЕсли Форма.СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.НеПривязано")
		ИЛИ Форма.СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.ЗапросОтправлен")
...
Функция ОбсужденияПодключены()
	...
	Если МожноПодключитьОбсуждения() Тогда
		МодульОбсужденияСлужебныйВызовСервера = ОбщегоНазначения.ОбщийМодуль("ОбсужденияСлужебныйВызовСервера");
		Возврат МодульОбсужденияСлужебныйВызовСервера.Подключены();
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура Подключаемый_ПолучитьСостояниеИнтеграции()
	...
	Если СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.ЗапросОтправлен") Тогда
...
Процедура ОбработкаОтветаМетода(Ответ)
	...
	Если СтатусЗапроса = СтатусыЗапросов.Выполнено Тогда
		...
		СостояниеПодключения = ПолучитьСостояниеПодключения(Объект.Организация);
		Если ОпросПлатформы
			И СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.ЗапросОтправлен") Тогда
				ПодключитьОбработчикОжидания("Подключаемый_ОбновитьСостояниеИнтеграцииСПлатформойСамозанятые", 1, Истина);
		ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.ЗапросОтклонен") Тогда
			ДанныеОбновлены = Истина;
			Ошибка = Истина;
	...
	ИначеЕсли СтатусЗапроса = СтатусыЗапросов.Ошибка Тогда
		...
		Если ОпросПлатформы Тогда
			Если ИмяМетодаВзаимодействия = "СтатусЗапросаНаПривязку"
					...
					Возврат;
			КонецЕсли;
			Если СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.Ошибка")
				Или СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.ЗапросОтправлен") Тогда
				ПодключитьОбработчикОжидания("Подключаемый_ОбновитьСостояниеИнтеграцииСПлатформойСамозанятые", 1, Истина);
			КонецЕсли;
		Иначе
...
Процедура ОбработкаОтветаМетода_ДетальныйСтатус(Ответ)
	...
	Если СтатусЗапроса = СтатусыЗапросов.Выполнено Тогда
		Если ЗаполнятьРеквизитыОрганизации Тогда
			ОбработкаОтветаМетода_ДетальныйСтатусНаСервере(Ответ, Объект.Организация);
			ОбновитьИнтерфейс();
		КонецЕсли;
		Закрыть();
	ИначеЕсли СтатусЗапроса = СтатусыЗапросов.Выполняется Тогда
...
&НаСервереБезКонтекста
Процедура ОбработкаОтветаМетода_ДетальныйСтатусНаСервере(Ответ, Организация)
	...
	Если ТипЗнч(Ответ.Результат) = Тип("Структура") Тогда
		...
		ОрганизацияОбъект.ИмяИП = Результат.Имя;
		ОрганизацияОбъект.ФамилияИП = Результат.Фамилия;
		ОрганизацияОбъект.ОтчествоИП = Результат.Отчество;
		ОрганизацияОбъект.НаименованиеСокращенное = ОбщегоНазначенияБПКлиентСервер.СокращенноеНаименованиеФизическогоЛица(
			Результат.Фамилия, Результат.Имя, Результат.Отчество, Ложь);
...
Функция КодыПредупреждений()
	...
	КодыПредупреждений.Вставить("ВнутренняяОшибкаСервиса", НСтр("ru = 'Попробуйте выполнить запрос позже.'"));
	КодыПредупреждений.Вставить("ОшибкаПарсингаЗапроса", НСтр("ru = 'Попробуйте выполнить запрос позже.'"));
	КодыПредупреждений.Вставить("ВременнаяРегистрация", НСтр("ru = 'Получение справок до завершения постановки на учет недоступно. Повторите запрос после получения статуса самозанятого.'"));
...
Функция КодыОшибок()
	...
	КодыОшибок.Вставить("НалогоплательщикНеЗарегистрирован", НСтр("ru = 'Налогоплательщик снят с учета или не обнаружен. Сначала нужно зарегистрироваться в сервсе Мой налог.'"));
	КодыОшибок.Вставить("НедостаточноПолномочий", НСтр("ru = 'Необходимые полномочия не предоставлены. Настройте список разрешений в Личном кабинете сервиса Мой налог.'"));
	КодыОшибок.Вставить("НалогоплательщикУжеПривязан", НСтр("ru = 'Налогоплательщик уже привязан. Попробуйте выполнить повторное подключение.'"));
...
Процедура ЗаписатьОшибкуВыполненияМетодаВзаимодействия(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки = Неопределено, КодОшибки = Неопределено)
	...
	Если ЗначениеЗаполнено(КодОшибки) И КодыПредупреждений().Свойство(КодОшибки, ОписаниеКодаОшибки) Тогда
		Предупреждение = Истина;
		Ошибка = Ложь;
		УровеньЖурнала = УровеньЖурналаРегистрации.Предупреждение;
	ИначеЕсли ЗначениеЗаполнено(КодОшибки) И КодыОшибок().Свойство(КодОшибки, ОписаниеКодаОшибки) Тогда
		Предупреждение = Ложь;
		Ошибка = Истина;
		УровеньЖурнала = УровеньЖурналаРегистрации.Ошибка;
	Иначе
		ОписаниеКодаОшибки = НСтр("ru = 'При подключении к сервису произошла ошибка'");
		Предупреждение = Ложь;
		Ошибка = Истина;
		УровеньЖурнала = УровеньЖурналаРегистрации.Ошибка;
	КонецЕсли;
