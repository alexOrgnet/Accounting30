...
Процедура ЗаполнитьОписаниеРазделаЗапасы(ОписаниеФункциональности)
	...
	Функциональность = ДобавитьОписаниеФункциональности(
		ОписаниеФункциональности, Раздел, "ИспользоватьНазначениеИспользованияСпецодежды",
		НСтр("ru = 'Назначения использования спецодежды'"),
		НСтр("ru = 'Для одной спецодежды можно задать несколько назначений использования с разными сроками эксплуатации'"));
	ДобавитьРодителя(Функциональность, "ВедетсяУчетСпецодеждыСпецоснастки");
	// добавить проверку ТарификацияБП.КонстантаФункциональностиПередЗаписью в событие ПередЗаписью константы
	Функциональность.ПроверяютсяУсловияТарификации = Истина;
...
Процедура ЗаполнитьОписаниеРазделаТорговля(ОписаниеФункциональности)
	...
	Функциональность = ДобавитьОписаниеФункциональности(
		...
		НСтр("ru = 'Продажи через маркетплейс'"),
...
Функция ДокументыПрепятствующиеОтключениюИспользоватьНазначениеИспользованияСпецодежды()
	...
	|	ПередачаМатериаловВЭксплуатациюСпецодежда.Ссылка КАК Представление
	|ПОМЕСТИТЬ ДокПередачаМатериалов
	|ИЗ
	...
	|		ПО ПередачаМатериаловВЭксплуатациюСпецодежда.НазначениеИспользования = НазначенияИспользования.Ссылка
	|ГДЕ
	|	НЕ НазначенияИспользования.Ссылка ЕСТЬ NULL
	|	И ПередачаМатериаловВЭксплуатациюСпецодежда.Ссылка.Дата > ДАТАВРЕМЯ(2021, 1, 1)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПередачаМатериаловВЭксплуатациюСпецодежда.Ссылка,
	...
	|	ПередачаМатериаловВЭксплуатациюСпецоснастка.Ссылка.Дата,
	|	ПередачаМатериаловВЭксплуатациюСпецоснастка.Ссылка
	|ИЗ
	|	Документ.ПередачаМатериаловВЭксплуатацию.Спецоснастка КАК ПередачаМатериаловВЭксплуатациюСпецоснастка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НазначенияИспользования КАК НазначенияИспользования
	|		ПО ПередачаМатериаловВЭксплуатациюСпецоснастка.НазначениеИспользования = НазначенияИспользования.Ссылка
	|ГДЕ
	|	НЕ НазначенияИспользования.Ссылка ЕСТЬ NULL
	|	И ПередачаМатериаловВЭксплуатациюСпецоснастка.Ссылка.Дата > ДАТАВРЕМЯ(2021, 1, 1)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПередачаМатериаловВЭксплуатациюСпецоснастка.Ссылка,
	|	ПередачаМатериаловВЭксплуатациюСпецоснастка.Ссылка.Дата,
	...
	|	ДокПередачаМатериалов.Дата КАК Дата,
	|	ДокПередачаМатериалов.Представление КАК Представление
	|ИЗ
	|	ДокПередачаМатериалов КАК ДокПередачаМатериалов
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата";
	...
	СтрокаТаблицыДокументов = ТаблицаРезультатовЗапросов.Добавить();
	СтрокаТаблицыДокументов.ИмяДокумента = "ПередачаМатериаловВЭксплуатацию";
	МетаданныеДокумента = Метаданные.Документы.Найти(СтрокаТаблицыДокументов.ИмяДокумента);
	СтрокаТаблицыДокументов.СинонимДокумента = МетаданныеДокумента.Синоним;
	СтрокаТаблицыДокументов.РезультатЗапроса = Запрос.Выполнить();
...
Функция ОтказСнятьИспользоватьНазначениеИспользованияСпецодежды()
...
Процедура СформироватьСписокДокументовПрепятствующихОтключениюОпции(Параметры, АдресХранилища) Экспорт
	...
	ИначеЕсли Параметры.Свойство("ИспользоватьНазначениеИспользованияСпецодежды") Тогда
		ТаблицаРезультатовЗапросов = ДокументыПрепятствующиеОтключениюИспользоватьНазначениеИспользованияСпецодежды();
		ОбластьШапка.Параметры.Заголовок = НСтр("ru = 'Назначения использования спецодежды'");
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюРасширенногоФункционала(Параметры, АдресХранилища) Экспорт
	...
	// Обесценение ОС
	Выборка = Результаты.ОбесценениеОС.Выбрать();
	Если Выборка.Количество() > 0 Тогда
		
		ПараметрыШапки = Новый Структура;
		
		ПараметрыШапки.Вставить("Заголовок",
			НСтр("ru = 'Обесценение основных средств'"));
		ПараметрыШапки.Вставить("Пояснение",
			НСтр("ru = 'Для отключения расширенного функционала удалите документы ""Обесценение ОС""'"));
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюИспользоватьНачисленияПоДоговорам(Параметры, АдресХранилища) Экспорт
	...
	Если Выборка.Количество() > 0 Тогда
		...
		ПарамерыШапки.Вставить("Заголовок",
			НСтр("ru = 'Договоры подряда'"));
		ПарамерыШапки.Вставить("Пояснение",
			НСтр("ru = 'Для отключения учета договоров подряда удалите договоры сотрудников'"));
...
Процедура ФункциональностьИспользоватьНазначениеИспользованияСпецодежды() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НазначенияИспользования.СрокПолезногоИспользования) КАК КоличествоРазличныхСроков,
	|	НазначенияИспользования.Владелец КАК Владелец
	|ПОМЕСТИТЬ НазначенияСРазнымСроком
	|ИЗ
	|	Справочник.НазначенияИспользования КАК НазначенияИспользования
	|ГДЕ
	|	НЕ НазначенияИспользования.СрокПолезногоИспользования = 0
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	НазначенияСРазнымСроком.Владелец КАК Владелец
	|ИЗ
	|	НазначенияСРазнымСроком КАК НазначенияСРазнымСроком
	|ГДЕ
	|	НазначенияСРазнымСроком.КоличествоРазличныхСроков > 1";
	
	УстановитьПривилегированныйРежим(Истина);

	РезультатЗапроса = Запрос.Выполнить();
	
	ИспользоватьНазначениеИспользованияСпецодежды = НЕ РезультатЗапроса.Пустой();
	
	Если ИспользоватьНазначениеИспользованияСпецодежды Тогда
		Обработки.ФункциональностьПрограммы.Изменить("ИспользоватьНазначениеИспользованияСпецодежды", Истина);
	КонецЕсли;
...
Функция ОтказСнятьИспользованиеКонстантыПоДаннымБазы(ИмяКонстанты) Экспорт
	...
	ИначеЕсли ИмяКонстанты = "ИспользоватьНазначениеИспользованияСпецодежды" Тогда
		ОтказСнять = ОтказСнятьИспользоватьНазначениеИспользованияСпецодежды();
...
Функция ОграниченияФункциональностиПоТарифу(ОписаниеФункциональности) Экспорт
	...
	Если Не ТарификацияБП.РазрешенУчетЗарплаты() Тогда
		ОтборПоРазделу = Новый Структура("Раздел", "ЗарплатаКадры");
		ФункциональностьРазделаЗарплатаКадры = ОписаниеФункциональности.НайтиСтроки(ОтборПоРазделу);
		Для Каждого СтрокаФункциональности Из ФункциональностьРазделаЗарплатаКадры Цикл
			Ограничения.ФункциональностьНедоступнаяПоТарифу.Добавить(СтрокаФункциональности.Имя);
		КонецЦикла;
	КонецЕсли;
...
Функция ИзменитьСписокФункциональностиПростойИнтерфейс(СписокФункциональности)
...
Функция РезультатЗапросаРасширенныйФункционал(ТолькоПроверка = Истина)
	...
	|	ПРЕДСТАВЛЕНИЕ(ОбесценениеОС.Ссылка) КАК Представление
	|ИЗ
	|	Документ.ОбесценениеОС КАК ОбесценениеОС
	|ГДЕ
	|	НЕ ОбесценениеОС.ПометкаУдаления
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОбесценениеОС.Дата
	|;
	...
	Результаты.Вставить("ОбесценениеОС", РезультатЗапроса[6]);
...
Процедура ДобавитьОграниченияОпцииВыплатыСамозанятым(УсловияДоступности)
	...
	НедоступныеВидыОпераций.Добавить(Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатаСамозанятому);
