...
Процедура ПослеВыполненияОбновления()
	...
	Если НужноОтправитьЗаявлениеВПФР() Тогда
...
Процедура ИзменитьОформлениеСтраницыЗаявлениеОдобрено()
	...
	ИначеЕсли НЕ ТребуетсяПолучениеРасписки 
		И НЕ ДокументЗаявление.ЭтоУдаленноеПереизданиеСертификата Тогда
...
Функция ПолучитьКрайнийСрокДЛ()
	...
	КрайнийСрокДЛ       = Дата(1,1,1);
	ПрошелКрайнийСрокДЛ = Ложь;
	РаспискаОтправлена  = Ложь;
		...
		Иначе
			...
			Если code = "0" Тогда
				...
				Если message = "Время до которого нужно предоставить электронную расписку" Тогда
					...
					КрайнийСрокДЛ = ТекущаяДатаСеанса() + remainingTime;
				...
				ИначеЕсли message = "Время для предоставления расписок истекло"
					...
					ПрошелКрайнийСрокДЛ = Истина;
					
				ИначеЕсли message = "Расписка приложена" Тогда
					
					РаспискаОтправлена = Истина;
