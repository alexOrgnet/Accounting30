...
Процедура ЗагрузитьДанныеДокументаДлительнаяОперация(Параметры, АдресРезультата) Экспорт
	...
	Иначе
		...
		ЭтоПродукцияМОТП = ШтрихкодированиеИС.ПрисутствуетТабачнаяПродукция(ВидыПродукцииДокументаОснования);
		
		Если Не ЭтоПродукцияМОТП Тогда
			
			Для Каждого ВидПродукции Из ВидыПродукцииДокументаОснования Цикл
				Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВидПродукции) Тогда
...
Процедура ВосстановитьДеревоДокументаПоРезультатамПроверки(Параметры, ДанныеДокумента, ОписаниеЗамера, АдресРезультата)
	...
	Пока ВыборкаРесурсовПроверки.Следующий() Цикл
		...
		ПоддерживаетсяОбъемноСортовойУчет = ИнтеграцияИСМПКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВыборкаРесурсовПроверки.ВидМаркируемойПродукции);
	...
	ИнтеграцияИСМПКлиентСервер.УстановитьПараметрСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, ЗапрашиватьДанныеСервисаИСМП);
	...
	Если РезультатПолученияДереваИСМП.ЕстьОшибки Тогда
		...
		Если РезультатПолученияДереваИСМП.ТребуетсяОбновлениеКлючаСессии
			И Не ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования) Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = '%1
