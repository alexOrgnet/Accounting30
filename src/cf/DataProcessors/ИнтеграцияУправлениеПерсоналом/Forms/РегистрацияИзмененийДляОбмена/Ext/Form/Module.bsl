...
Процедура ДобавитьРегистрациюОдинОбъект(Команда)
	...
	НачатьРегистрациюОдногоОбъекта(ТипыОбъектов[0], ИмяТаблицы);
...
&НаКлиенте
Процедура НачатьРегистрациюОдногоОбъекта(ТипОбъекта, ИмяТаблицы)
	...
	ПараметрыОповещения = Новый Структура("ТипОбъекта", ТипОбъекта);
	ОповещениеОЗавершении = Новый ОписаниеОповещения("РегистрацияОдногоОбъектаЗавершение", ЭтотОбъект, ПараметрыОповещения);
	ОткрытьФорму(ИмяФормыВыбора, ПараметрыВыбора, ЭтаФорма,,,,ОповещениеОЗавершении, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура РегистрацияОдногоОбъектаЗавершение(ВыбранныеСсылки, ПараметрыОповещения) Экспорт

	Если Не ЗначениеЗаполнено(ВыбранныеСсылки) Тогда
		Возврат;
	КонецЕсли;
	
	Результат = РегистрацияОдногоОбъектаЗавершениеНаСервере(ВыбранныеСсылки, ПараметрыОповещения.ТипОбъекта);
...
&НаСервере
Функция РегистрацияОдногоОбъектаЗавершениеНаСервере(ОбъектыКРегистрации, ТипОбъекта)
