...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СтруктураДоходовВычетов")
		...
		Если ПомощникЗаполнения3НДФЛКлиентСервер.ВычетыРаспределяютсяПоДоходам(Декларация3НДФЛВыбраннаяФорма)
			И Параметры.СтруктураДоходовВычетов.Свойство("КодВидаДохода", КодВидаДохода)
	...
	Иначе
		...
		Если ПомощникЗаполнения3НДФЛКлиентСервер.ВычетыРаспределяютсяПоДоходам(Декларация3НДФЛВыбраннаяФорма)
			И СтавкаНалога = УчетНДФЛПредпринимателяКлиентСервер.НалоговыеСтавкиПоУмолчанию(Период).СтавкаНДФЛ Тогда
...
Процедура КомандаОК(Команда)
	...
	ВыбранаСтавкаПрогрессивнойШкалы = ЭтоИдентификаторПрогрессивнойШкалы(Элементы.СтавкаНалога.ТекстРедактирования);
	...
	СтруктураРезультата.Вставить("КодВидаДохода", КодВидаДохода(
		...
		ВыбранаСтавкаПрогрессивнойШкалы));
...
&НаСервереБезКонтекста
Функция КодВидаДохода(Знач ВидДохода, Знач ВыбраннаяФорма, ВыбранаСтавкаПрогрессивнойШкалы)
	...
	ИначеЕсли РасширеннаяФункциональность И ВыбранаСтавкаПрогрессивнойШкалы Тогда 
...
Процедура ЗаполнитьСписокСтавокНалога()
	...
	НалоговыеСтавки = Отчеты.РегламентированныйОтчет3НДФЛ.НалоговыеСтавки(Декларация3НДФЛВыбраннаяФорма);
	...
	Если ТипЗнч(НалоговыеСтавки) = Тип("Соответствие") Тогда
		...
		Для Каждого ТекущаяСтавка Из НалоговыеСтавки Цикл
			...
			Если ЭтоИдентификаторПрогрессивнойШкалы(ТекущаяСтавка.Значение) Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоИдентификаторПрогрессивнойШкалы(СтавкаНалога)
	
	ИдентификаторСтавкиПрогрессивнойШкалы = ПомощникЗаполнения3НДФЛКлиентСервер.ИдентификаторСтавкиПрогрессивнойШкалы();
	
	Возврат (СтрЗаменить(СтавкаНалога, "%", "") = ИдентификаторСтавкиПрогрессивнойШкалы);
...
Функция ЗначениеСтавкиНалога()
	...
	Если СтавкаНалога = СтавкаНалогаПрогрессивнаяШкала() Тогда
		ЗначениеСтавки = УчетНДФЛПредпринимателяКлиентСервер.НалоговыеСтавкиПоУмолчанию(Период).СтавкаНДФЛ;
	Иначе
