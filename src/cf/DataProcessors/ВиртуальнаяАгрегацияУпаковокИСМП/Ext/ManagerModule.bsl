		...
		Пока ВыборкаИндексСтроки.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				Если ШтрихкодВключаетМРЦ И Выборка.МаксимальнаяРозничнаяЦена > 0 Тогда
					КодМаркировки = ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(Выборка.ПолныйКодМаркировки);
...
Процедура ЗаполнитьВложенныеЕдиницы(Параметры, ОстаткиПродукции)
	...
	Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетНаборы(Параметры.ВидПродукции) Тогда
		ЗаполнитьНаборы(Параметры, ОстаткиПродукции);
	КонецЕсли;
	
	Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(Параметры.ВидПродукции) Тогда
