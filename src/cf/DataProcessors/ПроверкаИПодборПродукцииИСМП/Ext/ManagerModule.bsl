...
Процедура ЗафиксироватьРезультатПроверкиИПодбора(Параметры, АдресРезультата) Экспорт
	...
	Если ЗаполнитьСпецификуСрокиГодности Тогда
		Если ТипЗнч(Параметры.ПроверяемыйДокумент) = Тип("ДокументСсылка.МаркировкаТоваровИСМП") Тогда
			...
			ЗаполнитьСпецификуСрокиГодности =
				...
				Или (ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Параметры.ВидМаркируемойПродукции)
	...
	Попытка
		...
		ЗаполнитьИдентификаторЗапросаГИСМТ = ИнтеграцияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(Параметры.ВидМаркируемойПродукции,
