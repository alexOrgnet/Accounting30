...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	ПараметрыСканированияСервер = ШтрихкодированиеИС.ПараметрыСканирования(ПроверяемыйДокумент,, ВидМаркируемойПродукции, ПараметрыРежимаИсправленияОшибок);
	...
	ПоддерживаетсяОбъемноСортовойУчет = ИнтеграцияИСМПКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВидМаркируемойПродукции)
...
Функция НеобходимоОбращениеКСервисуИСМП(ПараметрыСканирования, ПараметрыПроверкиКодовМаркировки)
	...
	Если Не ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, ОтключитьЗапросПоВидуПродукцииИОперации)
		И Не ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеНеизвестныхУпаковокИСМП(ПараметрыСканирования, ОтключитьЗапросПоВидуПродукцииИОперации)
...
Процедура ПереименоватьКолонкиПриВосстановленииДерева(ДеревоМаркированнойПродукцииОбъект)
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ИнтеграцияИС.ВерсияБиблиотеки(), "1.1.28.16") >= 0 Тогда
		ИменаКолонок.Добавить(
			Новый Структура("ИмяКолонкиСтарое, ИмяКолонкиНовое",
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	Если Не ПроверкаНеПоДокументу И ПараметрыСканированияСервер = Неопределено Тогда
		ПараметрыСканированияСервер = ШтрихкодированиеИС.ПараметрыСканирования(
			ПроверяемыйДокумент, Неопределено, ВидМаркируемойПродукции, ПараметрыРежимаИсправленияОшибок);
	КонецЕсли;
...
Процедура УправлениеЭлементамиКонтроляСтатусаКодовМаркировки(Форма)
	...
	Если Не Форма.ПараметрыСканирования = Неопределено Тогда
		...
		ДоступностьКомандОтключенияКонтроля = Не ИнтеграцияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(Форма.ВидМаркируемойПродукции,
		...
		Элементы.ФормаВключитьОтключитьКонтрольПоВладельцу.Доступность             = ДоступностьКомандОтключенияКонтроля;
...
Процедура ИнициализироватьПараметрыСканирования(НачальнаяИнициализация = Ложь)
	...
	ПараметрыСканирования.КонтролироватьРасхождениеСоставаУпаковокМеждуИБиИСМП = КонтрольСоставаУпаковок
		И ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования);
	...
	ПродукцияОнлайнПроверкиПриРозничнойПродаже = ИнтеграцияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(
		ВидМаркируемойПродукции,
		ПараметрыСканирования.ВидОперацииИСМП,
...
Функция РезультатОбработкиШтрихкода()
	Возврат ШтрихкодированиеИС.ИнициализироватьРезультатОбработкиШтрихкода();
КонецФункции
	...
	Если ШтрихкодированиеИСМПКлиент.ЭтоСобытиеПотоковойПечати(ЭтотОбъект, ДанныеШтрихкода, ПараметрыСканирования) Тогда
		Если ПараметрыСканирования.ДанныеВыбораПоМаркируемойПродукции.ТребуетВзвешивания
			...
			И (ДополнительныеПараметры = Неопределено Или Не ДополнительныеПараметры.Свойство("РезультатВзвешивания"))Тогда
				...
				ДанныеДляУточнения.ДанныеШтрихкода.Вставить("EAN", ШтрихкодированиеИСКлиентСервер.ШтрихкодEANИзGTIN(ДанныеДляУточнения.ДанныеШтрихкода.GTIN));
...
Процедура ПроверитьШтрихкодВДеревеМаркированнойПродукции(ДанныеШтрихкода)
	...
	ИначеЕсли ЭтоАдресВременногоХранилища(ДанныеШтрихкода.АдресДереваУпаковок)
		И (      ИнтеграцияИСКлиентСервер.ЭтоУпаковка(ДанныеШтрихкода.ТипУпаковки)
			Или ИнтеграцияИСКлиентСервер.ЭтоУпаковкаПоВиду(ДанныеШтрихкода.ВидУпаковки)) Тогда
...
Процедура Подключаемый_ПолученыДанныеИзТСД(Штрихкоды, ПараметрыОбработкиТСД) Экспорт
	...
	Если ЗагрузкаДанныхТСД.ТребуетсяАвторизацияИСМПРозница Тогда
		...
		ИнтерфейсАвторизацииИСМПКлиент.ЗапроситьКлючСессии(
			ИнтерфейсИСМПКлиентСервер.ПараметрыЗапросаКлючаСессииИСМПРозница(ПараметрыСканирования.Организация),
			Новый ОписаниеОповещения("ЗагрузитьИзТСДПослеАвторизации", ЭтотОбъект, ПараметрыОповещения));
...
Процедура ОбработатьПолученныеДанныеТСДНаСервере(ДанныеДляТСД, КэшированныеЗначения)
	...
	ВидПродукцииИспользуетНаборыИлиГрупповыеУпаковки = ИнтеграцияИСКлиентСерверПовтИсп.ВидПродукцииИспользуетНаборыИлиГрупповыеУпаковки(ВидМаркируемойПродукции);
	...
	Для Каждого СтрокаДанныхТСД Из ДанныеДляТСД.Штрихкоды Цикл
		...
		Если ПоддерживаетсяОбъемноСортовойУчет
			И ШтрихкодированиеИСКлиентСервер.ЭтоEANИлиGTIN(Штрихкод) Тогда
			Штрихкод = СтрШаблон("(02)%1(37)%2", ШтрихкодированиеИСКлиентСервер.GTINПоШтрихкодуEAN(Штрихкод), СтрокаДанныхТСД.Количество);
		КонецЕсли;
...
Функция ЗапроситьСокращеннуюИнформациюПоКодамМаркировки(СоответствиеШтрихкодовСтрокДерева, ВидПродукции, Организация, ИдентификаторФискальногоНакопителя)
	...
	РезультатЗапроса = ИнтерфейсИСМП.СокращеннаяИнформацияПоКМПриРозничнойПродаже(ТаблицаМаркировки,
		Организация,
...
Функция ХэшСуммаСтроки(НормализованныйШтрихкод)
	Возврат ИнтеграцияИС.ХэшСуммаСтроки(НормализованныйШтрихкод);
КонецФункции
