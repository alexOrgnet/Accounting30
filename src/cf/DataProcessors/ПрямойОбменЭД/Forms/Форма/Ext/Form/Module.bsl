		...
		ИначеЕсли Объект.ВариантЗагрузкиЭД = Перечисления.ВариантыЗагрузкиЭД.СчетНаОплату Тогда
			...
			Для Каждого ЭлементДанных Из ПрочитанныеДанные Цикл
				...
				СтрокаДанных.ТабличныйДокумент = ПоместитьВоВременноеХранилище(ЭлементДанных.ТабличныйДокумент,
					ИдентификаторФормы);
				СтруктураРеквизитов = ЭлементДанных.ДанныеДокумента;
				
				ДанныеРазбора = Новый Структура;
				ДанныеРазбора.Вставить("ДеревоРазбора"              , СтруктураРеквизитов.ДеревоРазбора);
				ДанныеРазбора.Вставить("СтрокаОбъекта"              , Неопределено);
				ДанныеРазбора.Вставить("ФорматЭлектронногоДокумента", Неопределено);
				
				СтруктураРеквизитов.ДанныеРазбора = ПоместитьВоВременноеХранилище(ДанныеРазбора, ИдентификаторФормы);
				СтрокаДанных.ФайлДанных = ПоместитьВоВременноеХранилище(СтруктураРеквизитов.ФайлДанных, ИдентификаторФормы);
				СтруктураРеквизитов.ФайлДанных    = Неопределено;
				СтруктураРеквизитов.ДеревоРазбора = Неопределено;
				СтрокаДанных.СтруктураРеквизитов = СтруктураРеквизитов;
			КонецЦикла;
...
Процедура ОбработатьВыборЭлектронногоДокумента(СтруктураРеквизитовЭД)
	...
	ИначеЕсли Объект.ВариантЗагрузкиЭД = ПредопределенноеЗначение("Перечисление.ВариантыЗагрузкиЭД.СчетНаОплату") Тогда
		...
		Если Не ЗначениеЗаполнено(СтруктураРеквизитовЭД.ШапкаДокумента.Организация) Тогда
			ЕстьОшибки = Истина;
			ТекущиеДанные.ТекстОшибки = СтрШаблон(НСтр(
				"ru = 'Не удалось найти организацию-получателя с ИНН %1'"), СтруктураРеквизитовЭД.ШапкаДокумента.ИННорганизации);
		КонецЕсли;
		Если Не ЕстьОшибки Тогда
			ИдентификаторСтроки = Элементы.ДанныеЭлектронныхДокументов.ТекущаяСтрока;
			ПодготовитьРеквизитыСчета(ИдентификаторСтроки, СтруктураРеквизитовЭД, ЕстьОшибки);
		КонецЕсли;
		Если Не ЕстьОшибки Тогда
			Закрыть(СтруктураРеквизитовЭД);
		ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.ТекстОшибки) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекущиеДанные.ТекстОшибки);
		КонецЕсли;
	ИначеЕсли Объект.ВариантЗагрузкиЭД = ПредопределенноеЗначение("Перечисление.ВариантыЗагрузкиЭД.РеализацияТоваровИУслуг") Тогда
...
&НаСервере
Процедура ПодготовитьРеквизитыСчета(ИдентификаторСтроки, Реквизиты, ЕстьОшибки)
	
	ТекущиеДанные = Объект.ДанныеЭлектронныхДокументов.НайтиПоИдентификатору(ИдентификаторСтроки);
	ДанныеРазбора = ПолучитьИзВременногоХранилища(ТекущиеДанные.СтруктураРеквизитов.ДанныеРазбора);
	ИдентификаторФормы = ?(ЗначениеЗаполнено(ИдентификаторФормыВладельца), ИдентификаторФормыВладельца, УникальныйИдентификатор);
	СтруктураЭД = ЭлектронноеВзаимодействиеБП.ПолучитьСтруктуруЭД(
		ДанныеРазбора, ИдентификаторФормы, ТекущиеДанные.ФайлДанных, ТекущиеДанные.Наименование);
	Реквизиты.ДанныеРазбора = СтруктураЭД.ДанныеФайлаРазбора;
	
	Обработки.ПрямойОбменЭД.ДополнитьСтруктуруРеквизитовСчета(Реквизиты, ЕстьОшибки);
