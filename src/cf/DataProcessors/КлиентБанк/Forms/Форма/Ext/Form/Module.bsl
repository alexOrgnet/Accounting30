...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		ЦифровойСчет = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчет, "ЦифровойСчет");
...
Процедура ЗагрузитьНастройкиДляБанковскогоСчета(БанковскийСчетУстановленИзФайла = Ложь, ЭтоИзменениеБанкСчета = Ложь)
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Если (ЗначениеЗаполнено(Объект.БанковскийСчет) Или ЗначениеЗаполнено(АдресВыписки))
			...
			СохранятьТекущуюНастройку = СоответствиеВыписки.Соответствует;
		...
		ИначеЕсли ЗначениеЗаполнено(Настройки.БанкДляПрямогоОбмена) Тогда
			...
			СпособЗагрузки = ?(ИспользоватьНесколькоБанковскихСчетовОрганизации, "ДиректБанкПоБанку", "ДиректБанк");
...
Функция ОтборСсылкиКСозданию()
	...
	Для Каждого СтрокаТаблицы Из ДокументыКИмпорту Цикл
		...
		Если (Не СтрокаТаблицы.Загружать Или Не ТребуетсяСозданиеНенайденныхВСтроке(СтрокаТаблицы))
			И Не ЭтоСсылкаНовогоБанковскийСчет Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЭтоСсылкаНовогоБанковскийСчет Тогда
			ОтборСсылки[ТипЗнч(БанковскийСчетОрганизации)].Вставить(БанковскийСчетОрганизации, Истина);
		КонецЕсли;
	КонецЦикла;
...
Процедура ИзменитьПараметрыВыбораБанковскогоСчета()
	...
	Элементы.БанковскийСчетВыгрузка.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
