...
Процедура ЗагрузитьДанныеИзЭлектронныхТаблиц(ДанныеЗагрузки, Результат, АдресРезультата)
	...
	Если ДанныеЗагрузки.Свойство(ИмяОборотноСальдоваяВедомость()) Тогда
		...
		ДлительныеОперации.СообщитьПрогресс( , "ВводНачальныхОстатков");
	КонецЕсли;
...
Процедура НайтиСоздатьОсновныеСредства(ЗагружаемыеДанные, Ошибки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаОС.НаименованиеВВедомости = СтрШаблон(
				"%1%2",
				?(ЗначениеЗаполнено(СтрокаОС.ИнвентарныйНомер), СтрокаОС.ИнвентарныйНомер + " ", ""),
				СтрокаОС.Наименование);
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеЗаполненияКонтрагента(СтрокаДанных, ДанныеИзСервиса)
	...
	Если ЭтоФизическоеЛицо Тогда
		...
		Если ЗначениеЗаполнено(СтрокаДанных.ОГРН) Тогда
			...
			ДанныеЗаполнения.Вставить("Самозанятый", Ложь);
		КонецЕсли;
	Иначе
...
Процедура ЗагрузитьОстаткиПоНоменклатуре(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	...
	Документ.Дата = Дата;
	Документ.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Товары;
...
Функция ПрочитатьОборотноСальдовуюВедомость(СоставДанных, Таблица)
	...
	ЗаполнитьТаблицуДанных(
		ТаблицаДанных, СоставДанных, Таблица, СтрокаШапкиТаблицы + 2, "", Неопределено, Ложь, ЗначенияЗаполнения);
	// Удаляем подвал с итогами
	ТаблицаДанных.Удалить(ТаблицаДанных.Количество() - 1);
	
	ЧисловыеРеквизиты = ЧисловыеРеквизитыОборотноСальдовойВедомости();
	СтрокиКУдалению = Новый Массив;
...
Функция ЧисловыеРеквизитыОборотноСальдовойВедомости()
	
	ИменаРеквизитов = Новый Массив;
	ИменаРеквизитов.Добавить("ДебетСумма");
	ИменаРеквизитов.Добавить("ДебетКоличество");
	ИменаРеквизитов.Добавить("КредитСумма");
	ИменаРеквизитов.Добавить("КредитКоличество");
...
Процедура ЗаполнитьСоответствияРеквизитовДокументовБезРазбивки(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "Сумма", "всего, ₽");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Комментарий", "комментарий");
...
Процедура ЗаполнитьСоответствияРеквизитовОборотноСальдоваяВедомость(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "ДебетСумма", "сумма");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ДебетКоличество", "количество");
	ДобавитьСоответствие(ТаблицаРеквизитов, "КредитСумма", "сумма");
	ДобавитьСоответствие(ТаблицаРеквизитов, "КредитКоличество", "количество");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ТипДокумента");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Дата");
...
Процедура ЗаполнитьСоответствияРеквизитовОсновныеСредства(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "Стоимость", "первоначальная стоимость, руб");
