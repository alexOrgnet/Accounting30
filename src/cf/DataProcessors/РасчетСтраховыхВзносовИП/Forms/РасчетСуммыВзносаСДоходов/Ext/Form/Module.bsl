...
Процедура ПотенциальноВозможныйДоходНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Документ.ОперацияСПатентом.ФормаСписка", СтруктураПараметров, ЭтотОбъект);
...
Процедура УправлениеФормой()
	...
	ИспользуетсяРасчетныйМетод = УчитыватьРасходы И РасчетЗаТекущийПериод
		И (ФиксированнаяЧастьВзносов > 0 Или (Всего - Уплачено - СуммаЗарезервированная) > 0);
	...
	Элементы.ГруппаСумма.Видимость = Не УчетнаяПолитика.ПлательщикЕНП(Организация, Период);
