...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтерфейсТаксиПростой = Константы.ИнтерфейсТаксиПростой.Получить();
...
Процедура НастройкаПодсказкиВводОстатковУСН(Элементы, Переход)
	Если НЕ ЗначениеЗаполнено(Переход) Тогда
		Элементы.ГруппаНапоминанияОВводеОстатков.Видимость = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура _ОКЗавершение(Результат, ИмяРегистра) Экспорт
	...
	ИначеЕсли ИмяРегистра = "НастройкиУчетаНДС" Тогда
		...
		Если (Не ОрганизацияВелаРаздельныйУчет И ЭтотОбъект.НастройкиУчетаНДС.РаздельныйУчетНДСНаСчете19 И ЕстьОстаткиДляРУ)
	...
	ИначеЕсли ИмяРегистра = "НастройкиУчетаСтраховыхВзносовИП" Тогда
		...
		НастройкиСистемыНалогообложенияФормыКлиент.ОбновитьИнтерфейсПослеЗаписиНастройкиУчетаСтраховыхВзносовИП(
			Новый Структура("УплачиватьДобровольныеВзносыВФССПередЗаписью, УплачиватьДобровольныеВзносыВФССПослеЗаписи",
				УплачиватьДобровольныеВзносыВФССПередЗаписью,
				УплачиватьДобровольныеВзносыВФССПослеЗаписи));
	КонецЕсли;
...
Процедура ЗаполнитьСвязанныеРеквизиты(ИмяРаздела = Неопределено)
	...
	Иначе
		ПоУмолчаниюКодОперации0 = "1011410";
	КонецЕсли;
...
Процедура ЗаписатьИзменения(ИмяРегистра)
	...
	ИначеЕсли ИмяРегистра = "НастройкиУчетаНДС" Тогда
		...
		Если ЭтотОбъект.НастройкиУчетаНДС.РаздельныйУчетНДСНаСчете19 Тогда
...
Процедура УправлениеФормой(Форма, ИмяРаздела = Неопределено)
	...
	Если ИмяРаздела = Неопределено ИЛИ ИмяРаздела = "НастройкиУчетаУСН" Тогда
		...
		НастройкаПодсказкиВводОстатковУСН(Элементы,
			НастройкиСистемыНалогообложенияФормыВызовСервера.НастройкиДляБаннераСменыОбъектаУСН(Форма.Организация));
	КонецЕсли;
...
Функция ИменаЭлементовСтраницыСистемыНалогообложенияДляУстановкиДоступности()
	...
	ИменаЭлементов.Добавить("ПодсказкаНапоминаниеОВводеОстатков");
