...
Процедура ОбработкаВыбораФайлаПредидущегоПериода(ВыбранныеФайлы, ДопПараметры) Экспорт
	...
	#Если ВебКлиент Тогда
		НачатьПомещениеФайлаНаСервер(
			Новый ОписаниеОповещения("ЧтениеТекстовогоФайлаВебКлиентЗавершение", ЭтотОбъект), , , , ВыбранныеФайлы[0], УникальныйИдентификатор);
	#Иначе
...
&НаКлиенте
Процедура ЧтениеТекстовогоФайлаВебКлиентЗавершение(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	Если ОписаниеПомещенногоФайла = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ЧтениеТекстовогоФайлаНаСервере(ОписаниеПомещенногоФайла.СсылкаНаФайл.Файл.ПолноеИмя);
КонецПроцедуры

&НаСервере
Процедура ЧтениеТекстовогоФайлаНаСервере(ПолноеИмя)
	ТекстФайла = Новый ЧтениеТекста(ПолноеИмя);
	ТекстВыгрузки.Очистить();
	Стр = "";
	Пока Стр <> Неопределено Цикл 
		Стр = ТекстФайла.ПрочитатьСтроку();
