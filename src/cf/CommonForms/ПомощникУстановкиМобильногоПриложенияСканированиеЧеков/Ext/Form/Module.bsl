...
Функция СтраницаОпубликована(АдресСтраницы)
	...
	ИнтернетПрокси = ПолучениеФайловИзИнтернета.ПолучитьПрокси(URI.Схема);
	
	HTTPСоединение = Новый HTTPСоединение(URI.Хост, URI.Порт,,,ИнтернетПрокси,3, ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение());
