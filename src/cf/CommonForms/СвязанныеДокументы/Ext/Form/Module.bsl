...
Функция ОбъектыПоКритериюОтбора(ЗначениеКритерияОтбора)
	...
	Для Каждого ЭлементСостава Из Метаданные.КритерииОтбора.СвязанныеДокументы.Состав Цикл
		...
		// Представление таблицы отличается от того, что в БСП, т.к. в БП есть документ НачислениеПеней с табличной частью
		// Задолженность с реквизитом Задолженность. И в этом случае запрос падает из-за неоднозначности Задолженность.
		ПредставлениеТаблицы = "ТабличнаяЧасть_" + Сред(ИмяТаблицы, Точка + 1);
