...
Процедура ОбновитьОтображениеПоляКоличество()
	...
	Если ТребуетВзвешивания И (РежимПечатиЭтикеток Или Не ЭтоEANИлиGTIN) Тогда
		...
		Элементы.ПолучитьВес.Видимость = ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукции);
	Иначе
...
Процедура ОбработатьШтрихкод(ДанныеШтрихкода, ШтрихкодНайден, ТекстОшибки = "")
	...
	Если МенеджерОборудованияКлиентСервер.ПроверитьКорректностьGTIN(ДанныеШтрихкода.Штрихкод) Тогда
		EAN = ШтрихкодированиеИСКлиентСервер.ШтрихкодEANИзGTIN(ДанныеШтрихкода.Штрихкод);
	КонецЕсли;
...
Процедура ПервоначальноеЗаполнениеПоЧастичномуВыбытию()
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции) Тогда
		...
		Если МожетВыбыватьЧастично И ЧастичноеВыбытие Тогда
			...
			ДанныеПоШтрихкодам = ШтрихкодированиеИС.ДанныеПоШтрихкодам(Штрихкоды, ПараметрыСканированияДанныхШтрихкода);
			...
			Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
				...
				Если ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканированияДанныхШтрихкода)
...
Процедура ОбновитьПараметрыЧастичногоВыбытия(Инициализация = Ложь)
	...
	ОбщийМодульИнтеграцияИСКлиентСерверПовтИсп = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияИСКлиентСерверПовтИсп");

	ДанныеЧастичногоВыбытияПолученыИзГИС = ОбщийМодульИнтеграцияИСКлиентСерверПовтИсп.ЗапрашиватьДанныеЧастичногоВыбытияИзСервиса(ВидПродукции);

	ОперацияИС = ШтрихкодированиеИСКлиентСервер.ОперацияИС(ПараметрыСканирования, ВидПродукции);
	МожетВыбыватьЧастично = ЗначениеЗаполнено(ЧастичноеВыбытиеВариантУчета)
		И ОбщийМодульИнтеграцияИСКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(ВидПродукции, ОперацияИС);
	...
	Если Инициализация Тогда
		...
		ЗаголовокРежимаВыбытия = ОбщийМодульИнтеграцияИСКлиентСерверПовтИсп.ЗаголовокРежимаВыбытияПоВидуОперации(ОперацияИС);
		...
		Для Каждого ЭлементСписка Из Элементы.РежимВыбытия.СписокВыбора Цикл
			...
			ПредставлениеРежима = ОбщийМодульИнтеграцияИСКлиентСерверПовтИсп.ПредставлениеРежимаВыбытияПоВидуПродукции(
	...
	Если РежимПечатиЭтикеток Тогда
		...
		ИначеЕсли ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.КреслаКоляски")
			...
			СтруктурыДанных.ДополнитьПолныйКодМаркировки = СтрШаблон("97%1",
				ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(ЗаводскойСерийныйНомер));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьДанныеGTIN()
	...
	Если ЗначениеЗаполнено(ВидПродукции)
		И (ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетНаборы(ВидПродукции)
		Или ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидПродукции)) Тогда
