...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_ПлатежныйДокумент_УплатаНалогов"
		...
		Или ИмяСобытия = "Запись_СуммаЗачетаЕНС" Тогда
...
Процедура ОтметитьЗадачуКакВыполненную(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтметитьЗадачуКакВыполненнуюЗавершение", ЭтотОбъект, Задача);
...
Процедура ВернутьАвтоматическийСтатус(Команда)
	...
	ВернутьАвтоматическийСтатусЗадачи(Задача);
...
&НаКлиенте
Процедура ОтметитьЗадачуКакВыполненнуюЗавершение(РезультатЗакрытия, Задача) Экспорт
	...
	ЗаписатьРучнойСтатусЗадачи(Задача, РезультатЗакрытия.Комментарий);
	Элементы.СписокЗадач.Обновить();
...
&НаСервереБезКонтекста
Процедура ВернутьАвтоматическийСтатусЗадачи(Задача)
	
	ЗадачаСоСтатусом = РегистрыСведений.ЗадачиБухгалтера.НовыйЗадачаСоСтатусом();
	ЗаполнитьЗначенияСвойств(ЗадачаСоСтатусом, Задача);
	ЗадачаСоСтатусом.Статус = "";
	
	// Сначала сбросим признак ручного статуса и запишем задачу
	ВыполнениеЗадачБухгалтера.ЗаписатьНовыйСтатусЗадачи(ЗадачаСоСтатусом, Ложь);
	...
	Для Каждого ЗадачаСоСтатусом Из ТаблицаЗадач Цикл
		ВыполнениеЗадачБухгалтера.ЗаписатьНовыйСтатусЗадачи(ЗадачаСоСтатусом, Ложь);
	КонецЦикла;
...
&НаСервереБезКонтекста
Процедура ЗаписатьРучнойСтатусЗадачи(Задача, Комментарий)
	...
	ПользовательУстановившийСтатус = Справочники.Пользователи.ПустаяСсылка();
	...
	Если Не ТекущийПользовательЯвляетсяСлужебным Тогда
		ПользовательУстановившийСтатус = ТекущийПользователь;
	КонецЕсли;
	
	ВыполнениеЗадачБухгалтера.ЗаписатьНовыйСтатусЗадачи(
		ЗадачаСоСтатусом,
		Ложь,
		Истина,
	...
	Если Задача.ИдентификаторЗадачи = "ЕдиныйНалоговыйСчет" Тогда
		...
		Обработки.ПомощникУплатыНалога.ЗаписатьСтатусПодчиненныхЗадачПоЕдиномуНалоговомуСчету(
			ТаблицаПодчиненныхЗадач, Статус, Истина, ТекущийПользователь, Комментарий);
	КонецЕсли;
