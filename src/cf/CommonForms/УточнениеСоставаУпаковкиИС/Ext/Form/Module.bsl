...
Процедура ЗаполнитьТребующиеУточненияДанныеУпаковки(СтрокаДерева, ДетализацияСтруктурыХранения)
	...
	Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
		...
		Если Не ТребуетсяУточнение
			...
			ТребуетсяУточнение = ИнтеграцияИС.ТребуетсяВыборСерии(СтрокаДерева, ПараметрыСканирования);
		КонецЕсли;
...
Функция ОбновитьЗаполненныеШтрихкодыУпаковок()
	...
	ОписаниеНоменклатуры = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(ДанныеДляУточнения.Выгрузить(,"Номенклатура").ВыгрузитьКолонку("Номенклатура"));
