			...
			Если ПротоколОбмена = Неопределено Тогда
				...
				Если Перечисления.ВидыОперацийСАТУРН.ЭтоАбстрактнаяОперация(Операция) Тогда
					...
					Для Каждого СообщениеJSON Из СообщенияJSON Цикл
						...
						Если Перечисления.ВидыОперацийСАТУРН.ЭтоАбстрактнаяОперация(СообщениеJSON.Операция)
							Или (ЗначениеЗаполнено(СообщениеJSON.ИдентификаторСтроки)
								И ЗначениеЗаполнено(РеквизитыСообщения.ИдентификаторСтроки)
								И СообщениеJSON.ИдентификаторСтроки <> РеквизитыСообщения.ИдентификаторСтроки) Тогда
							Продолжить;
						КонецЕсли;
						...
						СтрокаПротокола.Вставить("ЗапросТело",            ИнтеграцияИС.ОбъектВТекстJSON(ТелоЗапроса));
