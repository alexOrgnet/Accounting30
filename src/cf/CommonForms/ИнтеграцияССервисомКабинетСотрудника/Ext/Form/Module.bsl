...
Процедура НачатьПубликациюДанных()
	...
	ФоновыеЗадания.Выполнить("ИнтеграцияКабинетСотрудника.ВыполнитьОбменПослеПодключенияПриложения",
				ПараметрыМетода,
				ИнтеграцияКабинетСотрудника.КлючФоновогоЗаданияОбмен());
...
Функция ДлительнаяОперацияВыполнитьОбмен()
	...
	ПараметрыВыполненияВФоне.ОжидатьЗавершение = 0;
	ПараметрыВыполненияВФоне.КлючФоновогоЗадания = ИнтеграцияКабинетСотрудника.КлючФоновогоЗаданияОбмен();
...
Процедура ОбновитьДоступностьЭУИспользуемойФункциональностиПоВерсииПриложения()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ИнформацияОбОстаткахОтпусковНедоступна", "Видимость",
			ИспользоватьФормат503 И ДоступнаяФункциональностьСервиса.Отпуск);
