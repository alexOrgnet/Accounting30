...
//   Организация  - СправочникСсылка.Организации - Организация, для которой необходимо вернуть параметры настройки
//   Период       - Дата - Период, за который необходимо сформировать уведомление по ЕНС
//   СуммыКУплате - см. Обработка.РасчетСтраховыхВзносовИП.СвойстваВзносыКУплате()
//
// Возвращаемое значение:
...
Процедура СоздатьЗаполнитьОрганизацию(НастройкаИнтеграции, ДанныеОрганизации, КодВозврата, СообщениеОбОшибке)
	...
	Если Налогообложение.СистемаНалогообложения = "УпрощеннаяДоходы" Тогда
		Налогообложение.Вставить("СведенияОСтавкеНалога", СведенияОСтавкеНалога(ДанныеОрганизации));
	КонецЕсли;
...
Функция СведенияОСтавкеНалога(ДанныеОрганизации)
	
	СведенияОСтавкеНалога = НовыйСведенияОСтавкеНалогаПоУмолчанию();
...
Функция НовыйСведенияОСтавкеНалогаПоУмолчанию()
...
Процедура СоздатьЗаявлениеОЗачетеВСчетПредстоящейОбязанности(УчетнаяСистема, Параметры, РезультатВыполнения, Ошибка, СообщениеОбОшибке)
	...
	ПараметрыНалогов = НовыеПараметрыВзносыКУплате();
...
Функция НовыеПараметрыВзносыКУплате()
	
	Результат = Новый Структура(Обработки.РасчетСтраховыхВзносовИП.СвойстваВзносыКУплате());
	Для Каждого ТекущийПараметр Из Результат Цикл
		Результат[ТекущийПараметр.Ключ] = 0;
	КонецЦикла;
