...
Процедура ЗаполнитьТребованиямиНормативныхДокументов(ПутьКФайлуПравил = Неопределено) Экспорт
	...
	ТекущаяВерсияПравил = 
		Константы.ВерсияПравилПредставленияОтчетовУплатыНалогов.Получить();
	
	Если ЗначениеЗаполнено(ТекущаяВерсияПравил) И ТекущаяВерсияПравил >= ОбъектПравил.ВерсияПравил Тогда
		// В базе классификатор уже обновлен
		Возврат;
	КонецЕсли;
	
	Правила = ПредопределенныеПравила(ОбъектПравил);
	...
	КонецЦикла;
	Константы.ВерсияПравилПредставленияОтчетовУплатыНалогов.Установить(ОбъектПравил.ВерсияПравил);
...
Функция ТребованияНормативныхДокументовXML(ПоднятьВерсию = Ложь) Экспорт
	...
	ПредыдущаяВерсияПравил = ПредопределенныеПравила(ПредыдущийОбъектПравил);
...
Функция ПредопределенныеПравила(ОбъектXDTO)
