...
Процедура ОпределитьНомерПриказа()
	...
	Если Период > Год(Справочники.ВидыНалоговИПлатежейВБюджет.ДатаВторогоКлассификатора()) Тогда
		ИмяМакета      = Справочники.ВидыНалоговИПлатежейВБюджет.ИмяМакетаКБКНовый();
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 1 июня 2023 г. № 80н
			|(в редакции приказа Минфина от 20.11.2023 № 185н)'");
	Иначе
		ИмяМакета      = Справочники.ВидыНалоговИПлатежейВБюджет.ИмяМакетаКБКБезГода();
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 17 мая 2022 г. № 75н
			|(в редакции приказа Минфина от 28.08.2023 № 139н)'");
	КонецЕсли;
