...
//
// Параметры:
//  Правила      - ТаблицаЗначений - коллекция, которая будет дополнена правилами; можно использовать коллекцию,
//                  конструируемую в НовыйТаблицаПравил(). Колонки:
//                  * Правило - СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов - правило (обязательная)
//                  * ФинансовыйПериод - Дата - налоговый или иной финансовый период, в котором организация обязана
//                    применять правило. Если колонка в таблице есть, то правила будут детализированы по всем финансовым
//                    периодам, входящим в интервал составления перечня правил.
//                  * ВключеноПользователем - Булево - Истина, если правило действует на основании настройки в форме
//                    списка налогов и отчетов
...
//
Процедура ЗаполнитьПравилаОрганизации(Правила, Отбор, МенеджерВременныхТаблиц = Неопределено) Экспорт
...
Функция ВключенныеПравилаОрганизации(Организация, ТекущийПериод = Неопределено) Экспорт
	...
	ЗаполнитьПравилаОрганизации(Правила, ОтборПравил);
...
// Отказ от использования перечисления УдалитьУсловияПримененияТребованийЗаконодательства -
// замена на строковые идентификаторы условий.
//
Процедура ЗаполнитьУсловияПримененияТребованийЗаконодательстваСтроковымиЗначениями() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УсловияПравил.Ссылка КАК Ссылка,
	|	УсловияПравил.НомерСтроки КАК НомерСтроки,
	|	УсловияПравил.УдалитьУсловие КАК Условие
	|ИЗ
	|	Справочник.ПравилаПредставленияОтчетовУплатыНалогов.Условия КАК УсловияПравил
	|ГДЕ
	|	УсловияПравил.УдалитьУсловие <> ЗНАЧЕНИЕ(Перечисление.УдалитьУсловияПримененияТребованийЗаконодательства.ПустаяСсылка)
	|	И УсловияПравил.Условие = """"
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	НомерСтроки
	...
	КешУсловий = Новый Соответствие;
	
	Пока ВыборкаПравило.Следующий() Цикл
		
		Попытка
			
			Объект = ВыборкаПравило.Ссылка.ПолучитьОбъект();
			
			ВыборкаУсловие = ВыборкаПравило.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаУсловие.Следующий() Цикл
				
				Если Не ЗначениеЗаполнено(ВыборкаУсловие.Условие) Тогда
					Продолжить;
				КонецЕсли;
				
				ИндексСтроки = ВыборкаУсловие.НомерСтроки - 1;
				Объект.Условия[ИндексСтроки].Условие = Перечисления.УдалитьУсловияПримененияТребованийЗаконодательства.ИмяУсловия(
					ВыборкаУсловие.Условие,
					КешУсловий);
		...
		Исключение
			...
			Комментарий = СтрШаблон(
				НСтр("ru = 'Не удалось заполнить Условие (обработчик обновления ЗаполнитьУсловияПримененияТребованийЗаконодательстваСтроковымиЗначениями).
                      |%1'"),
			...
			ЗаписьЖурналаРегистрации(
				...
				Метаданные.Справочники.ПравилаПредставленияОтчетовУплатыНалогов,
				ВыборкаПравило.Ссылка,
				Комментарий);
...
//  Отбор - Структура - см. ОтборПравилЗаполнения()
//
Процедура СоздатьАктуальныеПравила(МенеджерВременныхТаблиц, Отбор)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ФинансовыйПериод Из ФинансовыеПериоды Цикл
			...
			Если ИнтерфейсыВзаимодействияБРО.ПервыйНалоговыйПериодНачинаетсяСДатыПостановкиНаУчет(Выборка.РасширенныйПервыйНалоговыйПериод) Тогда
				ДатаРегистрации = ОбщегоНазначенияБПВызовСервераПовтИсп.ДатаРегистрацииОрганизации(Отбор.Организация);
				КонецФинансовогоПериода = ИнтерфейсыВзаимодействияБРОКлиентСервер.КонецПериода(
					...
					ФинансовыйПериод);
				ДатаПостановкиНаУчет = УчетнаяПолитика.ДатаПостановкиНаУчетНалогоплательщика(Отбор.Организация, КонецФинансовогоПериода);
