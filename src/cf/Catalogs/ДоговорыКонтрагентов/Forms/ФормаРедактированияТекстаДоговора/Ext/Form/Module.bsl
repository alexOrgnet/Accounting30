...
Процедура ВставитьПараметр(ИмяПараметра)
	...
	ВременныйФорматированныйДокумент = ЗаменитьПараметрНаСервере(
		ИмяПараметра, ОбъектПечати, ШаблонДоговора, ШрифтПоУмолчанию, ЭлектронныйФормат);
...
Функция ЗаменитьПараметрНаСервере(Знач ИмяПараметра, Знач ОбъектПечати, Знач ШаблонДоговора, Знач ШрифтПоУмолчанию, Знач ЭлектронныйФормат)
	...
	Значение = ПечатьДоговоров.ЗначениеПараметра(ИмяПараметра, СведенияОСделке, ЗначенияСоставногоПараметра, Успешно, ЭлектронныйФормат);    
...
Процедура СнятьПометкуУдаленияФайлаДоговора(ОбновленныйФайлДоговора)
	...
	Исключение
		...
		ПодробныйТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать файл с текстом договора в элементе справочника ""Договоры контрагентов"" по причине: %1'"),
			ПодробноеПредставлениеОшибки(СведенияОбОшибке));
...
Функция ДокументЭлектронныйФорматДляПечати() 
	...
	Возврат ФорматированныйДокументБезСлужебныхМеток();
...
Процедура ПечатьДоговораЭлектронныйФормат(Команда)  
	...
	ФорматированныйДокументДляПечати = ДокументЭлектронныйФорматДляПечати(); 
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ФорматированныйДокументДляПечати", ФорматированныйДокументДляПечати);
	ПараметрыФормы.Вставить("ОбъектПечати", ДоговорКонтрагента);
