...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если СтатусФизЛицаИзменен Тогда
		УстановитьФункциональныеОпцииИнтерфейсаТакси(ЭтотОбъект, ТекущийОбъект);
	КонецЕсли;
	...
	Если Объект.КрупнейшийНалогоплательщик Тогда
		Если ИспользуетсяРасширенныйФункционал Тогда
			...
			Если ТребуетсяУказаниеРегиона
				...
				И (НЕ ЗначениеЗаполнено(КрупнейшийНалогоплательщикКодРегионаРегистрации) Или КрупнейшийНалогоплательщикКодРегионаРегистрации = "00") Тогда
...
Процедура ОбновитьВидимостьЭлементовНастроекПодключения()
	...
	Если ИнтеграцияДоступна Тогда
		НастройкиИнтеграцииСБП = ИнтеграцияССБПБП.НастройкиИнтеграцииСБП(Объект.Ссылка, БИКБанка);
...
Процедура УправлениеФормой(Форма)
	...
	ИПзакрыт = ЗначениеЗаполнено(Объект.ДатаЗакрытия);
	ЭтоСамозанятый = (Форма.ЭтоФизЛицо И Не ЭтоИндивидуальныйПредприниматель); // Физическое лицо, самозанятый без статуса ИП
	
	// Блок внесения показывается, если есть заявление на внесение изменений или снятие с учета
	// или это простой интерфейс
	Если Форма.ПростойИнтерфейс Тогда
		ПоказатьПомощникВнесенияИзмененийЕГР = ЗначениеЗаполнено(Объект.Ссылка)
			И Не Объект.ОбособленноеПодразделение
			И Форма.ЕГРДоступноВнесениеИзменений
			И Не ЭтоСамозанятый;
	Иначе
		ПоказатьПомощникВнесенияИзмененийЕГР = Форма.ЕГРДоступноВнесениеИзменений
			И ЗначениеЗаполнено(Объект.Ссылка)
			И ЗначениеЗаполнено(Форма.ЕГРЗаявление)
			И Не ЭтоСамозанятый;
	КонецЕсли;
	
	Если ИПзакрыт Тогда
		Элементы.ОписаниеИзмененийЕГР.Видимость = Ложь;
	Иначе
		Элементы.ОписаниеИзмененийЕГР.Видимость = Истина;
	КонецЕсли;
	
	Элементы.ВнесениеИзмененийЕГРФон.Видимость = ПоказатьПомощникВнесенияИзмененийЕГР;
	...
	Если Форма.ЕГРНужноОбновитьРеквизитыОрганизации Тогда
		Элементы.ЕГРСформироватьПакетДокументов.Заголовок = НСтр("ru = 'Показать пакет документов'");
	ИначеЕсли Форма.ЕГРВнесениеИзмененийНеЗавершено Тогда
		Элементы.ЕГРСформироватьПакетДокументов.Заголовок = НСтр("ru = 'Продолжить подготовку документов'");
	Иначе
		Если ЭтоИндивидуальныйПредприниматель Тогда
			Если Не ЗначениеЗаполнено(Форма.ЕГРЗаявление) Тогда
				Элементы.ЕГРСформироватьПакетДокументов.Заголовок = НСтр("ru = 'Внести изменения'");
			Иначе
				Элементы.ЕГРСформироватьПакетДокументов.Заголовок = НСтр("ru = 'Показать пакет документов'");
			КонецЕсли;
		Иначе
			Элементы.ЕГРСформироватьПакетДокументов.Заголовок = НСтр("ru = 'Сформировать пакет документов'");
		КонецЕсли;
	КонецЕсли;
	
	Если Форма.ЭтоФизЛицо И Не ЗначениеЗаполнено(Форма.Объект.ОГРН) Тогда
		Элементы.ВнесениеИзмененийЕГР.Заголовок = НСтр("ru = 'Регистрация ИП'");
		Элементы.ОписаниеИзмененийЕГР.Заголовок = НСтр("ru = 'Индивидуальный предприниматель зарегистрирован.'");
		Элементы.НадписьИзмененияЕстьВЕГР.Заголовок = НСтр("ru = 'Заполните реквизиты по ИНН
			|из Листа записи ЕГРИП'");
		Элементы.ПоказатьЛистЗаписиЕГР.Заголовок = НСтр("ru = 'Лист записи ЕГРИП'");
	ИначеЕсли Не ЗначениеЗаполнено(Форма.Объект.ОГРН) Тогда
		Элементы.ВнесениеИзмененийЕГР.Заголовок = НСтр("ru = 'Регистрация ООО'");
		Элементы.ОписаниеИзмененийЕГР.Заголовок = НСтр("ru = 'Организация зарегистрирована.'");
		Элементы.НадписьИзмененияЕстьВЕГР.Заголовок = НСтр("ru = 'Заполните реквизиты по ИНН
		|из Листа записи ЕГРЮЛ'");
		Элементы.ПоказатьЛистЗаписиЕГР.Заголовок = НСтр("ru = 'Лист записи ЕГРЮЛ'");
	ИначеЕсли ЭтоИндивидуальныйПредприниматель Тогда
		Элементы.ВнесениеИзмененийЕГР.Заголовок = НСтр("ru = 'Регистрация в ЕГРИП'");
		Если Форма.ЕГРПоказыватьЛистЗаписи Тогда
			Элементы.ОписаниеИзмененийЕГР.Заголовок = НСтр("ru = 'Укажите дату прекращения деятельности ИП'");
		Иначе
			Элементы.ОписаниеИзмененийЕГР.Заголовок =
				НСтр("ru = 'Изменение видов деятельности (кодов ОКВЭД), снятие с учета необходимо зарегистрировать в Едином государственном реестре индивидуальных предпринимателей.'");
		КонецЕсли;
	...
	Если Форма.ЕГРВнесениеИзмененийОшибка Тогда
		Элементы.ПояснениеИзмененияВЕГРОшибка.Видимость = Истина;
		Элементы.ГруппаПомощникВнесенияИзмененийЕГР.Видимость = Ложь;
		Элементы.ПояснениеИзмененияЕстьВЕГР.Видимость = Ложь;
		Элементы.ПояснениеИзмененийНетВЕГР.Видимость = Ложь;
		Элементы.СохранениеИзмененийЕГР.Видимость = Истина;
		Элементы.ОбновитьРеквизитыВПрограмме.Видимость = Ложь;
		Элементы.ПроверитьСтатусЗаявленияВЕГР.Видимость = Ложь;
		Элементы.ЕГРСформироватьПакетДокументов.Видимость = Истина;
		Элементы.ПоказатьЛистЗаписиЕГР.Видимость = Ложь;
	ИначеЕсли Форма.ЕГРОбновитьСтатус Тогда
		Элементы.ГруппаПомощникВнесенияИзмененийЕГР.Видимость = Истина;
		Элементы.ПояснениеИзмененияВЕГРОшибка.Видимость = Ложь;
		Элементы.ПояснениеИзмененияЕстьВЕГР.Видимость = Ложь;
		Элементы.СохранениеИзмененийЕГР.Видимость = Истина;
		Элементы.ПояснениеИзмененийНетВЕГР.Видимость = Истина;
		Элементы.ЕГРСформироватьПакетДокументов.Видимость = Ложь;
		Элементы.ОбновитьРеквизитыВПрограмме.Видимость = Ложь;
		Элементы.ПроверитьСтатусЗаявленияВЕГР.Видимость = Истина;
		Элементы.ПоказатьЛистЗаписиЕГР.Видимость = Ложь;
		Элементы.ОписаниеИзмененийЕГР.Заголовок = НСтр("ru = 'Заявление на рассмотрении в налоговом органе.
	...
	ИначеЕсли Не ЗначениеЗаполнено(Форма.Объект.ОГРН) Тогда
		...
		Элементы.ПояснениеИзмененияЕстьВЕГР.Видимость = Истина;
		Элементы.ПояснениеИзмененийНетВЕГР.Видимость = Ложь;
		
		Элементы.ПоказатьЛистЗаписиЕГР.Видимость = ПоказыватьЛистЗаписиЕГР(
			Форма.ЕГРАдресДанныхПомощника,
			Форма.ЕГРПоказыватьЛистЗаписи,
			Истина);
		
		Элементы.ОписаниеИзмененийЕГР.Заголовок = НСтр("ru = 'Передайте пакет документов в налоговый орган.
	...
	Иначе
		...
		Элементы.ГруппаПомощникВнесенияИзмененийЕГР.Видимость = Истина;
		Элементы.ПояснениеИзмененияВЕГРОшибка.Видимость = Ложь;
		Элементы.ОбновитьРеквизитыВПрограмме.Видимость = Форма.ТребуетОбновленияРеквизитов;
		Элементы.ПроверитьСтатусЗаявленияВЕГР.Видимость = Ложь;
		Элементы.СохранениеИзмененийЕГР.Видимость = ПредложитьСохранитьИзмененияЕГР;
		Элементы.ПояснениеИзмененияЕстьВЕГР.Видимость = Форма.ЕГРИзмененияДобавленыВРеестр И ПредложитьСохранитьИзмененияЕГР;
		Если ПоказыватьЛистЗаписиЕГР(Форма.ЕГРАдресДанныхПомощника, Форма.ЕГРПоказыватьЛистЗаписи) Тогда
			Элементы.ПоказатьЛистЗаписиЕГР.Видимость = Истина;
			Элементы.ПояснениеИзмененийНетВЕГР.Видимость = Ложь;
			Элементы.ЕГРСформироватьПакетДокументов.Видимость = Ложь;
		Иначе
			Элементы.ЕГРСформироватьПакетДокументов.Видимость = Истина;
	...
	Элементы.ОткрытьПомощникСнятияСУчетаВЕГР.Видимость = ЭтоИндивидуальныйПредприниматель И Не ЗначениеЗаполнено(Форма.ЕГРЗаявление);
	
	Если ИПзакрыт Тогда
		Элементы.ДатаЗакрытия.Видимость = Истина;
		Элементы.ПояснениеИзмененийНетВЕГР.Видимость = Ложь;
	ИначеЕсли Форма.ЕГРСнятиеСУчета Тогда
		Элементы.ДатаЗакрытия.Видимость = Не Форма.ЕГРОбновитьСтатус
			И Не Форма.ЕГРВнесениеИзмененийОшибка
			И Не Форма.ЕГРВнесениеИзмененийНеЗавершено;
	Иначе
		Элементы.ДатаЗакрытия.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.СтатусОбновленияЗаявления.Видимость = Форма.ВыполняетсяОбновлениеСтатусаЗаявленияЕГР;
	Элементы.ПроверитьСтатусЗаявленияВЕГР.Доступность = Не Форма.ВыполняетсяОбновлениеСтатусаЗаявленияЕГР;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПоказыватьЛистЗаписиЕГР(АдресДанныхПомощника, ИзмененияДобавленыВРеестр, ЭтоРегистрацияОрганизации = Ложь)
	
	Если ЭтоРегистрацияОрганизации Тогда
		
		Возврат ИзмененияДобавленыВРеестр;
		
	ИначеЕсли ЭтоАдресВременногоХранилища(АдресДанныхПомощника) Тогда
...
Процедура УстановитьСписокВыбораРегионов(КрупнейшийНалогоплательщик = Ложь)
	...
	ЭтотОбъект[ПрефиксИмени + "КодРегионаРегистрации"]= Формат(ЭтотОбъект[ПрефиксИмени + "РегистрацияВНалоговомОргане"].КодРегиона, "ЧЦ=2; ЧН=; ЧВН=");
...
Процедура ВыбратьКодИзКлассификатора(ИмяКлассификатора, Комментарий = "", Знач ИмяРеквизитаКод = "")
	...
	Если ИмяКлассификатора = "ОКВЭД2"
		Или ИмяКлассификатора = "ОКОПФ"
...
Процедура ИнициализироватьФорму()
	...
	Если Обработки.РегистрацияОрганизации.ДоступноВнесениеИзмененийЕГР(ЭтоФизЛицо, Объект.НаименованиеОКОПФ) Тогда
		ЕГРДоступноВнесениеИзменений = Истина;
	Иначе
		ЕГРДоступноВнесениеИзменений = Ложь;
	КонецЕсли;
	
	Если ЕГРДоступноВнесениеИзменений И ЗначениеЗаполнено(Объект.ОГРН) Тогда
		УстановитьСостояниеЗаявлениеОВнесенииИзмененийВЕГРН();
	Иначе
		// Проверяем, подано ли заявление на регистрацию из программы и выводим актуальное состояние
...
Процедура ПрочитатьРегистрациюВНалоговомОргане(КрупнейшийНалогоплательщик = Ложь)
	...
	Если ЗначениеЗаполнено(Объект[ПрефиксИмени + "РегистрацияВНалоговомОргане"])
		...
		ЭтотОбъект[ПрефиксИмени + "КодРегионаРегистрации"] = РегистрацияВНалоговомОргане.КодРегиона;
		...
		ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "СБППомощникПодключения" Тогда
			...
			Иначе
				НаименованиеНастройки = СтрШаблон(НСтр("ru = 'Настройка СБП в %1 (%2)'"),
					НаименованиеБанка, Объект.Наименование);
				ДополнительныеПараметры = Новый Структура;
...
Процедура ПоказатьЭлементыНастроекПодключения(Форма, ТекущийТекстНомераСчета = "")
	...
	Если (ЗначениеЗаполнено(Форма.БанковскийСчет.НомерСчета) Или ЗначениеЗаполнено(ТекущийТекстНомераСчета))
		И Форма.НомерСчетаКорректен Тогда
...
Процедура УстановитьСостояниеЗаявлениеОВнесенииИзмененийВЕГРН()
	...
	Если ЗначениеЗаполнено(ЕГРЗаявление) Тогда
		ДанныеЗаявления = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЕГРЗаявление, "ДанныеУведомления");
		Если ЗначениеЗаполнено(ДанныеЗаявления) Тогда
			ДанныеУведомления = ДанныеЗаявления.Получить();
			Если Не ЕГРСнятиеСУчета
				И ЗначениеЗаполнено(ДанныеУведомления.ДанныеПомощникаЗаполнения)
				И ДанныеУведомления.ДанныеПомощникаЗаполнения.Свойство("Изменено") Тогда
				...
				Для Каждого ТекущееИзменение Из ДанныеУведомления.ДанныеПомощникаЗаполнения.Изменено Цикл
			...
			ЕГРАдресДанныхПомощника = ПоместитьВоВременноеХранилище(
				ДанныеУведомления.ДанныеПомощникаЗаполнения,
				УникальныйИдентификатор);
	...
	Если ЕГРСнятиеСУчета Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ДатаЗакрытия) Тогда
			ПредложитьСохранитьИзмененияЕГР = Не СостояниеЗаявленияВЕГР.ЛистЗаписиЕГР И Не ЕГРВнесениеИзмененийОшибка;
			ЕГРНужноОбновитьРеквизитыОрганизации = СостояниеЗаявленияВЕГР.Документы;
		Иначе
			ПредложитьСохранитьИзмененияЕГР = Истина;
			ЕГРНужноОбновитьРеквизитыОрганизации = Истина;
		КонецЕсли;
	Иначе
...
Процедура УстановитьСостояниеЗаявлениеНаРегистрациюОрганизации()
	...
	Если ТребуетОбновленияРеквизитов И СостояниеРегистрацииОрганизации.ЛистЗаписиЕГР Тогда
		ЕГРПоказыватьЛистЗаписи = Истина;
	Иначе
		ЕГРПоказыватьЛистЗаписи = Ложь;
	КонецЕсли;
	
	ЕГРНужноОбновитьРеквизитыОрганизации = СостояниеРегистрацииОрганизации.ЛистЗаписиЕГР И ТребуетОбновленияРеквизитов;
...
&НаСервереБезКонтекста
Процедура УстановитьФункциональныеОпцииИнтерфейсаТакси(Форма, ТекущийОбъект = Неопределено, ВозможенПереходНаУСН = Неопределено)
	...
	УстановитьПривилегированныйРежим(Истина);
	Если ЭтоИндивидуальныйПредприниматель Или ЭтоЮрЛицо Тогда
		Если Форма.СтатусФизЛицаИзменен Или ЭтоЮрЛицо Тогда
			Константы.ИнтерфейсТаксиПростойИПиЮрЛица.Установить(Форма.ПростойИнтерфейс);
		КонецЕсли;
		Если ВозможенПереходНаУСН = Неопределено Тогда
			ВозможенПереходНаУСН = Обработки.ПомощникПоПереходуНаУСН.ВозможенПереходНаУСН(
				Объект.Ссылка, Форма.ПростойИнтерфейс);
		КонецЕсли;
...
Процедура ОткрытьПомощникЗаполнения(Знач ИдентификаторФормы = Неопределено)
	...
	Если Не ЕГРНужноОбновитьРеквизитыОрганизации И Не ЕГРВнесениеИзмененийНеЗавершено Тогда
...
Процедура ПриОтветеНаВопросОбработатьОшибкуУдалитьДокументыПоОрганизации(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПараметрыФормы.Вставить(
			"ИмяМетода",
			"Справочники.Организации.СформироватьСписокДокументовПрепятствующиеИзменениюВидаОрганизации");
...
Процедура ПроверитьВозможенПереходНаУСН()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ВозможенПереходНаУСН <> ВозможенПереходНаУСН() Тогда
			УстановитьФункциональныеОпцииИнтерфейсаТакси(ЭтотОбъект, , ВозможенПереходНаУСН);
		КонецЕсли;
	КонецЕсли;
