...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЭтоНовый() Тогда
		...
		РеквизитыДоЗаписи = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Наименование, НаименованиеПолное, РегистрацияВНалоговомОргане, КрупнейшийНалогоплательщикРегистрацияВНалоговомОргане, ОбособленноеПодразделение, ДатаЗакрытия");
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДатаЗакрытияДоЗаписи <> ДатаЗакрытия И ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		// Удаляем старую запись регистра с предыдущей датой закрытия
		Если ЗначениеЗаполнено(ДатаЗакрытияДоЗаписи) Тогда
			// По единому тарифу
			МенеджерЗаписи = РегистрыСведений.НастройкиУчетаСтраховыхВзносовИП_ПФР_ФФОМС.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.Период = ДатаЗакрытияДоЗаписи;
		...
		// Добавляем запись регистра с актуальной датой
