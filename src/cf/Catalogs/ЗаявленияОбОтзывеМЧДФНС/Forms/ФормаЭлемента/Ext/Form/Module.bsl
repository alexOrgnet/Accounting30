...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Объект.ВерсияФормата = ?(ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило(
			"Отзыв МЧД ФНС вер.5.02"), "5.02", "5.01");
...
Процедура ЗаполнитьНаОснованииДоверенности(Доверенность)
	...
	Если ЗначениеЗаполнено(Доверенность) Тогда
		...
		ЗаполнитьЗначенияСвойств(ДокОбъект, Доверенность,
			...
			| ЛицоБезДовФЛ_ИНН, ЛицоБезДовФЛ_ОГРН, ЛицоБезДовФЛ_СНИЛС, ЛицоБезДовЮЛ, ЛицоБезДовЮЛ_ИНН, ЛицоБезДовЮЛ_КПП,
			| ЛицоБезДовЮЛ_НаимОрг, ЛицоБезДовЮЛ_ОГРН");
		...
		Если Доверенность.Представители.Количество() = 0 Тогда
		...
		Иначе
			Для каждого Стр из Доверенность.Представители Цикл
				НоваяСтрока = ДокОбъект.Представители.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Стр);
				НоваяСтрока.ПредставительИспользуется = Истина;
			КонецЦикла;
