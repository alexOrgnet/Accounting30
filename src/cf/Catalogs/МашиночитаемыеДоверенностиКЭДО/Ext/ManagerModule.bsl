...
Функция ИмяВыгружаемогоФайла(ИмяФайлаДоверенности, ДатаДоверенности, Ссылка) Экспорт
	
	Если ЗначениеЗаполнено(ИмяФайлаДоверенности) Тогда
		Возврат ИмяФайлаДоверенности;
	КонецЕсли;
	
	Возврат КадровыйЭДОВызовСервера.ИмяФайлаКЭДО(ИдентификаторФайла(ДатаДоверенности, Ссылка) + ".xml");
...
Функция СвойстваДоверенностиВФорматеИФНС003(ДеревоXML)
	...
	Если ЗначениеЗаполнено(СвойстваДоверенности.НомерВРеестре) Тогда
		Если Не Справочники.МашиночитаемыеДоверенностиКЭДО.НайтиПоРеквизиту("НомерВРеестре", СвойстваДоверенности.НомерВРеестре).Пустая() Тогда
	...
	Если ТипДоверителя = "1" Тогда
		...
		ДанныеФизлица = ЭлектронныеТрудовыеКнижки.ПолучитьСвойствоXDTO(ДанныеФизлицаДоверителя, "СвФЛ");
...
Функция ДанныеДоверенностиВСтрокуXMLВерсия003(ДанныеДляВывода) Экспорт
	...
	URIПространстваИмен = Метаданные.ПакетыXDTO.ON_EMCHDКЭДО.ПространствоИмен;
	...
	Возврат ЗарплатаКадры.ЗаписатьПотокXML(ПотокВыгрузкиXML);
...
Функция ДанныеДоверенности(МЧД) Экспорт
	...
	Если ТипЗнч(МЧД) = Тип("СправочникОбъект.МашиночитаемыеДоверенностиКЭДО") Тогда
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Справочник.МашиночитаемыеДоверенностиКЭДО КАК ДоверенностиКЭДО
