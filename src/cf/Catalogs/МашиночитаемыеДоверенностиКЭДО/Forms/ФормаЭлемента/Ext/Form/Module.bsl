...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект = Неопределено)
	...
	ЗаписьСостояния = РегистрыСведений.СостоянияМЧДКЭДО.СоздатьМенеджерЗаписи();
...
Процедура ЗагрузитьДанныеФайлаНаСервере(ПомещенныйФайл)
	...
	СвойстваДоверенности = Справочники.МашиночитаемыеДоверенностиКЭДО.СвойстваДоверенностиИзФайла(АдресДоверенности);
	...
	Если ЗначениеЗаполнено(ОбъектДоверенности) Тогда
		...
		Если ЗначенияРеквизитов = Неопределено Тогда
			Если ТипЗнч(ОбъектДоверенности) = Тип("СправочникСсылка.Организации") Тогда
				ЗначенияРеквизитов = Справочники.МашиночитаемыеДоверенностиКЭДО.ДанныеОрганизации(
			...
			ИначеЕсли ТипЗнч(ОбъектДоверенности) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
				ЗначенияРеквизитов = Справочники.МашиночитаемыеДоверенностиКЭДО.ДанныеФизическогоЛица(
					ОбъектДоверенности, ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса()));
			КонецЕсли;
...
Функция НомерНовойМЧД()
	...
	Возврат Справочники.МашиночитаемыеДоверенностиКЭДО.ПолучитьНомерМЧД();
...
Функция ЗарегистрироватьМЧДНаСервере(ИмяФайлаДоверенности, АдресДоверенности, АдресПодписи, НомерДоверенности)
	Возврат Справочники.МашиночитаемыеДоверенностиКЭДО.ЗарегистрироватьМЧД(
...
Функция ИмяВыгружаемогоФайлаПоСсылке(Ссылка)
	Возврат Справочники.МашиночитаемыеДоверенностиКЭДО.ИмяВыгружаемогоФайлаПоСсылке(Ссылка);
КонецФункции
...
Функция ПолучитьСтатусТранзакцииМЧД(ИдентификаторТранзакции, НомерВРеестре)
	Возврат Справочники.МашиночитаемыеДоверенностиКЭДО.ПолучитьСтатусТранзакцииМЧД(
			ИдентификаторТранзакции,
			"",
			НомерВРеестре);
КонецФункции
