...
//
Функция ПроверитьДубли(Контрагент) Экспорт
...
Функция ПрочитатьДанныеXMLКарточки(АдресXMLФайла)
	...
	Если НЕ Карточка.Тип() = РаботаСФайламиБЭД.ПолучитьТипЗначенияCML("Контрагент") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
// Определяет, является ли самозанятым контрагент.
// Параметры:
...
//
Функция ЭтоСамозанятый(Контрагент) Экспорт
	
	Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "Самозанятый");
...
Процедура ЗаполнитьПризнакиКонтрагентаПоУмолчанию(КонтрагентОбъект) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Если НЕ КонтрагентОбъект.ПометкаУдаления
			...
			И НЕ КонтрагентОбъект.Самозанятый
...
Функция ЗадолженностьПоМассивуКонтрагентов(МассивКонтрагентов, ВестиУчетРасчетовСКонтрагентами, Организация = Неопределено) Экспорт
	...
	Если ВестиУчетРасчетовСКонтрагентами Тогда
		...
		|		ПО СчетНаОплатуПокупателю.ВалютаДокумента = Валюты.Валюта
		|ГДЕ
		|	СчетНаОплатуПокупателю.Контрагент В(&МассивКонтрагентов)
		|	И СчетНаОплатуПокупателю.Проведен
		|	И ЕСТЬNULL(СтатусыДокументов.Статус, ЗНАЧЕНИЕ(Перечисление.СтатусОплатыСчета.ПустаяСсылка)) В (&СтатусыНеоплаченныхСчетов)
		|	И СчетНаОплатуПокупателю.Организация В(&СписокОрганизаций)
		|
		...
		|			&СубконтоКонтрагентДоговор,
		|			Организация В (&СписокОрганизаций)
		|				И Субконто1 В (&МассивКонтрагентов)) КАК ХозрасчетныйОстатки
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Договор
		|;
	...
	Иначе
		...
		|	СУММА(ДанныеКонтрагентов.НеоплаченныеСчетаПокупателемКоличество) КАК НеоплаченныеСчетаПокупателемКоличество,
		|	СУММА(ДанныеКонтрагентов.НеоплаченныеСчетаПокупателемСумма) КАК НеоплаченныеСчетаПокупателемСумма,
		|	МАКСИМУМ(ДанныеКонтрагентов.Телефон) КАК Телефон
		|ИЗ
		|	(ВЫБРАТЬ
		|		НеоплаченныеСчета.Контрагент КАК Контрагент,
		|		НеоплаченныеСчета.НеоплаченныеСчетаПокупателемКоличество КАК НеоплаченныеСчетаПокупателемКоличество,
		...
		|		НомераТелефонов.Представление
		|	ИЗ
		|		НомераТелефонов КАК НомераТелефонов) КАК ДанныеКонтрагентов
		|
		|СГРУППИРОВАТЬ ПО
