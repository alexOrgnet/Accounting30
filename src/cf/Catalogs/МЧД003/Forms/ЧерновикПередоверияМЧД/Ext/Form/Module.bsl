...
Процедура ПриИзмененииДаты(ИмяЭлемента)
	...
	Если ЗначениеЗаполнено(СвПереДовер.СрокДейст)
		И СвПереДовер.ДатаВыдДовер > КонецДня(СвПереДовер.СрокДейст) Тогда
		СвПереДовер.ДатаВыдДовер = ПустаяДата;
		ТекстСообщения = НСтр("ru = 'Дата выдачи доверенности больше даты окончания. Введите корректную дату'"); 
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СвПереДовер.СрокДейст)
		И КонецДня(СвПереДовер.СрокДейст) < ТекущаяДатаСеанса() Тогда
		СвПереДовер.СрокДейст = ПустаяДата;
		ТекстСообщения = НСтр("ru = 'Дата окончания срока доверенности меньше текущей. Введите корректную дату'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
	СрокДействияМенееДопустимого = ЗначениеЗаполнено(СвПереДовер.ДатаВыдДовер)
		И ЗначениеЗаполнено(СвПереДовер.СрокДейст)
		И КонецДня(СвПереДовер.СрокДейст) > ТекущаяДатаСеанса()
		И НачалоДня(СвПереДовер.СрокДейст) - ТекущаяДатаСеанса() < 24*60*60;
...
Процедура ЗарегистрироватьМЧДПриНеобходимости(ПараметрыЗаписи)
	...
	МашиночитаемыеДоверенностиКлиент.ПодписатьЗарегистрироватьМЧД(Истина, ОбработчикПослеПодписания,
		Объект.Ссылка, , МашиночитаемыеДоверенностиКлиентСервер.ФорматМЧД_003());
