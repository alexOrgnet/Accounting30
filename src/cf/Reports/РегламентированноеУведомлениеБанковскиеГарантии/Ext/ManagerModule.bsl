...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "ФормаОтзыв501";
	Стр.ОписаниеФормы = "Заявление об отзыве банковской гарантии, по письму ФНС России от 27.03.2024 № ЕА-4-15/3458@";
	...
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "ФормаЗамена501";
	Стр.ОписаниеФормы = "Заявление о замене банковской гарантии, по письму ФНС России от 27.03.2024 № ЕА-4-15/3458@";
...
Функция ПроверитьДокументСВыводомВТаблицу_ФормаОтзыв501(Данные, УникальныйИдентификатор)
	...
	Иначе
		...
		Иначе
			Если Не ЗначениеЗаполнено(СтруктураДокумент.КодВидДок) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Если указан код документа", "Титульная", "КодВидДок"));
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СтруктураДокумент.СерНомДок) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Если указан серия и номер документа", "Титульная", "СерНомДок"));
			КонецЕсли;
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложения.ПроверкаОГРН(Титульная, ТаблицаОшибок, "Титульная", "ОГРНИП", Ложь, Ложь);
	КонецЕсли;
	...
	Для Каждого Элт Из Данные.ДанныеМногостраничныхРазделов.СведБГ Цикл 
		...
		УведомлениеОСпецрежимахНалогообложения.ПроверкаБИК(СведБГ, ТаблицаОшибок, "СведБГ", "БИК", СведБГ.КемВыд = "1" Или СведБГ.КемВыд = "2");
		УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(СведБГ, ТаблицаОшибок, "СведБГ", "ИНН", Истина, Истина);
		УведомлениеОСпецрежимахНалогообложения.ПроверкаКПП(СведБГ, ТаблицаОшибок, "СведБГ", "КПП", Истина);
	КонецЦикла;
...
Функция ПроверитьДокументСВыводомВТаблицу_ФормаЗамена501(Данные, УникальныйИдентификатор)
	...
	УведомлениеОСпецрежимахНалогообложения.ПроверкаБИК(Титульная, ТаблицаОшибок, "Титульная", "БИКГар", Титульная.КемВыдГар = "1" Или Титульная.КемВыдГар = "2");
	УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(Титульная, ТаблицаОшибок, "Титульная", "ИННГар", Истина, Истина);
	УведомлениеОСпецрежимахНалогообложения.ПроверкаКПП(Титульная, ТаблицаОшибок, "Титульная", "КППГар", Истина);
	...
	ИначеЕсли Титульная.СпосПред = "2" Тогда 
		...
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложения.ПроверкаГода(Титульная, ТаблицаОшибок, "Титульная", "ОтчетГод", Истина);
	...
	Для Каждого Элт Из Данные.ДанныеМногостраничныхРазделов.СведДокПредЗам Цикл 
		...
		УведомлениеОСпецрежимахНалогообложения.ПроверкаБИК(СведДокПредЗам, ТаблицаОшибок, "СведДокПредЗам", "БИК", СведДокПредЗам.КемВыд = "1" Или СведДокПредЗам.КемВыд = "2");
		УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(СведДокПредЗам, ТаблицаОшибок, "СведДокПредЗам", "ИНН", Истина, Истина);
		УведомлениеОСпецрежимахНалогообложения.ПроверкаКПП(СведДокПредЗам, ТаблицаОшибок, "СведДокПредЗам", "КПП", Истина);
	КонецЦикла;
