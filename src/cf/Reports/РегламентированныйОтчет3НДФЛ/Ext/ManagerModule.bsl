...
Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	...
	Если ПрименяетсяОсновнаяФорма2022(ВыбраннаяФорма)
		Или ВыбраннаяФорма = "ФормаОтчета2021кв1"
		...
		Если ПрименяетсяОсновнаяФорма2022(ВыбраннаяФорма) Тогда
			ДанныеПриложения3 = ПрочитатьДанныеДекларации(ДанныеОтчета, ВыбраннаяФорма, "ДанныеПриложения3");
			Для Каждого СтрокаПриложения3 Из ДанныеПриложения3 Цикл
				...
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли ВыбраннаяФорма = "ФормаОтчета2021кв1" Тогда
		...
		Для Каждого СтрокаДекларации Из ДанныеДекларации Цикл
			...
			Если ЗначениеЗаполнено(СтрокаДекларации.КБК) И ЗначениеЗаполнено(СтрокаДекларации.ОКТМО) Тогда
				ЕстьНалогКУплате = Ложь;
				Если ПрименяетсяОсновнаяФорма2022(ВыбраннаяФорма) Тогда
					ЕстьНалогКУплате = (СтрокаДекларации.СуммаНалога > 0);
				ИначеЕсли ВыбраннаяФорма = "ФормаОтчета2021кв1"
					Или ВыбраннаяФорма = "ФормаОтчета2020кв1"
					Или ВыбраннаяФорма = "ФормаОтчета2019кв1"
					Или ВыбраннаяФорма = "ФормаОтчета2018кв1" Тогда
					ЕстьНалогКУплате = (СтрокаДекларации.ПризнакУплатыВозврата = ПризнакУплата);
				КонецЕсли;
				
				Если ЕстьНалогКУплате Тогда
				...
				Если ЗначениеЗаполнено(СтрокаДекларации.КБКИП) Тогда
					...
					Если ЗначениеЗаполнено(СтрокаДекларации.ОКТМОИП1Квартал) Тогда
						ДобавитьДанныеРеглОтчетаВТаблицу(
							...
							СтрокаДекларации.СуммаНалогаИПКУплате1Квартал - УплаченныеАвансы,
							ТаблицаДанныхРеглОтчета,
							ВыбраннаяФорма);
					КонецЕсли;
		...
		Если СвойствоРаздела.МногостраничностьВРазделе Тогда
			...
			Если РазделМногострочный Тогда
				Если Не НомераДобавленныхСтрок.Свойство(ИмяРаздела, НомерСтроки) Тогда
					...
					ИначеЕсли СвойствоРаздела.ИмяСтраницы = "Раздел1" И Не ЭтоПустыеДанные(СтраницаРаздела.Данные) Тогда
						СтраницаРаздела = НоваяСтраницаМногостраничногоРаздела(ТаблицаРаздела, СвойствоРаздела, ПоказателиРаздела(СоставПоказателей, ИмяРаздела));
						НомерСтраницы = ТаблицаРаздела.Количество();
						НомераДобавленныхСтраниц.Вставить(ИмяРаздела, НомерСтраницы); // Сохраняем номер добавленной страницы.
					КонецЕсли;
...
Функция ИмяФайлаСхемы(ВыбраннаяФорма, ДатаПодписи)
	...
	Если ВыбраннаяФорма = РеквизитыФормы2024().ИмяОбъекта Тогда
...
Функция ИмяМакетаДереваВыгрузкиXML(ВыбраннаяФорма)
	...
	Если ВыбраннаяФорма = РеквизитыФормыОтчета2024().ФормаОтчета Тогда
...
Функция СвойстваПоказателей(ОтчетОбъект, ВыбраннаяФорма, УникальныйИдентификатор) Экспорт
	...
	Если ВыбраннаяФорма = РеквизитыФормыОтчета2024().ФормаОтчета
		Или ВыбраннаяФорма = РеквизитыФормыОтчета2023().ФормаОтчета Тогда
...
Функция СтруктураРеквизитовФормы(Организация, Период, ВыбраннаяФорма, ПараметрыОткрытойФормы = Неопределено) Экспорт
	...
	Если ПараметрыФормы.мВыбраннаяФорма = РеквизитыФормы2024().ИмяОбъекта Тогда
...
Функция ПределыВычетов(ВыбраннаяФорма) Экспорт
	...
	ВычетНаНалогоплательщика = СтандартныйВычетНаНалогоплательщика(РазмерыВычетов.СтандартныйВычетНаНалогоплательщика);
...
Функция РазмерыВычетов(ВыбраннаяФорма) Экспорт
	...
	РазмерыВычетов.СтандартныйВычетНаНалогоплательщика.Вставить(
		Перечисления.СтандартныеВычетыНаНалогоплательщика.ВычетПо281_1_1, 3000);
	РазмерыВычетов.СтандартныйВычетНаНалогоплательщика.Вставить(
		Перечисления.СтандартныеВычетыНаНалогоплательщика.ВычетПо281_1_2, 500);
	
	Возврат РазмерыВычетов;
	
КонецФункции

Функция СтандартныйВычетНаНалогоплательщика(ДанныеВычета) Экспорт
	
	ВычетРезультат = Новый Структура;
	
	ВычетРезультат.Вставить("СтандартныйВычетПо218_1_1",
		ДанныеВычета[Перечисления.СтандартныеВычетыНаНалогоплательщика.ВычетПо281_1_1]);
	
	ВычетРезультат.Вставить("СтандартныйВычетПо218_1_2",
		ДанныеВычета[Перечисления.СтандартныеВычетыНаНалогоплательщика.ВычетПо281_1_2]);
...
Функция ВидНалогаПоКБК(КодБК, ВыбраннаяФорма)
	...
	ИначеЕсли КодБК = КБКДекларации.ДоходыОблагаемыеПоПовышеннойСтавке Тогда
		Возврат Перечисления.ВидыНалогов.НДФЛ_ИП_НалоговаяБазаСвышеПредельнойВеличины;
	ИначеЕсли КодБК = КБКДекларации.НалогПоПовышеннойСтавкеНаОснованииУведомления Тогда
		Возврат Перечисления.ВидыНалогов.НДФЛ_ДоходыСвышеПредельнойВеличины;
	...
	ИначеЕсли КодБК = КБКДекларации.ДоходыСДивидендовПоПовышеннойСтавке Тогда
		Возврат Перечисления.ВидыНалогов.НДФЛ_Дивиденды_ДоходыСвышеПредельнойВеличины;
	Иначе
...
Функция КлючиПоказателейДекларации2021(ИмяРаздела = "") Экспорт
...
Функция СвойстваПоказателей2021(ОтчетОбъект, ВыбраннаяФорма, УникальныйИдентификатор) Экспорт
...
Функция КлючиПоказателейДекларации2022(ИмяРаздела = "") Экспорт
...
Функция СвойстваПоказателей2022(ОтчетОбъект, ВыбраннаяФорма, УникальныйИдентификатор) Экспорт
...
Функция РеквизитыФормы2023()
	...
	Если ОбщегоНазначения.ТекущаяДатаПользователя() < '20240101' Тогда
		// До 01.01.204 нужно подавать декларацию по старой форме
...
Функция КлючиПоказателейДекларации2023(ИмяРаздела = "") Экспорт
...
Функция СвойстваПоказателей2023(ОтчетОбъект, ВыбраннаяФорма, УникальныйИдентификатор) Экспорт
...
Функция КлючиПоказателейДекларации2024(ИмяРаздела = "") Экспорт
