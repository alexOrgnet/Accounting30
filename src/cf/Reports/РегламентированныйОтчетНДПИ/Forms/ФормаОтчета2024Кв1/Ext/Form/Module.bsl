...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П00001М101003")
		...
		Иначе
			Область.Значение = "";
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='КБК должен состоять из 20 цифр. Значение очищено'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяПоказателя = "П000720012003" И ЗначПоказателя > 0.3 Тогда
		Область.Значение = 0;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Значение коэффициента Кт не должно превышать 0,3. Значение очищено'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел51" Тогда
		...
		Если КодВидаПИ = "10017" ИЛИ КодВидаПИ = "04104" ИЛИ КодВидаПИ = "04106" Тогда
			УдалитьИмяАвтоРасчПоказателя(Форма, АвтоРасчПоказатели, ОбрабатываемаяОбласть.Имя);
			РазрешитьВводДанныхВОбласть(Форма, ОбрабатываемаяОбласть);
		КонецЕсли;
