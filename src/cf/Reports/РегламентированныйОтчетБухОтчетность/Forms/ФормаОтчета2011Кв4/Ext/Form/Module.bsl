...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	ПереченьСведений = "ИННЮЛ, НаимЮЛПол, ОКПО, ОКОПФ, ОКФС, ФормСобств, ОргПравФорм";
	...
	ЗаполнитьОбластьВРазделах(РазделыСРеквизитами, "ОргНазв",        Сведения.НаимЮЛПол);
	...
	ЗаполнитьОбластьВРазделе("РасчетАктивов", "ОргНазв", Сведения.НаимЮЛПол);
...
Процедура ЗаполнитьПоляПодписей()
	...
	ЗаполнитьОбластьВРазделах(ВсеРазделы(), "ОргДиректор", Сведения.ФИОРук);
...
Процедура ДобавитьФайл(ВидДополнительногоФайла)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ПоказатьПредупреждение(, ТекстСообщения, , НСтр("ru = 'Ошибка'"));
