							...
							Для каждого СтрокаМнЧПодч Из ДанныеМнЧПодч.Строки Цикл
								Если ЗаполненаСтрокаМногострочнойЧасти(ИмяФормы,
									...
									Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
										...
										СтатусПроверки = Ложь;
									ИначеЕсли НЕ НомерКонтрактаСоответствуетТребованиям(ЗначПок, ТекстСообщения) Тогда
										СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Движение средств -> Страница " + НомСтраницы + " -> Строка "
...
Функция НомерКонтрактаСоответствуетТребованиям(Знач НомерКонтракта, ТекстСообщения)
...
Функция УзелПуст(ИмяФормы, Узел, ПараметрыВыгрузки)
	...
	Иначе
		Если Узел.Код = "УникНомер" Тогда
			...
			// "Приложение 4 к Разделу 1" - в случае ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2023Кв3")
			// или ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2023Кв1");
			// "Приложение 7 к Разделу 1" - в случае ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв1");
...
Процедура ЗаписатьУзелДереваВXML(ИмяФормы,
	...
	Иначе
		...
		ИначеЕсли СтрокаДерева.Код = "ПерсСвСтрахЛиц"
			И (ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2023Кв3")
			ИЛИ ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2023Кв1")
			...
			Если ЗаполненРаздел(ИмяФормы, ДеревоРаздела, "Раздел3",
				...
				Для каждого Ветка Из ДеревоРаздела.Строки Цикл
					...
					ПрАннулир = СокрЛП(Дан["П000030001001"]);
					Если ЗначениеЗаполнено(ПрАннулир) Тогда
						ПотокXML.ЗаписатьАтрибут("ПрАннулир", ПрАннулир);
					КонецЕсли;
					
					ПотокXML.ЗаписатьНачалоЭлемента("ДанФЛПолуч");
					
					П000310002001 = СокрЛП(Дан["П000310002001"]);
					Если ЗначениеЗаполнено(П000310002001) Тогда
						ПотокXML.ЗаписатьАтрибут("ИННФЛ", П000310002001);
					КонецЕсли;
					ПотокXML.ЗаписатьАтрибут("СНИЛС",     СокрЛП(Дан["П000310003001"]));
					ПотокXML.ЗаписатьАтрибут("ДатаРожд",  Формат(Дан["П000310007001"], "ДФ=dd.MM.yyyy"));
					ПотокXML.ЗаписатьАтрибут("Гражд",     СокрЛП(Дан["П000310008001"]));
					ПотокXML.ЗаписатьАтрибут("Пол",       СокрЛП(Дан["П000310009001"]));
					ПотокXML.ЗаписатьАтрибут("КодВидДок", СокрЛП(Дан["П000310010001"]));
					// Особая обработка: конкатенация значений серии и номера документа, удостоверяющего личность.
					ПотокXML.ЗаписатьАтрибут("СерНомДок", СокрЛП(Дан["П000310011001"]) + " " + СокрЛП(Дан["П000310011002"]));
					
					ПотокXML.ЗаписатьНачалоЭлемента("ФИО");
					
					ПотокXML.ЗаписатьАтрибут("Фамилия", СокрЛП(Дан["П000310004001"]));
					ПотокXML.ЗаписатьАтрибут("Имя",     СокрЛП(Дан["П000310005001"]));
					П000310006001 = СокрЛП(Дан["П000310006001"]);
					Если ЗначениеЗаполнено(П000310006001) Тогда
						ПотокXML.ЗаписатьАтрибут("Отчество", П000310006001);
					КонецЕсли;
					...
					Если ПрАннулир <> "1" Тогда
						...
						ИмяГруппы_1 = СтруктураРеквизитовФормы.мИдГруппы321М1;
						ДеревоМнЧ_1 = Ветка.ДанныеМногострочныхЧастей[ИмяГруппы_1];
						
						ЗаполненПодраздел321 = Ложь;
						МассивИменГраф = СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[ИмяГруппы_1]["ИменаГраф"];
						Для каждого СтрокаМнЧ Из ДеревоМнЧ_1.Строки Цикл
							Для каждого ИмяГрафы Из МассивИменГраф Цикл
								...
								КонецЕсли;
							КонецЦикла;
							Если ЗаполненПодраздел321 Тогда
								Прервать;
							КонецЕсли;
						КонецЦикла;
						
						ИмяГруппы_2 = СтруктураРеквизитовФормы.мИдГруппы322М1;
						ДеревоМнЧ_2 = Ветка.ДанныеМногострочныхЧастей[ИмяГруппы_2];
						
						ЗаполненПодраздел322 = Ложь;
						МассивИменГраф = СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[ИмяГруппы_2]["ИменаГраф"];
						Для каждого СтрокаМнЧ Из ДеревоМнЧ_2.Строки Цикл
							Для каждого ИмяГрафы Из МассивИменГраф Цикл
								Если ЗначениеЗаполнено(СтрокаМнЧ.Данные[ИмяГрафы]) Тогда
						...
						Если ЗаполненПодраздел321 ИЛИ ЗаполненПодраздел322 Тогда
							...
							Для каждого СтрокаМнЧ Из ДеревоМнЧ_1.Строки Цикл
								...
								ДанСтрМнЧ = СтрокаМнЧ.Данные;
								
								ПотокXML.ЗаписатьНачалоЭлемента("СвВыплМК");
								
								ПотокXML.ЗаписатьАтрибут("Месяц",          СокрЛП(ДанСтрМнЧ["П00321М112001"]));
								ПотокXML.ЗаписатьАтрибут("КодКатЛиц",      СокрЛП(ДанСтрМнЧ["П00321М113001"]));
								ПотокXML.ЗаписатьАтрибут("СумВыпл",        СокрЛП(Формат(ДанСтрМнЧ["П00321М114001"], Ч_17_2)));
								Если ДанСтрМнЧ["П00321М115001"] > 0 Тогда
									ПотокXML.ЗаписатьАтрибут("ВыплОПС",    СокрЛП(Формат(ДанСтрМнЧ["П00321М115001"], Ч_17_2)));
								КонецЕсли;
								Если ДанСтрМнЧ["П00321М116001"] > 0 Тогда
									ПотокXML.ЗаписатьАтрибут("ВыплОПСДог", СокрЛП(Формат(ДанСтрМнЧ["П00321М116001"], Ч_17_2)));
								КонецЕсли;
								Если ДанСтрМнЧ["П00321М117001"] > 0 Тогда
									ПотокXML.ЗаписатьАтрибут("НачислСВ",   СокрЛП(Формат(ДанСтрМнЧ["П00321М117001"], Ч_17_2)));
								КонецЕсли;
							...
							Если ЗаполненПодраздел322 Тогда
								...
								ПотокXML.ЗаписатьНачалоЭлемента("ВыплСВДоп");
								
								Для каждого СтрокаМнЧ Из ДеревоМнЧ_2.Строки Цикл
									
									ДанСтрМнЧ = СтрокаМнЧ.Данные;
									
									ПотокXML.ЗаписатьНачалоЭлемента("ВыплСВДопМТ");
									
									ПотокXML.ЗаписатьАтрибут("Месяц",       СокрЛП(ДанСтрМнЧ["П00322М118001"]));
									ПотокXML.ЗаписатьАтрибут("КодСтрахЛиц", СокрЛП(ДанСтрМнЧ["П00322М119001"]));
									ПотокXML.ЗаписатьАтрибут("ВыплСВ",      СокрЛП(Формат(ДанСтрМнЧ["П00322М120001"], Ч_17_2)));
									ПотокXML.ЗаписатьАтрибут("НачислСВ",    СокрЛП(Формат(ДанСтрМнЧ["П00322М121001"], Ч_17_2)));
									
									ПотокXML.ЗаписатьКонецЭлемента();// "ВыплСВДопМТ"
									
								КонецЦикла;
								
								ПотокXML.ЗаписатьКонецЭлемента();// "ВыплСВДоп"
