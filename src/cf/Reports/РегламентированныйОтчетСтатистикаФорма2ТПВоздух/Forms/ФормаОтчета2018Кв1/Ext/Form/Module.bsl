...
Процедура ДополнитьПараметрыВыгрузки(ПараметрыВыгрузки)
	...
	//ТЗЗагрязняющиеВещества = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ЗагрязняющиеВещества").ТаблицаЗначений;
	Пока Истина Цикл 
		Инд = Инд + 1;
		...
		Если ЗначениеЗаполнено(КодВещества) Тогда 
			...
			//СтруктураПоиска.Код = КодВещества;
			//Строки = ТЗЗагрязняющиеВещества.НайтиСтроки(СтруктураПоиска);
			//Если СтрДлина(КодВещества) <> 4 И Строки.Количество() = 0 Тогда 
			//	СтруктураПоиска.Код = Прав("0000" + СтруктураПоиска.Код, 4);
			//	Строки = ТЗЗагрязняющиеВещества.НайтиСтроки(СтруктураПоиска);
			//КонецЕсли;
			//Если Строки.Количество() = 0 Тогда
			//	ПараметрыВыгрузки.Вставить("П800000010101_" + Формат(Итер, "ЧГ="), "");
			//Иначе
			//	ПараметрыВыгрузки.Вставить("П800000010101_" + Формат(Итер, "ЧГ="), Строки[0].Идентификатор);
			//КонецЕсли;
		КонецЕсли;
	КонецЦикла;
