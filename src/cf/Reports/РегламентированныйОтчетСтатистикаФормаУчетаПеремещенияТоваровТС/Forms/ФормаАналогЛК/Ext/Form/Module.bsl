...
Процедура ДобавитьТовар(Команда)
	Если Товары.Количество() >= 999 Тогда 
	...
	ВводТовара = ПолучитьФорму(ИмяФормыВводаТовара, ПараметрыФормы, ЭтотОбъект);
	ОписаниеОповещения = Новый ОписаниеОповещения("ВводТовараЗавершение", ЭтотОбъект, ПараметрыФормы);
	
	ВводТовара.ОписаниеОповещенияОЗакрытии = ОписаниеОповещения;
	ВводТовара.РежимОткрытияОкна = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
	ВводТовара.Открыть();
КонецПроцедуры
...
Процедура ПроверитьПоказателиОтчета(Команда)
	...
	Если Не ЗначениеЗаполнено(Заявитель_Телефон) Или Не ЗначениеЗаполнено(Заявитель_ЭлектроннаяПочта) Тогда
		...
		ЕстьОшибки = Истина;
	ИначеЕсли СтрНайти(Заявитель_Телефон, "+") > 0 Тогда 
		Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаСведенияОЗаявителе;
...
Функция Сохранить(Автосохранение = Ложь, 
	...
	СохраняемыеПоказатели.Вставить("ВерсияФТС", "5.21.0");
...
Процедура Инициализация(БезОткрытияФормы = Ложь, Знач ДокОснование = Неопределено) Экспорт
	...
	Попытка
		Если ЗначениеЗаполнено(ДокОснование) Тогда
			...
			ЗагрузитьТовары(ДокОснование, ДанныеПервичногоДокумента.ТаблицаТоварыДляВыгрузки, ДанныеПервичногоДокумента.ШтрихкодыУпаковок);
...
Функция ОбычныеРеквизитыДляСохранения()
	РеквизитыДляСохранения =  "РегНомерАннулируемойФормы,Графа4,Графа5Код,Графа5Наименование,Графа6Код,Графа6Наименование,Графа7Код,Графа7Сумма,"
			...
			+"Графа1Ошибка,Графа2Ошибка,Графа3Ошибка,Основание";
...
Процедура ДоступностьПолей(Форма)
	...
	Элементы.ТоварыДобавитьТовар.Доступность = (Форма.Товары.Количество() < 999);
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ИзвлечьСтруктуруXML(ИмяФормы, "СхемаВыгрузкиXMLЛК_5_21_0");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Иначе
		Для Каждого Стр Из Товары Цикл 
			...
			ПараметрыВыгрузкиСтрокиТовара.Свойство("Товар_GoodsDescription", ТоварСтрока.Области.НаименованиеТовара.Значение);
...
Процедура ЗаполнитьДаннымиПервичногоДокументаКлиент(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю("Необходимо выбрать направление перемещения", , "НаправлениеПеремещения");
		Возврат;
	КонецЕсли;
	ОткрытьФорму(ИмяФормыВыбора,
		Новый Структура("Отбор", Новый Структура("Организация", СтруктураРеквизитовФормы.Организация)),
		ЭтотОбъект,,,,ОО, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
&НаКлиенте
Процедура ЗаполнитьДаннымиПервичногоДокументаПослеВыбора(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если НачалоМесяца(РеквизитыДокумента.Дата) <> НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) Тогда 
		ОО = Новый ОписаниеОповещения("ЗаполнитьНаОснованииЗаявленияОВвозеПослеИзмененияПериода", ЭтотОбъект, РеквизитыДокумента);
		ПоказатьВопрос(ОО, "Период документа не совпадает с периодом отчета. Изменить период отчета?", РежимДиалогаВопрос.ДаНетОтмена);
	Иначе 
...
&НаКлиенте
Процедура ЗаполнитьНаОснованииЗаявленияОВвозеПослеИзмененияПериода(Результат, РеквизитыДокумента) Экспорт 
	...
	ЗаполнитьДаннымиПервичногоДокумента(РеквизитыДокумента.Ссылка);
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьДаннымиПервичногоДокумента(Ссылка)
	Попытка
		...
		ДополнитьКонтакты(ДанныеПервичногоДокумента.РеквизитыШапка);
		ЗагрузитьДокументы(ДанныеПервичногоДокумента.ТаблицаДокументы);
		ЗагрузитьТовары(Ссылка, ДанныеПервичногоДокумента.ТаблицаТоварыДляВыгрузки, ДанныеПервичногоДокумента.ШтрихкодыУпаковок);
		ЗагрузитьСведенияОЗаявителе(ДанныеПервичногоДокумента.РеквизитыШапка);
		ЗагрузитьГрафа1(ДанныеПервичногоДокумента.РеквизитыШапка);
		ЗагрузитьГрафа2(ДанныеПервичногоДокумента.РеквизитыШапка);
		ЗагрузитьГрафа3(ДанныеПервичногоДокумента.РеквизитыШапка);
...
Функция СтруктураТовар(СтрокаТовар)
	...
	Если ЗначениеЗаполнено(СтрокаТовар.ДопСведенияКод) Тогда
		НовСтр = ДополнительнаяИнформация.Добавить();
		НовСтр.AdditionalInformationCode = СтрокаТовар.ДопСведенияКод;
		НовСтр.AdditionalInformation = СтрокаТовар.ДопСведения;
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьТовары(Ссылка, ТаблицаТоварыДляВыгрузки, ШтрихкодыУпаковок)
	Товары.Очистить();
	Отбор = Новый Структура("ДокументСсылка, Номенклатура", Ссылка, Неопределено);
	Для Каждого Стр Из ТаблицаТоварыДляВыгрузки Цикл
		СтруктураТовараБезКодовМаркировки = СтруктураТовар(Стр);
			...
			Для Каждого ШК Из ШтрихкодыУпаковок.НайтиСтроки(Отбор) Цикл 
				НовСтр = СтруктураТовараБезКодовМаркировки.СредстваИдентификации.Добавить();
		...
		ЗаполнитьЗначенияСвойств(Товары.Добавить(),
			ФормаУчетаПеремещенияТоваровТС.СформироватьСтруктуруТовараПриЗагрузке(СтруктураТовараБезКодовМаркировки));
	КонецЦикла;
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьСведенияОЗаявителе(Реквизиты)
	ОтвЛицоПоказатели = ",ТелОтвСтатистика,ДолжнОтвСтатистика,АдресЭлПочтыОтвСтатистика,ФИООтвСтатистика";
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Реквизиты.Организация) Тогда 
		СтрокаСведений = "ИННЮЛ,НаимЮЛПол,КППЮЛ,НаимЮЛСокр,ОГРН" + ОтвЛицоПоказатели;
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
	...
	Иначе 
		СтрокаСведений = "ИННФЛ,ФИО,ОГРН" + ОтвЛицоПоказатели;
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Реквизиты.Организация, Реквизиты.Дата, СтрокаСведений);
	...
	Заявитель_Телефон = СведенияОбОрганизации.ТелОтвСтатистика;
...
Функция ОбщиеПоказателиГраф13(Реквизиты)
	...
	ПоказателиГрафы.Вставить("IdentityCardName", Реквизиты.ОтвЛицоФинУрПодНаименование);
