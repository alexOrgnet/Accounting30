	...
	Если "RU" = CountryCode Тогда 
		...
		ИначеЕсли СтрДлина(RFINN) = 12 Тогда 
			Если Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(RFINN, Ложь, "") Тогда 
				...
				ЕстьОшибки = Истина;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(RFOGRN) Или Не РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(RFOGRN, Ложь, "") Тогда
				Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаИдентификаторы;
				ОбщегоНазначенияКлиент.СообщитьПользователю("Не указан / неправильно указан ОГРН", , "RFOGRN");
				ЕстьОшибки = Истина;
			КонецЕсли;
...
Процедура ДоступностьПолей(Форма)
	...
	ИначеЕсли Форма.CountryCode = "BY" Тогда
		ПоказателиДляОчистки = "RFOGRN,RFINN,RFKPP,RKBIN,RKIIN,RAUNN,RASocialServiceNumber,RASocialServiceCertificate,KGINN,KGOKPO";
		ОчиститьПоказатели(Форма, ПоказателиДляОчистки);
	ИначеЕсли Форма.CountryCode = "KZ" Тогда 
		ПоказателиДляОчистки = "RFOGRN,RFINN,RFKPP,RBUNP,RBIdentificationNumber,RAUNN,RASocialServiceNumber,RASocialServiceCertificate,KGINN,KGOKPO";
		ОчиститьПоказатели(Форма, ПоказателиДляОчистки);
	ИначеЕсли Форма.CountryCode = "AM" Тогда 
		ПоказателиДляОчистки = "RFOGRN,RFINN,RFKPP,RBUNP,RBIdentificationNumber,RKBIN,RKIIN,KGINN,KGOKPO";
		ОчиститьПоказатели(Форма, ПоказателиДляОчистки);
	ИначеЕсли Форма.CountryCode = "KG" Тогда 
		ПоказателиДляОчистки = "RFOGRN,RFINN,RFKPP,RBUNP,RBIdentificationNumber,RKBIN,RKIIN,RAUNN,RASocialServiceNumber,RASocialServiceCertificate";
		ОчиститьПоказатели(Форма, ПоказателиДляОчистки);
	Иначе
		ПоказателиДляОчистки = "RFOGRN,RFINN,RFKPP,RBUNP,RBIdentificationNumber,RKBIN,RKIIN,RAUNN,RASocialServiceNumber,"
			+ "RASocialServiceCertificate,KGINN,KGOKPO";
		ОчиститьПоказатели(Форма, ПоказателиДляОчистки);
	КонецЕсли;
	...
	Если "RU" = Форма.CountryCode Тогда 
		...
		ИначеЕсли СтрДлина(Форма.RFINN) = 12 Тогда 
			Если Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(Форма.RFINN, Ложь, "") Тогда 
				Возврат Ложь;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Форма.RFOGRN) Или Не РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(Форма.RFOGRN, Ложь, "") Тогда
				Возврат Ложь;
			КонецЕсли;
		Иначе
		...
		Если Не ЗначениеЗаполнено(Форма.BusinessEntityTypeName) Или Не ЗначениеЗаполнено(Форма.PostalCode)Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
