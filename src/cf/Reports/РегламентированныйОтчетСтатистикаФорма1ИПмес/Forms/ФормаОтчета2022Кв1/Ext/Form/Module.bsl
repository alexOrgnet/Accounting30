...
Процедура ФормированиеТаблицыВыбораПродукции()
	...
	ТЗВыб = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель","П001001000101"))[0].ТаблицаЗначений.Выгрузить();
	ТЗВыб.ЗаполнитьЗначения(Ложь, "ПризнакОтбора");
	
	МассивОтбора = СвойстваПоказателей.НайтиСтроки(Новый Структура(
		"Показатель","П001001000100"))[0].ТаблицаЗначений.Выгрузить(Новый Структура("ПризнакОтбора", Ложь),	"Код");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001000100",
		250, ,                          , КоллекцияСписковВыбора["СправочникПриложениеДляВводаОКПД"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001000101",
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03_" + НомерСтроки].Значение));
	ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, Формат(ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "06_" + НомерСтроки].Значение, "ЧГ=")));
	Иначе
		...
		Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
			...
			Если ЗначениеЗаполнено(ВыборКод) Тогда
				Если ПредыдущееЗначениеВКолонке01 <> ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "01_" + НомерСтроки].Значение Тогда
					ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "04_" + НомерСтроки].Значение = "";
					ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "06_" + НомерСтроки].Значение = "";
				КонецЕсли;
			Иначе
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "04_" + НомерСтроки].Значение = "";
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "06_" + НомерСтроки].Значение = "";
	...
	Иначе
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
				...
				КопироватьДанныеФормы(ТЗВыбораПродукции, ТЗВыбора);
			...
			Иначе
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура(
					"Ключ", ТабличныйДокумент.Области["П001001000103_" + Сред(Область.Имя, 15)].Значение));
				ТЗВыбора.Очистить();
