...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ТабличныйДокумент.Вывести(ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы,
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяПоказателя = "П000030009003" Тогда
		Если ЗначПоказателя > 1 Тогда
			Область.Значение = 0;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Коэффициент Кв не должен превышать 1. Значение очищено'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьРаздел1()
	...
	Для Инд = 0 По ТЗ.Количество() - 1 Цикл
		...
		ТекСумма
		= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, ДанныеСтрокиМнЧР1["П000010002103"])
		- РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, ДанныеСтрокиМнЧР1["П000010002303"])
		- РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, ДанныеСтрокиМнЧР1["П000010002503"])
		- РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, ДанныеСтрокиМнЧР1["П000010002703"]);
...
Процедура СформироватьСтруктуруДанныхКС()
	...
	Для каждого СтрокаМнЧ Из Раздел1 Цикл
		...
		НоваяСтрока.Р01С021		= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, 
	...
	Для каждого Элемент Из Раздел2 Цикл
		...
		НоваяСтрока.Р02С140		= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, 
			Элемент.Данные.П000020014003);
		НоваяСтрока.Р02С260		= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, 
	...
	Для каждого Элемент Из Раздел21 Цикл
		...
		НоваяСтрока.Р21С050		= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, 
	...
	Для каждого Элемент Из Раздел3 Цикл
		...
		НоваяСтрока.Р03С130		= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, 
...
Процедура ПроверкаКС_1_07_код(Настройки)
	...
	Для каждого СтраницаР02 Из ДанныеКС.Р02 Цикл
		...
		РасчетнаяСумма = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, СуммаПоказателей / 13);
...
Процедура ПроверкаКС_1_10_код(Настройки)
	...
	Для каждого СтраницаР02 Из ДанныеКС.Р02 Цикл
		...
		РасчетнаяСумма = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Разность * Множитель);
...
Процедура ПроверкаКС_1_22()
	...
	РасчетнаяСумма2			= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, 0.49 * КоличествоР21);
