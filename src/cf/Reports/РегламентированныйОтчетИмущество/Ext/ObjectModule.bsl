...
Функция ПроверитьКонтрольныеСоотношенияДлительнаяОперация(СтруктураПараметров) Экспорт
	...
	Если НЕ СтрНайти(СтруктураПараметров.мВыбраннаяФорма, "2022Кв4") = 0 Тогда
...
#Область Проверки
	...
	ПроверкаКС_1_04_2023();
	ПроверкаКС_1_05_2023();
	ПроверкаКС_1_06_2023();
	ПроверкаКС_1_55_2023();
	ПроверкаКС_1_56_2023();
	ПроверкаКС_1_57_2023();
...
Процедура ПроверкаКС_1_04_2023()
...
Процедура ПроверкаКС_1_04_2023_код()
	...
	Пока ВыборкаР1.Следующий() Цикл
		...
		Если НастройкиКС.НомерКС = "1.4" И ЗначениеЗаполнено(ВыборкаР1.П000010000703_) Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура ПроверкаКС_1_55_2023()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Значение суммы налога, исчисленной к уменьшению в стр.040 Раздела 1 <= (стр.023 + '") +
		НСтр("ru='стр.025 + стр.027) Раздела 1'"));
...
Процедура ПроверкаКС_1_56_2023()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Раздел 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='1.56'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Значение суммы налога, подлежащей уплате в стр.030 Раздела 1 >= 0'"));
	...
	ПроверкаКС_1_56_2023_код();
	
КонецПроцедуры

Процедура ПроверкаКС_1_56_2023_код()
...
Процедура ПроверкаКС_1_57_2023()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Раздел 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='1.57'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Значение суммы налога, исчисленной к уменьшению в стр.040 Раздела 1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибочная сумма налога, исчисленная к уменьшению'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.040 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 21.02.2023 № ЕА-4-15/2048@'"));
