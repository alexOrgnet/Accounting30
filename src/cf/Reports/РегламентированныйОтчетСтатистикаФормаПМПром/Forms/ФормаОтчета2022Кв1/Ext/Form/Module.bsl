...
Процедура ФормированиеТаблицыВыбораПродукции()
	...
	НовСтрТЗВыб = ТЗВыб.Вставить(0);
	НовСтрТЗВыб.Код = "";
	НовСтрТЗВыб.Название = "Пустое значение";
	НовСтрТЗВыб.ПризнакОтбора = Истина;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(
			КолонкаПоиска, ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "03_" + НомерСтроки].Значение));
	ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(
			КолонкаПоиска, Формат(ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "05_" + НомерСтроки].Значение, "ЧГ=")));
	Иначе
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Тогда
		...
		Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
			...
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "03_" + НомерСтроки].Значение =
				?(ВыборКод = "0", "", ВыборКод);
			...
			Если ЗначениеЗаполнено(ВыборКод) Тогда
				Если ПредыдущееЗначениеВКолонке01 <> ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "01_" + НомерСтроки].Значение Тогда
					...
					ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "05_" + НомерСтроки].Значение = "";
				КонецЕсли;
			Иначе
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "04_" + НомерСтроки].Значение = "";
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "05_" + НомерСтроки].Значение = "";
		...
		ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
			...
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "05_" + НомерСтроки].Значение = РезультатВыбора["Код"];
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Иначе
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ",
					ПолеТабличногоДокументаФормаОтчета.Области["П002000100103_" + Сред(Область.Имя, 15)].Значение));
				ТЗВыбора.Очистить();
