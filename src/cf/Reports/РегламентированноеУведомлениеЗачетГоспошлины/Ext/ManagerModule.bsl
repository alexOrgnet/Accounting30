...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2023_1(Данные, УникальныйИдентификатор)
	...
	Иначе
		...
		Иначе
			...
			КонецЕсли;
			УведомлениеОСпецрежимахНалогообложения.Проверка_УдЛичнФЛ("КодВидДок,СерНомДок,ДатаДок,ВыдДок", УдЛичнФЛ, ТаблицаОшибок, "УдЛичнФЛ", Истина);
		КонецЕсли;
	КонецЕсли;
	...
	УведомлениеОСпецрежимахНалогообложения.ПроверкаОКТМО(Титульная, ТаблицаОшибок, "Титульная", "ОКТМОЗачет", Истина);
	УведомлениеОСпецрежимахНалогообложения.ПроверкаКБК(Титульная, ТаблицаОшибок, "Титульная", "КБКЗачет", Истина);
