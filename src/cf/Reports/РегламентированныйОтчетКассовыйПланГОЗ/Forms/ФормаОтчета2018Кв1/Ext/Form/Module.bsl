...
Процедура Расшифровать(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ОткрытьРасшифровкуОтчета(ИДОтчета(ИмяФормы), ИДРедакцииОтчета(ИмяФормы), ИДИменПоказателей, ПараметрыОтчета)
КонецПроцедуры
	...
	Если ПараметрыВыгрузки.ПричиныОтклоненияЗаполнены Тогда 
		...
		Для Каждого Стр Из СохрСтруктура.ДанныеМногоуровневыхРазделов.ПричиныОтклонения.Строки Цикл 
			...
			Если Не ПризнакиЗаполнены Тогда 
				СообщитьВТС("Не указаны признаки отклонения.", "ПричиныОтклонения", "Reason1", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ВыгрузкаОтчета(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "КассовыйПланГОЗ");
