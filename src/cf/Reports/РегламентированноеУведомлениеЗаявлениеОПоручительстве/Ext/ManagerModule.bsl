...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2023_1(Объект, Данные, УникальныйИдентификатор)
	...
	УведомлениеОСпецрежимахНалогообложения.ПроверкаКодаНО(Титульная.КодНОДог, ТаблицаОшибок, "Титульная", "КодНОДог");
	...
	Иначе
		Если Не ЗначениеЗаполнено(Титульная.ИНН) Тогда 
			УведомлениеОСпецрежимахНалогообложения.Проверка_УдЛичнФЛ("КодВидДок,СерНомДок,ВыдДок,ДатаДок", 
	...
	Если ЗначениеЗаполнено(Титульная.СвЗаинтЛицНаимОрг) Тогда 
		...
		Иначе
			УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(Титульная, ТаблицаОшибок, "Титульная", "СвЗаинтЛицИНН", Истина, Истина);
			УведомлениеОСпецрежимахНалогообложения.ПроверкаКПП(Титульная, ТаблицаОшибок, "Титульная", "СвЗаинтЛицКПП", Истина);
		КонецЕсли;
	Иначе
		УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(Титульная, ТаблицаОшибок, "Титульная", "СвЗаинтЛицИНН", Истина, Ложь);
