...
Функция СформироватьДеревоСтраниц(ИмяФормы)
	Постфикс = "";
	МакетПФПостфикс = "2017";
	Если ИмяФормы = "Форма2023_1" Тогда 
		Постфикс = "_2023";
		МакетПФПостфикс = "2023";
	КонецЕсли;
	...
	Стр001.ИмяМакета = "Заявление" + Постфикс;
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "Заявление";
	Стр001.МакетыПФ = "Печать_Форма" + МакетПФПостфикс + "_1_Заявление";
	
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Обязательство о" + Символы.ПС + "соблюдении условий";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Обязательство" + Постфикс;
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "Обязательство";
	Стр001.МакетыПФ = "Печать_Форма" + МакетПФПостфикс + "_1_Обязательство";
	
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "График платежей";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "График" + Постфикс;
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Истина;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "График";
	Стр001.МногострочныеЧасти.Добавить("МнгСтр");
	Стр001.МакетыПФ = "Печать_Форма" + МакетПФПостфикс + "_1_График";
	
	Возврат ДеревоСтраниц;
КонецФункции

Функция СоздатьНовоеУведомлениеПоДанным(Выборка, ИмяФормы)
	...
	СтруктураПараметров.Вставить("ДеревоСтраниц", СформироватьДеревоСтраниц(ИмяФормы));
...
Процедура КонвертироватьНаСервере(Выборка, ИмяФормы) 
	Попытка
		НовоеУведомление = СоздатьНовоеУведомлениеПоДанным(Выборка, ИмяФормы);
...
Процедура КонвертацияИзРегламентированногоОтчета(ВыборкаСтрока) Экспорт
	Попытка
		Если ВыборкаСтрока.ВыбраннаяФорма = "ФормаОтчета2017Кв1" Тогда
			КонвертироватьНаСервере(ВыборкаСтрока.Ссылка, "Форма2017_1");
		ИначеЕсли ВыборкаСтрока.ВыбраннаяФорма = "ФормаОтчета2023Кв1" Тогда
			КонвертироватьНаСервере(ВыборкаСтрока.Ссылка, "Форма2023_1");
		КонецЕсли;
	Исключение
	КонецПопытки;
КонецПроцедуры
