...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "Форма2024_1";
	Стр.ОписаниеФормы = "Заявление о применении заявительного порядка возмещения НДС (акциза), по письму ФНС России от 27.03.2024 № ЕА-4-15/3458@";
...
Функция ЭлектронноеПредставление(Объект, ИмяФормы, УникальныйИдентификатор) Экспорт
	Если ИмяФормы = "Форма2022_1" Или ИмяФормы = "Форма2023_1" Тогда 
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2024_1(Данные, УникальныйИдентификатор)
	...
	ИначеЕсли ЗаявВозм.СпосПред = "2" Тогда 
		...
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложения.ПроверкаГода(ЗаявВозм, ТаблицаОшибок, "ЗаявВозм", "ОтчетГод", Истина);
	...
	Если УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(СведВозм, "НаимБанк,БИК,НомСчет") Тогда 
		...
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложения.ПроверкаБИК(СведВозм, ТаблицаОшибок, "СведВозм", "БИК", Истина);
		УведомлениеОСпецрежимахНалогообложения.ПроверкаНомераСчета(СведВозм, ТаблицаОшибок, "СведВозм", "НомСчет", Истина);
	ИначеЕсли УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(Данные.ДанныеУведомления.СведВозм, "НаимВладСчет,КБКПолуч,ОКТМО,НомЛиц,НазнПлат") Тогда 
		...
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложения.ПроверкаОКТМО(СведВозм, ТаблицаОшибок, "СведВозм", "ОКТМО", Истина);
		УведомлениеОСпецрежимахНалогообложения.ПроверкаКБК(СведВозм, ТаблицаОшибок, "СведВозм", "КБКПолуч", Ложь);
	...
	Для Каждого Элт Из Данные.ДанныеМногостраничныхРазделов.РеквБГПор Цикл 
		...
		УведомлениеОСпецрежимахНалогообложения.ПроверкаБИК(РеквБГПор, ТаблицаОшибок, "РеквБГПор", "БИК", РеквБГПор.КемВыд = "1" Или РеквБГПор.КемВыд = "2");
		УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(РеквБГПор, ТаблицаОшибок, "РеквБГПор", "ИНН", Истина, Истина);
		УведомлениеОСпецрежимахНалогообложения.ПроверкаКПП(РеквБГПор, ТаблицаОшибок, "РеквБГПор", "КПП", Истина);
	КонецЦикла;
