...
Процедура ДобавитьСтраницуНаСервере()
	...
	УведомлениеОСпецрежимахНалогообложенияКлиентСервер.ПереименованиеСтраницЖурналаСправок(
		ДеревоСтраниц, УИДТекущаяСтраница, ДанныеМногостраничныхРазделов["СведУплПенсВзн"]);
КонецПроцедуры
...
Процедура ПечатьСправкиНаСервере() Экспорт 
	СохранитьДанные();
	СтруктураРеквизитовУведомления = Новый Структура(
		"СписокПечатаемыхЛистов", Отчеты[Объект.ИмяОтчета].СформироватьСправку(Объект.Ссылка, УИДТекущаяСтраница));
	
	УведомлениеОСпецрежимахНалогообложения.ПроставлениеКоличестваСтраниц(СтруктураРеквизитовУведомления.СписокПечатаемыхЛистов);
	Для Каждого Элт Из СтруктураРеквизитовУведомления.СписокПечатаемыхЛистов Цикл 
...
Функция СформироватьСправкуXMLНаСервере(УникальныйИдентификатор) Экспорт 
	СохранитьДанные();
	Возврат Отчеты[Объект.ИмяОтчета].ЭлектронноеПредставлениеСправки(Объект, УИДТекущаяСтраница);
КонецФункции

&НаСервере
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	Выгрузка = Отчеты[Объект.ИмяОтчета].ЭлектронноеПредставлениеСправки(Объект, УИДТекущаяСтраница);
...
Функция ПроверитьВыгрузкуСправкиНаСервере() Экспорт 
	СохранитьДанные();
	Возврат Отчеты[Объект.ИмяОтчета].ПроверитьСправкуСВыводомВТаблицу(Объект.Ссылка, УИДТекущаяСтраница);
КонецФункции
