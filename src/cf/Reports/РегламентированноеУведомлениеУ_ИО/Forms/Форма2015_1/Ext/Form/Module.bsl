...
Процедура СформироватьМакетНаСервере()
	...
	ДоступностьОбластейНумерации(ЭтотОбъект, ПредставлениеУведомления, Автонумерация);
КонецПроцедуры
...
Асинх Процедура ПечатьУведомления(Команда)
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Или Модифицированность Тогда 
		Если Ждать ВопросАсинх("Перед печатью необходимо сохранить изменения. Сохранить изменения?", РежимДиалогаВопрос.ДаНет) <> КодВозвратаДиалога.Да Тогда 
			Возврат;
		КонецЕсли;
		СохранитьДанные();
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура Перенумеровать(Команда)
	Если КодВозвратаДиалога.Да 
		= Ждать ВопросАсинх("При перенумерации страниц данные листов Г будут очищены. Продолжить?", РежимДиалогаВопрос.ДаНет) Тогда 
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДоступностьОбластейНумерации(Форма, ПредставлениеУведомления, Автонумерация)
	Если Автонумерация Тогда 
		Цвет = Форма["ЦФБ"];
		Защита = Истина;
	Иначе
