...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если ИмяТекРаздела = "Раздел1" Тогда
		...
		МакетРаздела1 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1");
	...
	ИначеЕсли ИмяТекРаздела = "Раздел2" Тогда
		...
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел2");
	...
	ИначеЕсли ИмяТекРаздела = "Раздел31" Тогда
		...
		МакетРаздела31 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел31");
		
		МакетРаздела32 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел32");
		
		МакетРаздела33 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел33");
	...
	ИначеЕсли ИмяТекРаздела = "Раздел41" Тогда
		...
		МакетРаздела41 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел41");
		
		МакетРаздела42 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел42");
	...
	ИначеЕсли ИмяТекРаздела = "Раздел5" Тогда
		...
		МакетРаздела5 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел5");
...
Процедура ПроверкаКС_1_01()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Сумма налога в стр.(030, 040, 050) Раздела 1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0, %2 >= 0, %3 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Укажите сумму налога в стр.030, 040, 050 Раздела 1 >= 0'"));
	...
	Для Счетчик = 1 По ДанныеКС.Раздел1.КолвоСтрокП0001000 Цикл
		...
		НастройкиКС.ЭтоОшибка = Показатели.Р01С030.Значение < 0 ИЛИ Показатели.Р01С040.Значение < 0 
			ИЛИ Показатели.Р01С050.Значение < 0;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
			Показатели.Р01С030.Значение, Показатели.Р01С040.Значение, Показатели.Р01С050.Значение);
...
Процедура ПроверкаКС_1_26()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно некорректное значение показателя отчета'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Проверьте заполнение стр.070 - 140 подр.3.2 Раздела 3'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 19.12.2023 № СД-4-3/15920'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.3.2 Раздела 3'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел31 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел312 Цикл
			Для каждого СтраницаПодраздела2 Из СтраницаПодраздела.Раздел3124 Цикл
				...
				НастройкиКС.ЭтоОшибка = 
					ЗначениеЗаполнено(Показатели.Р33С002.Значение) И ЗначениеЗаполнено(Показатели.Р33С003.Значение) 
					И МассивКодов.Найти(Показатели.Р33С001.Значение) = Неопределено;
				НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
					Показатели.Р33С001.ЗначениеСтр, Показатели.Р33С002.ЗначениеСтр, Показатели.Р33С003.ЗначениеСтр);
				НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
					НомерСтраницы(СтраницаПодраздела2.НомерСтраницы));
