...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		...
		Если Не ИнтернетПоддержкаПодключена Тогда 
			ИНН1С = "7709860400";
			Реквизиты1С = ОбработкаЗаявленийАбонентаПовтИсп.РеквизитыКонтрагента(ИНН1С);
			ИнтернетПоддержкаПодключена = (ТипЗнч(Реквизиты1С) = Тип("Структура")) 
				И (ИНН1С = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Реквизиты1С, "ИНН"));
		КонецЕсли;
	Исключение
	КонецПопытки;
...
Процедура ЗаполнитьНачальныеДанные() Экспорт 
	ДанныеУведомленияТитульный = ДанныеУведомления["Лист001"];
...
Процедура ДобавитьСтрокуНаСервере(ИмяОбласти)
	...
	ПоказателиМСЧ = ПоказателиМСЧ(МакетМСЧ);
	ИндСтр = РазделительНомераСтроки + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомерДобавляемойСтроки);
...
Процедура ЗагрузитьДанные(СсылкаНаДанные)
	...
	СохраненныеДанные = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаДанные, "ДанныеУведомления").Получить();
	...
	УведомлениеОСпецрежимахНалогообложения.ЗагрузкаМногострочныхЧастейУведомления(ЭтотОбъект, СохраненныеДанные);
	СохраненныеДанные.Свойство("РазрешитьВыгружатьСОшибками", РазрешитьВыгружатьСОшибками);
	ДобавитьНедостающиеПоказатели();
	
	Если СохраненныеДанные.Свойство("СтруктураПолейОтправки") Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, СохраненныеДанные["СтруктураПолейОтправки"]);
	КонецЕсли;
КонецПроцедуры
...
Функция ПроверкиПередОтправкой()
	...
	Если СтрДлина(РегистрирующийОрганКод) <> 4 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(РегистрирующийОрганКод) Тогда 
		...
		Возврат Ложь;
	КонецЕсли;
	Если ДанныеМногостраничныхРазделов.ЛистП.Количество() <> 1 Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			"Для целей отправки можно указать только одного заявителя (может быть заполнен только один Лист П)");
		Возврат Ложь;
	Иначе
		ЛистП = ДанныеМногостраничныхРазделов.ЛистП[0].Значение;
	...
	Если РегистрационныеФормыКлиентСервер.ОтправлятьЗаявлениеВXMLФормате(СпособПодачиДокументов, ФорматОтправкиДокументов) Тогда 
		Если КоличествоОшибокПроверкиВыгрузки() Тогда 
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				"При проверке формата произошли ошибки. Воспользуйтесь проверкой выгрузки в меню формы");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	Возврат Истина;
...
&НаСервере
Функция КоличествоОшибокПроверкиВыгрузки()
	ДокОбъект = РеквизитФормыВЗначение("Объект");
	СписокОшибок = ДокОбъект.ПроверитьДокументСВыводомВТаблицу(Неопределено);
	Результат = ?(ТипЗнч(СписокОшибок) = Тип("СписокЗначений"), СписокОшибок.Количество(), 0);
	Если ТипЗнч(СписокОшибок) = Тип("СписокЗначений") И Результат = 0 Тогда 
		АдресВыгрузки = ДокОбъект.ВыгрузитьДокумент()[0].АдресФайлаВыгрузки;
		АдресВременнойВыгрузки = ПоместитьВоВременноеХранилище(ПолучитьИзВременногоХранилища(АдресВыгрузки), Новый УникальныйИдентификатор);
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ПроверитьИПодписатьДокументы()
	...
	Если ("1" = ДанныеМногостраничныхРазделов.ЛистП[0].Значение.П01030200) Тогда
	...
	Если РегистрационныеФормыКлиентСервер.ОтправлятьЗаявлениеВXMLФормате(СпособПодачиДокументов, ФорматОтправкиДокументов) Тогда 
		Если ЭтоАдресВременногоХранилища(АдресВременнойВыгрузки) Тогда 
			ЗаявлениеВXML = Новый Массив;
			ЗаявлениеВXML.Добавить(АдресВременнойВыгрузки);
			РегистрационныеФормыКлиент.ДобавитьФайлВОписаниеФайлов(
				ВидДокумента, ЗаявлениеВXML, ПараметрыФормирования.ОписаниеФайлов, Истина);
		КонецЕсли;
	Иначе
