...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2011Кв4(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ЗаполнятьОФР
	   ...
	   И ВыводитьСведенияПоНоменклатурнымГруппам(ПараметрыОтчета) Тогда
		...
		Запрос.УстановитьПараметр("КорСчета", МассивСчетовВИерархииИзСписка("20.01, 23, 25, 26"));
		Запрос.Выполнить();
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2019Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	РассчитанныеПоказатели = Новый ТаблицаЗначений;
	РассчитанныеПоказатели.Колонки.Добавить("Организация");
	РассчитанныеПоказатели.Колонки.Добавить("Раздел");
	РассчитанныеПоказатели.Колонки.Добавить("Показатель");
	РассчитанныеПоказатели.Колонки.Добавить("Сумма");
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2011Кв4_БалансПоГрафе_Раздел2_2011Кв4(КонтекстВыполнения)
	...
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКДР_КонтрагентыДоговоры 76.27.2");
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКДР_КонтрагентыДоговоры 76.37.2");
	...
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКД 76.37.9");
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2011Кв4_БалансПоГрафе_Раздел4(КонтекстВыполнения)
	...
	Если НачалоПериодаОтчета >= '2015-10-01' Тогда
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 67.05");
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 67.25");
	КонецЕсли;
	...
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 76.27.1");
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 76.37.1");
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2011Кв4_БалансПоГрафе_Раздел5_2011Кв4(КонтекстВыполнения)
	...
	УчестьСоставляющую(ПроцессорПоказателя, "+ СККР_КонтрагентыДоговоры 76.27.2");
	УчестьСоставляющую(ПроцессорПоказателя, "+ СККР_КонтрагентыДоговоры 76.37.2");
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2011Кв4_БалансПоГрафе_Раздел5_2015Кв1(КонтекстВыполнения)
	...
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 67.24");
	Если НачалоПериодаОтчета >= '2015-10-01' Тогда
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 67.06");
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 67.26");
	КонецЕсли;
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока" + НомерСтроки) Тогда
		...
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.03");
		Если НачалоПериодаОтчета >= '2015-10-01' Тогда
			УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.05");
		КонецЕсли;
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока" + НомерСтроки) Тогда
		...
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.04");
		Если НачалоПериодаОтчета >= '2015-10-01' Тогда
			УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.06");
		КонецЕсли;
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока" + НомерСтроки) Тогда
		...
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.23");
		Если НачалоПериодаОтчета >= '2015-10-01' Тогда
			УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.25");
		КонецЕсли;
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока" + НомерСтроки) Тогда
		...
		УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.24");
		Если НачалоПериодаОтчета >= '2015-10-01' Тогда
			УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 66.26");
		КонецЕсли;
	...
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 62.32");
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКДО_КонтрагентыДоговоры 62.31");
	УчестьСоставляющую(ПроцессорПоказателя, "+ СКК 62.ОТ");
	Если СтрокаОтраженияСчета76_АВ = "1520" Тогда
		УчестьСоставляющую(ПроцессорПоказателя, "- СКД 76.АВ");
	КонецЕсли;
	УчестьСоставляющую(ПроцессорПоказателя, "+ СККР_ВидыПлатежей 68.01");
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2011Кв4_ОФР_ПоГрафе(Графа, ЗаголовокГрафы, КонтекстВыполнения, НачалоПериодаОтчета, КонецПериодаОтчета)
	...
	ОрганизацияПрименяетУСН = ЗаполнениеБухгалтерскойОтчетности.ПрименяетсяУСН(
		Организация, КонецПериодаОтчета);
	...
	Если ИспользуютсяМакетыЕНВД И ОрганизацияПрименяетЕНВД Тогда
		НомерСтроки = "21101";
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока23420") Тогда
		...
		Если НЕ ВедетсяУчетКурсовыхРазницВУЕ Тогда
			ТекстРасшифровки = "Обороты по кредиту счета 91.01 /объекты со значением реквизита ""Вид прочих доходов и расходов"" - ""Курсовые разницы""/"
			                 + " в корреспонденции со счетами 60.01, 60.02, 60.21, 60.22, 60.31, 60.32, 62.01, 62.02, 62.21, 62.22, 62.31, 62.32, 62.ОТ, 63, 68.32, 76.02, 76.05, 76.06, 76.07.2, 76.09, 76.22, 76.25, 76.26, 76.27.2, 76.29, 76.32, 76.35, 76.36, 76.37.2, 76.39, 76.К"
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока23423") Тогда
		...
		ЗначениеПоказателя = КО_91_01_ВПДР.ПереоценкаВнеоборотныхАктивов;
		УстановитьЗначениеПоказателя(РассчитанныеПоказатели, ИмяРаздела, ИмяОбластиПоказателя, ЗначениеПоказателя);
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока23532") Тогда
		...
		ЗначениеПоказателя = ДО_91_02_ВПДР.ПереоценкаВнеоборотныхАктивов;
		УстановитьЗначениеПоказателя(РассчитанныеПоказатели, ИмяРаздела, ИмяОбластиПоказателя, -ЗначениеПоказателя); // вычитаемый
	...
	Если КонецПериодаОтчета = КонецГода(КонецПериодаОтчета) Тогда
		...
		ОБ_80__83_01 = ОБ("80", "83.01", ОборотыМеждуСчетами);
		ОБ_81__83_01 = ОБ("81", "83.01", ОборотыМеждуСчетами);
		ОБ_82__83_01 = ОБ("82", "83.01", ОборотыМеждуСчетами);
		ОБ_84__83_01 = ОБ("84", "83.01", ОборотыМеждуСчетами);
		ОБ_86__83_01 = ОБ("86", "83.01", ОборотыМеждуСчетами);
		ОБ_83_01__80 = ОБ("83.01", "80", ОборотыМеждуСчетами);
		ОБ_83_01__81 = ОБ("83.01", "81", ОборотыМеждуСчетами);
		ОБ_83_01__82 = ОБ("83.01", "82", ОборотыМеждуСчетами);
		ОБ_83_01__84 = ОБ("83.01", "84", ОборотыМеждуСчетами);
		ОБ_83_01__86 = ОБ("83.01", "86", ОборотыМеждуСчетами);
		
		ЗначениеПоказателя = КО_83_01 - ОБ_80__83_01 - ОБ_81__83_01 - ОБ_82__83_01 - ОБ_84__83_01 - ОБ_86__83_01
		                   - ДО_83_01 + ОБ_83_01__80 + ОБ_83_01__81 + ОБ_83_01__82 + ОБ_83_01__84 + ОБ_83_01__86;
		...
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"-", "!ОБ " + Счет("80").Код + " " + Счет("83.01").Код,
			-ОБ_80__83_01, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("80"), Счет("83.01")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"-", "!ОБ " + Счет("81").Код + " " + Счет("83.01").Код,
			-ОБ_81__83_01, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("81"), Счет("83.01")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"-", "!ОБ " + Счет("82").Код + " " + Счет("83.01").Код,
			-ОБ_82__83_01, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("82"), Счет("83.01")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"-", "!ОБ " + Счет("84").Код + " " + Счет("83.01").Код,
			-ОБ_84__83_01, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("84"), Счет("83.01")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"-", "!ОБ " + Счет("86").Код + " " + Счет("83.01").Код,
			-ОБ_86__83_01, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("86"), Счет("83.01")));
		
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"-","!ДО " + Счет("83.01").Код,
			-ДО_83_01, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("83.01")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "!ОБ " + Счет("83.01").Код + " " + Счет("80").Код,
			ОБ_83_01__80, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("83.01"), Счет("80")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "!ОБ " + Счет("83.01").Код + " " + Счет("81").Код,
			ОБ_83_01__81, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("83.01"), Счет("81")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "!ОБ " + Счет("83.01").Код + " " + Счет("82").Код,
			ОБ_83_01__82, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("83.01"), Счет("82")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "!ОБ " + Счет("83.01").Код + " " + Счет("84").Код,
			ОБ_83_01__84, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("83.01"), Счет("84")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "!ОБ " + Счет("83.01").Код + " " + Счет("86").Код,
			ОБ_83_01__86, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("83.01"), Счет("86")));
...
Процедура ЗаполнитьФормаОтчета2011Кв4_ОИК(КонтекстВыполнения) Экспорт
	...
	Если ЗаполнятьГодПредшествующийПредыдущему Тогда
		...
		ОбщиеПараметрыРасшифровки.Вставить("КонецПериода", МоментКонцаПериодаГодаПредшествующегоПредыдущему);
	...
	Если ЗаполнятьГодПредыдущий Тогда
		...
		ЗаполнитьФормаОтчета2011Кв4_ОИК_ЗаГод("32", КонтекстВыполнения, ДатаНачалаПериодаПредыдущегоГода, МоментКонцаПериодаПредыдущегоГода);
	...
	НастройкиПолученияБухитогов.Вставить("НачалоПериода", НачалоПериодаОтчета);
	НастройкиПолученияБухитогов.Вставить("КонецПериода",  МоментКонцаПериодаОтчета);
...
Процедура ЗаполнитьФормаОтчета2011Кв4_ОДДС(КонтекстВыполнения) Экспорт
	...
	УчетС2019 = (ПривестиКДате(КонецПериодаОтчета) >= '2019-01-01');
	...
	СальдоИОборотыПоСчетам = ЗаполнениеБухгалтерскойОтчетностиИтоги.СальдоИОборотыПоСчетам(НастройкиПолученияБухитогов);
	ЗасчитанныеНалогиЕНС   = ЗаполнениеБухгалтерскойОтчетностиИтоги.ЗасчитанныеНалогиЕНС(
		Организация, НачалоПериодаОтчета, ПривестиКДате(КонецПериодаОтчета));
	
	СписокСчетовДДС = "50.01, 50.02, 50.21, 51, 52, 53, 55.01, 55.02, 55.04, 55.21, 55.24, 57.01, 57.21";
	Если НЕ УчетС2019 Тогда
		СписокСчетовДДС = СтрЗаменить(СписокСчетовДДС, ", 53", "");
	КонецЕсли;
	
	СчетаДДС = ТерминальныеСчетаВИерархииИзСписка(СписокСчетовДДС);
	СтруктураДДС = ПолучитьСтруктуруДДС(СчетаДДС, Организация, НачалоПериодаОтчета, КонецПериодаОтчета);
	
	ОрганизацияПерешлаНаУСН = ЗаполнениеБухгалтерскойОтчетности.ПрименяетсяУСН(
		Организация, ПривестиКДате(КонецПериодаОтчета));
	...
	Запрос.УстановитьПараметр("Счет_51", БухгалтерскийУчетПовтИсп.СчетаВИерархии(Счет("51")));
	...
	СНД_53    = СНД("53",    СальдоИОборотыПоСчетам);
	...
	ЗначениеПоказателя = СНД_50_01 + СНД_50_02 + СНД_50_21
	                   + СНД_51 + СНД_52 + ?(УчетС2019, СНД_53, 0)
	...
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, ЗаголовокРасшифровки,
		"+", "!СНД " + Счет("50.01").Код,
		СНД_50_01, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("50.01")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("50.02").Код,
		СНД_50_02, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("50.02")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("50.21").Код,
		СНД_50_21, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("50.21")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("51").Код,
		СНД_51, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("51")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("52").Код,
		СНД_52, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("52")));
	Если УчетС2019 Тогда
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "!СНД " + Счет("53").Код,
			СНД_53, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("53")));
	КонецЕсли;
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("55.01").Код,
		СНД_55_01, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("55.01")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("55.02").Код,
		СНД_55_02, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("55.02")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("55.04").Код,
		СНД_55_04, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("55.04")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("55.21").Код,
		СНД_55_21, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("55.21")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("55.24").Код,
		СНД_55_24, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("55.24")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("57.01").Код,
		СНД_57_01, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("57.01")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("57.03").Код,
		СНД_57_03, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("57.03")));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
		"+", "!СНД " + Счет("57.21").Код,
		СНД_57_21, ИмяРаздела, НастройкиОСВПоСчету(ОбщиеПараметрыРасшифровки, Счет("57.21")));
	...
	СубконтоКурсовыеРазницы.Добавить(Новый Структура("ВидСубконто, Отбор",
		...
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.КурсовыеРазницы")));
	...
	ОБ_57_21__91_01_КурсовыеРазницы = ОборотыМеждуСчетамиПоАналитике(НастройкиПолученияБухитоговКт, "57.21", "91.01");
	ОБ_91_02__57_21_КурсовыеРазницы = ОборотыМеждуСчетамиПоАналитике(НастройкиПолученияБухитоговДт, "91.02", "57.21");
	...
	ЗначениеПоказателя = ОБ_50_21__91_01_КурсовыеРазницы - ОБ_91_02__50_21_КурсовыеРазницы
	                   ...
	                   + ОБ_57_21__91_01_КурсовыеРазницы - ОБ_91_02__57_21_КурсовыеРазницы;
	УстановитьЗначениеПоказателя(РассчитанныеПоказатели, ИмяРаздела, "П000100" + НомерСтроки + Графа, ЗначениеПоказателя);
	
	ОтборПоАналитике = Новый Соответствие;
	ОтборПоАналитике.Вставить("Субконто1", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.КурсовыеРазницы"));
	
	ОтборПоАналитикеКор = Новый Соответствие;
	ОтборПоАналитикеКор.Вставить("КорСубконто1", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.КурсовыеРазницы"));
	...
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, "П000100" + НомерСтроки + Графа, "",
		"+", "Оборот с кредита счета 91.01 в дебет счета 57.21 по статье прочих доходов и расходов ""Курсовые разницы""",
		ОБ_57_21__91_01_КурсовыеРазницы, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("57.21"), Счет("91.01"), ОтборПоАналитикеКор));
	ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, "П000100" + НомерСтроки + Графа, "",
		"-", "Оборот с кредита счета 57.21 в дебет счета 91.02 по статье прочих доходов и расходов ""Курсовые разницы""",
		-ОБ_91_02__57_21_КурсовыеРазницы, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("91.02"), Счет("57.21"), ОтборПоАналитике));
	...
	Иначе
		...
		КонтекстВыполненияОДДС.Вставить("РасчетыПоНДС", РасчетыПоНДС);
		КонтекстВыполненияОДДС.Вставить("СальдоИОборотыПоСчетам", СальдоИОборотыПоСчетам);
		КонтекстВыполненияОДДС.Вставить("ЗасчитанныеНалогиЕНС", ЗасчитанныеНалогиЕНС);
		
		КонтекстВыполненияОДДС.Вставить("УчетС2023", ПривестиКДате(КонецПериодаОтчета) >= '2023-01-01');
		КонтекстВыполненияОДДС.Вставить("УчетС2024", ПривестиКДате(КонецПериодаОтчета) >= '2024-01-01');
		КонтекстВыполненияОДДС.Вставить("ЕНСРаспределенАвтоматически", КонтекстВыполнения.ЕНСРаспределенАвтоматически);
		
		ЗаполнитьФормаОтчета2011Кв4_ОДДС_2011(КонтекстВыполненияОДДС);
		
		КонтекстВыполнения.ЕНСРаспределенАвтоматически = КонтекстВыполненияОДДС.ЕНСРаспределенАвтоматически;
...
Процедура ЗаполнитьФормаОтчета2011Кв4_ОДДС_2011(ПараметрыВывода)
	...
	СальдоИОборотыПоСчетам = ПараметрыВывода.СальдоИОборотыПоСчетам;
	ЗасчитанныеНалогиЕНС   = ПараметрыВывода.ЗасчитанныеНалогиЕНС;
	
	УчетС2023 = ПараметрыВывода.УчетС2023;
	
	ВозможноРаспределениеЕНС = (СНД("68.90", СальдоИОборотыПоСчетам) <= 1000);
	
	ДО_57_03 = ДО("57.03", СальдоИОборотыПоСчетам);
	КО_57_03 = КО("57.03", СальдоИОборотыПоСчетам);
	...
	// Индивидуальное дополнение алгоритма строки 4119.
	ИмяОбласти = "П000100" + НомерСтроки + ПараметрыВывода.Графа;
	СтрокаПоказателя = ПараметрыВывода.РассчитанныеПоказатели.Найти(ИмяОбласти, "Показатель");
	ЗначениеПоказателя = СтрокаПоказателя.Сумма;
	
	ПоложительныеПлатежиПоПлатежнымКартам = Макс(0, ДО_57_03 - КО_57_03);
	
	ЗначениеПоказателя = ЗначениеПоказателя
		+ ПоложительныеПлатежиПоПлатежнымКартам;
	
	ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
		"+", "Сумма платежей по платежным картам (положительная разница оборотов по счету 57.03)",
		ПоложительныеПлатежиПоПлатежнымКартам, "ОДДС", НастройкиОСВПоСчету(ПараметрыВывода.ОбщиеПараметрыРасшифровки, Счет("57.03")));
	
	СтрокаПоказателя.Сумма = ЗначениеПоказателя;
	...
	// Индивидуальное дополнение алгоритма строки 4122.
	ИмяОбласти = "П000100" + НомерСтроки + ПараметрыВывода.Графа;
	СтрокаПоказателя = ПараметрыВывода.РассчитанныеПоказатели.Найти(ИмяОбласти, "Показатель");
	ЗначениеПоказателя = -СтрокаПоказателя.Сумма; // вычитаемый
	
	Если УчетС2023 И ВозможноРаспределениеЕНС Тогда
		ЗначениеПоказателя = ЗначениеПоказателя
			+ ЗасчитанныеНалогиЕНС.НДФЛ + ЗасчитанныеНалогиЕНС.СтраховыеВзносы;
		
		ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
			"+", "НДФЛ, засчитанный с единого налогового счета",
			ЗасчитанныеНалогиЕНС.НДФЛ, "ОДДС", ПараметрыВывода.ОбщиеПараметрыРасшифровки);
		ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
			"+", "Страховые взносы, засчитанные с единого налогового счета",
			ЗасчитанныеНалогиЕНС.СтраховыеВзносы, "ОДДС", ПараметрыВывода.ОбщиеПараметрыРасшифровки);
			
	КонецЕсли;
	
	СтрокаПоказателя.Сумма = -ЗначениеПоказателя; // вычитаемый
	...
	// Индивидуальное дополнение алгоритма строки 4124.
	ИмяОбласти = "П000100" + НомерСтроки + ПараметрыВывода.Графа;
	СтрокаПоказателя = ПараметрыВывода.РассчитанныеПоказатели.Найти(ИмяОбласти, "Показатель");
	ЗначениеПоказателя = -СтрокаПоказателя.Сумма; // вычитаемый
	
	Если УчетС2023 И ВозможноРаспределениеЕНС Тогда
		ЗначениеПоказателя = ЗначениеПоказателя + ЗасчитанныеНалогиЕНС.НалогНаПрибыль;
		
		ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
			"+", "Налог на прибыль, засчитанный с единого налогового счета",
			ЗасчитанныеНалогиЕНС.НалогНаПрибыль, "ОДДС", ПараметрыВывода.ОбщиеПараметрыРасшифровки);
	...
	// Индивидуальное дополнение алгоритма строки 4129.
	ИмяОбласти = "П000100" + НомерСтроки + ПараметрыВывода.Графа;
	СтрокаПоказателя = ПараметрыВывода.РассчитанныеПоказатели.Найти(ИмяОбласти, "Показатель");
	ЗначениеПоказателя = -СтрокаПоказателя.Сумма; // вычитаемый
	
	ОтрицательныеПлатежиПоПлатежнымКартам = -Мин(0, ДО_57_03 - КО_57_03);
	
	ЗначениеПоказателя = ЗначениеПоказателя + ОтрицательныеПлатежиПоПлатежнымКартам;
	
	ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
		"+", "Сумма платежей по платежным картам (отрицательная разница оборотов по счету 57.03)",
		ОтрицательныеПлатежиПоПлатежнымКартам, "ОДДС", НастройкиОСВПоСчету(ПараметрыВывода.ОбщиеПараметрыРасшифровки, Счет("57.03")));
	
	Если УчетС2023 Тогда
		Если ВозможноРаспределениеЕНС Тогда
			ЗначениеПоказателя = ЗначениеПоказателя
				- ЗасчитанныеНалогиЕНС.НДФЛ - ЗасчитанныеНалогиЕНС.НалогНаПрибыль
				- ЗасчитанныеНалогиЕНС.СтраховыеВзносы - ЗасчитанныеНалогиЕНС.НалогНаПрибыльСДивидендов
				- ЗасчитанныеНалогиЕНС.НДФЛСДивидендов;
			...
			ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
				...
				-ЗасчитанныеНалогиЕНС.СтраховыеВзносы, "ОДДС", ПараметрыВывода.ОбщиеПараметрыРасшифровки);
			ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
				"-", "Налог на прибыль с дивидендов, засчитанный с единого налогового счета",
				-ЗасчитанныеНалогиЕНС.НалогНаПрибыльСДивидендов, "ОДДС", ПараметрыВывода.ОбщиеПараметрыРасшифровки);
			ДобавитьСтрокуРасшифровки(ПараметрыВывода.ТаблицаРасшифровки, ИмяОбласти, "",
				"-", "НДФЛ с дивидендов, засчитанный с единого налогового счета",
				-ЗасчитанныеНалогиЕНС.НДФЛСДивидендов, "ОДДС", ПараметрыВывода.ОбщиеПараметрыРасшифровки);
			
			ПараметрыВывода.ЕНСРаспределенАвтоматически = Истина;
			
		Иначе
			ПараметрыВывода.ЕНСРаспределенАвтоматически = Ложь;
	...
	// Индивидуальное дополнение алгоритма строки 4322.
	ИмяОбласти = "П000100" + НомерСтроки + ПараметрыВывода.Графа;
	СтрокаПоказателя = ПараметрыВывода.РассчитанныеПоказатели.Найти(ИмяОбласти, "Показатель");
	ЗначениеПоказателя = -СтрокаПоказателя.Сумма; // вычитаемый
	
	Если УчетС2023 И ВозможноРаспределениеЕНС Тогда
		ЗначениеПоказателя = ЗначениеПоказателя + ЗасчитанныеНалогиЕНС.НалогНаПрибыльСДивидендов
			+ ЗасчитанныеНалогиЕНС.НДФЛСДивидендов;
...
Функция ПолучитьСтруктуруНДСвДДС2011(СчетаДДС, Организация, НачалоПериодаОтчета, КонецПериодаОтчета)
	...
	|	ПриходныйКассовыйОрдер.ВыручкаСНТТ КАК ВыручкаСНТТ
	|ПОМЕСТИТЬ ВТ_ДокументыПКО
	|ИЗ
	|	Документ.ПриходныйКассовыйОрдер КАК ПриходныйКассовыйОрдер
	|ГДЕ
	|	ПриходныйКассовыйОрдер.Дата МЕЖДУ &НачПериода И &КонПериода
	...
	|			ИЛИ ПриходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПКО.ВозвратОтПоставщика)
	|			ИЛИ ПриходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПКО.РозничнаяВыручка))
	|	И ПриходныйКассовыйОрдер.СчетКасса В(&СчетаДДС)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|	ОтчетОРозничныхПродажах.СчетКасса КАК СчетКасса,
	|	ОтчетОРозничныхПродажах.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств КАК ВидДвиженияДенежныхСредств
	...
	|	И ОтчетОРозничныхПродажах.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств ЕСТЬ НЕ NULL 
	|	И ОтчетОРозничныхПродажах.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств <> ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийДенежныхСредств.ПустаяСсылка)
	|	И ОтчетОРозничныхПродажах.СчетКасса В(&СчетаДДС)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Ссылка.СчетБанк КАК СчетДДС,
	|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств КАК ВидДДС,
	|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.СуммаПлатежа * ВЫБОР
	...
	|	КОНЕЦ КАК НДС,
	|	ВЫБОР
	|		КОГДА ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку)
	...
	|				И НЕ ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом В (&МассивСчетаРасчетов)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоОплатаОтКомитентаНаЗакупку
	...
	|
	|ВЫБРАТЬ
	|	ВТ_ДокументыПКО.СчетКасса,
	|	ПриходныйКассовыйОрдерРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств,
	|	ПриходныйКассовыйОрдерРасшифровкаПлатежа.СуммаПлатежа * ВЫБОР
	|		КОГДА ВТ_ДокументыПКО.ВалютаДокумента = &ВалютаРегУчета
	|			ТОГДА 1
	...
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ПриходныйКассовыйОрдерРасшифровкаПлатежа.ДоговорКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку)
	|				И ВТ_ДокументыПКО.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПКО.ОплатаПокупателя)
	|				И НЕ ПриходныйКассовыйОрдерРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом В (&МассивСчетаРасчетов)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ
	|ИЗ
	|	Документ.ПриходныйКассовыйОрдер.РасшифровкаПлатежа КАК ПриходныйКассовыйОрдерРасшифровкаПлатежа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДокументыПКО КАК ВТ_ДокументыПКО
	|		ПО ПриходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка = ВТ_ДокументыПКО.Ссылка
	|ГДЕ
	|	ВТ_ДокументыПКО.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПКО.РозничнаяВыручка)
	...
	|	И ВТ_ДокументыПКО.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПКО.РозничнаяВыручка)
	|	И ВТ_ДокументыПКО.ВыручкаСНТТ
	|	И ПриходныйКассовыйОрдерРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств ЕСТЬ НЕ NULL 
	|	И ПриходныйКассовыйОрдерРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств <> ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийДенежныхСредств.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВТ_ДокументыОРП.СчетКасса,
	|	ВТ_ДокументыОРП.ВидДвиженияДенежныхСредств,
	|	0,
	|	ОтчетОРозничныхПродажахТовары.СуммаНДС,
	|	ЛОЖЬ
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.Товары КАК ОтчетОРозничныхПродажахТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДокументыОРП КАК ВТ_ДокументыОРП
	|		ПО ОтчетОРозничныхПродажахТовары.Ссылка = ВТ_ДокументыОРП.Ссылка
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &СпособОценкиТоваровВРознице = ЗНАЧЕНИЕ(Перечисление.СпособыОценкиТоваровВРознице.ПоПродажнойСтоимости)
	|					И ВТ_ДокументыОРП.Склад.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.НеавтоматизированнаяТорговаяТочка)
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДокументыОРП КАК ВТ_ДокументыОРП
	|		ПО ОтчетОРозничныхПродажахАгентскиеУслуги.Ссылка = ВТ_ДокументыОРП.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	
	|	ВЫБРАТЬ
	|		ВТ_Поступления.СчетДДС,
	|		ВТ_Поступления.ВидДДС,
	|		ВЫБОР
	|			КОГДА ВТ_Поступления.ЭтоОплатаОтКомитентаНаЗакупку
	|				ТОГДА -ВТ_Поступления.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ВТ_Поступления.ЭтоОплатаОтКомитентаНаЗакупку
	|				ТОГДА 0
	|			ИНАЧЕ ВТ_Поступления.НДС
	|		КОНЕЦ,
	|		0,
	|		0
	|	ИЗ
	...
	|	ВЫБРАТЬ
	|		ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка),
	|		ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийДенежныхСредств.ОплатаОборотныхАктивов),
	|		0,
	|		0,
	...
	|		ВТ_Поступления КАК ВТ_Поступления
	|	ГДЕ
	|		ВТ_Поступления.ЭтоОплатаОтКомитентаНаЗакупку) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("СпособОценкиТоваровВРознице", ЗаполнениеБухгалтерскойОтчетности.СпособОценкиТоваровВРознице(
			Организация, ПривестиКДате(КонецПериодаОтчета)));
	
	МассивСчетаРасчетов = Новый Массив;
	МассивСчетаРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками); 			// 62
	МассивСчетаРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПрочимиПокупателямиИЗаказчиками); 	// 76.06
	МассивСчетаРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПрочимиПокупателямиИЗаказчикамиВал); 	// 76.26
	МассивСчетаРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПрочимиПокупателямиИЗаказчикамиУЕ); 	// 76.36
	
	МассивСчетаРасчетов = БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(МассивСчетаРасчетов);
	Запрос.УстановитьПараметр("МассивСчетаРасчетов", МассивСчетаРасчетов);
...
Функция ПолучитьСтруктуруПоВалютоОбменнымОперациям(Организация, НачалоПериодаОтчета, КонецПериодаОтчета, ВидДДСПолучения, ВидДДСПеречисления)
	...
	СчетаБанкаВИерархии = Новый Массив;
	СчетаБанкаВИерархии.Добавить(Счет51);
	СчетаБанкаВИерархии.Добавить(Счет("52"));
	
	СчетаБанка = БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(СчетаБанкаВИерархии);
	
	СчетаПриобретениеРеализацииВалютыВИерархии = Новый Массив;
	СчетаПриобретениеРеализацииВалютыВИерархии.Добавить(Счет("57.02"));
	СчетаПриобретениеРеализацииВалютыВИерархии.Добавить(Счет("57.22"));
	
	СчетаПриобретениеРеализацииВалюты =
		БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(СчетаПриобретениеРеализацииВалютыВИерархии);
	...
	|				&КонПериода,
	|				,
	|				СчетДт В (&СчетаРасчетныеСчета),
	|				&СубконтоСтатьиДДС,
	|				СчетКт В (&СчетаПрочиеДоходы),
	...
	Запрос.УстановитьПараметр("СчетаРасчетныеСчета",               БухгалтерскийУчетПовтИсп.СчетаВИерархии(Счет51));
	Запрос.УстановитьПараметр("СчетаПрочиеДоходы",
		БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПрочиеДоходы));
	Запрос.УстановитьПараметр("ПокупкаПродажаВалюты",
	...
	Запрос.УстановитьПараметр("СубконтоСтатьиДДС",
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиДвиженияДенежныхСредств);
	Запрос.УстановитьПараметр("СубконтоПрочиеДоходыРасходы",
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы);
...
Процедура ВывестиСтрокуОДДС(ПараметрыВывода)
	...
	Для Каждого ВидДДС Из ВидыДДС Цикл
		...
		Для Каждого ЭлементПоказателя Из НайденныеСтрокиПоВидуДДС Цикл
			...
			Если ПоВидуДДСПредусмотреноУменьшениеНаСуммуНДС(ВидДДС) Тогда
...
Процедура ВывестиСтрокуОДДС_НДС(ПараметрыВывода)
	...
	ФормулаРасчета     = ПараметрыВывода.ФормулаРасчета;
	ТаблицаРасшифровки = ПараметрыВывода.ТаблицаРасшифровки;
	ВыводитьНДСБюджет  = ?(ПараметрыВывода.Свойство("УчетС2024"), НЕ ПараметрыВывода.УчетС2024, Истина);
	...
	СвернутыйНДС = НДСПокупателей - НДСПоставщиков
		+ ?(ВыводитьНДСБюджет, -РасчетыПоНДС.ПеречисленныйНДС + РасчетыПоНДС.ПолученныйНДС, 0);
...
Функция ПоВидуДДСПредусмотреноУменьшениеНаСуммуНДС(ВидДДС)
	
	Возврат ВидДДС <> Перечисления.ВидыДвиженийДенежныхСредств.ОплатаТруда
		И ВидДДС <> Перечисления.ВидыДвиженийДенежныхСредств.НалогНаПрибыль;
...
Функция РассчитатьБухгалтерскиеВеличиныФормаОтчета2019Кв1_Баланс(КонтекстВыполнения)
	...
	РазделятьАвансыПоВидамАктивов = РегламентированнаяОтчетность.АвансыРазделеныПоВидамАктивов(
		Организация, КонтекстВыполнения.КонецОтчетногоПериода);
	
	СтруктураСКД_60_02 = ЗаполнениеБухгалтерскойОтчетностиИтоги.РазделенноеСальдоСчетаУчетаАвансов(
		Счет("60.02"), КонецПериодаОтчета, РазделятьАвансыПоВидамАктивов, Организация);
	СтруктураСКД_60_22 = ЗаполнениеБухгалтерскойОтчетностиИтоги.РазделенноеСальдоСчетаУчетаАвансов(
		Счет("60.22"), КонецПериодаОтчета, РазделятьАвансыПоВидамАктивов, Организация);
	СтруктураСКД_60_32 = ЗаполнениеБухгалтерскойОтчетностиИтоги.РазделенноеСальдоСчетаУчетаАвансов(
		Счет("60.32"), КонецПериодаОтчета, РазделятьАвансыПоВидамАктивов, Организация);
	СтруктураСКК_76_ВА = ЗаполнениеБухгалтерскойОтчетностиИтоги.РазделенноеСальдоСчета_76ВА(
		НачалоПериодаОтчета, КонецПериодаОтчета, РазделятьАвансыПоВидамАктивов, Организация);
	...
	АвансыПоВидамАктивов.Вставить("СтруктураСКК_76_ВА",            СтруктураСКК_76_ВА);
...
Функция ПараметрыАлгоритмовБаланса2019(КонтекстВыполнения, ОбщиеИтогиПоСчетам)
	...
	ПараметрыАлгоритмов.Вставить("ПрименяетсяФСБУ25", ЗаполнениеБухгалтерскойОтчетности.ПрименяетсяФСБУ25(
		КонтекстВыполнения.Организация, ПривестиКДате(КонтекстВыполнения.КонтекстОтчета.КонецПериодаОтчета)));
	ПараметрыАлгоритмов.Вставить("РазделятьАвансыПоВидамАктивов", ОбщиеИтогиПоСчетам.АвансыПоВидамАктивов.РазделятьАвансыПоВидамАктивов);
...
Процедура ЗаполнитьАлгоритмыБО2019Кв1_Баланс_Раздел1(Алгоритмы, ПараметрыЗаполнения)
	...
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКК 02.04"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКК 02.05"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКК 02.11"));
	...
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("- СКК 02.04 \ 01.01"));
	...
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("- СКК 02.04 \ 01.08"));
	...
	АлгоритмДопСтроки = ДобавитьНовыйАлгоритмСтроки(АлгоритмСтроки.ДопСтроки, "11510",
	...
	Если РазделятьАвансыПоВидамАктивов Тогда
		...
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКК_ПоВидуАктива 76.ВА", ВидАктиваАвансы));
	КонецЕсли;
	...
	Если РазделятьАвансыПоВидамАктивов Тогда
		...
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("- СКК_ПоВидуАктива 76.ВА", ВидАктиваАвансы));
	КонецЕсли;
...
#Область Строка1230
	...
	Если РазделятьАвансыПоВидамАктивов Тогда
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКД_АвансыДебЗад 60.02"));
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКД_АвансыДебЗад 60.22"));
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКД_АвансыДебЗад 60.32"));
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКК_АвансыДебЗад 76.ВА"));
	Иначе
	...
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКДР_КонтрагентыДоговоры 76.07.2"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКДР_КонтрагентыДоговоры 76.27.2"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКДР_КонтрагентыДоговоры 76.37.2"));
	...
	Если РазделятьАвансыПоВидамАктивов Тогда
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКД_АвансыДебЗад 60.02"));
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКД_АвансыДебЗад 60.22"));
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКД_АвансыДебЗад 60.32"));
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("- СКК_АвансыДебЗад 76.ВА"));
	Иначе
	...
	Если ЭтоИП Тогда
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКДР_ВидыПлатежей 68.14"));
	КонецЕсли;
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКДР_ВидыПлатежей 68.15"));
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКДР_ВидыПлатежей 68.16"));
	Если УчетС2018 Тогда
	...
	Если ЭтоИП И УчетС2023 Тогда
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКДР_ВидыПлатежей 69.06.6"));
	КонецЕсли;
	Если УчетС2023 Тогда
	...
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКДР_КонтрагентыДоговоры 76.07.2"));
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКДР_КонтрагентыДоговоры 76.27.2"));
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКДР_КонтрагентыДоговоры 76.37.2"));
...
#Область Строка1250
	...
	АлгоритмДопСтроки = ДобавитьНовыйАлгоритмСтроки(АлгоритмСтроки.ДопСтроки, "12513",
...
#Область Строка1260
	...
	Если НЕ ПрименяетсяФСБУ25 Тогда
		...
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКД 76.37.9"));
	КонецЕсли;
	...
	Если НЕ ПрименяетсяФСБУ25 Тогда
		...
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СКД 76.37.9"));
	КонецЕсли;
	
	Если РазделятьАвансыПоВидамАктивов Тогда
		АлгоритмДопСтроки = ДобавитьНовыйАлгоритмСтроки(АлгоритмСтроки.ДопСтроки, "12611",
		...
		АлгоритмДопСтроки = ДобавитьНовыйАлгоритмСтроки(АлгоритмСтроки.ДопСтроки, "12612",
...
#Область Строка1450
	...
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКК 76.27.1"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СКК 76.37.1"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКД 76.07.5"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКД 76.27.5"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКД 76.37.5"));
	Если ПрименяетсяФСБУ25 Тогда
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКД 76.07.9"));
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКД 76.27.9"));
		АлгоритмСтроки.Составляющие.Добавить(Составляющая("- СКД 76.37.9"));
	КонецЕсли;
...
#Область Строка1520
	...
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СККР_КонтрагентыДоговоры 76.27.2"));
	АлгоритмСтроки.Составляющие.Добавить(Составляющая("+ СККР_КонтрагентыДоговоры 76.37.2"));
	...
	Если ЭтоИП Тогда
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СККР_ВидыПлатежей 68.14"));
	КонецЕсли;
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СККР_ВидыПлатежей 68.15"));
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СККР_ВидыПлатежей 68.16"));
	Если УчетС2018 Тогда
	...
	Если ЭтоИП И УчетС2023 Тогда
		АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СККР_ВидыПлатежей 69.06.6"));
	КонецЕсли;
	Если УчетС2023 Тогда
	...
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СККР_КонтрагентыДоговоры 76.27.2"));
	АлгоритмДопСтроки.Составляющие.Добавить(Составляющая("+ СККР_КонтрагентыДоговоры 76.37.2"));
...
Функция ДобавитьНовыйАлгоритмСтроки(Алгоритмы, Номер, Наименование) Экспорт
	...
	АлгоритмСтроки.Вставить("Номер", Номер);
	АлгоритмСтроки.Вставить("Наименование", Наименование);
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2019Кв1_ОФР(КонтекстВыполнения) Экспорт
	...
	НачалоПериодаПредыдущегоГода = ДобавитьМесяц(НачалоПериодаОтчета, -12);
	КонецПериодаПредыдущегоГода  = КонецМесяца(ДобавитьМесяц(КонецПериодаОтчета, -12));
	МоментКонцаПериодаПредыдущегоГода = Новый МоментВремениСУточнениемПериода(КонецПериодаПредыдущегоГода,
		БухгалтерскийУчет.ВерхняяГраницаУточненияПериода());
	...
	ЗаполнитьОтчетБухОтчетностьФормаОтчета2019Кв1_ОФР_ПоГрафе(
		...
		НачалоПериодаОтчета, МоментКонцаПериодаОтчета);
	
	ЗаполнятьПредыдущиеПериоды = НЕ РасширенныйПериодОтчета(НачалоПериодаОтчета);
	Если ЗаполнятьПредыдущиеПериоды Тогда
		ЗаполнитьОтчетБухОтчетностьФормаОтчета2019Кв1_ОФР_ПоГрафе(
			"05", ЗаголовокДатаОтчетногоПериодаПрошлогоГода, КонтекстВыполнения,
			НачалоПериодаПредыдущегоГода, МоментКонцаПериодаПредыдущегоГода);
	КонецЕсли;
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2019Кв1_ОФР_ПоГрафе(Графа, ЗаголовокГрафы, КонтекстВыполнения, НачалоПериодаОтчета, КонецПериодаОтчета)
	...
	НастройкиПолученияБухитогов.Вставить("КорСчет", МассивСчетовВИерархииИзСписка("68.15"));
	НастройкиПолученияБухитогов.Вставить("Счет", МассивСчетовВИерархииИзСписка("99.01.1"));
	ОБ_99_01_1__68_15_Налог = ЗаполнениеБухгалтерскойОтчетностиИтоги.ОборотыМеждуСчетамиПоАналитике(НастройкиПолученияБухитогов);
	НастройкиПолученияБухитогов.Вставить("Счет", МассивСчетовВИерархииИзСписка("99.01"));
	ОБ_99_01__68_15_Налог = ЗаполнениеБухгалтерскойОтчетностиИтоги.ОборотыМеждуСчетамиПоАналитике(НастройкиПолученияБухитогов);
	
	НастройкиПолученияБухитогов.Вставить("КорСчет", МассивСчетовВИерархииИзСписка("68.16"));
	НастройкиПолученияБухитогов.Вставить("Счет", МассивСчетовВИерархииИзСписка("99.01.1"));
	ОБ_99_01_1__68_16_Налог = ЗаполнениеБухгалтерскойОтчетностиИтоги.ОборотыМеждуСчетамиПоАналитике(НастройкиПолученияБухитогов);
	НастройкиПолученияБухитогов.Вставить("Счет", МассивСчетовВИерархииИзСписка("99.01"));
	ОБ_99_01__68_16_Налог = ЗаполнениеБухгалтерскойОтчетностиИтоги.ОборотыМеждуСчетамиПоАналитике(НастройкиПолученияБухитогов);
	...
	НомерСтроки = "23427";
	НаименованиеСтроки = "Излишки при инвентаризации";
	
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока23427") Тогда
		ИмяОбластиПоказателя = "П00010" + НомерСтроки + Графа;
		ЗаголовокРасшифровки = ЗаголовокРасшифровкиПоСчетчику(НомерВРасшифровке, НаименованиеСтроки, ЗаголовокГрафы);
		
		ЗначениеПоказателя = КО_91_01_ВПДР.ИзлишкиПриИнвентаризации;
		УстановитьЗначениеПоказателя(РассчитанныеПоказатели, ИмяРаздела, ИмяОбластиПоказателя, ЗначениеПоказателя);
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока23531") Тогда
		...
		ИмяВПДР = "ПрочиеКосвенныеРасходы";
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, ЗаголовокРасшифровки,
			"+", "Оборот по дебету счета 91.02 " + ОписаниеОбъектовВПДР(ИмяВПДР), ДО_91_02_ВПДР[ИмяВПДР], ИмяРаздела,
			НастройкиОСВПоСчетуВПДР(ОбщиеПараметрыРасшифровки, Счет("91.02"), ИмяВПДР));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"-", "Оборот по дебету счета 91.02 " + ОписаниеОбъектовВПДР(ИмяВПДР) + " в корреспонденции со счетом 68.02",
			-ДО_91_02__68_02_ВПДР[ИмяВПДР], ИмяРаздела, НастройкиАнализаСчетаВПДР(ОбщиеПараметрыРасшифровки, Счет("91.02"), Счет("68.02"), ИмяВПДР));
	КонецЕсли;
	...
	ПараметрыЗаполнения.Вставить("ОрганизацияПрименяетАУСН",  ОрганизацияПрименяетАУСН);
	ПараметрыЗаполнения.Вставить("ОрганизацияПрименяетЕНВД",  ОрганизацияПрименяетЕНВД);
	ПараметрыЗаполнения.Вставить("ОрганизацияПрименяетНПД",   ОрганизацияПрименяетНПД);
	...
	ПараметрыЗаполнения.Вставить("ОБ_99_01_1__68_15_Налог", ОБ_99_01_1__68_15_Налог);
	ПараметрыЗаполнения.Вставить("ОБ_99_01_1__68_16_Налог", ОБ_99_01_1__68_16_Налог);
	...
	ПараметрыЗаполнения.Вставить("ОБ_99_01__68_15_Налог",   ОБ_99_01__68_15_Налог);
	ПараметрыЗаполнения.Вставить("ОБ_99_01__68_16_Налог",   ОБ_99_01__68_16_Налог);
	
	Если МакетыОФР = "61н_2019" Тогда
		ЗаполнитьФормаОтчета2019Кв1_ОФР_ПоГрафе_2410_Бланк2019(ПараметрыЗаполнения);
	ИначеЕсли МакетыОФР = "61н_2020" Тогда
		ЗаполнитьФормаОтчета2019Кв1_ОФР_ПоГрафе_2410_Бланк2020(ПараметрыЗаполнения);
	КонецЕсли;
...
Процедура ЗаполнитьФормаОтчета2019Кв1_ОФР_ПоГрафе_2410_Бланк2019(ПараметрыЗаполнения)
	...
	ОрганизацияПрименяетАУСН  = ПараметрыЗаполнения.ОрганизацияПрименяетАУСН;
	ОрганизацияПрименяетНПД   = ПараметрыЗаполнения.ОрганизацияПрименяетНПД;
	...
	ОБ_99_01_1__68_15_Налог = ПараметрыЗаполнения.ОБ_99_01_1__68_15_Налог;
	ОБ_99_01_1__68_16_Налог = ПараметрыЗаполнения.ОБ_99_01_1__68_16_Налог;
	...
	ИначеЕсли ОрганизацияПрименяетАУСН Тогда
		ЗначениеПоказателя = ОБ_99_01_1__68_15_Налог;
		УстановитьЗначениеПоказателя(РассчитанныеПоказатели, ИмяРаздела, ИмяОбластиПоказателя, -ЗначениеПоказателя); // вычитаемый
		
		ОтборПоАналитике = Новый Соответствие;
		ОтборПоАналитике.Вставить("Субконто1", Перечисления.ВидыПлатежейВГосБюджет.Налог);
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, ЗаголовокРасшифровки,
			"+", "Оборот с кредита счета 68.15 /субконто """ + Перечисления.ВидыПлатежейВГосБюджет.Налог + """/ в дебет счета 99.01.1",
			ОБ_99_01_1__68_15_Налог, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("68.15"), Счет("99.01.1"), ОтборПоАналитике));
		
	ИначеЕсли ОрганизацияПрименяетНПД Тогда
		ЗначениеПоказателя = ОБ_99_01_1__68_16_Налог;
		УстановитьЗначениеПоказателя(РассчитанныеПоказатели, ИмяРаздела, ИмяОбластиПоказателя, -ЗначениеПоказателя); // вычитаемый
		
		ОтборПоАналитике = Новый Соответствие;
		ОтборПоАналитике.Вставить("Субконто1", Перечисления.ВидыПлатежейВГосБюджет.Налог);
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, ЗаголовокРасшифровки,
			"+", "Оборот с кредита счета 68.16 /субконто """ + Перечисления.ВидыПлатежейВГосБюджет.Налог + """/ в дебет счета 99.01.1",
			ОБ_99_01_1__68_16_Налог, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("68.16"), Счет("99.01.1"), ОтборПоАналитике));
...
Процедура ЗаполнитьФормаОтчета2019Кв1_ОФР_ПоГрафе_2410_Бланк2020(ПараметрыЗаполнения)
	...
	Если ОрганизацияПрименяетПБУ18 Тогда
		...
		ОБ_99_02__84 = ОБ("99.02", "84", ОборотыМеждуСчетами);
		ОБ_84__99_02 = ОБ("84", "99.02", ОборотыМеждуСчетами);
		...
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			...
			ОБ_99_02__84, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("99.02"), Счет("84")));
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"-", "!ОБ " + Счет("84").Код + " " + Счет("99.02").Код,
			-ОБ_84__99_02, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("84"), Счет("99.02")));
...
Процедура ЗаполнитьФормаОтчета2019Кв1_ОФР_ПоГрафе_2460_Бланк2019(ПараметрыЗаполнения)
	...
	Если ОрганизацияПрименяетУСН ИЛИ ОрганизацияПрименяетАУСН ИЛИ ОрганизацияПрименяетНПД Тогда
		ЗначениеПоказателя = ДО_99_01_НалогНаПрибыль
		                   - ОБ_99_01_1__68_12_Налог
		                   + ДО_99_01_НалоговыеСанкции
		                   + ДО_99_09 - (КО_99_09 - ОБ_68__99_09)
...
Процедура ЗаполнитьФормаОтчета2019Кв1_ОФР_ПоГрафе_2460_Бланк2020(ПараметрыЗаполнения)
	...
	ОБ_99_01__68_15_Налог   = ПараметрыЗаполнения.ОБ_99_01__68_15_Налог;
	ОБ_99_01__68_16_Налог   = ПараметрыЗаполнения.ОБ_99_01__68_16_Налог;
	...
	ЗначениеПоказателя = КО_99_09 - ДО_99_09
		...
		+ ?(ОрганизацияПрименяетУСН,  ОБ_99_01__68_12_Налог, 0)
		+ ?(ОрганизацияПрименяетАУСН, ОБ_99_01__68_15_Налог, 0)
		+ ?(ОрганизацияПрименяетНПД,  ОБ_99_01__68_16_Налог, 0);
	...
	КонецЕсли;
	Если ОрганизацияПрименяетАУСН Тогда
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "Оборот с кредита счета " + Счет("68.15").Код + " /субконто """ + Перечисления.ВидыПлатежейВГосБюджет.Налог + """/ в дебет счета " + Счет("99.01").Код,
			ОБ_99_01__68_15_Налог, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("68.15"), Счет("99.01"), ОтборПоАналитике));
	КонецЕсли;
	Если ОрганизацияПрименяетНПД Тогда
		ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
			"+", "Оборот с кредита счета " + Счет("68.16").Код + " /субконто """ + Перечисления.ВидыПлатежейВГосБюджет.Налог + """/ в дебет счета " + Счет("99.01").Код,
			ОБ_99_01__68_16_Налог, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("68.16"), Счет("99.01"), ОтборПоАналитике));
	КонецЕсли;
	...
	Если ВключатьДопстрокуВОтчет(ДеревоНастроекСтрокРасшифровки, "ДопСтрока" + КодСтроки) Тогда
		...
		Если ОрганизацияПрименяетУСН Тогда
			...
			ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
				"-", "Оборот с кредита счета " + Счет("68.12").Код + " /субконто """ + Перечисления.ВидыПлатежейВГосБюджет.Налог + """/ в дебет счета " + Счет("99.01").Код,
				-ОБ_99_01__68_12_Налог, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("99.01"), Счет("68.12"), ОтборПоАналитике));
			
		ИначеЕсли ОрганизацияПрименяетАУСН Тогда
			ЗначениеПоказателя = ДО_99_01_НалогНаПрибыль
			                   - ОБ_99_01__68_15_Налог;
			...
			ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
				"-", "Оборот с кредита счета " + Счет("68.15").Код + " /субконто """ + Перечисления.ВидыПлатежейВГосБюджет.Налог + """/ в дебет счета " + Счет("99.01").Код,
				-ОБ_99_01__68_15_Налог, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("99.01"), Счет("68.15"), ОтборПоАналитике));
			
		ИначеЕсли ОрганизацияПрименяетНПД Тогда
			ЗначениеПоказателя = ДО_99_01_НалогНаПрибыль
			                   - ОБ_99_01__68_16_Налог;
			...
			ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбластиПоказателя, "",
				"-", "Оборот с кредита счета " + Счет("68.16").Код + " /субконто """ + Перечисления.ВидыПлатежейВГосБюджет.Налог + """/ в дебет счета " + Счет("99.01").Код,
				-ОБ_99_01__68_16_Налог, ИмяРаздела, НастройкиАнализаСчета(ОбщиеПараметрыРасшифровки, Счет("99.01"), Счет("68.16"), ОтборПоАналитике));
...
Функция НовыйПроцессорПоказателя() Экспорт
	...
	ПроцессорПоказателя.Вставить("ТаблицаРасшифровки", Новый ТаблицаЗначений);
...
Процедура УчестьСоставляющую(ПроцессорПоказателя, ОписаниеСоставляющей, ДопПараметры = Неопределено) Экспорт
	...
	Если СтрНачинаетсяС(Составляющая, "СКД_АвансыДебЗад") Тогда
		ТокенОстатка = "СКД_АвансыДебЗад";
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Сальдо на конец периода по дебету счета %1 по договорам, с признаком отражения в балансе не отнесенным к ""Прочим активам"", ""Запасам"" или ""Основным средствам""",
			Счет.Код);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКК_АвансыДебЗад") Тогда
		ТокенОстатка = "СКК_АвансыДебЗад";
		ТокенСчета = Сред(Составляющая, 17);
		Счет = СчетСРеквизитами(ТокенСчета, СчетаСРеквизитами);
		ИмяЗначения = ТокенОстатка + "_" + СтрЗаменить(ТокенСчета, ".", "_");
		ЗначениеСоставляющей = ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначения];
		
		ШаблонРасшифровкиПоказателя = СтрШаблон("Сальдо на конец периода по кредиту счета %1 по договорам, с признаком отражения в балансе не отнесенным к ""Прочим активам"", ""Запасам"" или ""Основным средствам""",
			Счет.Код);
...
Процедура УчестьСоставляющуюКМОП(ПроцессорПоказателя, ОписаниеСоставляющей, ДопПараметры = Неопределено) Экспорт
	...
	Если СтрНачинаетсяС(Составляющая, "СКД_АвансыДебЗад") Тогда
		ТокенСчета = Сред(Составляющая, 17);
		Счет = СчетСРеквизитами(ТокенСчета, СчетаСРеквизитами);
		ИмяЗначенияСКД = "СКД_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ИмяЗначенияСНД = "СНД_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ЗначениеСоставляющей = ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСКД][Перечисления.ВидыАктивовДляАвансов.ПустаяСсылка()]
		                     - ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСНД][Перечисления.ВидыАктивовДляАвансов.ПустаяСсылка()];
		
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по дебету счета %1 по договорам, с признаком отражения в балансе не отнесенным к ""Прочим активам"", ""Запасам"" или ""Основным средствам""",
			Счет.Код);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКК_АвансыДебЗад") Тогда
		ТокенСчета = Сред(Составляющая, 17);
		Счет = СчетСРеквизитами(ТокенСчета, СчетаСРеквизитами);
		ИмяЗначенияСКК = "СКК_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ИмяЗначенияСНК = "СНК_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ЗначениеСоставляющей = ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСКК][Перечисления.ВидыАктивовДляАвансов.ПустаяСсылка()]
		                     - ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСНК][Перечисления.ВидыАктивовДляАвансов.ПустаяСсылка()];
		
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по кредиту счета %1 по договорам, с признаком отражения в балансе не отнесенным к ""Прочим активам"", ""Запасам"" или ""Основным средствам""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = БезРасшифровки;
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКД_ПоВидуАктива") Тогда
		...
		ИмяЗначенияСКД = "СКД_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ИмяЗначенияСНД = "СНД_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ЗначениеСоставляющей = ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСКД][ДопПараметры]
		                     - ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСНД][ДопПараметры];
		
		Если ПараметрыСчета.Свойство("ШаблонОписания") Тогда
			ТекстШаблона = "Корректировка сальдо по дебету счета %1 " + ПараметрыСчета.ШаблонОписания;
		Иначе
			ТекстШаблона = "Корректировка сальдо по дебету счета %1 /объекты со значением реквизита ""Вид актива"" - ""%2""/";
		КонецЕсли;
		ШаблонРасшифровкиПоказателя = СтрШаблон(ТекстШаблона, Счет.Код, Строка(ДопПараметры));
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКК_ПоВидуАктива") Тогда
		...
		ИмяЗначенияСКК = "СКК_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ИмяЗначенияСНК = "СНК_ПоВидуАктива_" + СтрЗаменить(ТокенСчета, ".", "_");
		ЗначениеСоставляющей = ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСКК][ДопПараметры]
		                     - ПроцессорПоказателя.РассчитанныеЗначения[ИмяЗначенияСНК][ДопПараметры];
		
		Если ПараметрыСчета.Свойство("ШаблонОписания") Тогда
			ТекстШаблона = "Корректировка сальдо по кредиту счета %1 " + ПараметрыСчета.ШаблонОписания;
		Иначе
			ТекстШаблона = "Корректировка сальдо по кредиту счета %1 /объекты со значением реквизита ""Вид актива"" - ""%2""/";
		КонецЕсли;
		ШаблонРасшифровкиПоказателя = СтрШаблон(ТекстШаблона, Счет.Код, Строка(ДопПараметры));
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКДР_КонтрагентыДоговоры") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по дебету счета %1, развернутого по субконто ""Контрагенты"", ""Договоры""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуКонтрагентыДоговорыРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СККР_КонтрагентыДоговоры") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по кредиту счета %1, развернутого по субконто ""Контрагенты"", ""Договоры""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуКонтрагентыДоговорыРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКДР_Контрагенты") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по дебету счета %1, развернутого по субконто ""Контрагенты""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуКонтрагентыРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СККР_Контрагенты") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по кредиту счета %1, развернутого по субконто ""Контрагенты""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуКонтрагентыРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКДР_ВидыПлатежейУровни") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по дебету счета %1, развернутого по субконто ""Виды платежей в бюджет (фонды)"", ""Уровни бюджетов""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуВидыПлатежейУровниБюджетовРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СККР_ВидыПлатежейУровни") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по кредиту счета %1, развернутого по субконто ""Виды платежей в бюджет (фонды)"", ""Уровни бюджетов""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуВидыПлатежейУровниБюджетовРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКДР_ВидыПлатежей") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по дебету счета %1, развернутого по субконто ""Виды платежей в бюджет (фонды)""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуВидыПлатежейРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СККР_ВидыПлатежей") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по кредиту счета %1, развернутого по субконто ""Виды платежей в бюджет (фонды)""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуВидыПлатежейРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКДР_Работники") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по дебету счета %1, развернутого по субконто ""Работники организации""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуРаботникиРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СККР_Работники") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по кредиту счета %1, развернутого по субконто ""Работники организации""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуРаботникиРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКДР_Резервы") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по дебету счета %1, развернутого по субконто ""Оценочные обязательства и резервы""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуКонтрагентыРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СККР_Резервы") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка сальдо по кредиту счета %1, развернутого по субконто ""Оценочные обязательства и резервы""",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуКонтрагентыРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СКДР") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка развернутого сальдо по дебету счета %1",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
	...
	ИначеЕсли СтрНачинаетсяС(Составляющая, "СККР") Тогда
		...
		ШаблонРасшифровкиПоказателя = СтрШаблон("Корректировка развернутого сальдо по кредиту счета %1",
			Счет.Код);
		НастройкаОтчетаРасшифровки = НастройкиОСВПоСчетуРазвернуто(ПараметрыРасшифровки, Счет.Ссылка);
...
Функция БухгалтерскиеИтоги_Баланс2019Кв1(Организация, КонецПериодаОтчета, ИтогиПоСчетам)
	...
	СтруктураСКД_60_02 = АвансыПоВидамАктивов.СтруктураСКД_60_02;
	СтруктураСКД_60_22 = АвансыПоВидамАктивов.СтруктураСКД_60_22;
	СтруктураСКД_60_32 = АвансыПоВидамАктивов.СтруктураСКД_60_32;
	СтруктураСКК_76_ВА = АвансыПоВидамАктивов.СтруктураСКК_76_ВА;
	
	СКД_60_02_Очищенное = СКД("60.02", СальдоИОборотыПоСчетам);
	СКД_60_22_Очищенное = СКД("60.22", СальдоИОборотыПоСчетам);
	СКД_60_32_Очищенное = СКД("60.32", СальдоИОборотыПоСчетам);
	СКК_76_ВА_Очищенное = СКК("76.ВА", СальдоИОборотыПоСчетам);
	
	ВсеВидыАктивовАвансов = Новый СписокЗначений;
	
	Для Инд = 1 По Перечисления.ВидыАктивовДляАвансов.Количество() Цикл
		ВидАктиваАвансы = Перечисления.ВидыАктивовДляАвансов[Инд - 1];
		СКД_60_02_ПоВидуАктива = СтруктураСКД_60_02[ВидАктиваАвансы];
		СКД_60_22_ПоВидуАктива = СтруктураСКД_60_22[ВидАктиваАвансы];
		СКД_60_32_ПоВидуАктива = СтруктураСКД_60_32[ВидАктиваАвансы];
		СКК_76_ВА_ПоВидуАктива = СтруктураСКК_76_ВА[ВидАктиваАвансы];
		
		СКД_60_02_Очищенное = СКД_60_02_Очищенное - СКД_60_02_ПоВидуАктива;
		СКД_60_22_Очищенное = СКД_60_22_Очищенное - СКД_60_22_ПоВидуАктива;
		СКД_60_32_Очищенное = СКД_60_32_Очищенное - СКД_60_32_ПоВидуАктива;
		СКК_76_ВА_Очищенное = СКК_76_ВА_Очищенное - СКК_76_ВА_ПоВидуАктива;
		
		ВсеВидыАктивовАвансов.Добавить(ВидАктиваАвансы);
		
	КонецЦикла;
	
	РассчитанныеЗначения.Вставить("СКД_АвансыДебЗад_60_02", СКД_60_02_Очищенное);
	РассчитанныеЗначения.Вставить("СКД_АвансыДебЗад_60_22", СКД_60_22_Очищенное);
	РассчитанныеЗначения.Вставить("СКД_АвансыДебЗад_60_32", СКД_60_32_Очищенное);
	РассчитанныеЗначения.Вставить("СКК_АвансыДебЗад_76_ВА", СКК_76_ВА_Очищенное);
	...
	РассчитанныеЗначения.Вставить("СКД_ПоВидуАктива_60_02", СтруктураСКД_60_02);
	РассчитанныеЗначения.Вставить("СКД_ПоВидуАктива_60_22", СтруктураСКД_60_22);
	РассчитанныеЗначения.Вставить("СКД_ПоВидуАктива_60_32", СтруктураСКД_60_32);
	РассчитанныеЗначения.Вставить("СКК_ПоВидуАктива_76_ВА", СтруктураСКК_76_ВА);
...
Функция БухгалтерскиеИтоги_Корректировка2019Кв1(Организация, НачалоПериодаОтчета, КонецПериодаОтчета, ИтогиПоСчетам)
	...
	Для Каждого РБППоВидуАктива Из РБППоВидамАктива Цикл
		...
		СоздатьДополнитьСоответствие(РассчитанныеЗначения, "СНД_ПоВидуАктива_" + ТокенСчета, ВидАктива, РБППоВидуАктива.СНД);
		СоздатьДополнитьСоответствие(РассчитанныеЗначения, "ДО_ПоВидуАктива_" + ТокенСчета, ВидАктива, РБППоВидуАктива.ДО);
		СоздатьДополнитьСоответствие(РассчитанныеЗначения, "СКД_ПоВидуАктива_" + ТокенСчета, ВидАктива, РБППоВидуАктива.СКД);
	КонецЦикла;
	...
	ПараметрыРасчетаИтогов.НачалоПериода = НачалоПериодаОтчета;
	ПараметрыРасчетаИтогов.КонецПериода = КонецПериодаОтчета;
	ПараметрыРасчетаИтогов.РазделятьАвансыПоВидамАктивов = АвансыПоВидамАктивов.РазделятьАвансыПоВидамАктивов;
	...
	Для Каждого АвансПоВидуАктива Из АвансыПоВидамАктива Цикл
		...
		СоздатьДополнитьСоответствие(РассчитанныеЗначения, "СНК_ПоВидуАктива_" + ТокенСчета, ВидАктива, АвансПоВидуАктива.СНК);
		СоздатьДополнитьСоответствие(РассчитанныеЗначения, "КО_ПоВидуАктива_" + ТокенСчета, ВидАктива, АвансПоВидуАктива.КО);
		СоздатьДополнитьСоответствие(РассчитанныеЗначения, "СКК_ПоВидуАктива_" + ТокенСчета, ВидАктива, АвансПоВидуАктива.СКК);
...
Функция ТерминальныеСчетаВИерархииИзСписка(Знач СписокСчетов)
	
	Возврат ЗаполнениеБухгалтерскойОтчетности.ТерминальныеСчетаВИерархииИзСписка(СписокСчетов);
