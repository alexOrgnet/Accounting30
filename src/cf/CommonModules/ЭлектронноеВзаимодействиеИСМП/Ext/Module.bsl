...
Функция ДанныеДокументаСоответствуютДаннымУпаковок(ПроверяемыйДокумент, АвтоматическиФормироватьОСУ = Ложь) Экспорт
	...
	ВидыПродукции = ИнтеграцияИСМПВызовСервера.УчитываемыеВидыМаркируемойПродукции(НачалоДня(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроверяемыйДокумент,"Дата")), Ложь);
	...
	Если АвтоматическиФормироватьОСУ Тогда
		ТекстЗапроса = ТекстЗапроса + ШтрихкодированиеИС.ТекстЗапросаСвойстваМаркируемойПродукции()	+ "
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|";
	КонецЕсли;
	ИнтеграцияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(ТекстЗапроса, "ОписаниеИС", "ДанныеДокумента.Номенклатура");
	...
	Запрос.УстановитьПараметр("ВидыПродукцииПоддерживаютОСУ", ИнтеграцияИСМПКлиентСерверПовтИсп.ВидыПродукцииОбъемноСортовогоУчета());
...
//  РезультатРазбора  - см. ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода - Результат разбора,
...
Функция ЧастичноеСодержимое(ТаблицаУпаковок) Экспорт
	...
	ТекстЗапроса = 
		...
		|		ПО &ТоварыОписаниеНоменклатурыИС
		|";
	ИнтеграцияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(ТекстЗапроса, "ОписаниеНоменклатурыИС", "ДанныеШтрихкодовУпаковок.Номенклатура");
		...
		Для Каждого ДанныеШтрихКода Из ДочерниеЗаписи Цикл
			Если ЗначениеЗаполнено(ДанныеШтрихКода.Номенклатура) Тогда
				...
				Если РезультатРазбора <> Неопределено
					...
					СтрокаЗаполнения.Серия = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("СерияНоменклатуры");
					Продолжить;
				КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		...
		РезультатРазбора = Неопределено; // см. ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода
		...
		ИначеЕсли ШтрихкодированиеИСКлиентСервер.ВозможнаГрупповаяУпаковкаИлиНабор(
...
Процедура ДополнитьРезультатКодамиОСУ(ТаблицаТоваров, ПараметрыСканирования, ЧастичноеСодержимоеРезультат) Экспорт
	...
	Если ЕстьОшибки Тогда
		Возврат;
	КонецЕсли;
	Описание = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(НоменклатураДляОСУ);
