...
Функция УсловиеВыполненияРегламентнойОперации(ВидОперации, Организация, Период, ОднаОрганизация, КешНастроек) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеВзносовОтНесчастыхСлучаев Тогда
		
		Если Не ЗначениеНастройки(КешНастроек, "ИспользуетсяПодсистемаУчетаЗарплатыИКадров") Тогда
				Возврат Ложь;
		Иначе
			Возврат ЗначениеНастройки(КешНастроек, "ПрименяетсяАУСН", Организация, Период, Периодичность)
				И ЗначениеНастройки(КешНастроек, "НеобходимоНачислятьЗарплату", Организация, Период, Периодичность);
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеНалоговНаЕНС Тогда
		...
		Возврат КонецМесяца(Период) + 1 >= ЕдиныйНалоговыйСчет.НачалоПростогоУчета();
...
Функция СуществуетСправкаРасчет(ВидОперации) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.ИсключениеНЗПИзСоставаМатериальныхРасходовДляУСН
		...
		ИЛИ ВидОперации = Перечисления.ВидыРегламентныхОпераций.СписаниеДопРасходовДляУСН 
		ИЛИ ВидОперации = Перечисления.ВидыРегламентныхОпераций.СписаниеТаможенныхПлатежейДляУСН
		ИЛИ ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПереходНаФСБУ25 Тогда
		Возврат Ложь
	Иначе
		Возврат Истина;
	КонецЕсли;
