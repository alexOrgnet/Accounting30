...
Функция ЧекЕГАИСXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц, Операция = Неопределено) Экспорт
	...
	Если ИспользоватьФорматОбменаV4 Тогда
		...
		ЧекXDTO = ИнтеграцияЕГАИС.ОбъектXDTO(ПространствоИмен, "ChequeV4Type");
		...
		Если Шапка.ВидОперации = Перечисления.ВидыОперацийЧекаЕГАИС.РеализацияРозничномуПокупателю
			...
			ЧекXDTO.Header = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(ЧекXDTO, "Header");
			
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Date",     Формат(Шапка.Дата, "ДФ=yyyy-MM-ddTHH:mm:ss"), СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Kassa",    Шапка.СерийныйНомерККМ,                       СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Shift",    Шапка.НомерСмены,                             СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Number",   Шапка.НомерЧекаККМ,                           СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Type",     "Продажа",                                    СообщениеXML);
			
		ИначеЕсли Шапка.ВидОперации = Перечисления.ВидыОперацийЧекаЕГАИС.РеализацияЮридическомуЛицуСБезналичнойОплатой Тогда
			
			ЧекXDTO.HeaderTTN = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(ЧекXDTO, "HeaderTTN");
			
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.HeaderTTN, "Date",       Формат(Шапка.Дата, "ДФ=yyyy-MM-dd"), СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.HeaderTTN, "BillNumber", Шапка.СерийныйНомерККМ,                       СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.HeaderTTN, "TTNNumber",  Шапка.НомерСмены,                             СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.HeaderTTN, "Type",       "Продажа",                                    СообщениеXML);
		...
		ИначеЕсли Шапка.ВидОперации = Перечисления.ВидыОперацийЧекаЕГАИСВозврат.ВозвратОтРозничногоПокупателя Тогда
			...
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Shift",    Шапка.НомерСмены,                             СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Number",   Шапка.НомерЧекаККМ,                           СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Type",     "Возврат",                                    СообщениеXML);
		...
		ИначеЕсли Шапка.ВидОперации = Перечисления.ВидыОперацийЧекаЕГАИСВозврат.ВозвратОтЮридическогоЛицаСБезналичнойОплатой Тогда
			...
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.HeaderTTN, "BillNumber", Шапка.СерийныйНомерККМ,                       СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.HeaderTTN, "TTNNumber",  Шапка.НомерСмены,                             СообщениеXML);
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.HeaderTTN, "Type",       "Возврат",                                    СообщениеXML);	
			
		КонецЕсли;
		
		ЧекXDTO.Content = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(ЧекXDTO, "Content");
		...
		ПустойИдентификаторСтроки = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа(Метаданные.ОпределяемыеТипы.УникальныйИдентификаторИС);
		...
		Для Каждого СтрокаТЧ Из ТаблицаТовары Цикл
			...
			Если СтрокаТЧ.Количество = 0 Тогда
				...
				Для Каждого КодАкцизнойМарки Из СтрокаТЧ.АкцизныеМарки Цикл
					...
					ОбъектXDTO = ИнтеграцияИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, ИмяОбъектаXDTO, ЧекXDTO.Content);
					
					ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ОбъектXDTO, "barcode", КодАкцизнойМарки, СообщениеXML);
					
					Если ЗначениеЗаполнено(СтрокаТЧ.Штрихкод)
						И ЭтоШтрихкодEAN(СтрокаТЧ.Штрихкод)
						И Не ШтрихкодированиеИСКлиентСервер.ЭтоВнутреннийШтрихкод(СтрокаТЧ.Штрихкод) Тогда
						ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ОбъектXDTO, "ean", СтрокаТЧ.Штрихкод, СообщениеXML);
					Иначе
						ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ОбъектXDTO, "ean", "0000000000000", СообщениеXML);
					КонецЕсли;
					
					ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ОбъектXDTO, "price", Цена, СообщениеXML);
					
					Если СтрокаТЧ.ЧастичноеВыбытие Тогда
						ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ОбъектXDTO, "Volume", СтрокаТЧ.КоличествоЕГАИС, СообщениеXML);
					КонецЕсли;
		...
		ТекстСообщенияXML = ИнтеграцияЕГАИС.ОбъектXDTOВXML(ЧекXDTO, Шапка.ИдентификаторФСРАР, ПространствоИмен, ИмяТипа);
		
	ИначеЕсли Не ФорматОбменаМенееV4 Тогда
		
		ОписаниеНоменклатуры = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(Товары.ВыгрузитьКолонку("Номенклатура"));
		...
		ЧекXDTO = ИнтеграцияЕГАИС.ОбъектXDTO(ПространствоИмен, "ChequeV3Type");
		...
		Для Каждого СтрокаТЧ Из Товары Цикл
			...
			Если КодыАкцизныхМарок.Количество() >= СтрокаТЧ.Количество Тогда
				...
				Для Каждого КодАкцизнойМарки Из КодыАкцизныхМарок Цикл
					...
					Bottle = ИнтеграцияИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, "Bottle", ЧекXDTO.Content);
					...
					Если Не ЗначениеЗаполнено(СтрокаТЧ.Штрихкод) Тогда
						ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Bottle, "ean", "0000000000000", СообщениеXML);
					ИначеЕсли ЭтоШтрихкодEAN(СтрокаТЧ.Штрихкод) Тогда
						Если ШтрихкодированиеИСКлиентСервер.ЭтоВнутреннийШтрихкод(СтрокаТЧ.Штрихкод) Тогда
							ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Bottle, "ean", "0000000000000", СообщениеXML);
						Иначе
...
Функция ЧекXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц, Операция = Неопределено) Экспорт
	...
	Если ИспользоватьФорматОбменаV4 Тогда
		...
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Shift",    Шапка.НомерСмены,                             СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Number",   Шапка.НомерЧекаККМ,                           СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ЧекXDTO.Header, "Type",     ТипЧека,                                      СообщениеXML);
...
Функция ЭтоШтрихкодEAN(Штрихкод)
...
Процедура РаспределитьАкцизнуюМаркуПоТаблицеТоваров(ТаблицаТовары, СтруктураПоиска, СтрокаТЧ, КоличествоКРаспределению)
