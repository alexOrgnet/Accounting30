...
Функция ОперацияПолучениеОборотовПоСмене(
	...
	ПредставлениеЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ПараметрыНастройкиПодключения.РегистрационныйНомерККТ,
...
Функция ПрочитатьДанные_get_receipts_full(ДанныеСервиса)
	...
	Попытка
		...
		Для Каждого ДанныеЧекаИзСервиса Из ДанныеСервиса.receipts Цикл
			...
			Для Каждого ДанныеПоТоваруИзСервиса Из ДанныеЧекаИзСервиса.items Цикл
				...
				Если ДанныеПоТоваруИзСервиса.Свойство("productCode") Тогда // АПК:1416 Не обязательное свойство ответа сервиса.
				...
				Иначе
					ДанныеТовара.Вставить("КодПредметаРасчета", Неопределено);
				КонецЕсли;
...
Функция ИдентификаторПризнакаСпособаРасчета(ИдентификаторПоДаннымСервиса)
	...
	Если Врег(ИдентификаторПоДаннымСервиса) = "FULL_PREPAYMENT" Тогда
