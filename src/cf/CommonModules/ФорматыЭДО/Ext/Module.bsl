	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Попытка
			...
			ИначеЕсли URIПространстваИмен = "ON_AKTSVEROTP_1_972_01_05_01_01"
				...
				Или URIПространстваИмен = Обработки.ФорматДоговорныйДокумент101.ПространствоИмен()
				Или URIПространстваИмен = Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО.ПространствоИмен() Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(URIПространстваИмен) Тогда
					СвойстваУдаление = Новый Массив;
					СвойстваУдаление.Добавить("xsi:type");
					ДвоичныеДанныеФайла = УдалитьАтрибутыУзлов(ДвоичныеДанныеФайла, Кодировка, СвойстваУдаление);
					ТипОбъекта = ПолучитьТипЗначенияCML("Файл", URIПространстваИмен);
				КонецЕсли;
				ЧтениеXML.ОткрытьПоток(ДвоичныеДанныеФайла.ОткрытьПотокДляЧтения());
				ДанныеФайлаЭД = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML, ТипОбъекта);
			КонецЕсли;
...
//  ДвоичныеДанные
//
Функция УдалитьАтрибутыУзлов(Знач ДвоичныеДанныеФайла, Знач Кодировка, Знач Свойства)
	
	Если Свойства.Количество() = 0 Тогда
		Возврат ДвоичныеДанныеФайла;
	КонецЕсли;
	
	СвойстваДляПоиска = Новый Массив;
	Для Каждого Свойство Из Свойства Цикл
		СвойстваДляПоиска.Добавить(" " + Свойство + "=""");
	КонецЦикла;
	
	ЧтениеДанных = Новый ЧтениеДанных(ДвоичныеДанныеФайла, Кодировка);
	НачалоДанных = ЧтениеДанных.ПрочитатьДо(СвойстваДляПоиска);
	
	Если НЕ НачалоДанных.МаркерНайден Тогда
		ЧтениеДанных.Закрыть();
		Возврат ДвоичныеДанныеФайла;
	КонецЕсли;
	
	ПотокЗаписи = Новый ПотокВПамяти();
	ЗаписьДанных = Новый ЗаписьДанных(ПотокЗаписи, Кодировка);
	ЗаписьДанных.Записать(НачалоДанных.ПолучитьДвоичныеДанные());
	
	Пока Истина Цикл
		
		ЧтениеДанных.ПропуститьДо("""");
		ЧастьДанных = ЧтениеДанных.ПрочитатьДо(СвойстваДляПоиска);
		Если НЕ ЧастьДанных.МаркерНайден Тогда
			Прервать;
		КонецЕсли;
		ЗаписьДанных.Записать(ЧастьДанных.ПолучитьДвоичныеДанные());
...
// Табличный документ с данными ОКЕИ.
// Пробует получить макет из прикладного справочника единиц измерения, если макет отсутствует,
// то получает предопределенный макет из библиотеки электронных документов.
// 
// Возвращаемое значение:
//  ТабличныйДокумент:
//  * Области - КоллекцияОбластейТабличногоДокумента:
//  ** КодЧисловой - ОбластьЯчеекТабличногоДокумента
Функция ПолучитьКлассификаторЕдиницИзмерения() Экспорт
	
	ИмяМакетаСДаннымиКлассификатора = "КлассификаторЕдиницИзмерения";
	ПрикладноеИмяСправочникаВладельца = ОбщегоНазначенияБЭД.ИмяПрикладногоСправочника("ЕдиницыИзмерения");
	МетаданныеВладельцаМакета = Метаданные.Справочники.Найти(ПрикладноеИмяСправочникаВладельца);
	Если МетаданныеВладельцаМакета <> Неопределено И МетаданныеВладельцаМакета.Макеты.Найти(
		ИмяМакетаСДаннымиКлассификатора) <> Неопределено Тогда
		Менеджер = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(МетаданныеВладельцаМакета.ПолноеИмя());
		ПрикладнойМакет = Менеджер.ПолучитьМакет(ИмяМакетаСДаннымиКлассификатора);
		Если ТипЗнч(ПрикладнойМакет) = Тип("ТабличныйДокумент") И ПрикладнойМакет.Области.Найти("КодЧисловой")
			<> Неопределено Тогда
			Возврат ПрикладнойМакет;
		КонецЕсли;
	КонецЕсли;
	ПредопределенныйМакет = Обработки.ОбменСКонтрагентами.ПолучитьМакет(ИмяМакетаСДаннымиКлассификатора);
	Возврат ПредопределенныйМакет;
