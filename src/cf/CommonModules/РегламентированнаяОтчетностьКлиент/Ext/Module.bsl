...
Процедура ОтправитьОтчетИзФормыСписка(Форма, ИмяКоманды, КонтекстЭДОКлиент, ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Движение средств по счету в банке за пределами РФ"
		...
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2021Кв4"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.дв.ден.б.за пр.21к4") Тогда
			ПоказатьИВернутьОшибку(Форма,
				НСтр("ru = 'Отправка в электронном виде формы отчета не предусмотрена
						   |законодательством. Форма сдается в бумажном виде'"));
			Возврат;
		ИначеЕсли ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2023Кв4"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.дв.ден.б.за пр.23к4", '20240301') Тогда
			ПоказатьИВернутьОшибку(Форма,
				НСтр("ru = 'Отправка в электронном виде формы отчета не предусмотрена
						   |законодательством. Форма сдается в бумажном виде'"));
			Возврат;
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Расчет по группе компаний для налогового вычета по акцизу на нефтяное сырье"
		И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("группе компаний для налог",
		'20240201') Тогда
		ПоказатьИВернутьОшибку(Форма,
			НСтр("ru = 'Отправка в электронном виде формы отчета не предусмотрена
					   |законодательством. Форма сдается в бумажном виде'"));
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеЧерезВебСервис(Результат, ДополнительныеПараметры) Экспорт
	...
	ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПродолжение(Форма);
КонецПроцедуры
...
Процедура ПредставлениеЭталонаФорматаОбласти(ДополнительныеПараметры) Экспорт
	...
	Если Область.СодержитЗначение И Область.ТипЗначения.СодержитТип(Тип("Число")) И Область.Значение <> 0 Тогда
		...
		Если СтрНайти(ИмяМакета, "_") = 0 Тогда
			НаимТекущегоРаздела = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.СтруктураРеквизитовФормы, "НаимТекущегоРаздела", "");
			Если ЗначениеЗаполнено(НаимТекущегоРаздела) Тогда
				ИмяМакета = ИмяМакета + "_" + НаимТекущегоРаздела;
			КонецЕсли;
		КонецЕсли;
		
		ФорматЭталон = РегламентированнаяОтчетностьВызовСервера.ПолучитьЭталонФорматаОбласти(Форма.ИмяФормы, ИмяМакета, Область.Имя);
...
Асинх Процедура ПредставлениеНуля(ДополнительныеПараметры) Экспорт
	...
	Если Область.СодержитЗначение И Область.ТипЗначения.СодержитТип(Тип("Число")) И Область.Значение = 0 Тогда
		...
		Форма.РасчетНаКлиенте(
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ИмяСтраницыПанели", ""), 
			ДополнительныеПараметры.Элемент.ТекущаяОбласть.Имя);
		Форма.Модифицированность = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура СохранитьОтчетИОткрытьФормуПрисоединенныеФайлы(Форма) Экспорт
	...
	Если ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Если СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированныйОтчет") > 0 Тогда
			...
			Оп = Новый ОписаниеОповещения("СохранитьОтчетИОткрытьФормуПрисоединенныеФайлыПослеПредупреждения",
				ЭтотОбъект, ДопПараметры);
			
			Если ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ФНС") Тогда
				ПоказатьПредупреждение(Оп, НСтр("ru = 'Обратите внимание, присоединенные файлы не отправляются при отправке отчета.
					|
...
Процедура СохранитьУведомлениеИОткрытьФормуПрисоединенныеФайлы(Форма) Экспорт
	...
	Если ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Если СтрНачинаетсяС(Форма.ИмяФормы, "Отчет.РегламентированноеУведомление") Тогда
			...
			Оп = Новый ОписаниеОповещения("СохранитьУведомлениеИОткрытьФормуПрисоединенныеФайлыПослеПредупреждения",
				ЭтотОбъект, ДопПараметры);
			
			ПоказатьПредупреждение(Оп, НСтр("ru = 'Обратите внимание, присоединенные файлы не отправляются при отправке уведомления.
				|
...
Асинх Процедура ИзменитьСтатусОтправки(ПараметрыИзменения) Экспорт
	...
	Если ВыбСтатусЭлемент <> Неопределено
		...
		Если ВыбСтатусЭлемент.Значение = НСтр("ru = 'Сдано'") Или ВыбСтатусЭлемент.Значение = НСтр("ru = 'Сдано, есть замечания'") Тогда
...
Асинх Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПродолжение(Форма)
...
Асинх Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиенте(Форма)
	Если Форма.Модифицированность Тогда
		Форма.СохранитьНаКлиенте();
	КонецЕсли;
...
Процедура ПроверитьКонтрольныеСоотношения_СКП_ПоискКопий(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ИскатьКопии Тогда
