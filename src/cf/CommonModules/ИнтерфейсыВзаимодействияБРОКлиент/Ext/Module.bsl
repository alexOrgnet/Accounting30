...
Процедура ПриНачалеПодписания(Организации, Файл, Полномочия, Оповещение, СтандартнаяОбработка) Экспорт
	...
	ПараметрыОтбораFTS_10001 = Новый Структура("Код", "FTS_10001");
	ПараметрыОтбораFTS_10002 = Новый Структура("Код", "FTS_10002");
	ПараметрыОтбораRAR_00000001 = Новый Структура("Код", "RAR_00000001");
	Если Полномочия.НайтиСтроки(ПараметрыОтбораFTS_10001).Количество() <> 0
		ИЛИ Полномочия.НайтиСтроки(ПараметрыОтбораFTS_10002).Количество() <> 0
		ИЛИ Полномочия.НайтиСтроки(ПараметрыОтбораRAR_00000001).Количество() <> 0 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриНачалеПодписанияПослеПодписания",
			ЭтотОбъект, ДополнительныеПараметры);
		ДокументооборотСКОКлиент.ЗарегистрироватьМЧДРР(ОписаниеОповещения, ДоверенностьСсылка);
	КонецЕсли;
...
Процедура ПриОбработкеНавигационнойСсылкиОбновлениеПослеОбновления(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("ОтправкаСсылка") И Результат.ОтправкаСсылка <> Неопределено Тогда
		ДоверенностьСсылка = ?(ТипЗнч(ДополнительныеПараметры.Доверенность) = Тип("ФормаКлиентскогоПриложения"),
			ДополнительныеПараметры.Доверенность.Объект.Ссылка, ДополнительныеПараметры.Доверенность);
