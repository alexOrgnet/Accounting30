...
Процедура ИсправитьОшибкиУчастникаСчет(СтрокаДанныхИзБанка, Участник) Экспорт
	...
	Если Не БанковскиеПравила.СтрокаСоответствуетФорматуБанковскогоСчета(СтрокаДанныхИзБанка[Участник + "Счет"])
		Или Не СтрокаДанныхИзБанка[Участник + "Реквизиты"].Свойство("Наименование") Тогда
		Возврат;
	КонецЕсли;
	
	БалансовыйСчет = БанковскиеПравила.БалансовыйСчет(СтрокаДанныхИзБанка[Участник + "Счет"]);
	...
	Для Каждого ВозможныйНомерСчета Из СловаУчастника Цикл
		...
		Если Проблема = "РасчетыФилиалаБанкаСКлиентамиБанка" Или Проблема = "РасчетыЧерезБанкКорреспондент"
			...
			ВозможныйНомерСчетаБезСлеша = СокрЛП(СтрЗаменить(ВозможныйНомерСчета, "/", ""));
			
			Если БанковскиеПравила.СтрокаСоответствуетФорматуIBAN(ВозможныйНомерСчетаБезСлеша)
				И БанковскиеПравила.ПроверитьКонтрольныйКлючIBAN(ВозможныйНомерСчетаБезСлеша) Тогда
				ПлатежОтНерезидента = Проблема = "РасчетыЧерезБанкНерезидент";
				НомерСчета = ВозможныйНомерСчетаБезСлеша;
				Прервать;
			ИначеЕсли БанковскиеПравила.СтрокаСоответствуетФорматуБанковскогоСчета(ВозможныйНомерСчетаБезСлеша) Тогда
				НомерСчета = ВозможныйНомерСчетаБезСлеша;
				Прервать;
			ИначеЕсли (Проблема = "РасчетыЧерезБанкНерезидент" Или Проблема = "РасчетыФилиалаБанкаСКлиентамиБанка")
				И БанковскиеПравила.СтрокаСоответствуетФорматуНациональногоНомераСчета(ВозможныйНомерСчетаБезСлеша)
				И (НРег(ПредыдущееСлово) = ПризнакРС Или СтрНачинаетсяС(НРег(ПредыдущееСлово), "acc")) Тогда
				НомерСчета = ВозможныйНомерСчетаБезСлеша;
				Прервать;
			КонецЕсли;
			
			ПредыдущееСлово = ВозможныйНомерСчета;
		КонецЕсли;
	...
	СтрокаДанныхИзБанка[Участник + "Счет"] = НомерСчета;
