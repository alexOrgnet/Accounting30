...
Процедура ПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Документы.ПоступлениеНаРасчетныйСчет.ПроверитьСоздатьВидОплаты(ТекущийОбъект, Форма.СвойстваПлатежа);
...
Процедура ВидимостьДоступностьЭлементовФормы(Форма) Экспорт
	...
	Если НазваниеТабличнойЧасти = "РасшифровкаПлатежа" Или НазваниеТабличнойЧасти = "РасшифровкаПлатежаПлатежныеКарты" Тогда
		...
		Если Документы.ПоступлениеНаРасчетныйСчет.ЕстьРасшифровкаПлатежа(Объект.ВидОперации) Тогда
			...
			БанкИКассаФормыКлиентСервер.УстановитьВидимостьОтраженияДоходаПоступленияПоПлатежнымКартам(Форма, НазваниеТабличнойЧасти);
		КонецЕсли;
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(Форма) Экспорт
	...
	ОтражениеДоходаПоПлатежнымКартамВидимость =
		ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ОтражениеДоходаПоПлатежнымКартамВидимость")
		И Форма.ОтражениеДоходаПоПлатежнымКартамВидимость;
	...
	Для каждого СтрокаПлатеж Из РасшифровкаПлатежа Цикл
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаПлатеж.ПорядокОтраженияАванса)
			И (Форма.ОтражениеДоходаВидимость Или ОтражениеДоходаПоПлатежнымКартамВидимость) Тогда
...
Функция НовыеПараметрыПриИзмененииСчетаОрганизации(Знач ПараметрыОбъекта)
	...
	Если ПараметрыОбъекта.ВалютаДокумента = НоваяВалюта Тогда
		Возврат Новый Структура(
			"ВалютаДокумента, ЦифровыеДеньги, ПодразделениеОрганизации, СчетКонтрагента, КурсДокумента, КратностьДокумента",
			НоваяВалюта, НовыйЦифровыеДеньги, СвойстваСчета.ПодразделениеОрганизации, ПараметрыОбъекта.СчетКонтрагента,
...
Процедура УправлениеЭлементамиОтраженияВУСННаСервере(Форма) Экспорт
	...
	Если Не ЭтоФормаРасшифровкиПлатежа Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление
			...
			Элементы.СодержаниеУСН.Видимость = Не ПрименяетсяАУСН;
			...
			ВидимостьСуммыДоходовУСН = (Не ПрименяетсяАУСН Или Объект.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются)
				 И Не ПрименяетсяРазметкаАУСНБанка;
...
Процедура ЗаполнитьСпискиВыбораОтражениеВУСН(Форма)
	...
	Если ЭлементОтражениеВУСН <> Неопределено Тогда
		...
		// При оплате от покупателя вариант "Не принимаются" имеет особое представление.
		Если Форма.Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя Тогда
...
Функция СвойстваСтрокРасшифровкиПлатежаСервер(Знач ПараметрыПлатежа, Знач ПолучатьДоговор) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыПлатежа.ДоговорКонтрагента) Тогда
		...
		Если ПараметрыПлатежа.ВидОперации
			...
			СвойстваПлатежа.ВидОплатыРеквизиты = Документы.ПоступлениеНаРасчетныйСчет.ПараметрыВидОплатыПоРеквизитам(
				ПараметрыПлатежа.Организация, ПараметрыПлатежа.Контрагент, ПараметрыПлатежа.ДоговорКонтрагента);
		КонецЕсли;
	Иначе
...
Процедура УстановитьУсловноеОформлениеШапкаИВидимость(Форма, НачалоПутиКДаннымТЧ, ФормаДокументаОднострочная)
	...
	// РасшифровкаПлатежаПлатежныеКартыОтражениеДоходаПКПредставление
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаПлатежныеКартыОтражениеДоходаПКПредставление");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ЭлементУО.Отбор,
		"ОтражениеДоходаПоПлатежнымКартамВидимость",
		ВидСравненияКомпоновкиДанных.Равно,
