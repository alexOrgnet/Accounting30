...
Процедура НачальныеОперацииПриСозданииНаСервере(Форма) Экспорт
	...
	Реквизит_УИДПереключение = Новый РеквизитФормы("УИДПереключение", Новый ОписаниеТипов("УникальныйИдентификатор"));
	Реквизит_ПараметрыПозиционирования = Новый РеквизитФормы("ПараметрыПозиционирования", Новый ОписаниеТипов());
	...
	ДобавляемыеРеквизиты.Добавить(Реквизит_УИДПереключение);
	ДобавляемыеРеквизиты.Добавить(Реквизит_ПараметрыПозиционирования);
...
Функция ЗагрузитьСправку(СхемаВыгрузки, ДеревоЗагрузки, ПутьКСтрокеСДанными) Экспорт 
	...
	ДанныеСправкиПоСхеме = УведомлениеОСпецрежимахНалогообложения.НайтиУзелВДеревеПоПути(СхемаВыгрузки, ПутьКСтрокеСДанными, "Код");
...
Процедура ЗагрузитьСправкуРек(СтрокаСхемыВыгрузки, ДеревоЗагрузки, ПутьКСтрокеСДанными, ТекущиеДанные) Экспорт
	Если ЗначениеЗаполнено(СтрокаСхемыВыгрузки.Ключ) И ЗначениеЗаполнено(СтрокаСхемыВыгрузки.Код) Тогда 
		УзелЗагрузки = УведомлениеОСпецрежимахНалогообложения.НайтиУзелВДеревеПоПути(ДеревоЗагрузки, "Файл/" + ПутьКСтрокеСДанными, "ИмяЭлемента");
...
Процедура ПроверкаДанныхПолучателяСправки(Сведения, ИмяСтраницы, ТаблицаОшибок, ПризнакСовпадения) Экспорт 
	...
	КонецЕсли;
	УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(Сведения, ТаблицаОшибок, ИмяСтраницы, "ИННФЛ", Ложь, Ложь);
	Если Не ЗначениеЗаполнено(Сведения.ИННФЛ)
		Или УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(Сведения, "КодВидДок,СерНомДок,ДатаДок") Тогда 
	...
	Если ПризнакСовпадения = "0" Тогда 
		...
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(Сведения, ТаблицаОшибок, "СведОплФизУсл", "ИННФЛПолучатель", Ложь, Ложь);
		Если Не ЗначениеЗаполнено(Сведения.ИННФЛПолучатель)
			Или УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(Сведения, "КодВидДокПолучатель,СерНомДокПолучатель,ДатаДокПолучатель") Тогда 
	...
	ИначеЕсли ПризнакСовпадения = "1" Тогда 
		Если УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(Сведения, 
		...
		Для Каждого КЗ2 Из КЗ1.Значение Цикл 
			Если СтрНайти(КЗ2.Ключ, "___") = 0 И ЗначениеЗаполнено(КЗ2.Значение) И ТипЗнч(КЗ2.Значение) = Тип("Дата") Тогда
				Если Год(КЗ2.Значение) = 1 И Месяц(КЗ2.Значение) = 1 И День(КЗ2.Значение) = 1 Тогда 
					Продолжить;
				ИначеЕсли Год(КЗ2.Значение) < 1900 Или Год(КЗ2.Значение) > 2100 Тогда 
					ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				...
				Для Каждого Кол Из Данные.ДанныеДопСтрокБД[КЗ1.Ключ].Колонки Цикл 
					Если ЗначениеЗаполнено(Стр[Кол.Имя]) И ТипЗнч(Стр[Кол.Имя]) = Тип("Дата") Тогда
						Если Год(Стр[Кол.Имя]) = 1 И Месяц(Стр[Кол.Имя]) = 1 И День(Стр[Кол.Имя]) = 1 Тогда 
							Продолжить;
						ИначеЕсли Год(Стр[Кол.Имя]) < 1900 Или Год(Стр[Кол.Имя]) > 2100 Тогда 
							ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
...
Процедура ПроверкаКПП(Данные, ТаблицаОшибок, Страница, Поле, Обязательность, ИндексСтроки = Неопределено) Экспорт 
	...
	Если ЗначениеЗаполнено(Данные[Поле]) Тогда
		Если Не РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(Данные[Поле], "") Или Данные[Поле] = "000000000" Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
...
Процедура РаскраситьАвтозаполняемыеЯчейки(Форма) Экспорт 
...
Процедура ЗагрузкаМногострочныхЧастейУведомления(Форма, СтруктураПараметров) Экспорт 
