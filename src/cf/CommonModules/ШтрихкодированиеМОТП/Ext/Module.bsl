...
Функция ВложенныеШтрихкодыУпаковок(ШтрихкодыУпаковок, ПараметрыСканирования, СортироватьДанные = Ложь) Экспорт
	...
	Если ИзвестныеУпаковки.Количество() > 0 Тогда
		...
		ШтрихкодированиеИСМПСлужебный.УстановитьПризнакОтказаОтПроверкиКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования);
...
Функция ВыполнитьПроверкиПередПолучениемДанныхПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Если ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, ДанныеПоШтрихкодам.ОтключитьЗапросВСервисПоОперацииИПродукции) Тогда
		...
		РезультатПроверкиАвторизации = ШтрихкодированиеИСМПСлужебный.ТребуетсяАвторизацияПользователя(ПараметрыСканирования);
		...
		Если ЗначениеЗаполнено(СтрокаДанных.ВидПродукции)
			...
			Если ВидыПродукцииПоКоду.Количество() > 0 Тогда
				...
				Если РезультатПроверкиАвторизации.ТребуетсяОбновлениеКлючаСессии Тогда
					...
					ШтрихкодированиеИСМПСлужебный.ЗаполнитьРезультатОбработкиТребуетсяАвторизация(РезультатОбработкиШтрихкода, ПараметрыСканирования);
				...
				Если РезультатПроверкиАвторизации.ТребуетсяОбновлениеКлючаСессииРозница Тогда
					...
					ШтрихкодированиеИСМПСлужебный.ЗаполнитьРезультатОбработкиТребуетсяАвторизация(РезультатОбработкиШтрихкода, ПараметрыСканирования, Истина);
...
Функция ПравилаПроверкиНаОшибки(ПараметрыСканирования, ДанныеПоШтрихкодам, ПроверенныеШтрихкоды = Неопределено)
	...
	Если ПараметрыСканирования.ДопустимыеСтатусыМОТП.Количество() > 0
		И (ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, ДанныеПоШтрихкодам.ОтключитьЗапросВСервисПоОперацииИПродукции) Или НеобходимыПроверкиРозничнойПродажи)
	...
	Если ЗначениеЗаполнено(ПараметрыСканирования.ДопустимыйСпособВводаВОборот) Тогда
		Если (ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, ДанныеПоШтрихкодам.ОтключитьЗапросВСервисПоОперацииИПродукции) Или НеобходимыПроверкиРозничнойПродажи)
			И Не ИнтеграцияИСМПКлиентСерверПовтИсп.ДействуетАварийныйРежимДляРозничныхПродаж() Тогда
...
Процедура ОбработатьДеревоПолученноеИзМОТП(ДеревоУпаковок, ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыОбработки) Экспорт
	...
	Если ПараметрыСканирования <> Неопределено
		...
		Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияМОТП(ДопустимыйВидПродукции) Тогда
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольПолногоКодаМаркировки") Тогда
			...
			Если СтрокаДанных <> Неопределено Тогда
				...
				Если ВыполнятьКонтроль
					...
					Если ПараметрыСканирования.ТребоватьПолныйКодМаркировкиИСМП
						Или КодВключаетКриптоХвост Тогда
...
Процедура ЗапросИРасчетМРЦПоСтрокамДерева(ЗапрашиваемыеСтроки, ВычисляемыеСтроки, ПараметрыСканирования) Экспорт
	...
	Если Не ИнтеграцияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования) Тогда
		Возврат;
	КонецЕсли;
...
Функция СтрокиТаблицыДанныхДляПолученияСпецифики(ДанныеКодовМаркировки, СпецификаШтрихкодовГрупповыхИЛогистическихУпаковок, ПараметрыСканирования)
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		ПеречитыватьДляОбязательныхТоварныхГруппПриРозничнойПродажеГИСМТ =
			Не ИнтеграцияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(СтрокаДанных.ВидПродукции,
				ПараметрыСканирования.ВидОперацииИСМП,
				ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ)
