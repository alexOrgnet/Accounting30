...
Функция КодМаркировкиСоответствуетШаблону(ПараметрыРазбораКодаМаркировки, Настройки, ШаблонКодаМаркировки) Экспорт
	...
	Если ЗначениеЗаполнено(ШаблонКодаМаркировки.ВидУпаковки) Тогда
		Если ШаблонКодаМаркировки.ВидУпаковки = ВидУпаковкиНабор
			И ШтрихкодированиеИСКлиентСервер.ЭтоВнутреннийШтрихкод(СоставКодаМаркировки.EAN) Тогда
			Возврат Неопределено;
		КонецЕсли;
	ИначеЕсли СоставКодаМаркировки.Свойство("EAN")
		И ШтрихкодированиеИСКлиентСервер.ЭтоВнутреннийШтрихкод(СоставКодаМаркировки.EAN)
...
// Возвращаемое значение:
//    Структура:
//     * ШаблоныПоИдентификатору - Соответствие:
//        ** Ключ     - Строка - см. ИнициализироватьНастройкиШаблонаРазобранногоКодаМаркировки.ИдентификаторДопустимыхШаблонов
//        ** Значение - см. ИнициализироватьНастройкиШаблонаРазобранногоКодаМаркировки.
//     * ПараметрыШаблонаПоПолномуХешуШаблонаСУчетомGTIN - Соответствие:
//        ** Ключ     - Строка.
//        ** Значение - Булево.
	...
	Если ПозицияПоследнегоРазделителя > 0 Тогда
		...
		Если Лев(Окончание, 2) = "97" Тогда
			...
			Если Не Base64 Тогда
				Результат = ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(Результат);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// * РезультатРазбора - Неопределено, МенеджерОборудованияМаркировкаКлиентСервер.РазобратьСтрокуШтрихкодаGS1 - Предварительный разбор при флаге СодержитРазделительGS.
// * ДлинаКодаМаркировки - Число - Длина кода маркировки.
// * ФильтрПоВидуПродукции - Структура - Пользовательский фильтр по виду продукции:
...
// * МодульКонтекста - ОбщийМодуль - РазборКодаМаркировкиИССлужебныйКлиент / РазборКодаМаркировкиИССлужебный.
// * ОбщиеМодулиРазбораКМ - Соответствие - Соответствие из имени общего модуля и самого общего модуля.
...
// * КодМаркировки - Строка.
// * ВидыПродукции - Неопределено, ПеречислениеСсылка.ВидыПродукцииИС, Массив Из ПеречислениеСсылка.ВидыПродукцииИС.
// * Настройки - см. РазборКодаМаркировкиИССлужебныйКлиент.НастройкиРазбораКодаМаркировки, см. РазборКодаМаркировкиИССлужебный.НастройкиРазбораКодаМаркировки
...
Функция РазобратьКодМаркировкиУпрощенноПоШаблону(ПараметрыОбработкиКодаМаркировки, ПараметрыПроверяемогоШаблона, КешНастроекРазбораКодаМаркировки)
	...
	Если ПараметрыОбработкиКодаМаркировки.ПользовательскиеПараметры = Неопределено
		...
		Для Каждого ОписаниеЭлементаКМ Из ДанныеШаблона.ОписаниеЭлементовКодаМаркировки Цикл
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(ОписаниеЭлементаКМ.Алфавит)
				И Не ШтрихкодированиеИСКлиентСервер.КодСоответствуетАлфавиту(
	...
	Для Каждого ОписаниеЭлементаКМ Из ДанныеШаблона.ОписаниеЭлементовКодаМаркировки Цикл
		...
		РазборКодаМаркировкиИССлужебныйКлиентСервер.ЗаполнитьСоставКодаМаркировки(СоставКодаМаркировки, ОписаниеЭлементаКМ, Значение);
		
	КонецЦикла;
	
	ДанныеРезультата = РазборКодаМаркировкиИССлужебныйКлиентСервер.НовыйРезультатРазбораКодаМаркировки(ПараметрыОбработкиКодаМаркировки.РасширеннаяДетализация);
	...
	ДанныеРезультата.НормализованныйКодМаркировки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировкиПоБазовомуШаблону(
		ДанныеРезультата.КодМаркировки,
...
Функция ЭтоПродукцияИзНатуральногоМеха(КодМаркировки, ДополнительныеПараметры, ПроверятьАлфавитЭлементов, Алфавит, ЗначенияЭлементовКМ = Неопределено)
	...
	Если ЗначенияЭлементовКМ = Неопределено Тогда
		...
		Если ПроверятьАлфавитЭлементов Тогда
			Если Не ШтрихкодированиеИСКлиентСервер.КодСоответствуетАлфавиту(
				Сред(КодМаркировки, 4, 6), Алфавит.БуквыПрописныеЦифры) Тогда
				Возврат Ложь;
			ИначеЕсли Не ШтрихкодированиеИСКлиентСервер.КодСоответствуетАлфавиту(
...
//
// Параметры:
//   Значения - Неопределено, ПеречислениеСсылка.ВидыПродукцииИС, Массив из ПеречислениеСсылка.ВидыПродукцииИС
// Возвращаемое значение:
...
Функция ХешВидовПродукции(Значения) Экспорт
	...
	ИначеЕсли ТипЗнч(Значения) = Тип("ПеречислениеСсылка.ВидыПродукцииИС") Тогда
		Если Значения.Пустая() Тогда
			Возврат "";
		Иначе
			Возврат Формат(ИнтеграцияИСКлиентСервер.ИндексВидаПродукцииИС(Значения), "ЧЦ=2; ЧН=; ЧВН=");
		КонецЕсли;
	ИначеЕсли ТипЗнч(Значения) = Тип("Массив") Тогда
		...
		ИначеЕсли Значения.Количество() = 1 Тогда
			Если ТипЗнч(Значения[0]) = Тип("ПеречислениеСсылка.ВидыПродукцииИС") И Не Значения[0].Пустая() Тогда
				Возврат Формат(ИнтеграцияИСКлиентСервер.ИндексВидаПродукцииИС(Значения[0]), "ЧЦ=2; ЧН=; ЧВН=");
			Иначе
				Возврат "";
			КонецЕсли;
		Иначе
			...
			Для Каждого Значение Из Значения Цикл
				Если ТипЗнч(Значение) = Тип("ПеречислениеСсылка.ВидыПродукцииИС") И Не Значение.Пустая() Тогда
					СортируемыйСписок.Добавить(Формат(ИнтеграцияИСКлиентСервер.ИндексВидаПродукцииИС(Значение), "ЧЦ=2; ЧН=; ЧВН="));
				КонецЕсли;
			КонецЦикла;
