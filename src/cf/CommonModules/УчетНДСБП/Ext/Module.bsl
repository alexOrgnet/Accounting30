...
Процедура ТекстЗапросаРеестрНДСПриложение(Запрос, ПараметрыОтчета, ЭтоПриложение5) Экспорт
	...
	Если УчетНДСВызовСервера.КлассифицироватьНоменклатуруПоОперациям0(ДатаЗапроса,Организация) Тогда
		...
		Если ЭтоПриложение5 Тогда
			ДоступныеКодыОпераций = КодыОперацийРеестрНДСПриложение(5,ДатаЗапроса);
		Иначе
			ДоступныеКодыОпераций = КодыОперацийРеестрНДСПриложение(1,ДатаЗапроса);
		КонецЕсли;
	
		Запрос.УстановитьПараметр("ДоступныеКодыОпераций", ДоступныеКодыОпераций);
...
Функция КодыОперацийРеестрНДСПриложение (НомерРеестраНДСПриложение, ДатаЗапроса)
	
	МассивКодовОпераций = Новый Массив;
	
	Если НомерРеестраНДСПриложение = 5 Тогда
	...
	Иначе
		...
		Если ДатаЗапроса >='20200401' Тогда
		
			Макет = Отчеты.РегламентированныйОтчетРеестрНДСПриложение1.ПолучитьМакет("СпискиВыбора2020Кв2");
		...
		Иначе
			...
			Макет = Отчеты.РегламентированныйОтчетРеестрНДСПриложение1.ПолучитьМакет("СпискиВыбора2015Кв4");
...
Функция ТекстЗапросаРаздел5КодыОпераций()
	...
	|		ПО ДанныеПервичныхДокументов.Документ = НДСЗаписиКнигиПокупок.ДокументОтгрузки
	|ГДЕ
	|	&СУчетомКорректировок
	|	И НДСЗаписиКнигиПокупок.Организация В(&Организация)
	|	И НДСЗаписиКнигиПокупок.ЗаписьДополнительногоЛиста
