...
//
Процедура ПодключитьОбработчикВыполненияОбменаНаКлиентеПоРасписанию(НачалоРаботыСистемы = Истина) Экспорт
	
	ОтключитьОбработчикОжидания("ОбработчикОжиданияВыполненияОбменаНаКлиентеПоРасписанию");
	
	ОбменНастроен = Ложь;
	Если НачалоРаботыСистемы Тогда
		ОбменНастроен = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиентаПриЗапуске().ПодключитьОбработчикВыполненияОбменаНаКлиентеПоРасписанию;
	Иначе
		ОбменНастроен = ИнтеграцияЕГАИСВызовСервера.ДоступноВыполнениеОбменаНаКлиентеПоРасписанию()
	КонецЕсли;
