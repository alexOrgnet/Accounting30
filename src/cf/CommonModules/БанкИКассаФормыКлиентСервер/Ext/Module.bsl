...
// Управляет видимостью поля выбора отражения поступления по платежным картам в доходах.
//
// Параметры:
//  Форма      - ФормаКлиентскогоПриложения - настраиваемая форма документа поступления денег.
...
//               См. например, ПриходныйКассовыйОрдерФормыКлиентСервер.НазваниеТаблицыРасшифровкиПлатежа
//
Процедура УстановитьВидимостьОтраженияДоходаПоступленияПоПлатежнымКартам(Форма, НазваниеТабличнойЧасти) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		...
		УстановитьВидимостьПоля(
			Форма.Элементы,
			"ОтражениеДоходаПКПредставление",
			НазваниеТабличнойЧасти,
			Форма.ОтражениеДоходаПоПлатежнымКартамВидимость И НепосредственноеРедактирование);
