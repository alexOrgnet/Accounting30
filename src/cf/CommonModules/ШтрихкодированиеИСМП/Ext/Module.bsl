...
Процедура СвернутьОднородныеОбъемноСортовыеКоды(ВложенныеШтрихкодыУпаковок, ШтрихкодыУПД) Экспорт
	...
	Для Каждого СтрокаGTIN Из ОбъемноСортовыеКоды Цикл
		...
		Для Индекс = 0 По ИсходныеСтроки.ВГраница() Цикл
			...
			Приемник.ШтрихкодBase64 = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(НормализованныйКодМаркировки);
			//Дерево
...
Функция ДобавитьИзменитьКодМаркировкиКегаНаОборудованииРозлива(ПараметрыСканирования, Товары, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	ПараметрыЗаполнения.ОбработкаГрупповыхУпаковок = Ложь;
...
Функция ПараметрыЗаполненияТаблицыИзДереваУпаковок()
	...
	ВозвращаемоеЗначение.Вставить("ОбработкаГрупповыхУпаковок", Истина);
...
Процедура ЗаполнитьТаблицуШтрихкодовИзДереваУпаковокРекурсивно(ТаблицаПриемник, ДеревоУпаковок, ПараметрыЗаполнения)
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если ПараметрыЗаполнения.ОбработкаГрупповыхУпаковок
