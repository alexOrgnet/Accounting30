...
#КонецОбласти
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.95.6";
	Обработчик.Процедура           = "РегистрыСведений.КлассификаторОКВЭД2.ОбновитьНаименованияОКВЭД2";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.96.15";
	Обработчик.Процедура           = "Обработки.ФункциональностьПрограммы.ФункциональностьИспользоватьНазначениеИспользованияСпецодежды";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.102.7";
	Обработчик.Процедура           = "Константы.ОтображатьОстаткиИЦеныВСпискеТовары.УстановкаЗначенияПриОбновленииИнформационнойБазы";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.104.9";
	Обработчик.Процедура           = "Константы.ОтображатьОстаткиПоВзаиморасчетамВСпискеКонтрагентов.УстановкаЗначенияПриОбновленииИнформационнойБазы";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.108.5";
	Обработчик.Процедура           = "Константы.ФормированиеУставногоКапиталаПростойИнтерфейс.УстановитьКонстантуПриОбновленииИнформационнойБазы";
	Обработчик.НачальноеЗаполнение = Ложь; 
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.116.13";
	Обработчик.Процедура           = "РегистрыСведений.ВариантыПримененияТребованийЗаконодательства.ЗаполнитьУсловияПримененияТребованийЗаконодательстваСтроковымиЗначениями";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.116.22";
	Обработчик.Процедура           = "Справочники.ПравилаПредставленияОтчетовУплатыНалогов.ЗаполнитьУсловияПримененияТребованийЗаконодательстваСтроковымиЗначениями";
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.ОбщиеДанные         = Истина;
	...
	Обработчик.Процедура           = "РегистрыСведений.КлассификаторОКВЭД2.ЗаполнитьКлассификаторПриОбновленииИнформационнойБазы";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.140.2";
	Обработчик.Процедура           = "УчетКассовыхЧековПодотчетныхЛиц.УстановитьОпциюУстановкаМобильногоПриложенияОтложенно";
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения     = "Отложенно";
	Обработчик.Идентификатор       = Новый УникальныйИдентификатор("1ed86a6f-2cbe-6182-9723-7782921dde34");
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.144.25"; 
	Обработчик.Процедура           = "Справочники.ШаблоныДоговоров.ОбновлениеТиповыхШаблоновДоговоровОтложенно";
	Обработчик.НачальноеЗаполнение = Истина; 
	Обработчик.РежимВыполнения 	   = "Отложенно";
	Обработчик.Идентификатор       = Новый УникальныйИдентификатор("097cc719-879a-4acf-bcaa-605476ab9bcb");
...
Процедура УстановитьДатуТекущейВерсии() Экспорт
	...
	Константы.ДатаТекущейВерсии.Установить(Дата(2024,04,09));
...
Процедура КонтактнаяИнформацияОбновлениеИБ() Экспорт
	...
	// КонтактныеЛица
	Справочники.КонтактныеЛица.ЗаполнитьВидыКонтактнойИнформации();
	...
	ПараметрыВида.Наименование = НСтр("ru = 'Адрес местонахождения'");
	ПараметрыВида.МожноИзменятьСпособРедактирования = Ложь;
	ПараметрыВида.ВидРедактирования                 = "Диалог";
	ПараметрыВида.ОбязательноеЗаполнение            = Ложь;
	ПараметрыВида.Порядок                           = 1;
	...
	Справочники.ОбъектыВыполненияРабот.УстановитьСвойстваВидовКонтактнойИнформации();
...
Процедура ЗаполнитьДатуПоследнейЗагрузкиЭкземпляраПодключаемогоОборудованияЭвотор() Экспорт
	...
	|ГДЕ
	|	ПодключаемоеОборудование.ТипОборудования = ЗНАЧЕНИЕ(Перечисление.ТипыПодключаемогоОборудования.УдалитьККМОфлайн)
...
Процедура ЗарегистрироватьИзмененияОбъектовМаркировки(Параметры) Экспорт
	...
	Для Каждого ПланОбмена Из ПланыОбменаРИБ Цикл
		...
		Если ВсеУзлы.Количество() > 0 Тогда
			...
			ПланыОбмена.ЗарегистрироватьИзменения(ВсеУзлы, Метаданные.РегистрыСведений.СтатусыПроверкиИПодбораДокументовИСМП);
