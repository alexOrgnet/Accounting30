...
Функция ПрочитатьРегистрациюГоловногоПодразделения(Знач Родитель) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	ПодразделенияОрганизаций.Ссылка В(&ВсеРодители)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРодители КАК Родители
		|		ПО (Родители.Ссылка = ИспользованиеРегистрацийВОрганеСтатистики.СтруктурнаяЕдиница)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Родители.Ссылка.Наименование ИЕРАРХИЯ");
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		РодителиСРегистрацией = РезультатЗапроса.Выгрузить();
		Строка = РодителиСРегистрацией[РодителиСРегистрацией.Количество() - 1];
		МенеджерЗаписи.СтруктурнаяЕдиница = Строка.СтруктурнаяЕдиница;
