...
Функция СНИЛССовпадает(Форма, Субъект)
	...
	СНИЛСБезРазделителей = ОбработкаЗаявленийАбонентаКлиентСервер.СНИЛСБезРазделителей(Форма.ВладелецЭЦПСНИЛС);
	СНИЛССовпадает = СНИЛССертификата = СНИЛСБезРазделителей;
...
Функция ФИОВладельцаСовпадает(Форма, Субъект)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Субъект, "GN") Тогда
		ПолеИмя     = "GN";
		ПолеФамилия = "SN";
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Субъект, "OID2_5_4_42") Тогда
		ПолеИмя     = "OID2_5_4_42";
...
Процедура ОтобразитьРезультатПроверки(Элемент, РезультатПроверки) Экспорт
	...
	Элемент.Видимость = ЗначениеЗаполнено(РезультатПроверки.ТекстОшибки);
