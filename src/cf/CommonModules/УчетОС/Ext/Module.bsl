...
//
// Параметры:
//  Записи - ПолучитьПустуюТаблицуАмортизацииОС
...
Функция ПодготовитьРасчетАмортизацииОС(ТаблицаОС, ТаблицаРеквизиты, Отказ, Знач РежимСправкиРасчета = Ложь, ОкончательныйРасчет = Ложь) Экспорт
	...
	ДопускаетсяПрименениеАлгоритмовПБУ6 = Истина;
	Если ПересчетЗаГод <> Неопределено Тогда
		ДопускаетсяПрименениеАлгоритмовПБУ6 = Ложь;
	КонецЕсли;
	...
	Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.РегламентнаяОперация")
		ИЛИ Реквизиты.Регистратор = Неопределено Тогда
...
Процедура УстановитьДанныеБухгалтерскогоУчета(СтрокаСправкаРасчет, СтрокаАмортизации, ЗаписьОценкаСтоимости, ДанныеПостоянныхРазниц)
	...
	// Разницы отражаем на сводной записи
	УстановитьСуммыПроводкиБУ(
		СтрокаСправкаРасчет,
		СтрокаАмортизации,
		ЗаписьОценкаСтоимости.СуммаПроводки,
		ЗаписьОценкаСтоимости.Амортизация,
		ДанныеПостоянныхРазниц);
...
Процедура УстановитьСуммыПроводкиБУ(СтрокаСправкаРасчет, СтрокаАмортизации, СуммаПроводки, Амортизация, ДанныеПостоянныхРазниц)
...
Процедура СформироватьДвиженияВосстановлениеАмортизационнойПремии(ТаблицаОС, ТаблицаСтоимости, ТаблицаРеквизиты, СправкаРасчет, Движения, Отказ) Экспорт
	...
	Иначе
		...
		Для Каждого ОбъектАмортПремии Из ТаблицаАмортПремий Цикл
			...
			// Понадобится для дальнейшего определения убытка
			СтрокиСправкиРасчета = СправкаРасчет.НайтиСтроки(Новый Структура("Объект", ОбъектАмортПремии.ОсновноеСредство));
			Если СтрокиСправкиРасчета.Количество() Тогда
				
				СтрокаСправкиРасчета = СтрокиСправкиРасчета[0];
				СтрокаСправкиРасчета.СтоимостьНУ = СтрокаСправкиРасчета.СтоимостьНУ + ОбъектАмортПремии.СуммаАморПремии;
				СтрокаСправкиРасчета.ОстаточнаяСтоимостьНУ = СтрокаСправкиРасчета.ОстаточнаяСтоимостьНУ + ОбъектАмортПремии.СуммаАморПремии;
				
				Если ИспользоватьСуммыРазниц Тогда
					СтрокаСправкиРасчета.СтоимостьВР = СтрокаСправкиРасчета.СтоимостьВР - ОбъектАмортПремии.СуммаАморПремии;
					СтрокаСправкиРасчета.ОстаточнаяСтоимостьВР = СтрокаСправкиРасчета.ОстаточнаяСтоимостьВР - ОбъектАмортПремии.СуммаАморПремии;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПодготовитьТаблицыАмортизацииОСИСуммАмортизационнойПремии(ТаблицаОС, ТаблицаРеквизиты, Отказ, ОкончательныйРасчет = Ложь) Экспорт
...
Функция ПодготовитьПараметрыОбесценениеОС(ТаблицаОбесценение, ТаблицаРеквизиты)
	...
	СписокОбязательныхКолонок = ""
	+ "ОсновноеСредство,"           // <СправочникСсылка.ОсновныеСредства>
	+ "ИзменениеОбесценения,";      // <Ссылка на справочник подразделений>

	Параметры.Вставить("ТаблицаОбесценение",
		ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(ТаблицаОбесценение, СписокОбязательныхКолонок));
...
Процедура СформироватьДвиженияОбесценениеОС(ТаблицаОбесценение, ТаблицаРеквизиты, Движения, Отказ) Экспорт

	Если Не ЗначениеЗаполнено(ТаблицаОбесценение) Тогда
		Возврат;
	КонецЕсли;
	
	Параметры = ПодготовитьПараметрыОбесценениеОС(ТаблицаОбесценение, ТаблицаРеквизиты);
	Реквизиты = Параметры.Реквизиты[0];
	
	ОтражатьВНалоговомУчете = УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Реквизиты.Организация, Реквизиты.Период);
	ПоддержкаПБУ18          = УчетнаяПолитика.ПоддержкаПБУ18(Реквизиты.Организация, Реквизиты.Период);

	Для каждого СтрокаТаблицы Из Параметры.ТаблицаОбесценение Цикл
		...
		Если СтрокаТаблицы.ИзменениеОбесценения > 0 Тогда
			...
			Проводка.Содержание = "Убыток от обесценения основных средств";
			...
			Проводка.Сумма = СтрокаТаблицы.ИзменениеОбесценения;
		...
		Иначе 
			...
			Проводка.Содержание = "Восстановление убытка от обесценения";
...
Процедура СформироватьДвиженияУбытокОтРеализацииОС(ПараметрыПроведения, ПараметрыВыбытия, ТаблицаРеализации, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаСправки Из ПараметрыВыбытия.НачислениеАмортизации.СправкаРасчет Цикл
		
		Если СтрокаСправки.ОстатокСрокаПолезногоИспользованияНУ <= 1 Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаРеализацииОС = ТаблицаРеализации.Найти(СтрокаСправки.Объект, "Номенклатура");
		
		Если СтрокаРеализацииОС = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		УбытокОтРеализацииОС = СтрокаСправки.ОстаточнаяСтоимостьНУ
			- СтрокаСправки.СуммаАмортизацииНУ 
			- СтрокаРеализацииОС.СуммаБезНДСРуб;
		...
		Если УбытокОтРеализацииОС > 0 Тогда 
			...
			БухгалтерскийУчет.УстановитьСубконто(ПроводкаУбыток.СчетДт, ПроводкаУбыток.СубконтоДт, "ОсновныеСредства", СтрокаСправки.Объект);
			БухгалтерскийУчет.УстановитьСубконто(ПроводкаУбыток.СчетКт, ПроводкаУбыток.СубконтоКт, "ПрочиеДоходыИРасходы", Справочники.ПрочиеДоходыИРасходы.ПустаяСсылка());
			...
			СрокИспользованияОбъекта = СтрокаСправки.СрокПолезногоИспользованияНУ - СтрокаСправки.ОстатокСрокаПолезногоИспользованияНУ + 1;
			
			ОстатокСрокаПолезногоИспользованияНУСУчетомКоэффицента = ?(СтрокаСправки.КоэффициентАмортизацииНУ = 0,
				СтрокаСправки.ОстатокСрокаПолезногоИспользованияНУ - 1,
				СтрокаСправки.СрокПолезногоИспользованияНУ / СтрокаСправки.КоэффициентАмортизацииНУ - СрокИспользованияОбъекта);
			...
			ОстатокСрокаПолезногоИспользованияНУСУчетомКоэффицента =
				...
				ОстатокСрокаПолезногоИспользованияНУСУчетомКоэффицента, Окр((ОстатокСрокаПолезногоИспользованияНУСУчетомКоэффицента + 0.5), 0));
				
			ПроводкаУбыток.Содержание = СтрШаблон(СодержаниеШаблон, СтрокаСправки.Объект, ОстатокСрокаПолезногоИспользованияНУСУчетомКоэффицента);
