...
Процедура ИзменитьПометкуНовостей(Знач МассивНовостей, Знач Пометка = 1) Экспорт
	...
	Если ТипЗнчМассивНовостей <> Тип("СправочникСсылка.Новости") // Единственная новость
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПометкуНовостей: параметр [МассивНовостей] недопустимого типа [%1].'"), ТипЗнчМассивНовостей);
	...
	Если (ТипЗнчПометка <> Тип("Число"))
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПометкуНовостей: параметр [Пометка] недопустимого типа [%1] или выходит за границы [0..9] (сейчас: %2).'"), ТипЗнчПометка, Пометка);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьПризнакПрочтенностиНовостей(Знач МассивНовостей, Знач ПризнакПрочтенности = Истина) Экспорт
	...
	Если ТипЗнчМассивНовостей <> Тип("СправочникСсылка.Новости") // Единственная новость
			И ТипЗнчМассивНовостей <> Тип("Массив") Тогда // Массив новостей
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакПрочтенностиНовостей: параметр [МассивНовостей] недопустимого типа [%1].'"), ТипЗнчМассивНовостей);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если (ТипЗнчПризнакПрочтенности <> Тип("Булево")) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакПрочтенностиНовостей: параметр [ПризнакПрочтенности] недопустимого типа [%1].'"), ТипЗнчПризнакПрочтенности);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьПризнакУдаленияИзСпискаНовостей(Знач МассивНовостей, Знач ПризнакУдаленияИзСписка = Истина) Экспорт
	...
	Если ТипЗнчМассивНовостей <> Тип("СправочникСсылка.Новости") // Единственная новость
			И ТипЗнчМассивНовостей <> Тип("Массив") Тогда // Массив новостей
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакУдаленияИзСпискаНовостей: параметр [МассивНовостей] недопустимого типа [%1].'"), ТипЗнчМассивНовостей);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если (ТипЗнчПризнакУдаленияИзСписка <> Тип("Булево")) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакУдаленияИзСпискаНовостей: параметр [ПризнакУдаленияИзСписка] недопустимого типа [%1].'"), ТипЗнчПризнакУдаленияИзСписка);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьПризнакПрочитатьПозже(Знач МассивНовостей, Знач ПрочитатьПозже = Истина) Экспорт
	...
	Если ТипЗнчМассивНовостей <> Тип("СправочникСсылка.Новости") // Единственная новость
			И ТипЗнчМассивНовостей <> Тип("Массив") Тогда // Массив новостей
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакПрочитатьПозже: параметр [МассивНовостей] недопустимого типа [%1].'"), ТипЗнчМассивНовостей);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если (ТипЗнчПрочитатьПозже <> Тип("Булево")) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакПрочитатьПозже: параметр [ПрочитатьПозже] недопустимого типа [%1].'"), ТипЗнчПрочитатьПозже);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьПризнакОповещенияНовостей(Знач МассивНовостей, Знач ОповещениеВключено = Ложь, Знач ДатаНачалаОповещения = '00010101') Экспорт
	...
	Если ТипЗнчМассивНовостей <> Тип("СправочникСсылка.Новости") // Единственная новость
			И ТипЗнчМассивНовостей <> Тип("Массив") Тогда // Массив новостей
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакОповещенияНовостей: параметр [МассивНовостей] недопустимого типа [%1].'"), ТипЗнчМассивНовостей);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если (ТипЗнчОповещениеВключено <> Тип("Булево")) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакОповещенияНовостей: параметр [ОповещениеВключено] недопустимого типа [%1].'"), ТипЗнчОповещениеВключено);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если (ТипЗнчДатаНачалаОповещения <> Тип("Дата")) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПризнакОповещенияНовостей: параметр [ДатаНачалаОповещения] недопустимого типа [%1].'"), ТипЗнчДатаНачалаОповещения);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Функция ПолучитьСостояниеОповещения(МассивНовостей) Экспорт
	...
	Если ТипЗнчМассивНовостей <> Тип("Массив") Тогда // Массив новостей
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ПолучитьСостояниеОповещения: параметр [МассивНовостей] недопустимого типа [%1].'"), ТипЗнчМассивНовостей);
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат Результат;
	КонецЕсли;
...
Процедура ИзменитьПользовательскуюВидимостьЛентНовостей(ЛентыНовостей, Видимость = Ложь, Пользователь = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущийПользователь) <> ТипПользователь Тогда // Внешний пользователь, например.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПользовательскуюВидимостьЛентНовостей: Передан параметр [Пользователь] неправильного типа: [%1].'"),
			ТипЗнч(Пользователь));
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПользовательскуюВидимостьЛентНовостей: Передан параметр [ЛентыНовостей] неправильного типа: [%1].'"),
...
Процедура ИзменитьВидимостьЛентНовостейДляОбластиДанных(
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьВидимостьЛентНовостейДляОбластиДанных: Передан параметр [МассивЛентНовостей] неправильного типа: [%1].'"),
...
Процедура ИзменитьПользовательскиеСпособыОповещенияЛентНовостей(ЛентыНовостей, НастройкиОповещения, Пользователь = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущийПользователь) <> ТипПользователь Тогда // Внешний пользователь, например.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПользовательскиеСпособыОповещенияЛентНовостей: Передан параметр [Пользователь] неправильного типа: [%1].'"),
			ТипЗнч(Пользователь));
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИзменитьПользовательскиеСпособыОповещенияЛентНовостей: Передан параметр [ЛентыНовостей] неправильного типа: [%1].'"),
			ТипЗнч(ЛентыНовостей));
		ЗаписатьОшибкуВалидацииПараметров(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ТипЗнч(НастройкиОповещения.СпособОповещения) = ТипСпособОповещения Тогда
				СпособОповещенияПользователяОНовостях = НастройкиОповещения.СпособОповещения;
			Иначе // Строка
				...
				ИначеЕсли НастройкиОповещения.СпособОповещения = "СистемаВзаимодействия" Тогда
					СпособОповещенияПользователяОНовостях = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия;
				Иначе
			...
			Запись = РегистрыСведений.НастройкиЛентНовостейПользовательские.СоздатьМенеджерЗаписи();
				...
				Запись.СпособОповещенияПользователяОНовостях = СпособОповещенияПользователяОНовостях;
			Запись.Записать(Истина);
...
#Область Обработка
	...
	Если НастройкиОтображенияНовостейЭталон.Использование = Истина Тогда
		Форма.Новости.Вставить("СпособОтображенияПанелиКонтекстныхНовостей", НастройкиОтображенияНовостейЭталон.СпособОтображенияПанелиКонтекстныхНовостей);
		// Кнопка / подменю.
		...
		Если (НастройкиЗаполненияНовостямиЭталон.ПолучатьНовостиНаСервере = Истина) Тогда
			Если (НастройкиОтображенияНовостейЭталон.СпособОтображенияПанелиКонтекстныхНовостей = "СписокНовостей")
				...
				СостояниеПанелиКонтекстныхНовостей = ОбработкаНовостейСлужебный.ПолучитьСостояниеПанелиКонтекстныхНовостей(ИдентификаторМетаданных, ИдентификаторФормы);
