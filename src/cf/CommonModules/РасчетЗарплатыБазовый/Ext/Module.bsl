...
Процедура ЗаполнитьОтработанноеВремя(СтрокиНачислений, МесяцНачисления, Организация, ПерваяПоловинаМесяца) Экспорт
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, СотрудникиНачислений, "ДатаПриема,ДатаЗавершенияРаботы", МесяцНачисления);
	
	ТаблицаОтборовСотрудников = КадровыеДанные.Скопировать(, "Период,Сотрудник,ДатаПриема");
	Для Каждого СтрокаТаблицы Из ТаблицаОтборовСотрудников Цикл
		Если СтрокаТаблицы.ДатаПриема > МесяцНачисления
			И СтрокаТаблицы.ДатаПриема < КонецМесяца(МесяцНачисления) Тогда
			
			СтрокаТаблицы.Период = СтрокаТаблицы.ДатаПриема;
		КонецЕсли;
	КонецЦикла;
	
	КадровыеДанныеДоляНеполногоРабочегоВремени = КадровыйУчет.КадровыеДанныеСотрудниковПоТаблицеЗначений(Истина, ТаблицаОтборовСотрудников, "ДоляНеполногоРабочегоВремени");
	...
	Для Каждого Строка Из СтрокиНачислений Цикл
		...
		Если КатегорииНачислений[Строка.Начисление] = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПовременнаяОплатаТруда Тогда
			...
			Если КадровыеДанныеДоляНеполногоРабочегоВремени.Количество() > 0 Тогда
				ДанныеДоляНеполногоРабочегоВремени = КадровыеДанныеДоляНеполногоРабочегоВремени.НайтиСтроки(Новый Структура("Сотрудник", Строка.Сотрудник));
				Если ЗначениеЗаполнено(ДанныеДоляНеполногоРабочегоВремени)
					И ЗначениеЗаполнено(ДанныеДоляНеполногоРабочегоВремени[0].ДоляНеполногоРабочегоВремени) Тогда
					
					ДоляНеполногоРабочегоВремени = ДанныеДоляНеполногоРабочегоВремени[0].ДоляНеполногоРабочегоВремени;
				КонецЕсли;
