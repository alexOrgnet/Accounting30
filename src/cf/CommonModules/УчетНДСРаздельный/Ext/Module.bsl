...
Процедура СформироватьДвиженияПоступлениеДопРасходовОтПоставщика(ТаблицаТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ДанныеДвижений = ПодготовитьДанныеДвиженийПоступлениеТоваровУслугОтПоставщика(Параметры.Товары, Услуги, Реквизиты);
	СформироватьДвиженияПоступлениеЦенностей(ДанныеДвижений, Реквизиты, Движения, Отказ);
...
Функция ПолучитьТаблицуСписанныеПартииНДС(Товары, СписанныеТоварыБухУчет, Реквизиты, Отказ) Экспорт
	...
	ДатаСписанияДопРасходовПоФИФО = ДатаСписанияДопРасходовПоФИФО();
	СписаниеДопРасходовПоФИФО = ДатаСписанияДопРасходовПоФИФО <= Реквизиты.Период;
	...
	Для каждого СтрокаДокумента Из Товары Цикл
		...
		Для каждого СписаннаяПартияБухУчет Из СписанныеТоварыПоСтрокеБухУчет Цикл
			...
			Для каждого ОстатокПартии Из ОстаткиПартийНДСПоСтроке Цикл
				...
				Если СписаниеДопРасходовПоФИФО Тогда
					СуммаДопРасходовПоАналитикеЗатрат = ИтогиПоОстаткамДопРасходов.Итог("СуммаБезНДС");
					СуммаСписанияДопРасходов = Окр(СуммаДопРасходовПоАналитикеЗатрат / ОстатокПоАналитикеУчетаЗатрат * Количество, 2);
					ИтогСуммаСписанияДопРасходов = ИтогиПоОстаткамДопРасходов.Итог("СписаннаяСуммаБезНДС");
					Если ИтогСуммаСписанияДопРасходов + СуммаСписанияДопРасходов > СуммаДопРасходовПоАналитикеЗатрат Тогда
						СуммаСписанияДопРасходов = СуммаДопРасходовПоАналитикеЗатрат - ИтогСуммаСписанияДопРасходов;
					КонецЕсли;
				...
				Для Каждого СтрокаОстаткаДопРасхода Из ОстаткиДопРасходов Цикл
					Если СписаниеДопРасходовПоФИФО Тогда 
						Если СуммаСписанияДопРасходов = 0 Тогда
							Прервать;
						КонецЕсли;
						Если (СтрокаОстаткаДопРасхода.СуммаБезНДС - СтрокаОстаткаДопРасхода.СписаннаяСуммаБезНДС) <= СуммаСписанияДопРасходов Тогда
							СписаннаяСуммаБезНДС = СтрокаОстаткаДопРасхода.СуммаБезНДС - СтрокаОстаткаДопРасхода.СписаннаяСуммаБезНДС;
							СписанныйНДС = СтрокаОстаткаДопРасхода.НДС - СтрокаОстаткаДопРасхода.СписанныйНДС;
							СуммаСписанияДопРасходов = СуммаСписанияДопРасходов - СписаннаяСуммаБезНДС;
							
						Иначе
							
							СписаннаяСуммаБезНДС = СуммаСписанияДопРасходов;
							СписанныйНДС = Окр(СуммаСписанияДопРасходов *СтрокаОстаткаДопРасхода.НДС / СтрокаОстаткаДопРасхода.СуммаБезНДС,2);
							СуммаСписанияДопРасходов = 0;
						КонецЕсли;
					Иначе
						СписаннаяСуммаБезНДС	= Окр(СтрокаОстаткаДопРасхода.СуммаБезНДС * КоэффициентСписанияДопРасходов, 2);
						СписанныйНДС			= Окр(СтрокаОстаткаДопРасхода.НДС * КоэффициентСписанияДопРасходов, 2);
...
Функция ДатаСписанияДопРасходовПоФИФО()
