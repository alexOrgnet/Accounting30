...
Процедура СформироватьДвиженияНачислениеАмортизации(ТаблицаЗатрат, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	СвойстваСчетаОбесценения = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ПланыСчетов.Хозрасчетный.ОбесценениеОС_01);
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаЗатрат Цикл
		...
		ИначеЕсли СтрокаТаблицы.СчетАмортизации = СчетРасходыНаНИОКР Тогда
		...
		//Корректировка амортизации от обесценения
		...
		Проводка.СубконтоКт.Вставить(ВидСубконтоОсновныеСредства, СтрокаТаблицы.ОбъектУчета);
...
Процедура РассчитатьПостоянныеРазницы(СтрокаСправкаРасчет, СуммаПроводки, ДанныеПостоянныхРазниц) Экспорт
	...
	ИначеЕсли ДанныеПостоянныхРазниц.ОстатокПР > 0 И Разница > 0 Тогда
		...
		КонецЕсли;
		СтрокаСправкаРасчет.СуммаАмортизацииПР = Мин(СтрокаСправкаРасчет.СуммаАмортизацииПР, Разница);
	ИначеЕсли ДанныеПостоянныхРазниц.ОстатокПР < 0 И Разница < 0 Тогда
		Если ДанныеПостоянныхРазниц.ОстатокНУ > 0 Тогда
		...
		Иначе
			СтрокаСправкаРасчет.СуммаАмортизацииПР = ДанныеПостоянныхРазниц.ОстатокПР;
		КонецЕсли;
		СтрокаСправкаРасчет.СуммаАмортизацииПР = Макс(СтрокаСправкаРасчет.СуммаАмортизацииПР, Разница);
	Иначе
		СтрокаСправкаРасчет.СуммаАмортизацииПР = 0;
	КонецЕсли;
...
//  ПоказателиОтчета - Структура - структура, в которую добавляются показатели отчета.
//
Процедура ПолучитьСведенияОПоказателяхОтчетаИмуществоФормаОтчета2019_2022(ПоказателиОтчета, ИДРедакцииОтчета, ПараметрыОтчета) Экспорт
	...
	Если ИДРедакцииОтчета = "ФормаОтчета2019Кв4" 
		 ...
		 Или ИДРедакцииОтчета = "ФормаОтчета2022Кв4" Тогда
	...
	Если ИДРедакцииОтчета <> "ФормаОтчета2022Кв4" Или ЭтоИностраннаяОрганизация Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2022Кв4" Тогда
...
Процедура ЗаполнитьЗаявлениеОЛьготеПоТранспортномуЗемельномуНалогу(ПараметрыОтчета, Контейнер) Экспорт
	...
	КатегорииТСПоКодуВида = РегистрыСведений.РегистрацияТранспортныхСредств.КатегорииТСПоКодуВида();
...
Процедура ЗаполнитьСообщениеОНаличииТранспортаИЗемли(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументСверкаСФНС) Тогда
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РасчетТранспортногоНалога.ОсновноеСредство КАК ОсновноеСредство,
		|	ЕСТЬNULL(РасчетТранспортногоНалога.КодВидаТранспортногоСредства, """") КАК КодВидаТС,
		|	ЕСТЬNULL(РасчетТранспортногоНалога.ИдентификационныйНомер, """") КАК ИдНомТС,
		|	ЕСТЬNULL(РасчетТранспортногоНалога.Марка, """") КАК МаркаТС,
		|	ЕСТЬNULL(РасчетТранспортногоНалога.РегистрационныйЗнак, """") КАК РегЗнакТС,
		|	ЕСТЬNULL(РасчетТранспортногоНалога.ДатаРегистрации, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаРегТС,
		|	ЕСТЬNULL(РасчетТранспортногоНалога.ДатаПрекращенияРегистрации, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаСнУчетТС
		|ИЗ
		|	ДанныеСверки КАК ДанныеСверки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РасчетТранспортногоНалога КАК РасчетТранспортногоНалога
		|		ПО ДанныеСверки.ОсновноеСредство = РасчетТранспортногоНалога.ОсновноеСредство
		|ГДЕ
		|	РасчетТранспортногоНалога.Организация В(&Организации)
		|	И РасчетТранспортногоНалога.ПериодРасчета = &ПериодРасчета
		|
	...
	Иначе
		...
		Запрос.Текст = ТекстЗапросаВыборкиТранспортныхСредствДляСообщенияВФНС();
	...
	Иначе
		...
		Запрос.Текст = ТекстЗапросаВыборкиЗемельныхУчастковДляСообщенияВФНС();
...
Функция ТекстЗапросаВыборкиТранспортныхСредствДляСообщенияВФНС(ПоОдномуОбъекту = Ложь) Экспорт
	
	// Текст запроса может быть ниже модифицирован через СхемаЗапроса.
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РасчетТранспортногоНалога.ОсновноеСредство КАК ОсновноеСредство,
	|	РасчетТранспортногоНалога.КодВидаТранспортногоСредства КАК КодВидаТС,
	...
	|	РасчетТранспортногоНалога.ДатаПрекращенияРегистрации КАК ДатаСнУчетТС
	|ИЗ
	|	РегистрСведений.РасчетТранспортногоНалога КАК РасчетТранспортногоНалога
	|ГДЕ
	|	РасчетТранспортногоНалога.Организация В (&Организации)
	...
	Если ПоОдномуОбъекту Тогда
		СхемаЗапроса = Новый СхемаЗапроса;
		СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
		// Отбор по указанному ТС
		СхемаЗапроса.ПакетЗапросов[0].Операторы[0].Отбор.Добавить("РасчетТранспортногоНалога.ОсновноеСредство = &ОсновноеСредство");
		// ПЕРВЫЕ 1
...
Функция ТекстЗапросаВыборкиЗемельныхУчастковДляСообщенияВФНС(ПоОдномуОбъекту = Ложь) Экспорт

	// Текст запроса может быть ниже модифицирован через СхемаЗапроса.
	
	ТекстЗапроса = ТекстЗапросаВременнаяТаблицаРасчетЗемельногоНалога() +
		ОбщегоНазначения.РазделительПакетаЗапросов() +
		ТекстЗапросаВременнаяТаблицаПостановкиИСнятияСУчетаЗемельныхУчастков() +
		ОбщегоНазначения.РазделительПакетаЗапросов() +
	    ТекстИтоговогоЗапросаВыборкиЗемельныхУчастковДляСообщенияВФНС();
		
	Если ПоОдномуОбъекту Тогда
		// Для одного ОС добавляем условие в запрос первой временной таблицы РасчетНалога. Этого достаточно, т.к. последующие 
		// таблицы отбираются по ней. 
		// Неоптимальность запроса для одного ОС (индексирование, избыточные поля и соединения) считаем допустимой.
		СхемаЗапроса = Новый СхемаЗапроса;
		СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
		// Отбор по указанному земельному участку в первом запросе
		СхемаЗапроса.ПакетЗапросов[0].Операторы[0].Отбор.Добавить("РасчетЗемельногоНалога.ОсновноеСредство = &ОсновноеСредство");
		// ПЕРВЫЕ 1 в последнем запросе
		КоличествоЗапросов = СхемаЗапроса.ПакетЗапросов.Количество(); 
		СхемаЗапроса.ПакетЗапросов[КоличествоЗапросов - 1].Операторы[0].КоличествоПолучаемыхЗаписей = 1;
		ТекстЗапроса = СхемаЗапроса.ПолучитьТекстЗапроса();
	КонецЕсли;
...
Процедура ЗаполнитьУведомлениеОГибелиТС(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументСверкаСФНС) Тогда
		...
		|	ЕСТЬNULL(ДанныеРегистрацииПогибшихТС.КодВидаТранспортногоСредства, """") КАК КодВидаТС,
...
Функция ТекстЗапросаВременнаяТаблицаРасчетЗемельногоНалога() Экспорт

	// Текст запроса может быть по месту модифицирован через СхемаЗапроса.
...
Функция ТекстЗапросаВременнаяТаблицаПостановкиИСнятияСУчетаЗемельныхУчастков() Экспорт

	// Текст запроса может быть по месту модифицирован через СхемаЗапроса.
	
	// Основную часть данных о земельных участках берем из расчета налога.
	// Но там отсутствуют данные о датах постановки и снятия земельных участков с учета.
	// Для получения этих данных используем такой алгоритм:
	// 1. Получаем все даты регистрации земельных участков. По конкретному участку их может быть несколько из-за того, что один 
	//    и тот же участок в программе (основное средство) может неоднократно ставиться и сниматься с учета. При этом 
	//    фильтруем данные только по тем филиалам, где ОС хотя бы 1 месяц состояло на учете как зем участок (т.е. попало в расчет налога).
	//    Также учитываем регистрации с разными кадастровыми номерами как уникальные записи, потому что именно 
	//    кадастровый номер идентифицирует объект налогообложения.
	//    См. врем таблицу ПостановкиНаУчет.
	// 2. Аналогично выбираем все события снятия с учета.
	//    См. врем таблицу СнятияСУчета.
	// 3. К каждой постановке на учет находим ближайшее после нее снятие с учета.
	//    См. врем таблицу БлижайшиеСнятияСУчета.
	// 4. Для каждого снятия с учета ищем самую раннюю предшествующую ему регистрацию. Это позволяет найти первую 
	//    фактическую регистрацию участка, если введено несколько регистраций подряд без снятия с учета.
	//    В итоге получаем периоды регистрации земельных участков (непересекающиеся пары Постановка - Снятие).
	//    См. врем таблицу ПостановкиИСнятияСУчета.
	// 5. В конце выбираем самый последний период владения. Считаем, что более ранние периоды владения не имеют значения с точки зрения
	...
	|ГДЕ
	|	РегистрацияЗемельныхУчастков.Период <= &ПериодРасчета
	|	И РегистрацияЗемельныхУчастков.ВключатьВНалоговуюБазу
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПостановкиНаУчет.Организация КАК Организация,
	|	ПостановкиНаУчет.ОсновноеСредство КАК ОсновноеСредство,
	|	ПостановкиНаУчет.КадастровыйНомер КАК КадастровыйНомер,
	|	ПостановкиНаУчет.Период КАК ПостановкаНаУчет,
	|	МИНИМУМ(ЕСТЬNULL(СнятияСУчета.Период, ДАТАВРЕМЯ(1, 1, 1))) КАК СнятиеСУчета
	|ПОМЕСТИТЬ БлижайшиеСнятияСУчета
	|ИЗ
	|	ПостановкиНаУчет КАК ПостановкиНаУчет
	|		ЛЕВОЕ СОЕДИНЕНИЕ СнятияСУчета КАК СнятияСУчета
	|		ПО ПостановкиНаУчет.Организация = СнятияСУчета.Организация
	|		И ПостановкиНаУчет.ОсновноеСредство = СнятияСУчета.ОсновноеСредство
	|		И ПостановкиНаУчет.КадастровыйНомер = СнятияСУчета.КадастровыйНомер
	|		И ПостановкиНаУчет.Период < СнятияСУчета.Период
	|СГРУППИРОВАТЬ ПО
	|	ПостановкиНаУчет.Организация,
	|	ПостановкиНаУчет.ОсновноеСредство,
	|	ПостановкиНаУчет.КадастровыйНомер,
	|	ПостановкиНаУчет.Период
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	БлижайшиеСнятияСУчета.Организация КАК Организация,
	|	БлижайшиеСнятияСУчета.ОсновноеСредство КАК ОсновноеСредство,
	|	БлижайшиеСнятияСУчета.КадастровыйНомер КАК КадастровыйНомер,
	|	МИНИМУМ(БлижайшиеСнятияСУчета.ПостановкаНаУчет) КАК ПостановкаНаУчет,
	|	БлижайшиеСнятияСУчета.СнятиеСУчета КАК СнятиеСУчета
	|ПОМЕСТИТЬ ВсеПостановкиИСнятияСУчета
	|ИЗ
	|	БлижайшиеСнятияСУчета КАК БлижайшиеСнятияСУчета
	|СГРУППИРОВАТЬ ПО
	|	БлижайшиеСнятияСУчета.Организация,
	|	БлижайшиеСнятияСУчета.ОсновноеСредство,
	|	БлижайшиеСнятияСУчета.СнятиеСУчета,
	|	БлижайшиеСнятияСУчета.КадастровыйНомер
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВсеПостановкиИСнятияСУчета.Организация КАК Организация,
	|	ВсеПостановкиИСнятияСУчета.ОсновноеСредство КАК ОсновноеСредство,
	|	ВсеПостановкиИСнятияСУчета.КадастровыйНомер КАК КадастровыйНомер,
	|	МАКСИМУМ(ВсеПостановкиИСнятияСУчета.ПостановкаНаУчет) КАК ПостановкаНаУчет
	|ПОМЕСТИТЬ АктуальныйПериодВладения
	|ИЗ
	|	ВсеПостановкиИСнятияСУчета КАК ВсеПостановкиИСнятияСУчета
	|СГРУППИРОВАТЬ ПО
	|	ВсеПостановкиИСнятияСУчета.Организация,
	|	ВсеПостановкиИСнятияСУчета.ОсновноеСредство,
	|	ВсеПостановкиИСнятияСУчета.КадастровыйНомер
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ОсновноеСредство,
	...
	|	ВсеПостановкиИСнятияСУчета.ОсновноеСредство КАК ОсновноеСредство,
	|	ВсеПостановкиИСнятияСУчета.КадастровыйНомер КАК КадастровыйНомер,
	|	ВсеПостановкиИСнятияСУчета.ПостановкаНаУчет КАК ПостановкаНаУчет,
	|	ВсеПостановкиИСнятияСУчета.СнятиеСУчета КАК СнятиеСУчета
	|ПОМЕСТИТЬ ПостановкиИСнятияСУчета
	|ИЗ
	|	ВсеПостановкиИСнятияСУчета КАК ВсеПостановкиИСнятияСУчета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ АктуальныйПериодВладения КАК АктуальныйПериодВладения
	|		ПО ВсеПостановкиИСнятияСУчета.Организация = АктуальныйПериодВладения.Организация
	|		И ВсеПостановкиИСнятияСУчета.ОсновноеСредство = АктуальныйПериодВладения.ОсновноеСредство
	|		И ВсеПостановкиИСнятияСУчета.КадастровыйНомер = АктуальныйПериодВладения.КадастровыйНомер
	|		И ВсеПостановкиИСнятияСУчета.ПостановкаНаУчет = АктуальныйПериодВладения.ПостановкаНаУчет
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ОсновноеСредство,
...
Функция ТекстИтоговогоЗапросаВыборкиЗемельныхУчастковДляСообщенияВФНС() Экспорт

	// Текст запроса может быть ниже модифицирован через СхемаЗапроса.
	// Итоговый запрос формирует выборку регистраций по каждому уникальному кадастровому номеру.	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РасчетНалога.КадастровыйНомер КАК НомКадастрЗУ,
	|	РасчетНалога.Наименование КАК Комментарий,
	|	ЕСТЬNULL(ПостановкиИСнятияСУчета.ПостановкаНаУчет, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаРегПрав,
	|	ЕСТЬNULL(ПостановкиИСнятияСУчета.СнятиеСУчета, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПрекрПрав
	|ИЗ
	|	РасчетНалога КАК РасчетНалога
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПостановкиИСнятияСУчета КАК ПостановкиИСнятияСУчета
	|		ПО РасчетНалога.Организация = ПостановкиИСнятияСУчета.Организация
	|		И РасчетНалога.ОсновноеСредство = ПостановкиИСнятияСУчета.ОсновноеСредство
	|		И РасчетНалога.КадастровыйНомер = ПостановкиИСнятияСУчета.КадастровыйНомер
	|УПОРЯДОЧИТЬ ПО
	|	НомКадастрЗУ,
	|	ДатаРегПрав";
