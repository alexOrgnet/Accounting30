...
Функция ДанныеЗаполненияЗаказаНаЭмиссиюКодовМаркировкиСУЗ(Организация) Экспорт
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ДанныеЗаполнения.ВидПродукции) Тогда
...
Функция ДанныеЗаполненияОтгрузкиТоваровИСМП(Организация, Контрагент = Неопределено) Экспорт
	...
	ДанныеЗаполнения.Вставить("GLNКонтрагента",   ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("GLN"));
...
Процедура ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(ОбъектЗаполнения, МассивСтрок = Неопределено, Знач ДополнительныеПоляПоиска = "") Экспорт
	...
	Если ДополнительныеПоляПоиска.Найти("Шаблон") = Неопределено Тогда
		ОбрабатыватьГрупповыеУпаковки = ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(Объект.ВидПродукции);
		...
		ОбрабатыватьНаборы = ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетНаборы(Объект.ВидПродукции);
	...
	Иначе
		ЗаполнятьТоварныйЗнак = ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(Объект.ВидПродукции);
	КонецЕсли;
