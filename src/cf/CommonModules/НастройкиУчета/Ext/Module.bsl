...
Процедура ЗаписатьУчетнуюПолитикуОбособленногоПодразделения(Организация) Экспорт
	...
	СкопироватьЗаписиУчетнойПолитикиДляОбособленногоПодразделения("НастройкиВариантаРасчетаКорректировокВВалюте", Организация);
КонецПроцедуры
...
Процедура СинхронизацияНастроекУчета(ТекущаяЗапись, ИмяРегистра)
	...
	ИначеЕсли УчетнаяПолитика.ПрименяетсяУСН(Организация, Период) Тогда
		...
		НастройкиНалогаПредыдущийПериод = РегистрыСведений.НастройкиСистемыНалогообложения.СрезПоследних(ПредыдущийПериод, ОтборНастроек);
		Если ЗначениеЗаполнено(НастройкиНалогаПредыдущийПериод) Тогда
			...
			НапоминатьОВведенииОстатков = ((НастройкиСистемыНалогообложения.ПрименяетсяУСН
				...
				И (НастройкиСистемыНалогообложения.ПрименяетсяУСНДоходыМинусРасходы = НастройкиНалогаПредыдущийПериод.ПрименяетсяУСНДоходы));
	...
	Если ЭтоЮрЛицо ИЛИ УчетЗарплаты.ИПИспользуетТрудНаемныхРаботников(Организация) Тогда
		Если Константы.УчетЗарплатыИКадровСредствамиБухгалтерии.Получить() Тогда
			...
			Если НастройкиНалога.Количество() = 0
				Или УчетнаяПолитика.ПрименяетсяАУСН(Организация, Период) Тогда
...
Процедура СоздатьНастройкиУчетнойПолитикиПоУмолчанию(ДанныеЗаполнения)
	...
	НастройкиВариантаРасчетаКорректировокВВалюте = РегистрыСведений.НастройкиВариантаРасчетаКорректировокВВалюте.СоздатьНаборЗаписей();
	...
	ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НастройкиВариантаРасчетаКорректировокВВалюте);
...
Процедура СоздатьПрименяемыеТарифыСтраховыхВзносов(НаборЗаписей)
	...
	ВидТарифаАУСН                                 = Справочники.ВидыТарифовСтраховыхВзносов.АУСН;
	...
	Для каждого Строка Из НаборЗаписей Цикл
		...
		Если Выборка.Количество() > 0 Тогда
			...
			ИначеЕсли Строка.СистемаНалогообложения = Перечисления.СистемыНалогообложения.АУСН
				И Выборка.ВидТарифа = ВидТарифаАУСН Тогда
	...
	Для каждого Строка Из НаборЗаписей Цикл
		...
		Если ЭтоЮрЛицо ИЛИ УчетЗарплаты.ИПИспользуетТрудНаемныхРаботников(Строка.Организация) Тогда
			Если Константы.УчетЗарплатыИКадровСредствамиБухгалтерии.Получить() Тогда
				...
				НастройкиНалога = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СрезПоследних(ПериодНастройки, Новый Структура("Организация", Строка.Организация));
				Если НастройкиНалога.Количество() = 0 Тогда
					НастройкиНалога = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СрезПервых(ПериодНастройки, Новый Структура("Организация", Строка.Организация));
					Если НастройкиНалога.Количество() = 0 Тогда
						...
						Запись.Записать(Истина);
					Иначе
						СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев", Строка.Организация, НастройкиНалога[0].Период, ПериодНастройки);
					КонецЕсли;
				КонецЕсли;
				...
				Если Строка.ПрименяетсяАУСН Тогда
					...
					ДанныеЗаполненияСтраховыеВзносы = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.ЗаписьПоУмолчанию();
					ДанныеЗаполненияСтраховыеВзносы.Период = ПериодНастройки;
					ДанныеЗаполненияСтраховыеВзносы.Ставка = УчетЗарплаты.НулеваяСтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев();
...
Функция ПорядокУплатыАвансов()
	...
	УсловиеНалогНаПрибыльАвансыЕжеквартально = УсловияПримененияТребованийЗаконодательства.УсловиеНалогНаПрибыльАвансыЕжеквартально();
	УсловиеНалогНаПрибыльАвансыЕжемесячно = УсловияПримененияТребованийЗаконодательства.УсловиеНалогНаПрибыльАвансыЕжемесячно();
	УсловиеНалогНаПрибыльАвансыПоФактическойПрибыли = УсловияПримененияТребованийЗаконодательства.УсловиеНалогНаПрибыльАвансыПоФактическойПрибыли();
	Условия = Новый Массив;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Условия", Условия);
	Запрос.УстановитьПараметр("НалогНаПрибыльАвансыПоФактическойПрибыли", УсловиеНалогНаПрибыльАвансыПоФактическойПрибыли);
	Запрос.УстановитьПараметр("НалогНаПрибыльАвансыЕжемесячно", УсловиеНалогНаПрибыльАвансыЕжемесячно);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВариантыПримененияТребованийЗаконодательства.Организация КАК Организация,
	|	ВЫБОР
...
Процедура ДобавитьНастройкуБухгалтерскогоУчетаДляПБУ18(РСНастройкиБухгалтерскогоУчета)
	...
	Для Каждого Организация Из Организации Цикл
		...
		НастройкаНаСледующийГод.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы;
