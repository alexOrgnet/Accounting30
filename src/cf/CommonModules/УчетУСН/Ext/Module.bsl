...
Функция ПодготовитьТаблицуНДСПоРеализацииНеплательщиком(ТаблицаВзаиморасчетов, ТаблицаРеквизитов) Экспорт
	...
	|		ИНАЧЕ ВТ_СуммыДокументов.СуммаДокумента * ВТ_СуммыДокументов.КурсДокумента / ВТ_СуммыДокументов.КратностьДокумента -
	|			ВТ_НачисленныйНДС.НДС
	...
	|	ВТ_СуммыДокументов.СуммаДокумента КАК ВалютнаяСуммаРеализации
	|ИЗ
...
Процедура ЗаполнитьДекларацию2021ОбъектДоходы(ПараметрыЗаполнения, ДанныеКУДиР, СтавкиНалога, ЗаполняемыйРаздел, КлючиРаздела)
	...
	Для Каждого СтрокаНалоговойБазы Из ДанныеКУДиР.Раздел1 Цикл
		...
		РасходыУменьшающиеНалог = СуммаРасходаУменьшающегоНалог(ДанныеКУДиР.ВзносыПодлежащиеУплате, ОтборКвартал);
...
Функция ДанныеКУДиРЗаПериодПоКварталам(Организация, НачалоПериода, КонецПериода, Раздел1 = Истина, Раздел4 = Ложь, Раздел5 = Ложь)
	...
	ДанныеКУДиР = Новый Структура("Раздел1, Раздел4, Раздел5, ВзносыПодлежащиеУплате");
	...
	Если УчетСтраховыхВзносовИППовтИсп.УпрощенныйЗачетФиксированныхВзносов(Организация, КонецПериода) Тогда
		...
		ДанныеКУДиР.ВзносыПодлежащиеУплате = Запрос.Выполнить().Выгрузить();
	КонецЕсли;
...
Процедура РассчитатьНалогУСНОбъектДоходы(КонтекстРасчета, ТаблицаПроводок, ТаблицаСправкиРасчета, Ошибки)
	...
	РасходыУменьшающиеНалог = РасходыУменьшающиеНалог + ВзносыПодлежащиеУплате;
...
Функция СоздатьНачисленныйНДСПоРеализации(МенеджерВременныхТаблиц, Период, Организация, ДокументыРеализации)
	...
	|	СУММА(РублевыеСуммыДокументовВВалюте.НалоговаяБазаНДС) КАК СуммаБезНДС,
	...
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПродаж.Обороты(, &КонецПериода,, Организация = &Организация
	...
	|	ВТ_НачисленныйНДСВВалюте.СуммаБезНДС,
	|	ВТ_НачисленныйНДСВВалюте.НДС,
	...
	|	ВТ_НачисленныйНДСВРублях.СуммаБезНДС,
	|	ВТ_НачисленныйНДСВРублях.НДС,
	|	ЛОЖЬ
	|ИЗ
...
Функция ВзносыИППодлежащиеУплате(Организация, НачалоПериода, КонецПериода)
	...
	ВзносСДоходов = УчетСтраховыхВзносовИП.ВзносыСДоходовПодлежащиеУплате(Организация, КонецПериода).Всего;
	...
	ВзносыПодлежащиеУплате = Макс(ИспользованныеВзносы.УСН_Доход, ВзносыПодлежащиеУплате);
	
	// Полученная сумма взноса не должна быть больше, чем в предыдущем квартале и неиспользованный остаток взносов
	ВзносыПодлежащиеУплате = Мин(ИспользованныеВзносы.УСН_Доход + НеиспользованныйОстаток, ВзносыПодлежащиеУплате);
	...
	РазрядностьОкругленияПоказателей = УчетУСНКлиентСервер.РазрядностьОкругленияПоказателей(КонецПериода);
	
	Возврат Окр(ВзносыПодлежащиеУплате - ЗачтеноПоЗаявлениямЗаТекущийГод, РазрядностьОкругленияПоказателей);
