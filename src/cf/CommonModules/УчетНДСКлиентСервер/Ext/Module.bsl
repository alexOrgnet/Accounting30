...
Процедура УстановитьНастройкиУчетаНДСВСтрокеБилетаПоУмолчанию(ДанныеСтроки) Экспорт
	...
	ДанныеСтроки.ПредъявленСФ = Истина;
	
	// Поле СпособУчетаНДС также может заполняться механизмом заполнения счетов учета
	Если Не ЗначениеЗаполнено(ДанныеСтроки.СпособУчетаНДС) Тогда
...
// ОбщаяСтавкаНДС - Общая ставка НДС, актуальная на дату (НДС 18% или НДС 20%)
...
// Определяется расчетная ставка НДС для подстановки в Счета-фактуры
// Параметры:
//  Период               - Дата - дата- на которую определяется общая расчетная ставка НДС
// Возвращаемое значение:
//   Ставка НДС    - ПеречислениеСсылка.СтавкиНДС - могут быть различные варианты:
//   - Заполнена "Основная ставка НДС" в пользовательских настройках
//        - тогда возвращается соответствующая расчетная ставка НДС, ставке из пользовательских настроек
//   - Не заполнена "Основная ставка НДС" в пользовательских настройках
//        - тогда возвращается общая расчетная ставка НДС - актуальная, на переданную дату.
Функция РасчетнаяСтавкаНДСПользователя(Период) Экспорт
	
	СтавкаНДСПользователя = УчетНДСВызовСервера.СтавкаНДСПользователя();
	
	Возврат ?(ЗначениеЗаполнено(СтавкаНДСПользователя),
		УчетНДСВызовСервера.РасчетнаяСтавкаНДСПоОбычной(СтавкаНДСПользователя),
		ОбщаяРасчетнаяСтавкаНДС(Период));
