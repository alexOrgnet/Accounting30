...
Процедура СформироватьДвиженияУСН(ДокументОбъект, СтруктураДопПараметров = Неопределено, ЭтоОбъект = Истина, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
		...
		Если НЕ СтруктураШапкиДокумента.УдержатьВознаграждение Тогда
			...
			ОтразитьВУСН(ДокументСсылка, ВидОперацииСписания, СтруктураШапкиДокумента, 2, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		Иначе
			...
			Если ВыделятьНДСУСН И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
				...
				ОтразитьВУСН(ДокументСсылка, ВидОперацииСписания, СтруктураШапкиДокумента,2,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
			Иначе
				ОтразитьВУСН(ДокументСсылка, ВидОперацииСписания, СтруктураШапкиДокумента,1,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
			КонецЕсли;
		КонецЕсли;
		
		ОтразитьВУСН(ДокументСсылка, "ЗачетВыручкиКомиссионера", СтруктураШапкиДокумента,3,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
	...
	ИначеЕсли ВидОперации = "ЗачетВознагражденияКомиссионера" Тогда
		...
		Если ОтражатьВознаграждениеПоПатенту Тогда
			...
			Если ЗначениеЗаполнено(ПатентДокумента) Тогда
				...
				ЗаписьКУДиРПатент.Графа4 = СуммаДоходов;
			КонецЕсли;
		Иначе
			...
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
				...
				Если Не ВыделятьНДСУСН Тогда
					КУДиР.НДС = ДокументСсылка.Товары.Итог("СуммаНДСВознаграждения")
						+ ДокументСсылка.Услуги.Итог("СуммаНДСВознаграждения");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ДвиженияРегистров <> Неопределено Тогда
			...
			Если (ДвиженияРегистров.КУДиР.Количество() > 0)
				...
				ИначеЕсли ВидОперации = "Поступление" И СтруктураШапкиДокумента.ВидДокумента = "ПоступлениеТоваровУслуг" Тогда
					...
					КонецЦикла; 
					ДеревоРасходов = ТаблицуЗначенийВТаблицуСГруппировкой(ТаблицаРасходов, "ВидРасхода, ОплатаИзВыручкиКомиссионера", "Сумма, НДС", ВидОперации);
				Иначе
...
Функция ВидОперацииДДС(СтруктураШапкиДокумента) Экспорт
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым)
		Или (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатаСамозанятому) Тогда
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		...
		Если КонтролироватьСумму Тогда
			...
			Если Расход.Сумма = 0 Тогда
				Продолжить;
			КонецЕсли;
			Если Расход.ВыделятьНДСУСНДокумент И
		...
		Если (СтрокаКорректировкиРасход.Валюта <> ВалютаРеглУчета)
			...
			И ЗначениеЗаполнено(Расход.СтруктураКурса)
			И (ПризнаватьРасход Или ВидОперации = "Оплата") Тогда
		...
		Если (ВидОперации = "Оплата") И
			(СтрокаКорректировкиПриход.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеСписаноПринято)
		...
		Если Не ТаблицаРасходов.Колонки.Найти("СтатусСписания") = Неопределено Тогда
			...
			ИначеЕсли Расход.Количество > Расход.КоличествоПоСтатусуСписания Тогда
				...
				ПроверитьСторноРасходовПОСписанию(СтруктураШапкиДокумента, НомерСтрокиРасходов, Расход.ВыделятьНДСУСНДокумент,
		...
		Если (НомерСтрокиРасходов > 0) И (СтрокаКорректировкиПриход.НДС > 0)
			И (Не ВыделятьНДСУСН) И Расход.ВыделятьНДСУСНДокумент И (Не ВидОперации = "Оплата") Тогда
