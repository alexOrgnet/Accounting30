...
Процедура НайтиИУстановитьСертификатСТокена(
	...
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("Форма", Форма);
	ДополнительныеПараметры.Вставить("ВсеСертификаты", Форма.ВсеСертификаты);
	ДополнительныеПараметры.Вставить("ОповещениеОЗавершении", ВыполняемоеОповещения);
	...
	КриптографияЭДКОКлиент.ПолучитьКонтейнерыКлючей(ОписаниеОповещения, Истина, ВыводитьСообщения);
...
Процедура НайтиСертификатыНаТокене_ПослеПолученияКонтейнеров(Результат, ВходящийКонтекст) Экспорт
	...
	Для каждого ИмяФайлаКонтейнера Из Результат.КонтейнерыКлючей Цикл
		Если СтрНачинаетсяС(Врег(ИмяФайлаКонтейнера), "SCARD") Тогда
	...
	Если Контейнеры.Количество() = 0 Тогда
		ДлительнаяОтправкаКлиент.ЗакрытьПростуюФормуОжидания(ВходящийКонтекст.Форма);
		СообщитьОбОтсутствииПодходящихСертификатов(ВходящийКонтекст.Форма);
		Возврат;
	КонецЕсли;
...
Процедура НайтиСертификатыНаТокене_ПослеПолученияBase64Сертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Попытка
		Результат = ОбработкаЗаявленийАбонентаВызовСервера.ЗаполнитьСвойстваСертификатаСТокена(РеквизитыСтруктурой, Контейнер);
...
Процедура ПредложитьВыборСертификатаСТокена_ПослеВыбора(Контейнер, ВходящийКонтекст) Экспорт
	...
	ВходящийКонтекст.Удалить("Номер");
