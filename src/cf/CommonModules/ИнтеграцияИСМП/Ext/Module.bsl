...
Процедура ЗаблокироватьСписокПлощадок(МассивПлощадок, ВремяБлокировки = 0) Экспорт
	...
	Если ВремяБлокировки = 0 Тогда
		ВремяБлокировки = ИнтеграцияИСМПКлиентСерверПовтИсп.ВремяБлокировкиCDNПлощадки();
	КонецЕсли;
...
Функция ТребуетсяОбновлениеСпискаCDNПлощадок() Экспорт
	...
	Если ИнтеграцияИСМПВызовСервера.СписокУчитываемойПродукцииТребующейОбязательнойПроверкиПриПродажеВРозницуНаДату().Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ВремяОбновленияПлощадок",  ИнтеграцияИСМПКлиентСерверПовтИсп.ЧастотаОбновленияCDNПлощадок() * 3600);
	Запрос.УстановитьПараметр("ТекущаяУниверсальнаяДата", ТекущаяУниверсальнаяДата());
...
Функция ДанныеПокдлюченияКегаКОборудованиюРозлива() Экспорт
	...
	ДанныеПодключения.Вставить("Организация",             ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("Организация"));
	ДанныеПодключения.Вставить("Подразделение",           ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("Подразделение"));
	ДанныеПодключения.Вставить("Склад",                   ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("Склад"));
...
Функция ПодключитьКегКОборудованиюРозлива(ДанныеПодключения) Экспорт
	...
	РезультатПодключения = РегистрыСведений.КегиНаОборудованииРозливаИСМП.ПодключитьКегКОборудованиюРозлива(
...
Функция СостояниеОбмена(Организации) Экспорт
	...
	Запрос.УстановитьПараметр("ПустаяОрганизация",                ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("Организация"));
...
Функция ТаблицаОрганизаций(Знач Организации)
	...
	Если Организации.Количество() = 0 Тогда
		Организации = ИнтеграцияИС.ДоступныеОрганизации();
	КонецЕсли;
