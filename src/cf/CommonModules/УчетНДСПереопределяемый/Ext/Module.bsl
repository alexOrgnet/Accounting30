...
Функция ВерсияПостановленияНДС1137(Период) Экспорт
	...
	Если Период >= '20231001' Тогда      // ФЗ № 389 вступает в силу с 1 октября 2023 года.
		Возврат 6;
...
Функция ПараметрыСчетаФактуры(СчетФактура) Экспорт
	...
	Результат = Новый Структура(
		...
		|ИдентификаторГосКонтракта,ЕстьПрослеживаемыеТовары");
	...
	Иначе
		Для каждого ТипОснования Из ТипыОснований Цикл
			...
			ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
				Если ЭтоПолученныйСФ Тогда
					ТекстЗапроса = ТекстЗапросаВозвратПоставщикуПолученныйСФ();
				Иначе
...
Процедура НачислитьЕНППриВвозеТоваровИзЕАЭС(Реквизиты, СтрокаТаблицыНачислений, Движения) Экспорт
	
	Проводка = Движения.Хозрасчетный.Добавить(); 
	
	ЕНПНеплательщикаНДС  = НЕ УчетнаяПолитика.ПлательщикНДС(Реквизиты.Организация, Реквизиты.Период);
	Проводка.Организация = Реквизиты.Организация;
	Проводка.Содержание  = НСтр("ru = 'Единый налоговый счет: Начисление налога за товары из Таможенного союза'");
	Проводка.СчетДт      = ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате; // 68.42
	Проводка.СчетКт      = ПланыСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет;      // 68.90
	Если ЕНПНеплательщикаНДС Тогда
		Проводка.Сумма = СтрокаТаблицыНачислений.СуммаБУ;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Договоры", СтрокаТаблицыНачислений.КорСубконто2);
	Иначе
		Проводка.Сумма = СтрокаТаблицыНачислений.НДС;
	...
	БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "СФПолученные", Реквизиты.Регистратор);
...
//
Функция ВедетсяПростойУчетЕНП(Период) Экспорт
	
	Возврат Период >= ЕдиныйНалоговыйСчет.НачалоПростогоУчета();
...
Функция ТекстЗапросаВозвратПоставщикуПолученныйСФ()
