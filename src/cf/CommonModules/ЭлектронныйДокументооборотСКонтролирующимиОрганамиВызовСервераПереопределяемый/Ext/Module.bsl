...
Процедура ПолучитьИзображенияПрисоединенныхФайловИсточников(ФайлыИсточников, ИдентификаторФормыВладельца) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтеграцияС1СДокументооборотом") И ФайлыИсточников.Количество() > 0 Тогда
		...
		Если Настройки.ИспользоватьИнтеграциюДО2 Или Настройки.ИспользоватьИнтеграциюДО3 Тогда
			...
			Для Каждого ФайлыИсточника Из ФайлыИсточников Цикл
				
				ИспользоватьДОДляХраненияФайловОбъекта =
					МодульИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ИспользоватьПрисоединенныеФайлы1СДокументооборотаДляОбъекта(
						ФайлыИсточника.Ключ);
				...
				СвязанныйОбъектДО =
					МодульИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ДанныеОбъектаДОПоВнешнемуОбъекту(
						ФайлыИсточника.Ключ);
				Если СвязанныйОбъектДО = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ОбъектXDTO = МодульИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(
					Прокси,
				...
				Для Каждого ФайлXDTO Из ОбъектXDTO.files Цикл
					Если ДопустимыеРасширения.Найти(ВРег(ФайлXDTO.extension)) <> Неопределено Тогда
				...
				НужныеФайлыXDTO = МодульИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъекты(
					Прокси,
					НужныеФайлы,
					"name,extension,size,binaryData");
				
				Для Каждого ФайлXDTO Из НужныеФайлыXDTO Цикл
					ОписаниеФайла = Новый Структура("Имя, Размер, АдресДанных");
					ОписаниеФайла.Имя = СтрШаблон("%1.%2", ФайлXDTO.name, ФайлXDTO.extension);
					ОписаниеФайла.Размер = ФайлXDTO.size;
					ОписаниеФайла.АдресДанных = ПоместитьВоВременноеХранилище(ФайлXDTO.binaryData, ИдентификаторФормыВладельца);
					
					ФайлыИсточника.Значение.Добавить(ОписаниеФайла);
				КонецЦикла;
