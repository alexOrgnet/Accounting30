...
Функция СоздатьЗаданиеНаВыгрузку(Организация, Операция, ВидПродукции, ПараметрыЗапроса) Экспорт
	...
	ТелоЗапроса.Вставить("productGroupCode", ИнтерфейсИСМПСлужебный.ТоварнаяГруппаЧислом(ВидПродукции));
	...
	РезультатОтправкиЗапроса = ИнтерфейсМОТПСлужебный.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON(РезультатЗапроса);//, Ложь);
...
Функция ПривестиТипЗначения(ИмяКолонки, ДанныеКолонки, ВидПродукции, КешТипов = Неопределено)
	...
	Если ИмяКолонки = "СтатусКодаМаркировки" Тогда
		Результат = ИнтерфейсИСМПСлужебный.СтатусКодаМаркировки(ДанныеКолонки);
	...
	ИначеЕсли ИмяКолонки = "ВидУпаковки" Тогда
		Результат= ИнтерфейсИСМПСлужебный.ВидУпаковки(ДанныеКолонки, ВидПродукции);
		КешТипов.Вставить(ДанныеКолонки, Результат);
