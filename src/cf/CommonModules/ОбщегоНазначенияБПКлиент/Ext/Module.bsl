...
//  УстанавливатьСтатусСФ - Булево - признак того, что нужно установить переданный статус и для связанных счетов-фактур.
//
Процедура УстановитьСтатусыВыделенныхДокументов(Форма, СтатусДокумента, УстанавливатьСтатусСФ = Ложь) Экспорт
	...
	Если Не СтатусИзменен 
		И Не УстанавливатьСтатусСФ Тогда
		Возврат;
	Иначе
		Оповестить("ИзмененСтатусДокументов", СписокДокументов.ВыгрузитьЗначения()); // оповещение открытых форм документов
	КонецЕсли;
	
	Если УстанавливатьСтатусСФ Тогда
		УстановитьСтатусыОригиналаСчетаФактуры(Форма, СтатусДокумента, Ложь, ВыделенныеДокументы);
	КонецЕсли;
...
//
Процедура УстановитьСтатусыОригиналаСчетаФактуры(Форма, СтатусДокумента, ЭтоСписокСчетовФактур = Ложь, ВыделенныеДокументы = Неопределено) Экспорт
