...
Процедура ЗагрузитьРеестрСчетовФактурПоставщика(ПараметрыРеестра, АдресХранилища) Экспорт
	...
	ДокументОбъект.НалоговыйПериод = НачалоКвартала(РеквизитыРеестра.НалоговыйПериод);
...
Процедура СформироватьРеестрСчетовФактур(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	СведенияОбОрганизации = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(ПараметрыОтчета.Организация, , , , , Истина);
...
Процедура ЗаполнитьОбщиеРеквизитыРеестра(Результат, ОбъектXDTO, Письмо)
	...
	Результат.НалоговыйПериод                 = НачалоКвартала(Результат.НалоговыйПериод);
...
Процедура ДополнитьТаблицуРеестров(ТаблицаРеестров, ОписаниеОшибок, Письма)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РеестрСчетовФактурПоставщика.Организация КАК Организация,
	|	РеестрСчетовФактурПоставщика.Контрагент КАК Контрагент,
	|	НАЧАЛОПЕРИОДА(РеестрСчетовФактурПоставщика.НалоговыйПериод, КВАРТАЛ) КАК НалоговыйПериод,
	|	РеестрСчетовФактурПоставщика.Дата КАК Дата
	|ИЗ
	|	Документ.РеестрСчетовФактурПоставщика КАК РеестрСчетовФактурПоставщика
	|ГДЕ
	...
	Для Каждого Реестр Из ТаблицаРеестров Цикл
		...
		ОтборРеестров.Вставить("НалоговыйПериод", НачалоКвартала(Реестр.НалоговыйПериод));
