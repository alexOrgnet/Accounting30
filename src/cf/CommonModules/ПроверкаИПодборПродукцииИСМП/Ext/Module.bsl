...
Процедура ЗаполнитьПризнакАвтоматическийОСУИСВСтроке(СтрокаТовары) Экспорт
	...
	Если Не ИнтеграцияИСМПКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(СтрокаТовары.ВидПродукцииИС) Тогда 
		Возврат;
	КонецЕсли;
	...
	ШтрихкодированиеИС.ЗаполнитьПроверяемыеGTIN(Таблица, МассивGTIN, СоответствиеGTIN, Ложь);
...
Процедура ЗаполнитьПризнакАвтоматическийОСУИСВТаблице(ТаблицаТовары, ЗаполнитьКолонкуGTIN = Ложь) Экспорт
	...
	Для Каждого СтрокаТЧ Из ТаблицаТовары Цикл
		Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(СтрокаТЧ.ВидПродукцииИС) Тогда
	...
	Таблица.Колонки.Добавить("ПодходитGTIN", ОбщегоНазначения.ОписаниеТипаЧисло(1));
	Таблица.Колонки.Добавить("GTIN", ОбщегоНазначения.ОписаниеТипаСтрока(14));
...
Процедура ПриОчисткеСопоставленияМаркированнойПродукции(Форма, ИдентификаторыДереваМаркированнойПродукции, КэшированныеЗначения) Экспорт
	...
	Для Каждого СтрокаДерева Из МассивСтрокДерева Цикл
		...
		ЭтоШтрихкодВводаОстатков = ШтрихкодированиеИСКлиентСервер.ЭтоШтрихкодВводаОстатков(СтрокаДерева.GTIN);
...
Функция ПрерватьОбработкуЗагрузкиДанных(ДанныеДокумента, ПараметрыСканирования, ДанныеНоменклатурыПоДаннымУПД, РезультатПолученияДанных, ВосстановлениеДетализацииТребуетОбращенияКСервису) Экспорт
	...
	Если ТребуетсяПрерватьОбработкуЗагрузкиДанных Тогда
		...
		Если ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ДанныеДокумента.ВидМаркируемойПродукции) Тогда
			ПараметрыЗапросаКлючаСессии = ИнтерфейсМОТПКлиентСервер.ПараметрыЗапросаКлючаСессии(ДанныеДокумента.Организация);
		Иначе
			ПараметрыЗапросаКлючаСессии = ИнтерфейсИСМПКлиентСервер.ПараметрыЗапросаКлючаСессии(ДанныеДокумента.Организация);
		КонецЕсли;
...
Процедура ОбработкаСтрокиЧастичногоВыбытияПриВосстановленииДанных(СтрокаПриемника, СтрокаИсточника, ПараметрыСканирования, ВидПродукции) Экспорт
	...
	Если ШтрихкодированиеИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
...
Процедура ВывестиИнформациюОПроблемахУПДПоТовару(СтрокиНоменклатураШтрихкод, ВидПродукции, ШаблонСообщения, НаименованиеСправочника, УникальныйИдентификаторФормы, КоличествоВыводимыхШтрихкодов = 3)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатур Цикл
		...
		ИнтеграцияИС.СообщитьПользователюВФорму(УникальныйИдентификаторФормы, ТекстСообщения);
...
Процедура ДополнитьТаблицуПодобраннойМаркируемойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ВидПродукции, ПараметрыСканирования) Экспорт
	...
	ДопустимаПроизвольнаяЕдиницаУчета = ИнтеграцияИС.ЕстьПроизвольныеЕдиницыУчета(ВидПродукции)
		Или ИнтеграцияИСКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(ВидПродукции, ПараметрыСканирования.ВидОперацииИСМП);
	
	ПоддерживаетсяОбъемноСортовойУчет = ИнтеграцияИСМПКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВидПродукции)
	...
	Контекст.Вставить("ДопустимУчетМерныхТоваров",         ИнтеграцияИС.ЕстьПродукцияТребующаяВзвешивания());
...
Функция СформироватьТаблицуИзмененийПодобраннойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ПараметрыСканирования, ВидПродукции)
	...
	ПустаяХарактеристикаНоменклатуры = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("ХарактеристикаНоменклатуры");
	ПустаяСерияНоменклатуры          = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("СерияНоменклатуры");
...
Функция КолонкиДляТаблицыИзмененийПодобраннойПродукции(ВидПродукции) Экспорт
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукции)
		Или ИнтеграцияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(ВидПродукции) Тогда
