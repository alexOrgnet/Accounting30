...
Функция ЗагрузитьВходящиеДокументыУниверсально(ПараметрыЗагрузки, ПараметрыОбмена) Экспорт
	...
	ВидыПродукцииУчета = ИнтеграцияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции(, Ложь);
	...
	Для Каждого ОрганизацияДляОбмена Из Организации Цикл
		...
		Для Каждого ВидПродукции Из ВидыПродукцииУчета Цикл
			...
			Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукции)
...
Процедура ВыполнитьАктуализациюCDNПлощадокДлительнаяОперация(ПараметрыФоновогоЗадания, АдресРезультата) Экспорт
	...
	Иначе
		ТребуетсяОбновлениеCDNПлощадок = ИнтеграцияИСМП.ТребуетсяОбновлениеСпискаCDNПлощадок();
	КонецЕсли;
	...
	РезультатОбновления = ИнтерфейсИСМП.АктуализацияСпискаCDNПлощадок(ПараметрыФоновогоЗадания.Организация);
...
Функция ОбязательныеПоляОписанияОстатковПоСтрокеДокумента(СтрокаТовары, ВидПродукции) Экспорт
	...
	ИначеЕсли ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции) Тогда
	...
	Если ПараметрыОписанияОстатков.ИспользоватьТоварныйЗнак Тогда
		...
		Если Не ЗначениеЗаполнено(ТоварныйЗнак)
			И (ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции)
...
Функция СообщениеКПередачеJSON(ДокументСсылка, ДальнейшееДействие, ДополнительныеПараметры = Неопределено)
	...
	Если ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеИдентификаторСоединения Тогда
		...
		ТелоСообщенияJSON = ИнтерфейсМОТПСлужебный.ОбъектВТекстJSON(
...
Функция ПодготовитьКПередачеСообщениеАгрегацииКодовМаркировкиИСМП(СтрокаДереваВерхнегоУровня, ОбщийЭлементОчереди, Знач ЗагружатьДо, СообщенияJSON,
	...
	Если СтрокаДереваВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор Тогда
		...
		Если ДанныеУровняАгрегации.КоличествоКодовНаборов + ТекущееКоличествоКодов >= ПараметрыАгрегации.МаксимальноеКоличествоКодовВДокументе
			...
			СообщениеJSON.ЗагружатьДо        = ЗагружатьДо;
			СообщениеJSON.Версия             = ОбщийЭлементОчереди.РеквизитыИсходящегоСообщения.Версия;
			СообщениеJSON.ТекстСообщенияJSON = ИнтерфейсМОТПСлужебный.ОбъектВТекстJSON(ДанныеУровняАгрегации.ТелоЗапросаСоздание, Истина);
			СообщениеJSON.ПараметрыЗапроса   = ПараметрыЗапроса;
...
Процедура ОбработкаЗагрузкиПереданногоДокумента(ЭлементОчереди, ПараметрыОбмена, РезультатОбработкиЗаявки, ПараметрыОбновленияСтатуса)
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса
		...
		Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена Тогда
			...
			Если УдалитьПолныеКодовМаркировкиИзПула Тогда
				ЭтоМолочнаяПродукция = ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ЭлементОчереди.ВидПродукции);
	...
	ИначеЕсли (ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.СписаниеЭмитированныхКодовМаркировки
			И ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.ИСМП
...
Процедура ЗагрузитьРезультатОбработкиАбстрактнойОперацииРасчетСтатуса(ЭлементОчереди, ПараметрыОбмена)
	...
	Попытка
		...
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			...
			ПротоколОбмена = ИнтеграцияИСМПСлужебный.ИнициализироватьТаблицуПротоколОбмена();
			ОтветТело = СтрШаблон(
...
Функция СтрокаДляПоискаGTINМаркировкиОстатков(ВидПродукции, СтрокаТовары)
	...
	Если Не ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции) Тогда
...
Функция ЗагрузитьДокументы(ЭлементОчереди, ПараметрыОбмена, ПараметрыЗагрузки)
	...
	Для Каждого ИдентификаторДокумента Из ДокументыДляЗагрузки Цикл
		...
		Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ЭлементОчереди.ВидПродукции) Тогда
			ПараметрыОбработки = ИнтерфейсИСМП.НовыеПараметрыОбработкиСоставаДокумента();
			...
			Для Каждого ДанныеДокумента Из СтруктураОбработкиДокумента.ДанныеДокумента Цикл
				...
				Для Каждого СтрокаКодаМаркировки Из СоставДокумента Цикл
					...
					ИначеЕсли ЭтоОбъемноСортовойУчет Тогда
						Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ДокументОбъект.ВидПродукции) Тогда
...
Функция ВходящиеДокументы(ЭлементОчереди, ПараметрыЗагрузки)
	...
	СведенияОбОрганизации = ИнтеграцияИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(ЭлементОчереди.Организация);
...
Процедура ДополнитьТабличныеЧастиПоДаннымИСМП(ДокументПриемка, ДанныеШтрихкодов, РезультатЗапроса)
	...
	ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(ДокументПриемка, Неопределено, ВидПродукции);
	...
	Если ЭтоПриемкаПродукцииИзМеха Тогда
		...
		МаркированныеТоварыИСМП = РезультатПолученияДанных.ВложенныеШтрихкоды.МаркированныеТовары;
		МаркированныеТоварыИСМП.Индексы.Добавить("Штрихкод");
		...
		Для Каждого СтрокаДанных Из ДанныеШтрихкодов Цикл
			...
			Если Не ЗначениеЗаполнено(GTIN) Тогда
				ПараметрыОтбора.Штрихкод = СтрокаДанных.Штрихкод;
				НайденныеСтроки = МаркированныеТоварыИСМП.НайтиСтроки(ПараметрыОтбора);
				Если НайденныеСтроки.Количество() Тогда
					GTIN = НайденныеСтроки[0].GTIN;
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработкаДокументаОбъемноСортовогоУчета(ДокументПриемка, ДанныеШтрихкодов)
	...
	ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(ДокументПриемка, Неопределено, ВидПродукцииДокумента);

	Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукцииДокумента) Тогда
	...
	Для Каждого СтрокаШтрихкода Из ДанныеШтрихкодов Цикл
		ШтрихкодыEAN.Добавить(ШтрихкодированиеИСКлиентСервер.ШтрихкодEANИзGTIN(СтрокаШтрихкода.Штрихкод));
		GTINПотребительскихУпаковок.Вставить(СтрокаШтрихкода.Штрихкод, Истина);
	КонецЦикла;
	ДанныеПоШтрихкодамEAN = ШтрихкодированиеИС.ДанныеПоШтрихкодамEAN(
	...
	Для Каждого СтрокаДанных Из ДанныеШтрихкодов Цикл
		...
		СтруктураПоиска = Новый Структура();
		СтруктураПоиска.Вставить("ШтрихкодEAN", ШтрихкодированиеИСКлиентСервер.ШтрихкодEANИзGTIN(СтрокаДанных.Штрихкод));
		...
		ИначеЕсли ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукцииДокумента)
...
// Параметры:
//  ЭлементОчереди - СтрокаТаблицыЗначений - (См. ИнтеграцияИСМП.ИнициализиоватьОчередьСообщений()).
//  СлужебныйПротоколОбмена - См. ИнициализироватьТаблицуПротоколОбмена
//  СтатусОбработки - ПеречислениеСсылка.СтатусыОбработкиСообщенийИСМП - Статус обработки сообщения.
//  Операция - ПеречислениеСсылка.ВидыОперацийИСМП -  операция обмена.
// 
// Возвращаемое значение:
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если Не КодыАгрегированы Тогда // При вводе в оборот в разрезе кодов или АТК.
		Для Каждого СтрокаДерева Из ВложенныеШтрихкоды.ДеревоУпаковок.Строки Цикл
			Результат = НайтиСформированныеАгрегатыПоДаннымИСМП(СтрокаДерева);
			Если Результат.ЕстьОшибки Тогда
				ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(СообщениеJSON, Результат.ТекстОшибки);
				СообщенияJSON.Добавить(СообщениеJSON);
				ЕстьОшибки = Истина;
				Прервать;
			КонецЕсли;
	...
	Если Не ЕстьОшибки Тогда
		...
		ЭтоМолочнаяПродукцияИСМП = ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ДанныеШапки.ВидПродукции);
	...
	Если ДанныеШапки.ОтчетПроизводственнойЛинии Тогда
		...
		Для Каждого СтрокаДанных Из РезультатОбработкиДанныхОтчета.ОбработанныеДанныеОтчета Цикл
			...
			Для Каждого ИменаКолонок Из РезультатОбработкиДанныхОтчета.ГруппыКолонок Цикл
				...
				СтруктураЗначения = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкодаПоДаннымРазбора(
...
Процедура ПодготовитьСообщенияПоАбстрактнойАгрегации(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если Не ДанныеШапки.ОтчетПроизводственнойЛинии Тогда
		...
		Если ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ЭлементОчереди.ВидПродукции) Тогда
	...
	Для Каждого КлючИЗначение Из СообщенияПоОрганизациям Цикл
		...
		Если СертификатыПользователяДляПодписиПоОрганизации = Неопределено Тогда
			ДоступныеСертификаты.Вставить(
				Организация,
				ИнтеграцияИС.СертификатыПользователяДляПодписиПоОрганизации(Организация, Истина));
		КонецЕсли;
...
Процедура ЗаписатьЛогЗапросовВоВременноеХранилищеВозвращаемогоЗначения(ПараметрыОбмена, Приемник) Экспорт
	...
	Если ИнтеграцияИСКлиентСерверПовтИсп.ЭтоФоновоеЗаданиеНаСервере() Тогда
		ПараметрыЛогирования = ЛогированиеЗапросовИСМП.ПараметрыЛогированияЗапросов();
...
Функция ДеревоУпаковокПоДаннымОтчетаПроизводственнойЛинии(ДанныеШапки, ИсточникДанных, КешДанныхРазбора, ПараметрыЗаполнения = Неопределено,
	...
	Иначе
		...
		КонецЦикла;
		ИнформацияПоШтрихкодамEAN = ШтрихкодированиеИС.ДанныеПоШтрихкодамEAN(ДанныеEAN, ПараметрыСканирования, ПараметрыЗаполнения.GTINПотребительскихУпаковок);
	КонецЕсли;

	ЭтоПродукцияМОТП = ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ДанныеШапки.ВидПродукции);
...
Процедура ПодготовитьДанныеКОпределениюВидаУпаковки(КодМаркировки, GTIN, ДополнительныеПараметры) Экспорт
	...
	Если КОпределениюВидаУпаковки = Неопределено Тогда
		...
		ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(ФормаПроверки);
