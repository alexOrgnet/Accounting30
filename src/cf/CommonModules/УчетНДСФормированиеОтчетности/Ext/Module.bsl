...
#Область ПодготовкаОбщихДанных
	...
	ОбщиеПараметрыЗапросов.Вставить("КодОперацииПоУмолчанию",
		УчетНДСВызовСервера.КодОперацииПоСтавке0ПоУмолчанию(ПараметрыОтчета.мДатаКонцаПериодаОтчета,
	...
	ИначеЕсли ПараметрыОтчета.ПравилаЗаполнения = "2023Кв1" Тогда
...
#Область Раздел3
	...
	Если Контейнер.Свойство("Раздел3")
		...
		Если Раздел3Начисления.Следующий() Тогда
			...
			Если ПараметрыОтчета.ПравилаЗаполнения = "2023Кв1"
			 ИЛИ ПараметрыОтчета.ПравилаЗаполнения = "2021Кв3"
			 ИЛИ ПараметрыОтчета.ПравилаЗаполнения = "2020Кв4"
...
Функция ТекстЗапросаРаздел3Начисления()
	...
	|						ТОГДА НДСЗаписиКнигиПродажОбороты.НДСОборот
	|					ИНАЧЕ 0
	|				КОНЕЦ КАК ЧИСЛО(15, 2))), 0) КАК П000300010005
...
Процедура ЗаполнитьРеестрДляПодтвержденияСтавки0(Контейнер, ПараметрыОтчета)
	...
	УчетНДСПереопределяемый.ТекстЗапросаРеестрНДСПриложение(Запрос,ПараметрыОтчета,ЭтоПриложение5);
