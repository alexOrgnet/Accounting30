...
Процедура ПоказатьРаспознанныйДокумент(
	...
	Если Не ПустаяСтрока(ИмяФормыОбработчика) Тогда
		...
		Если КоличествоСтрокТаблицы > КоличествоСтрокПриКоторойВозможнаДолгаяЗагрузка Тогда
...
Функция КонтекстСозданияРаспознанногоДокумента(Форма, ПараметрыОперации)
	...
	ЭтоСчетНаОплату = (ПараметрыОперации.ТипДокумента = "СчетНаОплатуПокупателю") Или (ПараметрыОперации.ТипДокумента = "СчетНаОплатуПоставщика");
