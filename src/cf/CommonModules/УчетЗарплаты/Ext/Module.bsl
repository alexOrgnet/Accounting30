		...
		Если ЗначениеЗаполнено(ТаблицаНалоговВзносовСФОТ) Тогда
			...
			Для Каждого СчетУчетаСтраховыхВзносов Из СчетаУчетаСтраховыхВзносов Цикл
				...
				Для Каждого СтрокаМассива Из МассивСтрок Цикл
					...
					Проводка = Движения.Хозрасчетный.Добавить();
					Проводка.Период      = СрокУплаты;
					Проводка.Организация = Реквизиты.Организация;
					Проводка.Сумма       = СтрокаМассива.Сумма;
					
					Проводка.СчетКт = СчетУчетаЕНС;
					
					Проводка.Содержание  = НСтр("ru = 'Учет страховых взносов в составе совокупной налоговой обязанности'", ОбщегоНазначения.КодОсновногоЯзыка());
					
					Проводка.СчетДт = СтрокаМассива.КорСчет;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СтрокаМассива.КорСубконто1);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СтрокаМассива.КорСубконто2);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СтрокаМассива.КорСубконто3);
...
Процедура СформироватьПроводкиПоОтражениюРезервовВУчете(ТаблицаРезервов, Реквизиты, Движения, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРезервов Цикл
		...
		Если СтрокаТаблицы.Сумма > 0 
			...
			Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Списание Тогда
				Проводка.СчетКт = СтрокаТаблицы.Счет;
...
Процедура СформироватьДвиженияПоНалогамИВзносамСФОТ(ТаблицаРеквизиты, ТаблицыУплатаНалогов, Движения, Отказ) Экспорт
	...
	Если ТаблицыУплатаНалогов.Свойство("ТаблицаУплатыПоЕдиномуНалоговомуСчету")
		...
		Если СчетаУчета.Найти(ПланыСчетов.Хозрасчетный.НДФЛ) <> Неопределено
			Или СчетаУчета.Найти(ПланыСчетов.Хозрасчетный.НДФЛ_ДоходыСвышеПредельнойВеличины) <> Неопределено Тогда
...
Процедура СформироватьДвиженияНачислениеВзносовОтНесчастныхСлучаев(ПараметрыПроведения,	Движения) Экспорт
	...
	Параметры = ПодготовитьПараметрыНачислениеВзносовОтНесчастыхСлучаев(ПараметрыПроведения.ТаблицаВзносыОтНесчастныхСлучаев);
...
Функция ПодготовитьПараметрыНачислениеВзносовОтНесчастыхСлучаев(ТаблицаРеквизиты)
		...
		Если ЗначениеЗаполнено(ТаблицаОтраженияВУчете) Тогда
			...
			Для Каждого СчетУчетаСтраховыхВзносов Из СчетаУчетаСтраховыхВзносов Цикл
				...
				Для Каждого СтрокаМассива Из МассивСтрок Цикл
					...
					Проводка.СчетДт = СтрокаМассива.СчетКт;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СтрокаМассива.СубконтоКт1);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СтрокаМассива.СубконтоКт2);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СтрокаМассива.СубконтоКт3);
...
Функция ПараметрыУчетаРезервовВОрганизации(Организация, Период) Экспорт
	...
	ФормироватьРезервБУ = НастройкиДействующихРезервов.Количество() <> 0;
...
Функция РолиПросмотраДанных() Экспорт
	...
	Роли.Добавить("ЧтениеДокументКадровогоЭДО");
	...
	Роли.Добавить("ЧтениеСогласийНаПрисоединениеККЭДО");
...
Функция РолиРедактированияДанных() Экспорт
	...
	Роли.Добавить("ДобавлениеИзменениеДокументКадровогоЭДО");
	...
	Роли.Добавить("ДобавлениеИзменениеСогласийНаПрисоединениеККЭДО");
...
Функция РолиНастройки() Экспорт
	...
	Роли.Добавить("ИспользованиеОбработкиУничтожениеПерсональныхДанных");
...
Функция ИПИспользуетТрудНаемныхРаботниковЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Возврат Не Результат[0].Пустой() Или Не Результат[1].Пустой();
...
Процедура ЗаполнитьНомераДоговоровРаботыУслуги() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ШаблонСообщения = НСтр("ru = 'Не удалось обработать договор подряда
			|%1'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
...
Функция ИспользуетсяТрудИностранцевСОсобымиВзносами() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользуетсяТрудИностранцевСОсобымиВзносами");
...
Процедура УстановкаНастроекПоУмолчаниюПрименяемыеТарифыСтраховыхВзносов(Запись, ДанныеЗаполнения)
	...
	Если УчетнаяПолитика.ПрименяетсяАУСН(Запись.ГоловнаяОрганизация, Запись.Период) Тогда
		ВидТарифа = Справочники.ВидыТарифовСтраховыхВзносов.АУСН;
	ИначеЕсли УчетнаяПолитика.ПрименяетсяУСН(Запись.ГоловнаяОрганизация, Запись.Период) Тогда
...
Функция ТарифВзносовСоответствуетУчетнойПолитике(ГоловнаяОрганизация, Период) Экспорт
	...
	Если НастройкиНалога.Количество() <> 0 Тогда
		ВидТарифа = НастройкиНалога[0].ВидТарифа;
		Если УчетнаяПолитика.ПрименяетсяАУСН(ГоловнаяОрганизация, Период)
			И ВидТарифа = Справочники.ВидыТарифовСтраховыхВзносов.АУСН Тогда
...
Функция ИсчисленныйНДФЛЗаПериод(Организация, Период, ПлательщикЕНП, Аванс) Экспорт
	
	Если Аванс Тогда
		ТаблицаОтраженияВУчете = ДанныеДляОтражениеВУчетеНДФЛАванса(Организация, Период);
	Иначе
...
Функция ДанныеДляОтражениеВУчетеНДФЛАванса(Организация, Период) Экспорт
...
Функция УдержанныйНДФЛЗаПериод(Организация, Период, ВтораяПоловинаМесяца = Ложь) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровСредствамиБухгалтерии") Тогда
		...
		Для Каждого СтрокаТаблицы Из УдержаноНалогаЗаПериод Цикл
			...
			Если ПустаяСтрока(РеквизитыРегистрации.КодПоОКТМО) Тогда
				РеквизитыРегистрации.КодПоОКТМО = КодТерритории;
			КонецЕсли;
			Если ПустаяСтрока(РеквизитыРегистрации.КПП) Тогда
				РеквизитыРегистрации.КПП = КПП;
			КонецЕсли;
			...
			Если СтрокаТаблицы.Сумма <> 0 Тогда
				...
				НоваяСтрока.ОКАТО                       = РеквизитыРегистрации.КодПоОКТМО;
				НоваяСтрока.Сумма                       = СтрокаТаблицы.Сумма;
...
Функция УдержанныйНДФЛПоДокументу(Организация, ДокументСсылка) Экспорт
	...
	|	СведенияОбУдержанномНДФЛНДФЛ.ВидНалога КАК ВидНалога
	|ИЗ
	|	Документ.СведенияОбУдержанномНДФЛ КАК СведенияОбУдержанномНДФЛ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СведенияОбУдержанномНДФЛ.НДФЛ КАК СведенияОбУдержанномНДФЛНДФЛ
	|		ПО СведенияОбУдержанномНДФЛ.Ссылка = СведенияОбУдержанномНДФЛНДФЛ.Ссылка
	...
	|	СведенияОбУдержанномНДФЛНДФЛ.ВидНалога";
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		...
		Если СтрокаТаблицы.Сумма <> 0 Тогда
			НоваяСтрока = ТаблицаНДФЛ.Добавить();
			НоваяСтрока.Организация = Организация;
			НоваяСтрока.ВидНалога = СтрокаТаблицы.ВидНалога;
			Если ЗначениеЗаполнено(НоваяСтрока.ВидНалога) Тогда
				НоваяСтрока.Налог     = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(НоваяСтрока.ВидНалога);
			Иначе
				НоваяСтрока.Налог     = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоКБК(СтрокаТаблицы.КодБК);
				НоваяСтрока.ВидНалога = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрока.Налог, "ВидНалога");
			КонецЕсли;
...
Процедура ПроверитьВозможностьЗаписиДокумента(Источник, ТекстИсключения, Отказ) Экспорт
	...
	ИначеЕсли Не РасчетЗарплатыДляНебольшихОрганизаций.РасчетЗарплатыДляНебольшихОрганизацийВозможен() Тогда
		...
		ДопустимоеКоличествоСотрудников = РасчетЗарплатыДляНебольшихОрганизаций.ПорогЗапрета();
		Отказ = Истина;
...
Функция ТекстПредупрежденияОбОграниченияхРежимаРасчетаЗарплатыДляНебольшихОрганизаций(Организация, ПредыдущееКоличествоСотрудников) Экспорт
	...
	ПорогЗапрета = РасчетЗарплатыДляНебольшихОрганизаций.ПорогЗапрета();
...
Функция РезультатПроверкиУсловийТарификацииПоКоличествуСотрудников(Организация) Экспорт
	...
	КоличествоСотрудников = РасчетЗарплатыДляНебольшихОрганизаций.КоличествоСотрудниковОрганизации(Организация);
...
Функция СписокСчетовАгентскогоНДФЛ() Экспорт
	...
	СписокСчетовАгентскогоНДФЛ.Добавить(ПланыСчетов.Хозрасчетный.НДФЛ_ДоходыСвышеПредельнойВеличины);
...
Функция ЕстьВзносыЗаРаботников(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	СчетаВзносовЗаРаботников = Новый Массив;
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ФСС_НСиПЗ);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ФСС);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ПФР_страх);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ПФР_нак);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ПФР_доп);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ПФР_доп_шахтеры);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ПФР_доп_ВредныеУсловияТруда);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ПФР_доп_ТяжелыеУсловияТруда);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ПФР_ОПС);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ФФОМС);
	СчетаВзносовЗаРаботников.Добавить(ПланыСчетов.Хозрасчетный.ТФОМС);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(РасходыУменьшающиеНалогПоОтдельнымРежимамОбороты.СуммаРасходаОборот, 0) > 0 КАК ЕстьРаботникиУСН,
		|	ЕСТЬNULL(РасходыУменьшающиеНалогПоОтдельнымРежимамОбороты.СуммаРасходаПСНОборот, 0) > 0 КАК ЕстьРаботникиПСН
		|ИЗ
		...
		|			,
		|			Организация = &Организация
		|				И СчетУчета В (&СчетаВзносовЗаРаботников)) КАК РасходыУменьшающиеНалогПоОтдельнымРежимамОбороты";
	
	Запрос.УстановитьПараметр("КонецПериода", КонецПериода);
	Запрос.УстановитьПараметр("НачалоПериода", НачалоПериода);
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("СчетаВзносовЗаРаботников", БухгалтерскийУчет.СформироватьМассивСубсчетов(СчетаВзносовЗаРаботников));
...
Функция СпособОтраженияНачисленияНеВключаемыеВРасходы()
...
Функция УплаченныйНДФЛЗаПериод(Организация, НалоговыйПериод, ДатаСведений, ИсключаемыйДокументУплаты = Неопределено)
	...
	СчетаНачисленияНалога = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.НДФЛ_НалоговогоАгента); // 68.01
...
Функция ДанныеРасчетаНДФЛАУСН(ТаблицаПеречислениеЗарплаты, ОплатаПоДоговорамПодряда, ТаблицаРеквизитов) Экспорт
	...
	ТаблицаРасчетыНДФЛ.Свернуть("Ведомость,ФизическоеЛицо,НалоговыйПериод,КодДохода,КодВычета,КодНеоблагаемогоВзносамиДохода", 
		"СуммаДохода,СуммаВычета,СуммаНДФЛ,СуммаНДФЛУдержаный,СуммаНДФЛПеречисленный,СуммаНеоблагаемаяВзносами");
...
Функция НоваяТаблицаРасчетыНДФЛАУСН()
	...
	ТаблицаРасчетыНДФЛ.Колонки.Добавить("ФизическоеЛицо",  
		Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
	ТаблицаРасчетыНДФЛ.Колонки.Добавить("НалоговыйПериод", 
		Новый ОписаниеТипов("Дата",,,,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	ТаблицаРасчетыНДФЛ.Колонки.Добавить("КодДохода", 
		Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(4)));
	ТаблицаРасчетыНДФЛ.Колонки.Добавить("КодВычета", 
		Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(3)));
	ТаблицаРасчетыНДФЛ.Колонки.Добавить("КодНеоблагаемогоВзносамиДохода", 
...
Функция УдержанныйНДФЛПоДокументуВыплаты(Движения)
	...
	Для Каждого СтрокаТаблицы Из УдержаноНалогаЗаПериод Цикл
		...
		Если СтрокаТаблицы.СуммаСПревышения <> 0 Тогда
			НоваяСтрока = ТаблицаУдержанногоНДФЛ.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Сумма      = СтрокаТаблицы.СуммаСПревышения;
			НоваяСтрока.СрокУплаты = ГрафикПереноса[СтрокаТаблицы.КрайнийСрокУплаты];
			НоваяСтрока.СчетУчета  = СчетУчетаНДФЛСвышеПредельнойВеличины;
		КонецЕсли;
	КонецЦикла;
...
Процедура СформироватьДвиженияПоУдержанномуНДФЛПриВыплатеДохода(Движения, Параметры, Отказ) Экспорт
	...
	СформироватьПроводкиПоУдержанномуНДФЛ(Движения, Реквизиты, ТаблицаУдержанныйНДФЛ, Отказ)
...
Процедура СформироватьДвиженияПоУдержанномуНДФЛ(ТаблицаРеквизиты, ТаблицаУдержанныйНДФЛ, Движения, Отказ) Экспорт
	...
	Если Реквизиты.Период < НачалоПростогоУчета Или Реквизиты.Месяц < НачалоПростогоУчета Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьПроводкиПоУдержанномуНДФЛ(Движения, Реквизиты, ТаблицаУдержанныйНДФЛ, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаУдержанныйНДФЛ Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СчетУчета)
			...
			Проводка = Движения.Хозрасчетный.Добавить();
			Проводка.Период      = СтрокаТаблицы.СрокУплаты;
			Проводка.Организация = Реквизиты.Организация;
			Проводка.Сумма       = СтрокаТаблицы.Сумма;
			
			Проводка.СчетКт = СчетУчетаЕНС;
			
			Проводка.Содержание  = НСтр("ru = 'Учет удержанного НДФЛ в составе совокупной налоговой обязанности'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			НомерСубконто = ЕдиныйНалоговыйСчет.НомерВидаСубконтоНаСчете(СвойстваСчета, "РегистрацияВНалоговомОргане", "СправочникСсылка.РегистрацииВНалоговомОргане");
			...
			КонецЕсли;
			НомерСубконто = ЕдиныйНалоговыйСчет.НомерВидаСубконтоНаСчете(СвойстваСчета, "ВидыПлатежейВГосБюджет", "ПеречислениеСсылка.ВидыПлатежейВГосБюджет");
			Если НомерСубконто <> 0 Тогда
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, НомерСубконто, Перечисления.ВидыПлатежейВГосБюджет.Налог);
			КонецЕсли;
