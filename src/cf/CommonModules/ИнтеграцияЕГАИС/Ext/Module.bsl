...
Процедура ПроверитьЗаполнениеШтрихкодовУпаковок(Объект, Отказ) Экспорт
	...
	ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(Объект, Неопределено, Перечисления.ВидыПродукцииИС.Пиво); 
...
Процедура ЗаполнитьВXDTOОрганизациюСЮрАдресом_v2(ОбъектXDTOРодитель, ИмяПоля, Источник, Префикс = "", СообщениеXML) Экспорт
	...
	ОрганизацияXDTO.address_ur = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(ОрганизацияXDTO, "address_ur");
	
	ЗаполнитьСвойствоXDTO(ОрганизацияXDTO.address_ur, "Country",     Формат(Реквизиты.КодСтраны, "ЧЦ=3; ЧН=; ЧВН="), СообщениеXML);
	ЗаполнитьСвойствоXDTO(ОрганизацияXDTO.address_ur, "RegionCode",  Формат(Реквизиты.КодРегиона, "ЧЦ=2; ЧВН="),     СообщениеXML);
	ЗаполнитьСвойствоXDTO(ОрганизацияXDTO.address_ur, "description", Реквизиты.ПредставлениеАдреса,                  СообщениеXML);
...
Процедура ЗаполнитьВXDTOАкцизныеМаркиПоСправке2_v4(ОбъектXDTO, Источник, ИсточникАкцизныеМарки, СообщениеXML) Экспорт
	...
	Если КодыАкцизныхМарок.Количество() > 0 Тогда
		...
		ОбъектXDTO.MarkInfo = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO, "MarkInfo");
		Для Каждого КодАкцизнойМарки Из КодыАкцизныхМарок Цикл
			ЗаполнитьСвойствоXDTO(ОбъектXDTO.MarkInfo, "amc", КодАкцизнойМарки, СообщениеXML);
		КонецЦикла;
...
Процедура ПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ПодключитьОбработчикВыполненияОбменаНаКлиентеПоРасписанию",
		ИнтеграцияЕГАИСВызовСервера.ДоступноВыполнениеОбменаНаКлиентеПоРасписанию());
...
Функция ОбъектXDTO(ПространствоИмен, ИмяТипа) Экспорт
	...
	Возврат ИнтеграцияИС.ОбъектXDTO(ПространствоИмен, ИмяТипа);
...
Функция ЗаполнитьСправку2ВТабличнойЧасти(Товары, АлкогольнаяПродукция, Справка2,
	...
	Если ОписанияНоменклатуры = Неопределено Тогда
		...
		Для Каждого Строка Из Товары Цикл
			...
			КонецЕсли;
		КонецЦикла;
		ОписанияНоменклатуры = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(МассивНоменклатуры);
	КонецЕсли;
