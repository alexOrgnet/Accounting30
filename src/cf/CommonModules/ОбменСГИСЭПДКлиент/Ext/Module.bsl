	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда	
		...
		Если ТипЗнч(Форма.МППривязки) = Тип("ФиксированноеСоответствие") Тогда
			...
			Если СтруктураПривязкиМП <> Неопределено Тогда
				Форма.МПНаименование = СтруктураПривязкиМП.МПНаименование;
				Форма.МПТелефон = СтруктураПривязкиМП.МПТелефон;
				Форма.МПФото = СтруктураПривязкиМП.МПФото;
				Форма.МПИдентификатор = СтруктураПривязкиМП.МПИдентификатор;
				Форма.МПИдентификаторЗаписи = СтруктураПривязкиМП.МПИдентификаторЗаписи;	
			Иначе
	...
	МассивЗаполненныхТаблиц = Новый Массив;
 	Для Каждого ТаблицаКиЗ Из ТаблицыФормы Цикл  
		...
		Иначе
			...
			Для Каждого СтрокаСтруктура Из ТаблицаКиЗ.Значение Цикл	
				НоваяСтрокаФормы = Неопределено;
//				Если ЗначениеЗаполнено(ИдентификаторСтрокиРодителя) Тогда
//					Если СтрокаСтруктура.Свойство("ИдентификаторСтрокиРодителя")
//						И СтрокаСтруктура.ИдентификаторСтрокиРодителя = ИдентификаторСтрокиРодителя Тогда
