...
Функция ХарактеристикиУбыткиОтРеализацииОсновныхСредств()
	...
	// счета учета
	Параметры.Вставить("СчетРасходов", ПланыСчетов.Хозрасчетный.ПрочиеРасходы);
	Параметры.Вставить("СчетУбытков",  ПланыСчетов.Хозрасчетный.УбыткиОтРеализацииОС);
	// аналитика на этих счетах
	Параметры.Вставить("ВидыСтатей",                              Новый ТаблицаЗначений);
	Параметры.ВидыСтатей.Колонки.Добавить("ВидСтатьи",            Новый ОписаниеТипов("ПеречислениеСсылка.ВидыПрочихДоходовИРасходов"));
	// описание расчета
	Параметры.Вставить(
		"ШаблонОписания",
		НСтр("ru = 'Оборот по дебету счета [СчетРасходов] по статьям вида ""[ВидСтатьи]"" в корреспонденции со счетом [СчетУбытков][ЭтапРасчета]'"));
	Параметры.Вставить("ПараметрыОписания", Новый Структура);
	Параметры.ПараметрыОписания.Вставить("СчетРасходов", Параметры.СчетРасходов);
	Параметры.ПараметрыОписания.Вставить("СчетУбытков",  Параметры.СчетУбытков);
	Параметры.ПараметрыОписания.Вставить("ВидСтатьи");            // должен быть установлен при подготовке текста описания
...
Функция РезультатЗапросаУбыткиОтРеализацииАмортизируемыхОсновныхСредств(ХарактеристикиРасходов, ПроцессорПолученияДанных)
	...
	|ПОМЕСТИТЬ ВТ_Обороты
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В (&СчетаРасходов), &ВидыСубконтоСчетаРасходов, Организация В (&Организации), КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.УбыткиОтРеализацииОС), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства)) КАК ХозрасчетныйОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОбороты.Субконто1,
...
Процедура ПолучитьРеализацияАмортизируемогоИмущества(ПроцессорПолученияДанных)
	...
	РезультатЗапроса = РезультатЗапросаУбыткиОтРеализацииАмортизируемыхОсновныхСредств(ХарактеристикиРасходов, ПроцессорПолученияДанных);
	...
	// 5. Количество реализованных объектов
	ШагРасчета = ШагиРасчета.Добавить();
	...
	// 7. Количество объектов, реализованных с убытком
	ШагРасчета = ШагиРасчета.Добавить();
