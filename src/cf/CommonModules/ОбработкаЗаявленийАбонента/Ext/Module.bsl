...
Процедура ЗаполнитьАдресаОрганизации_ВНовомФормате(СтруктураРеквизитов, ДанныеОрганизации) Экспорт
	...
	Иначе
		СтруктураРеквизитов.Вставить("АдресЮридический_JSON", "");
	КонецЕсли;
...
Функция ИсходныйВладелецСертификатаПоФИОиСНИЛС(Организация, ТекущиеДанныеОрганизации = Неопределено, Фамилия, Имя, Отчество, СНИЛС) Экспорт
	...
	Если ЗначениеЗаполнено(Руководитель)
		И (ФИОСовпадаетСФИОВладельца(ФИОРуководителя, Фамилия, Имя, Отчество)
	...
	ИначеЕсли ЗначениеЗаполнено(ГлБухгалтер)
		И (ФИОСовпадаетСФИОВладельца(ФИОБухгалтера, Фамилия, Имя, Отчество)
...
Функция ДанныеРуководителяЮЛИз1СКонтрагента(Форма, РеквизитыКонтрагента) Экспорт
	...
	Если Форма.ЭтоЮридическоеЛицо Тогда
		...
		Если Руководители.Количество() = 0 Тогда 
			Возврат Неопределено;
		ИначеЕсли Руководители.Количество() > 1 И ЗначениеЗаполнено(Форма.Руководитель) Тогда
...
Процедура ЗаполнитьДанныеЕГРЮЛДляСравнения(Форма) Экспорт
	...
	Попытка
		РеквизитыКонтрагента = ОбработкаЗаявленийАбонентаПовтИсп.РеквизитыКонтрагентаДляПроверки(Форма.ИНН);
	Исключение
...
Функция ЭтоСертификатФНС(СвойстваСертификата) Экспорт
	...
	Если ДокументооборотСКОВызовСервера.ИспользуетсяРежимТестирования() Тогда
		Возврат Истина;
	Иначе
		Возврат ИмяУЦ = НСтр("ru = 'Федеральная налоговая служба'");
	КонецЕсли;
