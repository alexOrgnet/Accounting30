...
Функция ЦепочкаСвойствДействительна(Объект, Знач СтрокаСвойствОбъекта)
	...
	Попытка
		...
		Если СтрНайти(СтрокаСвойствОбъекта, ".") = 0 Тогда
			Значение = Объект[СтрокаСвойствОбъекта];
		Иначе
			Значение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, СтрокаСвойствОбъекта);
		КонецЕсли;
