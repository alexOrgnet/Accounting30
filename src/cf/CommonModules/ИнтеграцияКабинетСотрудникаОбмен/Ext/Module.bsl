...
Функция ВыгрузитьДокументы(МенеджерОбмена) Экспорт
	...
	Для каждого СтрокаТЗ Из ТаблицаИзменений Цикл
		...
		ИначеЕсли СтрокаТЗ.КатегорияДокумента = Перечисления.КатегорииДокументовКадровогоЭДО.ЗаявлениеСотрудника Тогда
			ЗаполнитьЗначенияСвойств(ОтменитьРегистрацию.Добавить(), СтрокаТЗ);
		ИначеЕсли ТипДанных = Тип("ДокументСсылка.СправкаНДФЛ") Тогда
	...
	Если ЗначениеЗаполнено(Данные.РезультатыСогласования) Тогда
		...
		Для каждого СтрокаТЗ Из Данные.РезультатыСогласования Цикл
			Если НеВыгруженныеРезультатыСогласования[СтрокаТЗ.ИдентификаторОбъекта] = Неопределено Тогда
				ОбработанныеЗаявки.Вставить(Данные.ИдентификаторЗаявка[СтрокаТЗ.ИдентификаторДокумента]);
			КонецЕсли;
		КонецЦикла;
...
Функция ВыгрузитьМЧД(МенеджерОбмена)
	...
	ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.МашиночитаемаяДоверенностьКЭДО;
...
Функция ДанныеДляВыгрузкиМЧД(ТаблицаИзменений)
	...
	Если ЗначениеЗаполнено(ДанныеМЧД) Тогда 
		...
		МЧДПубличныйИдентификатор = ИнтеграцияУправлениеПерсоналомОбмен.СсылкаПубличныйИдентификатор(МЧД, Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.МашиночитаемаяДоверенностьКЭДО);
		...
		Для каждого СтрокаТЗ Из ДанныеМЧД Цикл
			...
			ДанныеДоверенности = СтрокаТЗ.Доверенность.Получить();
			ХешСумма = ИнтеграцияУправлениеПерсоналом.ХешСуммаДвоичныхДанных(ДанныеДоверенности);
			
			УстановленныеПодписи = ЭлектроннаяПодписьКЭДО.УстановленныеПодписи(СтрокаТЗ.МЧД);
...
Функция ДанныеДляВыгрузки2НДФЛ(МенеджерОбмена, ДокументыКЭДО)
	...
	Для каждого СтрокаТЗ Из ДанныеСправок Цикл
		...
		Если ТребуетсяЭП И СтрокаТЗ.ДанныеЭлектронногоДокумента <> Неопределено Тогда
			...
			РезультатыСогласования = РезультатыСогласования(СтрокаТЗ.ИдентификаторДокумента, СтрокаТЗ.ДанныеЭлектронногоДокумента);
...
Функция ВыгрузитьСогласияКЭДО(МенеджерОбмена, ДокументыКЭДО, ТаблицаОбработано)
	...
	Если ЗначениеЗаполнено(РезультатыСогласования) Тогда
		...
		Если ЗначениеЗаполнено(НеВыгруженныеРезультатыСогласования) Тогда
			Для каждого СтрокаТЗ Из РезультатыСогласования Цикл
				Если НеВыгруженныеРезультатыСогласования[СтрокаТЗ.ИдентификаторОбъекта] = Истина Тогда
					ВыгруженныеДокументы.Удалить(СтрокаТЗ.ИдентификаторДокумент[ИдентификаторДокумента]);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция ДанныеДляВыгрузкиСогласийКЭДО(МенеджерОбмена, ДокументыКЭДО)
	...
	Для каждого СтрокаТЗ Из ДанныеСогласий Цикл
		...
		РезультатыСогласования = РезультатыСогласования(ДанныеДокументаНаПодпись.ИдентификаторДокумента, ДанныеДокументаНаПодпись.ДанныеЭлектронногоДокумента);
		Если ЗначениеЗаполнено(РезультатыСогласования) Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатыСогласования, Данные.РезультатыСогласования);
		КонецЕсли;
...
Функция ВыгрузитьЗаработанныеПраваНаОтпуск(МенеджерОбмена)
	...
	Если ЗначениеЗаполнено(Изменения) Тогда
		...
		Данные = ДанныеЗаработанныхПравНаОтпуск(Изменения, СотрудникиФизическихЛиц);
		...
		Если ЗначениеЗаполнено(Данные) Тогда
			...
			Для каждого ФизическоеЛицо Из ВыгруженыФизическиеЛица Цикл
				...
				Для каждого Сотрудник Из Сотрудники Цикл
					ОбработанныеСотрудники.Вставить(Сотрудник, Истина);
				КонецЦикла;
			КонецЦикла;
			
			Для каждого СтрокаТЗ Из Изменения Цикл
				Если ОбработанныеСотрудники[СтрокаТЗ.Ссылка] = Истина Тогда
					НоваяСтрока = ТаблицаОбработано.Добавить();
			...
			ИнтеграцияКабинетСотрудника.ОбновитьАктуальностьИнформацииОбОтпуске(ОбработанныеСотрудники, Данные.АктуальностьИнформацииОбОтпуске);
...
Функция ДанныеЗаработанныхПравНаОтпуск(ТаблицаИзменений, СотрудникиФизическихЛиц)
	
	Данные = Новый Структура("ИнформацияОбОтпусках,ПрогнозируемыеНеиспользованныеДниОтпуска,АктуальностьИнформацииОбОтпуске,ОтменитьРегистрацию");
	
	ИнформацияОбОтпусках 						= НоваяТаблицаИнформацияОбОтпусках();
	ПрогнозируемыеНеиспользованныеДниОтпуска 	= НоваяТаблицаПрогнозируемыеНеиспользованныеДниОтпуска();
	ОтменитьРегистрацию 						= ТаблицаИзменений.СкопироватьКолонки("Ссылка,ТипОбъекта");
	
	Сотрудники = ТаблицаИзменений.ВыгрузитьКолонку("Ссылка");
	ДанныеОбОтпусках = ИнтеграцияУправлениеПерсоналом.ДанныеОбОтпусках(Сотрудники, СотрудникиФизическихЛиц);
	...
	Данные.АктуальностьИнформацииОбОтпуске 			= ДанныеОбОтпусках.АктуальностьИнформацииОбОтпуске;
	Данные.ОтменитьРегистрацию 						= ОтменитьРегистрацию;
...
Функция ВыгрузитьДанныеГрафиковРаботы(МенеджерОбмена)
	...
	Для каждого СтрокаТЗ Из ТаблицаИзменений Цикл
		Если Не СтрокаТЗ.ЕстьВПравилах Тогда
			ЗаполнитьЗначенияСвойств(ОтменитьРегистрацию.Добавить(), СтрокаТЗ);
		ИначеЕсли СтрокаТЗ.ГрафикВыгружался Тогда
			ЗаполнитьЗначенияСвойств(Изменения.Добавить(), СтрокаТЗ);
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеШаблоновДокументов(ТаблицаИзменений)
	...
	Для каждого СтрокаТЗ Из Данные Цикл
		...
		Иначе
			...
			НоваяСтрока.НеИспользуется 						= СтрокаТЗ.ВАрхиве;
...
Функция ДанныеДляВыгрузкиРезультатовВыполненияЗаявок(МенеджерОбмена, ТаблицаИзменений)
	...
	Для каждого СтрокаТЗ Из РезультатыВыполненияЗаявок Цикл
		...
		Данные.ИдентификаторЗаявка.Вставить(СтрокаТЗ.ИдентификаторЗаявки, СтрокаТЗ.Заявка);
		...
		Комментарий = "";
		Если РезультатВыполнения = Перечисления.РезультатыСогласованияБЗК.Отклонено
		...
		Если ИспользуетсяКадровыйЭДО И ЗначениеЗаполнено(СтрокаТЗ.ЭлектронныйДокумент) И СтрокаТЗ.ЗаявкаПодписывается Тогда
			...
			РезультатыСогласования = РезультатыСогласованияЗаявкиСПодписью(СтрокаТЗ.ИдентификаторЗаявки, РезультатВыполнения,
						СтрокаТЗ.ЭлектронныйДокумент, СтрокаТЗ.ИдентификаторДокумента, Комментарий);
		...
		Если СтрокаТЗ.ТипЗаявкиКабинетСотрудника = Перечисления.ТипыЗаявокКабинетСотрудника.СправкаОбОстаткеОтпуска
			И РезультатВыполнения = Перечисления.РезультатыСогласованияБЗК.Согласовано Тогда
...
Функция НоваяТаблицаИнформацияОбОтпусках()
...
Функция РезультатыСогласования(ИдентификаторДокумента, ДанныеЭлектронногоДокумента)
...
Функция РезультатыСогласованияЗаявкиСПодписью(ИдентификаторЗаявки, РезультатВыполнения, ПрисоединенныйФайл, ИдентификаторДокумента, Комментарий)
...
Функция РезультатВыгрузкиРасчетныхЛистков(МенеджерОбмена, ПараметрыВыгрузки)
	...
	Результат.БылиОшибки 	= Данные.БылиОшибки;
	...
	Если Не ЗначениеЗаполнено(Данные.ДокументыНаПодпись) Тогда
		Возврат Результат;
	КонецЕсли;
	
	СтрокиРасчетныхЛистков = Новый Соответствие;
	Для каждого СтрокаТЗ Из Данные.РасчетныеЛистки Цикл
		СтрокиРасчетныхЛистков.Вставить(СтрокаТЗ.ИдентификаторДокумента, СтрокаТЗ);
	КонецЦикла;
	
	РазмерПакета = 500;
	КоличествоДокументов = Данные.ДокументыНаПодпись.Количество();
	Пакеты = Новый Массив; 
	ДокументыНаПодпись = НоваяТаблицаДокументыНаПодпись();
	СчПакета = 0;
	Для Ид = 0 По КоличествоДокументов - 1 Цикл
		СтрокаТЗ = Данные.ДокументыНаПодпись[Ид];
		Если СчПакета = РазмерПакета Тогда
			Пакеты.Добавить(ДокументыНаПодпись);
			СчПакета = 0;
			ДокументыНаПодпись = НоваяТаблицаДокументыНаПодпись();
		КонецЕсли;
		СчПакета = СчПакета + 1;
		ЗаполнитьЗначенияСвойств(ДокументыНаПодпись.Добавить(), СтрокаТЗ);
	КонецЦикла;
	Пакеты.Добавить(ДокументыНаПодпись);
	...
	КоличествоВыгружено = 0;
	Для каждого ДокументыНаПодпись Из Пакеты Цикл
		...
		Для Каждого ИдентификаторДокумента Из РезультатВыгрузки.Выгружено Цикл
			...
			ЗаполнитьЗначенияСвойств(РасчетныеЛистки.Добавить(), СтрокиРасчетныхЛистков[ИдентификаторДокумента]);
		КонецЦикла;
		
		Если ЗначениеЗаполнено(РасчетныеЛистки) Тогда
			
			ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.РасчетныйЛисток;
			РезультатВыгрузки = МенеджерОбмена.РезультатВыгрузкиОбъектов(РасчетныеЛистки, ТипОбъекта);
			Результат.БылиОшибки = Результат.БылиОшибки Или РезультатВыгрузки.БылиОшибки;
...
Функция ДанныеДляВыгрузкиРасчетныхЛистков(МенеджерОбмена, ПараметрыВыгрузки)
	...
	Данные = Новый Структура("СоставныеЧастиЗарплаты,РасчетныеЛистки,ДокументыНаПодпись,НеОбработаны,БылиОшибки");
	Данные.СоставныеЧастиЗарплаты 	= Новый Массив;
	Данные.РасчетныеЛистки 			= НоваяТаблицаРасчетныеЛистки();
	Данные.ДокументыНаПодпись 		= НоваяТаблицаДокументыНаПодпись();
	Данные.БылиОшибки 				= Ложь;
	...
	Данные.РасчетныеЛистки.Колонки.Добавить("СтарыйФайл");
	Данные.РасчетныеЛистки.Колонки.Добавить("НовыйФайл");
	Данные.РасчетныеЛистки.Колонки.Добавить("ДокументКЭДО");
	Данные.РасчетныеЛистки.Колонки.Добавить("ИдентификаторДокумента");
	...
	Для каждого ФизическоеЛицо Из СписокФизическихЛиц Цикл
		...
		НоваяСтрока = Данные.ДокументыНаПодпись.Добавить();
		НоваяСтрока.ИдентификаторДокумента			= ДокументКЭДООбъект.ИдентификаторДокумента;
		НоваяСтрока.Организация						= Организация;
		...
		РасчетныйЛисток = Данные.РасчетныеЛистки.Добавить();
		...
		РасчетныйЛисток.СтрокиРасчетногоЛистка 	= СтрокиРасчетногоЛистка;
		РасчетныйЛисток.ДокументыНаПодпись 		= ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Новый Структура("ID",ДокументКЭДООбъект.ИдентификаторДокумента));
		РасчетныйЛисток.ИдентификаторДокумента	= ДокументКЭДООбъект.ИдентификаторДокумента;
