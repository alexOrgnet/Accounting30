...
Процедура НачатьПроверкуПодписей(ДанныеДляПроверкиПодписей, Индекс) Экспорт
	...
	Если ВыводитьСообщения Тогда
		Состояние(НСтр("ru = 'Проверка подписей'"), , Строка(ДанныеЭлектронногоДокумента.ДокументКЭДО));
	Иначе
		Состояние(НСтр("ru = 'Проверка подписей'"), Цел(Индекс / ДанныеДляПроверкиПодписей.Количество() * 100), Строка(ДанныеЭлектронногоДокумента.ДокументКЭДО));
	КонецЕсли;
...
Процедура ПроверкаПодписиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеСтроки.ПодписьВерна Тогда
		...
		КадровыйЭДОВызовСервера.ОбновитьРезультатПроверкиПодписи(
			ПодписанныйОбъект, ДанныеСтроки.ПорядковыйНомер, ДанныеСтроки.ДатаПроверкиПодписи, Истина)
	КонецЕсли;
