...
Процедура ДобавитьНастройкиОплаты(
	...
	// Заполнять шаблоны имеет смысл только для существующих настроек.
...
Процедура ЗаполнитьДекорацияНастройкиШаблонов(
	...
	Если Шаблоны.Количество() <> 0 Тогда
		Форма.Элементы.ДекорацияНастройкиШаблонов.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
//
// НастройкаПодключения:
//  Интеграция - СправочникСсылка.НастройкиПодключенияКСистемеБыстрыхПлатежей - настройка подключения
